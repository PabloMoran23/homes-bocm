# Gijón/Xixón — investigación portal ayuntamiento

Municipio: **Gijón/Xixón** (`gijon-xixon`) — Asturias / provincia Gijón/Xixón  
Boletín: BOPA (`boletin_source_id: bopa`, 1 entrada histórica en cola)  
Nota: mismo municipio que `gijon` (INE 33024); nombre bilingüe oficial.
...
2. **Sede TAO — información pública:** La página `SEDE_URBANISMO` embebe metadata JSON en `metadata_GJ_META_URBANISMO_PLANEAMIENTO_INFOPUBLICA` con filas DataTable (expediente, asunto, enlace detalle). Estudios de detalle en trámite.

3. **Sede TAO — instrumentos planeamiento/gestión:** Tablas `GJ_META_URBANISMO_PLANEAMIENTO_INSTRUMENTOS` y `GESTION` con posible carga dinámica (a menudo vacías en HTML estático).
...
- ~300 instrumentos RPGUR implican varios minutos de scrape con delay 0.35s por detalle.
