...
1. **Proyectos:** semillas PDF web urbanismo + carpeta transparencia sede (Wicket) + instrumentos GVA.
2. **Licencias:** tablón `/board` (regex urbanismo/licencias) + trámites informativos sede.
3. **Geometría:** fetch ICV por `featureId` conocidos o escaneo paginado; matching textual título↔`denominaci`/`zon_suelo` (p. ej. «industrial», «playa», «casco»).
...
- `queue.yaml` tiene `provincia: Guardamar de la Safor` (debería ser `Valencia`).
- Municipio pequeño (622 hab., 1.10 km²) — bajo volumen en boletines.

## Adapter (pendiente)

- Clase sugerida: `municipio.adapters.guardamar_de_la_safor:GuardamarDeLaSaforAyuntamientoAdapter`
