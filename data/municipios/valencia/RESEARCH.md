...
1. **NSF Domino (`mhv.valencia.es`)** — Las páginas Liferay de instrumentos en trámite/aprobados embeben un iframe apuntando a `urbanismo2.nsf` con vista `vTramitacionWebNW`. El listado es HTML estático (`ul.listadoBusqueda > li > a`) con código de expediente y título. Además, la vista `vTramitacionWebv?ReadViewEntries&Navigate=1.1` expone ~200 instrumentos MPG en XML Domino (sin paginación adicional). Cada ficha (`OpenDocument`) incluye PDFs, número de expediente y fecha de publicación DOGV.
2. **Tablón de edictos sede** — `https://sede.valencia.es/sede/edictos/index/materia/{UR|LA}` lista anuncios en HTML (`rotuloAnuncio`) con fecha `(dd/mm/yyyy)` y enlace `/sede/edictos/detalle/{hash}`.
3. **Liferay Asset Publisher** — `proyectos-urbanos` publica tarjetas con `span.enlace-title` y enlace `/cas/urbanismo/proyectos-urbanos/-/content/...`.
4. **Participación pública** — Enlaces `/-/content/planeam-planesparticipaciónpública?uid=...` con título en `bloque_titulo` dentro de la ficha.
5. **Transparencia** — Sección G.2 instrumentos de planeamiento (enlaces a las mismas rutas de urbanismo).
...
- El **tablón de edictos** (`/sede/edictos/index/materia/UR` y `/LA`) es HTML scrapeable sin POST; materia UR aporta IP/modificaciones PGOU, LA aporta IP art. 55 obras.
...
- Edictos urbanísticos integrados vía scrape HTML materias UR/LA.
