...
DIPSEGOVIA_BASE = "https://www.dipsegovia.es/web/ayuntamiento-de-aguilafuente"
SEDE_BASE = "https://aguilafuente.sedelectronica.es"
PLAI_BASE = "https://servicios.jcyl.es/PlanPublica"
WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/ows"
MUNICIPIO = "Aguilafuente"
ID_PREFIX = "aguilafuente"
PLAI_MUNICIPIO = 4
PLAI_PROVINCIA = 40
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=004",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=004",
...
RE_PREVIEW = re.compile(
    r'href="(https://aguilafuente\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://aguilafuente\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_SECTOR = re.compile(
    ...
    r"el pastel|la chimenea|carravieja|palomar|eras de santa mar[ií]a|la cerca|"
    r"arroyo malucas|marinos|la calera)",
)
...
def _sector_tokens(text: str) -> list[str]:
    ...
    for pat in (
        ...
        r"carravieja",
        r"palomar",
        r"eras de santa mar[ií]a",
        r"la cerca",
        r"arroyo malucas",
        r"marinos",
        r"la calera\s*(?:i|ii|1|2)?",
        r"sector\s*a",
        r"u[1-6]",
        r"d[1-6]",
    ):
...
class AguilafuenteAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-aguilafuente/1.0")},
        )
