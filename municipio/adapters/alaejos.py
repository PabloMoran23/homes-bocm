...
SEDE_BASE = "https://alaejos.sedelectronica.es"
...
DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    (
        f"{SEDE_BASE}/catalog/t/ba729040-e3e4-4b08-abf0-78b91d5048e4",
        "Declaración Responsable de Obras fuera del perímetro BIC",
    ),
    (
...
RE_PREVIEW = re.compile(
    r'href="(https://alaejos\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://alaejos\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class AlaejosAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + PlanPublica PLAU + IDECyL WFS (web corporativa inactiva)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=004"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=004"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-alaejos/1.0")},
        )
