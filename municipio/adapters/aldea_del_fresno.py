...
WP_BASE = "https://www.aldeadelfresno.es"
SEDE_BASE = "https://aldeadelfresno.sedelectronica.es"
MUNICIPIO = "Aldea del Fresno"
ID_PREFIX = "aldea-del-fresno"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "ALDEA DEL FRESNO"

WP_URBANISMO_CATEGORIES: tuple[int, ...] = ()  # sin categorías WP útiles

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/normativa-municipal/",
    f"{WP_BASE}/privada/ordenanzas/",
    f"{WP_BASE}/documentos-categoria/ordenanzas/",
    f"{WP_BASE}/documentos-categoria/bandos/",
    f"{WP_BASE}/documentos-categoria/avisos-bandos/",
]
...
RE_BOARD_PREVIEW = re.compile(
    r'href="(https://aldeadelfresno\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
class AldeaDelFresnoAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-aldea-del-fresno/1.0")},
        )
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        tramites: list[dict[str, Any]] = [
            ...
            {
                "id": _stable_id("lic", f"{self.wp_base}/normativa-municipal/"),
                "fecha_concesion": None,
                "tipo": "ordenanza fiscal licencias urbanísticas",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Ordenanza fiscal reguladora de la tasa por otorgamiento de licencias urbanísticas",
                "url": f"{self.wp_base}/normativa-municipal/",
                "source": "ayuntamiento",
                "nota": "Normativa municipal publicada en BOCM",
                "origen": "wp_tramite",
            },
            {
        ...
        try:
            html = self._fetch(f"{self.wp_base}/normativa-municipal/")
        except urllib.error.URLError:
            return tramites
        seen_urls: set[str] = set()
        for m in re.finditer(
            r'href="(https://aldeadelfresno\.sedelectronica\.es/[^"]+)"',
            html,
            re.I,
        ):
            url = m.group(1)
