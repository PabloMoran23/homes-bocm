...
SEDE_BASE = "https://aldeaviejadetormes.sedelectronica.es"
PLAU_BASE = "https://servicios.jcyl.es/PlanPublica"
MUNICIPIO = "Aldeavieja de Tormes"
ID_PREFIX = "aldeavieja-de-tormes"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=024",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=024",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        "http://www.lasalina.es/Aplicaciones/GestorInter.jsp?codMunicipio=24&funcion=VerNormasUrbanisticas&nombre=Aldeavieja+de+Tormes&prestacion=NormasUrbanisticas",
        "Normas urbanísticas y planeamiento (Diputación de Salamanca)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://aldeaviejadetormes\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://aldeaviejadetormes\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class AldeaviejaDeTormesAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + PLAU JCyL + IDECyL WFS (geometría partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=024"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=024"
        )
    ...
    def _bootstrap_session(self) -> None:
        ...
        req = urllib.request.Request(
            self.board_url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-aldeavieja-de-tormes/1.0")},
        )
        try:
    ...
    def _fetch(self, url: str, *, sede: bool = True) -> str:
        if sede:
    ...
    def _collect_plau(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(self.plau_url, sede=False)
        except (urllib.error.URLError, OSError, ConnectionError):
            return []
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
    ...
    def _collect_plai_seed(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(self.plai_url, sede=False)
        except (urllib.error.URLError, OSError, ConnectionError):
            return []
        rows: list[dict[str, Any]] = []
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if RE_LICENCIA.search(blob) and not RE_PROYECTO.search(blob):
            return None
        if row.get("procedimiento", "").lower() == "licencias urbanísticas":
            pass
