...
SEDE_BASE = "https://alfarb.sede.dival.es"
WEB_BASE = "https://alfarb.es"
MUNICIPIO = "Alfarb"
ID_PREFIX = "alfarb"
INE_COD_MUN = "46026"
...
NORMES_URL = f"{WEB_BASE}/normes-subsidiaries/"

GVA_WFS = "https://terramapas.icv.gva.es/0702_Planeamiento"
GVA_WFS_TYPE = "Planeamiento.Zonificacion"
GVA_WFS_OFFSETS = [0, 2000, 4000, 6000, 10500, 13500]

NORMES_SEEDS = [
    ("Normes Subsidiàries de Planejament", NORMES_URL, "normas subsidiarias"),
    ("Classificació del sòl — estructura general", NORMES_URL, "clasificación suelo"),
    ("Zonificació Casc Urbà", NORMES_URL, "zonificación casc urbà"),
    ("Zonificació El Puntal", NORMES_URL, "zonificación el puntal"),
    ("Zonificació Almaguer", NORMES_URL, "zonificación almaguer"),
    ("Zonificació Los Lagos", NORMES_URL, "zonificación los lagos"),
]
...
RE_LICENCIA = re.compile(
    ...
    r"primera ocupaci[oó]n|inicio de obra|ocupaci[oó]n de la via|material d'obra|"
    r"reserva d'aparcament.*obra)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|normes subsidi|"
    r"informaci[oó]n p[uú]blica|expediente|proyecto|modificaci[oó]n|reparcel|"
    r"aprobaci[oó]n|edicto|bop|dogv|ordenanza|sector|urbanizaci[oó]n|zonificaci|"
    r"homologaci|suelo industrial|puntal|almaguer|los lagos|casc urb)",
)
RE_TABLON_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"jurat|jurado|premis.*valenci|fundaci[oó]n amancio|electors|iae|cobranza|"
    r"sepe|ccll|llibre de festes|protecci[oó] de dades|subvenci[oó]n)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_DOC_LINK = re.compile(
    r'href="((?:https://alfarb\.sede\.dival\.es)?/tablondeanuncios/documento\.aspx\?id=\d+[^"]*)"',
    re.I,
)
RE_TRAMITE = re.compile(
    r'href="(https://alfarb\.sede\.dival\.es/carpetaciudadana/tramite\.aspx\?idtramite=\d+)"[^>]*>\s*([^<]+)',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    n = (title or "").lower()
    if "normes subsidi" in n or "normas subsidi" in n:
        return "normas subsidiarias"
    if "plan parcial" in n or "sector" in n or "suelo industrial" in n:
        return "plan parcial"
    if "plan general" in n or "pgou" in n:
        return "PGOU"
    if "zonificaci" in n or "classificaci" in n:
        return "zonificación"
    if "ordenanza" in n:
...
class AlfarbAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress alfarb.es (captcha) + sede Dival (Sedipualba) tablón RSS + ICV GVA WFS partial."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.normes_url = str(self.config.get("normes_url") or NORMES_URL)
    ...
    def _fetch(self, url: str, *, timeout: int = 90) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-alfarb/1.0")},
        )
    ...
    def _fetch_geometry(self, titulo: str) -> dict[str, Any] | None:
        ...
        for feat in feats:
            ...
            for k in keys:
                if k in label or k in title_low:
                    score += 1.0
            if "normes subsidi" in title_low and "normas subsidi" in label:
                score += 2.0
            if "suelo industrial" in title_low and "industrial" in label:
                score += 2.0
            if score <= 0:
                continue
        ...
        if "normes subsidi" in title_low or "normas subsidi" in title_low:
            matched = [
                f["geom"]
                for f in feats
                if "normas subsidi" in (f.get("label") or "").lower()
                or "normes subsidi" in (f.get("label") or "").lower()
            ]
        if "suelo industrial" in title_low or "homolog" in title_low:
            matched = [f["geom"] for f in feats if "industrial" in (f.get("label") or "").lower()]
    ...
    def _collect_catalogo_obra_tramites(self) -> list[dict[str, Any]]:
        try:
            ...
            rows.append(
                {
                    ...
                    "tipo": "trámite sede (obra / vía pública)",
                    "distrito": None,
                    "lat": None,
                    "lon": None,
                    "titulo": title[:500],
        ...
        return [
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Tauler d'anuncis — sede electrónica Alfarb",
                "url": self.tablon_url,
                "source": "ayuntamiento",
                "nota": "Edictos y licencias publicados en alfarb.sede.dival.es",
                "origen": "sede_tablon",
            },
            {
                ...
                "source": "ayuntamiento",
                "nota": "Trámites de ocupación vía pública y obra; sin licencia urbanística en línea",
                "origen": "sede_tramite",
            },
            {
                "id": _stable_id("lic", self.normes_url),
                "fecha_concesion": None,
                "tipo": "normas subsidiarias",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Normes Subsidiàries de Planejament",
                "url": self.normes_url,
                "source": "ayuntamiento",
                "nota": "Documentación de planeamiento en web municipal (SiteGround captcha en scraper)",
                "origen": "web_tramite",
            },
        ]
    ...
    def _collect_normes_seeds(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for titulo, url, tipo in NORMES_SEEDS:
            rec: dict[str, Any] = {
                "id": _stable_id("proy", f"normes:{titulo}"),
                "municipio": MUNICIPIO,
                "titulo": titulo,
                "fecha": None,
                "tipo": tipo,
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_licencia_info_pages():
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for rec in self._collect_catalogo_obra_tramites():
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for seed in self._collect_normes_seeds():
            add(seed)
        ...
        return {
            ...
            "normes": sum(1 for r in rows if r.get("origen") == "normes_subsidiaries"),
