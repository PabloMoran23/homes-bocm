...
SEDE_BASE = "https://alhaurinelgrande.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board/"
URBANISMO_URL = "https://alhaurinelgrande.es/concejalia-de-urbanismo/"
MUNICIPIO = "Alhaurín el Grande"
ID_PREFIX = "alhaurin-el-grande"

RE_LICENCIA = re.compile(
    r"(?i)(solicitud de licencia|licencia(?:s)?(?: de)?(?: obra| urban| municipal| de actividad)?|"
    r"notificaci[oó]n.*licencia|edicto.*(?:licencia|actividad|industria)|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|inicio de obra|"
    r"obra (?:mayor|menor)|establecimiento hosteler|venta productos|v[ií]a p[uú]blica)",
)
RE_PROYECTO = re.compile(
    ...
    r"divisi[oó]n poligonal|adaptaci[oó]n|clasificaci[oó]n)",
)
...
RE_PDF_HREF = re.compile(
    r'href="(https://alhaurinelgrande\.es/wp-content/uploads/[^"]+\.pdf)"',
    re.I,
)
RE_BOARD_ROW = re.compile(r'<tr[^>]*>\s*<td class="class_name".*?</tr>', re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://alhaurinelgrande\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
def _fecha_from_pdf_name(url: str) -> str | None:
    ...
    if m:
        try:
            ...
            pass
    m = RE_YEAR.search(Path(url).stem)
    if m:
...
def _pdf_title(url: str) -> str:
    name = Path(url).stem.replace("_", " ").replace("-", " ")
    return re.sub(r"\s+", " ", name).strip()


class AlhaurinElGrandeAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona (tablón) + WordPress PDFs urbanismo."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-alhaurin-el-grande/1.0")},
        )
            ...
            rows.append(
                {
                    "titulo": title,
                    "url": url,
                    "fecha": _fecha_from_pdf_name(url),
                    "blob": f"{title} {url}",
                }
            )
        return rows
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        elif re.search(r"(?i)informaci[oó]n p[uú]blica|audiencia", blob):
    ...
    def _pdf_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        blob = row.get("blob") or ""
        tipo = "planeamiento"
        if re.search(r"(?i)memoria", blob):
            tipo = "memoria planeamiento"
        elif re.search(r"(?i)planos", blob):
            tipo = "planos planeamiento"
        elif re.search(r"(?i)plan especial|surs", blob):
            tipo = "plan especial"
        elif re.search(r"(?i)adaptaci[oó]n", blob):
            tipo = "adaptación PGOU"
        return {
