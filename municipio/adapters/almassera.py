...
SEDE_BASE = "https://almassera.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WEB_BASE = "https://www.almassera.es"
MUNICIPIO = "Almàssera"
ID_PREFIX = "almassera"
COD_INE_MUN = "46014"

TRANSPARENCIA_NORMATIVA_URL = f"{WEB_BASE}/es/transparencia/35-normativa-urbanistica-planes-parciales"
TRANSPARENCIA_PUAM_URL = f"{WEB_BASE}/es/transparencia/38-plan-urbano-actuacion-municipal-almassera"
URBANISMO_TRAMITES_URL = f"{WEB_BASE}/es/content/impresos-solicitudes-urbanismo"
BANDO_URL = f"{WEB_BASE}/es/servicios/canal-bando-municipal"

ICV_WFS_BASE = "https://terramapas.icv.gva.es/0702_Planeamiento"
ICV_TYPE_NAME = "Planeamiento.Zonificacion"
ICV_OFFSETS = list(range(0, 15000, 500))

DEFAULT_SEED_PAGES: list[str] = [
    TRANSPARENCIA_NORMATIVA_URL,
    TRANSPARENCIA_PUAM_URL,
    URBANISMO_TRAMITES_URL,
    BANDO_URL,
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|puam|convenio|"
    ...
    r"cambio de uso|normas subsidiarias|homologaci[oó]n|ordenanza|e[oó]lic|iate|font baixa|sr-1)",
)
RE_BOARD_NON_URBAN = re.compile(
    ...
    r"ayudas a entidades culturales|baja en el padr[oó]n|alta en el padr[oó]n|"
    r"convocatoria junta de gobierno|bolsa de polic[ií]a|acta tribunal)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://almassera\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_PDF_HREF = re.compile(
    r'href="((?:https://www\.almassera\.es)?/sites/www\.almassera\.es/files/[^"]+\.(?:pdf|PDF))"',
    re.I,
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_SECTOR = re.compile(r"(?i)sector\s*([0-9IVXLC]+|sr[\s-]*\d+)")
...
def _proyecto_tipo(denominaci: str) -> str:
    ...
    if "plan general" in n or "pgou" in n:
        return "PGOU"
    if "plan parcial" in n or "sector" in n or "sr-" in n:
        return "plan parcial"
    if "modificaci" in n:
...
class AlmasseraAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal portalesmunicipales DigitalValue + sede espublico gestiona + ICV WFS zonificación."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.transparencia_normativa_url = str(
            self.config.get("transparencia_normativa_url") or TRANSPARENCIA_NORMATIVA_URL
        )
        self.transparencia_puam_url = str(self.config.get("transparencia_puam_url") or TRANSPARENCIA_PUAM_URL)
        self.urbanismo_tramites_url = str(
            self.config.get("urbanismo_tramites_url") or URBANISMO_TRAMITES_URL
        )
        self.bando_url = str(self.config.get("bando_url") or BANDO_URL)
    ...
    def _fetch(self, url: str, *, timeout: int = 90) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-almassera/1.0")},
        )
    ...
    def _match_icv_zone(self, titulo: str) -> dict[str, Any] | None:
        ...
        for zone in self._collect_icv_zones():
            ...
            if den and den in norm:
                score = 100.0
            elif sector and sector in den.replace(" ", ""):
                score = 80.0
            else:
    ...
    def _collect_web_links(self) -> list[dict[str, Any]]:
        ...
        for page in self.seed_pages[: self.max_crawl_pages]:
            ...
            for m in RE_PDF_HREF.finditer(html):
                ...
                if "puam" in titulo.lower() or "plan parcial" in titulo.lower():
                    tipo = "planeamiento"
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "id": _stable_id("lic", self.urbanismo_tramites_url),
                "fecha_concesion": None,
                "tipo": "formularios urbanismo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Impresos y solicitudes de urbanismo",
                "url": self.urbanismo_tramites_url,
                "source": "ayuntamiento",
                "nota": "Licencias obra mayor/menor, apertura, actividades",
                "origen": "web_tramite",
            },
            {
    ...
    def _collect_proyecto_info_pages(self) -> list[dict[str, Any]]:
        return [
            {
                "id": _stable_id("proy", self.transparencia_normativa_url),
                "municipio": MUNICIPIO,
                "titulo": "Transparencia — normativa urbanística y planes parciales",
                "fecha": None,
                "tipo": "normativa",
            ...
            {
                "id": _stable_id("proy", self.transparencia_puam_url),
                "municipio": MUNICIPIO,
                "titulo": "Plan Urbano de Actuación Municipal (PUAM)",
                "fecha": None,
                "tipo": "planeamiento",
                "url": self.transparencia_puam_url,
                "source": "ayuntamiento",
                "origen": "transparencia",
            },
            {
                "id": _stable_id("proy", self.bando_url),
                "municipio": MUNICIPIO,
                "titulo": "Canal bando municipal — anuncios y edictos",
                "fecha": None,
                "tipo": "anuncios",
                "url": self.bando_url,
                "source": "ayuntamiento",
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        tipo = "urbanismo"
        if re.search(r"(?i)planeamiento|homologaci[oó]n|sector|plan parcial|puam", blob):
            tipo = "planeamiento"
