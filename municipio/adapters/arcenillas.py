...
RE_PREVIEW = re.compile(
    r'href="(https://arcenillas\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://arcenillas\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
def _proyecto_tipo(blob: str, instrumento: str = "") -> str:
    ...
    if "proyecto regional" in n or "pyic" in n or "otpyic" in n:
        return "proyecto regional"
    if "delimitaci" in n and "suelo urbano" in n:
...
class ArcenillasAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress arcenillas.es + sede espublico + PlanPublica PLAU + IDECyL WFS (Zamora)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=49&municipio=010"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=49&municipio=010"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-arcenillas/1.0")},
        )
