...
SEDE_BASE = "https://archidona.sedelectronica.es"
...
RE_PROYECTO = re.compile(
    ...
    r"cambio de uso|sunp|supi|ordenanza|rectificaci[oó]n descriptiva|depuradora|"
    r"sistema general|innovaci[oó]n|ur-\d+|actuaci[oó]n)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"cobranza iae|padrones|bopma.*polic[ií]a local|activa-t joven|pe[oó]n (?:construcci|forestal)|"
    r"acreedores por fallecimiento|notoriedad del t[ií]tulo|plan municipal de empleo|"
    r"concurso.*fotogr[aá]fica|romer[ií]a)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://archidona\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
def _proyecto_tipo(title: str, procedimiento: str = "") -> str:
    ...
    if "plan especial" in blob or "pe_" in blob or "depuradora" in blob:
    ...
    if "pgom" in blob or "avance" in blob:
        return "planeamiento"
    if "pgou" in blob or "plan general" in blob or "innovaci" in blob:
        return "PGOU"
    ...
    if "cambio de uso" in blob or "ur-" in blob:
...
class ArchidonaAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona: tablón + SITUA (PGOU regional)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-archidona/1.0")},
        )
    ...
    def _collect_situa_seed(self) -> dict[str, Any]:
        return {
            "id": _stable_id("proy", self.situa_url),
            "municipio": MUNICIPIO,
            "titulo": "PGOU Archidona — consulta SITUADIFusión (Junta de Andalucía)",
            "fecha": None,
            "tipo": "PGOU",
            "url": self.situa_url,
            "source": "ayuntamiento",
            "origen": "situa",
            "nota": "Planeamiento vigente regional; sin enlace por expediente del tablón municipal",
        }
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if (
            ...
            and "genérico" not in proc
            and "urbanismo" not in cat
            and "actuaciones" not in proc
        ):
            return None
