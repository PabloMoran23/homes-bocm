...
DEFAULT_PLAU_URLS: list[tuple[str, str]] = [
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=47&municipio=008",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=47&municipio=008",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_TRAMITE = re.compile(
    r'href="(https://sede\.aytoarroyo\.es/carpetaciudadana/tramite\.aspx\?idtramite=\d+)"[^>]*>([^<]+)',
    re.I,
)
RE_LICENCIA = re.compile(
    ...
    r"obra (?:mayor|menor)|primera ocupaci[oó]n|tenada|ganado)",
)
RE_PROYECTO = re.compile(
    ...
    r"parcela|suelo|sector|unidad(?:es)? de ejecuci[oó]n|\b(?:UE|UBZ|UNC|SUR)[- ]?[A-Z0-9]+\b|"
    r"actuaci[oó]n|autorizaci[oó]n de uso|ruina|desafectaci[oó]n)",
)
...
def _normalize_sector_code(raw: str) -> str:
    ...
    code = code.replace("SU-NC", "UNC")
    m = re.fullmatch(r"SE(\d+)", code)
    if m:
        return f"SE-{m.group(1).zfill(2)}"
    return code
...
def _proyecto_tipo(blob: str, instrumento: str = "") -> str:
    ...
    if "uso excepcional" in n or "autorizaci" in n and "uso" in n:
        return "autorización uso excepcional"
    if "licencia" in n:
        return "licencia"
    if "planeam" in n or "pgou" in n or "sector" in n or "ubz" in n or "unc" in n:
        return "planeamiento"
    return "urbanismo"


class ArroyoDeLaEncomiendaAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal aytoarroyo.es + sede Sedipualba + PlanPublica PLAU + IDECyL WFS (partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.catalogo_url = str(self.config.get("catalogo_url") or CATALOGO_URBANISMO)
        self.plau_url = str(
            self.config.get("plau_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=47&municipio=008"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=47&municipio=008"
        )
        raw_seeds = self.config.get("plau_urls") or DEFAULT_PLAU_URLS
        self.plau_urls: list[tuple[str, str]] = []
        for item in raw_seeds:
            if isinstance(item, dict):
                self.plau_urls.append((str(item["url"]), str(item.get("titulo") or item["url"])))
            elif isinstance(item, (list, tuple)) and len(item) >= 2:
                self.plau_urls.append((str(item[0]), str(item[1])))
            else:
                self.plau_urls.append((str(item), str(item)))
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-arroyo-de-la-encomienda/1.0")},
        )
    ...
    def _collect_tablon_rss(self) -> list[dict[str, Any]]:
        try:
            raw = self._fetch_bytes(self.tablon_rss)
            ...
            rows.append(
                {
                    "titulo": title[:500],
                    "url": url,
                    "fecha": _parse_rss_date((date_el.text or "").strip() if date_el is not None else ""),
                    "origen": "tablon_rss",
                }
            )
        return rows
    ...
    def _collect_tramites(self) -> list[dict[str, Any]]:
        ...
        for m in RE_TRAMITE.finditer(html):
            url, titulo = m.group(1), unescape(_strip_html(m.group(2)))
            if url in seen:
                continue
            seen.add(url)
    ...
    def _collect_seed_pages(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for url, fallback_title in self.plau_urls:
            rows.append(
                {
