...
WEB_BASE = "https://www.arucas.org"
SEDE_BASE = "https://arucas.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
URBANISMO_URL = f"{WEB_BASE}/modules.php?mod=portal&file=ver_gen&tipo=99&id=TkRrM05RPT0="
PMUS_URL = f"{WEB_BASE}/modules.php?mod=portal&file=ver_gen&id=T1RNMU5RPT0="
MUNICIPIO = "Arucas"
ID_PREFIX = "arucas"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-arucas"
GEOBDP_MUNICIPIO = "https://geobdp.grafcan.es/core/municipios/35006/"

DEFAULT_SEED_PAGES: list[str] = [
    URBANISMO_URL,
    PMUS_URL,
    f"{WEB_BASE}/modules.php?mod=portal&file=ver_servicios&id=TlRBMk13PT0=",
    f"{SEDE_BASE}/transparency",
    f"{SEDE_BASE}/transparency/ac8b9712-ed87-457b-a683-263683c6b144/",
    BOARD_URL,
    f"https://opendata.sitcan.es/dataset/{SITCAN_PACKAGE}",
]
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgo|pgou|pamu|pmus|convenio urban|"
...
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?arucas\.org)?/?[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Ayuntamiento de Arucas.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
class ArucasAyuntamientoAdapter(AyuntamientoAdapter):
    """Portal municipal PHP + sede espublico + SITCAN + GEOBDP Grafcan (INE 35006)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-arucas/1.0")},
        )
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo — área municipal y trámites",
                "url": URBANISMO_URL,
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
            {
                "id": _stable_id("lic", f"{SEDE_BASE}/dossier"),
                "fecha_concesion": None,
                "tipo": "trámite informativo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Sede electrónica — catálogo de trámites (licencias y urbanismo)",
                "url": f"{SEDE_BASE}/dossier",
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
        ]
