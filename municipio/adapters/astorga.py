...
WEB_BASE = "https://aytoastorga.es"
SEDE_BASE = "https://ayuntamientoastorga.sedelectronica.es"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    ...
    (
        f"{WEB_BASE}/_contenidos/noticias/2021/ANUNCIO_APROBACION_PROVISIONAL_PLAN_GENERAL.html",
        "Aprobación provisional Plan General de Ordenación Urbana",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=008",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=008",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://ayuntamientoastorga\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://ayuntamientoastorga\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_TABLON_LINK = re.compile(
    r'href=["\']([^"\']*/_contenidos/(?:tablon-de-anuncios|noticias)/[^"\']+)["\']',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra menor|"
    r"primera ocupaci[oó]n|primera utilizaci[oó]n|ambiental|obras en)",
)
RE_PROYECTO = re.compile(
    ...
    r"desbroce de terrenos|ordenanza.*terrazas)",
)
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"S\.?\s*U\.?\s*\.?\s*N\.?\s*C\.?\s*[-.]?\s*(?:R\d+|[A-Z]\d+)?|"
    r"SU[- ]?NC\.?\s*(?:R\d+|[A-Z]\d+)?|"
    r"SU[- ]?NC[- ]?R\d+"
    r")\b",
)
...
def _fecha_from_blob(text: str) -> str | None:
    ...
    if m:
        try:
            ...
            pass
    years = [int(x.group(1)) for x in re.finditer(r"\b(20\d{2})\b", text or "") if 2010 <= int(x.group(1)) <= 2035]
    if years:
...
def _proyecto_tipo(blob: str, instrumento: str = "") -> str:
    ...
    if "licencia" in n:
        return "licencia"
    if "planeam" in n or "sector" in n or "su-nc" in n or "s.u.n.c" in n:
        return "planeamiento"
    return "urbanismo"


class AstorgaAyuntamientoAdapter(AyuntamientoAdapter):
    """OpenCms web (Diputación León) + sede espublico + PLAU JCyL + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.info_url = str(self.config.get("info_url") or f"{self.sede_base}/info.1")
        self.dossier_url = str(self.config.get("dossier_url") or f"{self.sede_base}/dossier")
        self.plau_url = str(
            self.config.get("plau_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=008"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=008"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-astorga/1.0")},
        )
    ...
    def _collect_plau(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for url, origen in ((self.plau_url, "plau_jcyl"), (self.plai_url, "plai_jcyl")):
            try:
                html = self._fetch(url)
            except (urllib.error.URLError, OSError, ConnectionError):
                continue
            for item in self._parse_plau_rows(html, url):
    ...
    def _collect_web_tablon(self) -> list[dict[str, Any]]:
        tablon_url = f"{self.web_base}/ayuntamiento/tablon-de-anuncios/"
        try:
        ...
        rows: list[dict[str, Any]] = []
        for url in links:
            titulo = Path(urllib.parse.urlparse(url).path).stem.replace("_", " ")
            fecha = _fecha_from_blob(url)
            try:
                page = self._fetch(url)
                h2 = RE_H2.search(page)
                if h2:
                    titulo = _strip_html(h2.group(1))
                fecha = _fecha_from_blob(page) or fecha
            except (urllib.error.URLError, OSError, ConnectionError):
                pass
            rows.append(
                {
    ...
    def _collect_seed_pages(self) -> list[dict[str, Any]]:
        ...
        for url, fallback_title in self.seed_urls:
            ...
            try:
                ...
                h2 = RE_H2.search(html)
                if h2:
                    titulo = _strip_html(h2.group(1)) or titulo
                fecha = _fecha_from_blob(html)
            except (urllib.error.URLError, OSError, ConnectionError):
                pass
    ...
    def _web_tablon_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        blob = row["titulo"]
        if RE_LICENCIA.search(blob) and not RE_PROYECTO.search(blob):
            return None
    ...
    def _web_tablon_to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        if not RE_LICENCIA.search(row["titulo"]):
            return None
        if RE_PROYECTO.search(row["titulo"]) and not re.search(
            r"(?i)licencia|obra|ambiental", row["titulo"]
        ):
            return None
        return {
            "id": _stable_id("lic", row["url"]),
            "fecha_concesion": row.get("fecha"),
            "tipo": "licencia / información pública",
            "distrito": None,
            "lat": None,
            "lon": None,
            "titulo": row["titulo"],
    ...
    def _plau_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not RE_PROYECTO.search(blob) and row.get("origen") not in ("plau_jcyl", "plai_jcyl"):
            return None
        key = f"plau:{row.get('doc_id') or row['url']}"
        rec: dict[str, Any] = {
            "id": _stable_id("proy", key),
            "municipio": MUNICIPIO,
            "titulo": row["titulo"],
            "fecha": row.get("fecha"),
            "tipo": _proyecto_tipo(blob, row.get("instrumento") or ""),
            "url": row["url"],
            "source": "ayuntamiento",
            "origen": row.get("origen"),
    ...
    def _tramite_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if RE_LICENCIA.search(row["titulo"]) and not re.search(
            r"(?i)planeam|actuaci[oó]n urban|modificaci[oó]n del planeamiento|pgou|ruina",
            row["titulo"],
        ):
            return None
        rec: dict[str, Any] = {
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_board():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_web_tablon():
            rec = self._web_tablon_to_licencia(item)
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_web_tablon():
            add(self._web_tablon_to_proyecto(item))
        ...
        return {
            ...
            "web_tablon": sum(1 for r in rows if r.get("origen") == "web_tablon"),
            "source": "ayuntamiento",
            "adapter": self.__class__.__name__,
            "at": datetime.now(timezone.utc).isoformat(),
        }
