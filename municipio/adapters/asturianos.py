...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    ...
    (
        f"{PLAU_BASE}/openDocuIndice.do?cDocId=279745",
        "Sin Planeamiento General (SPG) — documentación vigente",
    ),
    (
        "https://idecyl.jcyl.es/siur/index.html?id=49017",
        "SIUR — visor urbanístico IDECyL (Asturianos)",
    ),
]
...
DEFAULT_LICENCIA_PAGES: list[tuple[str, str]] = [
    ...
    (f"{WP_BASE}/index.php/tablon-de-anuncios/", "tablón de anuncios — web municipal"),
]

RE_PREVIEW = re.compile(
    r'href="(https://asturianos\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra menor|"
    r"primera ocupaci[oó]n|primera utilizaci[oó]n|autoconsumo|instalaci[oó]n de generaci[oó]n)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|spg|"
    ...
    r"normas urban|instrumento|memoria|planos|bocyl|sin planeamiento|"
    r"autoconsumo|instalaci[oó]n de generaci[oó]n|renovable)",
)
RE_EXCLUDE = re.compile(
    ...
    r"electores|icio\b|ordenanza fiscal|video aereo|drone|diputado|historia se escribe|"
    r"caminos de la autovia|hola mundo|cuotas nacionales)",
)
...
def _proyecto_tipo(blob: str) -> str:
    n = blob.lower()
    if "sin planeamiento" in n or "spg" in n:
        return "sin planeamiento general"
    if "autoconsumo" in n or "generaci" in n and "renovable" in n:
...
class AsturianosAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress aytoasturianos.es + sede espublico + PLAU JCyL + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.wp_api = str(self.config.get("wp_api") or f"{self.wp_base}/index.php/wp-json/wp/v2")
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-asturianos/1.0")},
        )
    ...
    def _collect_wp_posts(self) -> list[dict[str, Any]]:
        ...
        page = 1
        while page <= 5:
            url = f"{self.wp_api}/posts?per_page=50&page={page}"
            try:
    ...
    def _wp_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        blob = f"{row.get('titulo', '')} {row.get('descripcion', '')}"
        if RE_EXCLUDE.search(blob):
            return None
