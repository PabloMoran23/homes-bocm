...
BASE = "https://aviles.es"
LEGACY_BASE = "http://www.aviles.es/html_IIS/planes_urban"
SEDE_BASE = "https://sedeelectronica.aviles.es"
...
WFS_FILTER = "Instrumento LIKE '%AVILES%'"

DEFAULT_SEED_PAGES: list[str] = [
    f"{BASE}/urbanismo-licencias-normativa",
    f"{BASE}/planes-municipales1",
    f"{BASE}/urbanismo",
    f"{LEGACY_BASE}/index.html",
    f"{LEGACY_BASE}/I/I-GEN-PLAN.html",
    f"{LEGACY_BASE}/I/I-GEN-GEST.html",
    f"{LEGACY_BASE}/I/I-INF_PUB.html",
    f"{LEGACY_BASE}/I/I-INF-AMBIENTALES.html",
]

DEFAULT_LICENCIA_PAGES: list[str] = [
    f"{BASE}/licencias-para-particulares",
    f"{BASE}/licencias-para-comercio-y-oficinas",
    f"{BASE}/licencias-para-hosteleria",
    f"{BASE}/licencias-para-industria",
    f"{BASE}/certificaci%C3%B3n-urban%C3%ADstica",
    f"{BASE}/urbanismo-licencias-normativa",
]
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|pgotm|convenio|"
    r"informaci[oó]n p[uú]blica|expediente|proyecto|modificaci[oó]n|reparcel|"
    r"estudio (?:ac[uú]stico|ambiental|de detalle|de implantaci[oó]n)|normas? (?:provisionales|subsidiarias)|"
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|programa de actuaci|gesti[oó]n urban)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https?://(?:www\.)?aviles\.es)?/documents/[^"]+\.pdf[^"]*|'
    r'https?://www\.aviles\.es/html_IIS/[^"]+\.(?:pdf|html?)[^"]*)"',
    re.I,
)
RE_LEGACY_LINK = re.compile(
    r'href="((?:\.\./)?[^"]*\.html?)"',
    re.I,
)
...
RE_TABLON_ROW = re.compile(
    r'<tr class="GridView(?:Row|AlternatingRow)">\s*<td>\s*([^<]+?)\s*</td>\s*'
    r'<td>\s*(\d{1,2}/\d{1,2}/\d{4})\s*</td>.*?'
    r'href="(?:https://sedeelectronica\.aviles\.es/)?((?:DetalleTablon|Tablon_ver_doc)\.aspx\?id=\d+)"',
    re.S,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Avil[eé]s.*$", "", t, flags=re.I).strip()
...
def _tipo_proyecto(denominacion: str, clasificacion: str = "") -> str:
    ...
    if "plan general" in blob or "pgou" in blob or "pgotm" in blob or "pgo " in blob:
        return "PGOU"
...
class AvilesAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay aviles.es + legado html_IIS + RPGUR Asturias; geometría parcial WFS."""
    ...
    def _crawl_portal_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in RE_DOC_LINK.finditer(html):
                ...
                title = re.sub(r"\.(pdf|html?)$", "", title, flags=re.I).replace("+", " ").strip()
            ...
            for m in RE_LEGACY_LINK.finditer(html):
                href = unescape(m.group(1))
                link = self._abs_url(href, page_url)
                if "html_IIS/planes_urban" in link and link not in visited and link not in queue:
                    queue.append(link)

            for m in re.finditer(r'href="(https://aviles\.es/[^"#?]+)"', html, re.I):
    ...
    def _collect_tablon_licencias(self) -> list[dict[str, Any]]:
        ...
        for m in RE_TABLON_ROW.finditer(html):
            ...
            fecha = _parse_fecha_dmy(m.group(2))
            link_path = m.group(3)
            if not RE_LICENCIA.search(title) and not RE_PROYECTO.search(title):
                continue
            detail_url = f"{SEDE_BASE}/{link_path.lstrip('/')}"
            rows.append(
                {
                    "id": _stable_id("lic", detail_url),
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        for rec in self._collect_tablon_licencias():
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)

        info_pages = [
            (f"{BASE}/licencias-para-particulares", "Licencias para particulares"),
            (f"{BASE}/urbanismo-licencias-normativa", "Urbanismo, licencias y normativa"),
            (f"{SEDE_BASE}/Tablon.aspx", "Tablón de anuncios — sede electrónica"),
        ]
