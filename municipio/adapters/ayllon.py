...
WEB_BASE = "https://www.ayllon.es"
SEDE_BASE = "https://ayllon.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (f"{WEB_BASE}/urbanismo", "Urbanismo — normativa y documentación"),
    (f"{WEB_BASE}/urbanismo1", "Urbanismo, Obras Públicas y Medio Ambiente"),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=024",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=024",
        "Planeamiento en información pública (PLAI JCYL)",
    ),
    (
        f"{SEDE_BASE}/transparency/40aee2ea-0a61-4686-aa8b-a7a363c1b308/",
        "Transparencia — urbanismo, obras públicas y medio ambiente",
    ),
    (f"{SEDE_BASE}/dossier", "Catálogo de trámites — sede electrónica"),
]

RE_PREVIEW = re.compile(
    r'href="((?:https://ayllon\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://ayllon\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
def _sector_tokens(text: str) -> list[str]:
    ...
    for pat in (
        ...
        r"la dehesa",
        r"el r[ií]o",
        r"prado de las se[nñ]oritas",
        r"molino ramos",
        r"la pe[nñ]a del gato",
        r"las requijadas",
        r"concepcionistas",
        r"la mesa",
        r"riaguas",
...
class AyllonAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay ayllon.es + sede espublico + PLAI JCYL + geometría parcial IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-ayllon/1.0")},
        )
    ...
    def _normalize_sede_url(self, url: str) -> str:
        if url.startswith("http"):
            return url
        if url.startswith("/"):
            return f"{self.sede_base}{url}"
        return f"{self.sede_base}/{url}"
    ...
    def _parse_board_table(self, html: str, origen: str) -> list[dict[str, Any]]:
        ...
        for tr in re.findall(r"<tr>(.*?)</tr>", tbody_m.group(1), re.S):
            ...
            if len(cells) < 6:
                ...
                url = self._normalize_sede_url(link_m.group(1))
                rows.append(
                    {
                        "titulo": titulo[:500],
            ...
            doc_url = self._normalize_sede_url(link_m.group(1)) if link_m else self.board_url
            rows.append(
                {
                    "titulo": titulo[:500],
                    "doc_label": doc[:500],
                    ...
                    "url": doc_url,
                    "pdf_url": doc_url if link_m else None,
                    "origen": origen,
                }
            )
        return rows
    ...
    def _parse_board_links(self, html: str, origen: str) -> list[dict[str, Any]]:
        ...
        for m in RE_PREVIEW.finditer(html):
            url = self._normalize_sede_url(m.group(1))
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tramites": sum(1 for r in rows if r.get("origen") in ("catalogo_tramites", "portal_tramite")),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
            "semillas": sum(1 for r in rows if r.get("origen") == "portal_semilla"),
        }
