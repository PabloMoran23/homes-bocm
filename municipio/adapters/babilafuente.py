...
WEB_BASE = "https://babilafuente.es"
SEDE_BASE = "https://babilafuente.sedelectronica.es"
...
RE_PREVIEW = re.compile(
    r'href="((?:https://babilafuente\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://babilafuente\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class BabilafuenteAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress noticias + sede espublico + PlanPublica JCyL + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-babilafuente/1.0")},
        )
