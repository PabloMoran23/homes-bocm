...
WEB_BASE = "https://baltanas.es"
SEDE_BASE = "https://baltanas.sedelectronica.es"
MUNICIPIO = "Baltanás"
ID_PREFIX = "baltanas"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        f"{WEB_BASE}/ayuntamiento/urbanismo-y-vivienda/",
        "Urbanismo y vivienda",
    ),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=34&municipio=18",
        "Planeamiento en información pública (Junta CYL)",
    ),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=34&municipio=18",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        f"{PLAU_BASE}/lplanes.plau?municipio=3402250001701",
        "Planeamientos aprobados definitivamente (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://baltanas\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://baltanas\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_PLAU_ROW = re.compile(
    r'title="([^"]+)"[^>]*>\s*<a[^>]+onclick="openDocumento\((\d+)\)',
    re.I,
)
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|normas urban|"
    ...
    r"sector|edicto|actuaci[oó]n urban|urbanizaci[oó]n|delimitaci[oó]n de suelo|"
    r"instrumento|memoria|planos|bocyl|\bSE[- ]?\d{2}(?:[- ][A-Z])?\b)",
)
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"SE[- ]?\d{2}(?:[- ][A-Z])?|"
    r"S\.?\s*U\.?\s*\.?\s*N\.?\s*C\.?\s*[-.]?\s*(?:SE)?[- ]?\d{2}(?:[- ][A-Z])?"
    r")\b",
)
...
def _normalize_sector_code(raw: str) -> str:
    ...
    code = re.sub(r"SECTOR", "", code)
    if re.fullmatch(r"SE\d{2}[A-Z]?", code):
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "plan parcial" in n or "industrial" in n:
        return "plan parcial"
    if "plan especial" in n or re.search(r"\bse[- ]?\d{2}", n):
        return "sector de planeamiento"
    if "urbanizaci" in n or "actuaci" in n:
        return "actuación urbanística"
    if "delimitaci" in n and "suelo" in n:
...
class BaltanasAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Elementor + sede espublico + PlanPublica JCyL + IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, dossier: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-baltanas/1.0")},
        )
        if dossier:
    ...
    def _ensure_sede_session(self) -> None:
        try:
    ...
    def _collect_plau_docs(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for path, origen in (
            ("searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=34&municipio=18", "plau_jcyl"),
            ("searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=34&municipio=18", "plai_jcyl"),
        ):
            url = f"{PLAU_BASE}/{path}"
            try:
                html = self._fetch(url)
            except (urllib.error.URLError, OSError, ConnectionError):
                continue
            for m in re.finditer(
                r'title="([^"]+)"[^>]*onclick="openDocumento\((\d+)\)"',
                html,
                re.I,
            ):
                titulo = unescape(m.group(1)).strip()
                if titulo.lower() in {"abrir documento", "listado de ficheros", "vista de impresión"}:
                    continue
                doc_url = f"{PLAU_BASE}/openDocumento.do?cDocId={m.group(2)}"
                if doc_url in seen:
                    continue
                seen.add(doc_url)
                rows.append(
            ...
            for m in re.finditer(r'title="([^"]+)"[^>]*>\s*<a[^>]+cDocId=(\d+)', html, re.I):
                titulo = unescape(m.group(1)).strip()
                if titulo.lower() in {"abrir documento", "listado de ficheros"}:
                    continue
                doc_url = f"{PLAU_BASE}/openDocumento.do?cDocId={m.group(2)}"
                if doc_url in seen:
                    continue
        ...
        for url, fallback_title in self.seed_urls:
            ...
            try:
                ...
                if title_m:
                    ...
                    titulo = re.sub(r"\s*[-|]\s*(?:Ayuntamiento de )?Baltan[aá]s.*$", "", titulo, flags=re.I).strip()
            except (urllib.error.URLError, OSError, ConnectionError):
                pass
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_wfs_proyectos():
            add(item)
        for item in self._collect_plau_docs():
        ...
        return {
            ...
            "plau": sum(1 for r in rows if str(r.get("origen") or "").startswith("plau")),
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
        }
