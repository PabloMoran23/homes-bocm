...
WP_BASE = "https://barlovento.es"
SEDE_BASE = "https://barlovento.sedelectronica.es"
MUNICIPIO = "Barlovento"
ID_PREFIX = "barlo"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-barlovento"
GEOBDP_MUNICIPIO = "https://geobdp.grafcan.es/core/municipios/38007/"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/anuncios/",
    f"{WP_BASE}/anuncios/ayudas-rehabilitacion-vivienda-privada/",
    f"{WP_BASE}/anuncios/asfalto-llanadas-de-bona/",
    f"{WP_BASE}/noticias/barlovento-licita-la-obra-de-urbanizacion-entre-la-calle-el-drago-y-la-transversal-de-la-calle-la-constitucion/",
    f"{WP_BASE}/noticias/el-ayuntamiento-de-barlovento-impulsa-el-desarrollo-del-suelo-urbano-de-la-fajana/",
    f"{WP_BASE}/noticias/barlovento-comienza-las-obras-de-renovacion-del-alumbrado-en-su-trama-urbana/",
    f"{WP_BASE}/noticias/barlovento-finaliza-la-obra-de-renovacion-del-alumbrado-publico-de-su-trama-urbana/",
    f"{WP_BASE}/noticias/comienzan-las-obras-de-rehabilitacion-de-la-montana-de-la-centinela-y-las-fuentes-de-la-fajana-la-faya-felipe-y-la-gallega/",
    f"{WP_BASE}/noticias/barlovento-contrata-el-proyecto-de-rehabilitacion-del-canal-de-la-galeria-de-los-roques-de-los-arboles/",
    f"{WP_BASE}/noticias/bases-reguladoras-por-rehabilitacion-de-viviendas-de-titularidad-privada-en-el-medio-rural-del-plan-vivienda-de-canarias-2020-2025/",
]

RE_PREVIEW = re.compile(
    r'href="(https://barlovento\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_EXCLUDE = re.compile(
    ...
    r"iae|cobranza|calendario fiscal|selecci[oó]n de personal|tribunal|"
    r"convocatoria pleno|incendio forestal|bolsa de trabajo|bop n|"
    r"licitaci[oó]n.*ludoteca|contrataciones)",
)
...
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?barlovento\.es)?/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
class BarloventoAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress + sede espublico gestiona + SITCAN CKAN + GEOBDP Grafcan (Canarias)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-barlovento/1.0")},
        )
    ...
    def _parse_board_table(self, html: str) -> list[dict[str, Any]]:
        ...
        for tr in re.findall(r"<tr>(.*?)</tr>", tbody_m.group(1), re.S):
            ...
            blob = f"{titulo} {expediente} {procedimiento} {descripcion}"
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Tablón de anuncios — sede electrónica Barlovento",
                "url": self.board_url,
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        rows = self._dedupe(raw)
        self._write_jsonl(out_jsonl, rows)
        return {"rows": len(rows), "status": "ok", "source": "wordpress_sitcan_geobdp_board"}
