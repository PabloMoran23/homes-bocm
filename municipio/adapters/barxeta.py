...
SEDE_BASE = "https://barxeta.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WEB_BASE = "https://www.barxeta.es"
PGOU_URL = f"{WEB_BASE}/ca/pagina/pla-general-dordenacio-urbana"
...
RE_PROYECTO = re.compile(
    ...
    r"cambio de uso|normas subsidiarias|homologaci[oó]n|ordenanza|e[oó]lic|iate|parcell)",
)
RE_BOARD_NON_URBAN = re.compile(
    ...
    r"ayudas a entidades culturales|lloguer jove|alumnat|inventari municipal)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://barxeta\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_PGOU_PDF = re.compile(
    r'href="((?:https://www\.barxeta\.es)?/sites/www\.barxeta\.es/files/[^"]+\.pdf)"[^>]*>([^<]+)',
    re.I,
)
...
class BarxetaAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal Digital Value (portales) + sede espublico gestiona + ICV WFS zonificación (partial)."""
    ...
    def _fetch(self, url: str, *, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-barxeta/1.0")},
        )
    ...
    def _match_icv_zone(self, titulo: str) -> dict[str, Any] | None:
        ...
        for zone in self._collect_icv_zones():
            ...
            if "PLAN GENERAL" in norm and "PLAN GENERAL" in den:
                score += 20.0
            if "PGOU" in norm and ("PLAN GENERAL" in den or "PGOU" in den):
                score += 15.0
    ...
    def _enrich_geometry(self, rec: dict[str, Any], *, licencia: bool = False) -> None:
        ...
        if rec.get("origen") == "tablon" and licencia:
            if not re.search(r"(?i)pol[ií]gono|parcela|sector|diseminad|parcell", rec.get("titulo") or ""):
                return
    ...
    def _collect_pgou_pdfs(self) -> list[dict[str, Any]]:
        ...
        for m in RE_PGOU_PDF.finditer(html):
            ...
            titulo = label or "Documento PGOU Barxeta"
            tipo = "PGOU"
            if re.search(r"(?i)memoria", titulo):
                tipo = "memoria PGOU"
            elif re.search(r"(?i)normativa|cat[aà]leg", titulo):
                tipo = "normativa PGOU"
            rec: dict[str, Any] = {
                "id": _stable_id("proy", url),
                "municipio": MUNICIPIO,
                "titulo": titulo[:500],
                "fecha": "2007-09-13" if "R070913" in url else None,
                "tipo": tipo,
                "url": url,
                "source": "ayuntamiento",
                "origen": "web_pgou",
        ...
        return [
            {
                ...
                "municipio": MUNICIPIO,
                "titulo": "Pla General d'Ordenació Urbana — web municipal",
                "fecha": None,
                "tipo": "PGOU",
                "url": self.pgou_url,
                "source": "ayuntamiento",
                "origen": "web",
                "nota": "PGOU suspendido (R070913): catálogo, normativa y memoria PDF",
            },
            {
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if re.search(r"(?i)planeamiento|homologaci[oó]n|sector|plan parcial", blob):
            tipo = "planeamiento"
        elif re.search(r"(?i)informaci[oó]n p[uú]blica|iate|dogv|parcell", blob):
