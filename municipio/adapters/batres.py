...
WEB_BASE = "https://www.batres.es"
SEDE_BASE = "https://batres.sedelectronica.es"
MUNICIPIO = "Batres"
ID_PREFIX = "batres"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "BATRES"

NORMAS_URL = f"{WEB_BASE}/ayuntamiento/normativa/normas-subsidiarias"
TRAMITES_URL = f"{WEB_BASE}/tramites/urbanismo"
DOCS_URL = f"{WEB_BASE}/areas-municipales/urbanismo-e-infraestructuras/documentos-urbanisticos"
BOARD_URL = f"{SEDE_BASE}/board"

RE_PREVIEW = re.compile(
    r'href="(https://batres\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"\b(?:APD|UE|UA|AD|AN|AI|PAU|SAU|S)-\d+[A-Z0-9-]*\b)",
)
...
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:APD|UE|UA|AD|AN|AI|PAU|SAU|S)-\d+[A-Z0-9-]*)\b",
)
...
def _sector_ilike_parts(text: str) -> list[str]:
    s = text.strip()
    low = s.lower()
    for marker in (" del pgou", " pgou", " bocm", " aprob", " anuncio", " nnss", " batres", " cotorredondo", " montebatres"):
        if marker in low:
...
class BatresAyuntamientoAdapter(AyuntamientoAdapter):
    """Joomla web (NNSS + trámites PDF) + sede espublico (tablón) + WFS SITCM (geometría partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.docs_url = str(self.config.get("docs_url") or DOCS_URL)
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-batres/1.0")},
        )
    ...
    def _scrape_pdf_links(self, page_url: str, origen: str) -> list[dict[str, Any]]:
        try:
        ...
        for href, label_raw in RE_PDF_LINK.findall(html):
            ...
            pdf_url = urllib.parse.urljoin(f"{self.web_base}/", unescape(href))
            if pdf_url in seen:
                continue
            seen.add(pdf_url)
            titulo = label if len(label) > 3 else Path(unescape(href)).name
            rows.append(
                {
                    "titulo": titulo[:500],
    ...
    def _collect_tramites_pdfs(self) -> list[dict[str, Any]]:
        return self._scrape_pdf_links(self.tramites_url, "tramites_web")

    def _collect_nnss_pdfs(self) -> list[dict[str, Any]]:
        return self._scrape_pdf_links(self.normas_url, "nnss_pdf")
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "url": self.normas_url,
                "source": "ayuntamiento",
                "nota": "Documentación NNSS y modificaciones puntuales",
                "origen": "normas_web",
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Trámites y formularios de urbanismo (licencias y declaraciones)",
                "url": self.tramites_url,
                "source": "ayuntamiento",
                "nota": "Solicitudes licencia obra, declaraciones responsables, primera ocupación, etc.",
                "origen": "tramites_index",
            },
            {
                "id": _stable_id("lic", self.docs_url),
                "fecha_concesion": None,
                "tipo": "documentos urbanísticos",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Documentos urbanísticos del ayuntamiento",
                "url": self.docs_url,
                "source": "ayuntamiento",
                "nota": "Sección urbanismo web municipal",
                "origen": "docs_web",
            },
        ]
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_board():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_tramites_pdfs():
        ...
        return {
            ...
            "info": sum(
                1
                for r in rows
                if r.get("origen") in ("sede_tablon", "normas_web", "tramites_index", "docs_web")
            ),
        }
