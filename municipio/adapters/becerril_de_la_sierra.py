...
WEB_BASE = "https://www.becerrildelasierra.org"
SEDE_BASE = "https://becerrildelasierra.sedelectronica.es"
MUNICIPIO = "Becerril de la Sierra"
ID_PREFIX = "becerril-de-la-sierra"

URBANISMO_URL = f"{WEB_BASE}/index.php/servicios-municipales/109-urbanismo-y-licencias"
BOARD_URL = f"{SEDE_BASE}/board/974e6d5e-f59b-11de-b600-00237da12c6a/"
CM_BOCM_PDF = "https://www.comunidad.madrid/sites/default/files/doc/urbanismo/becerril.pdf"
TRANSPARENCY_ORDENANZAS = f"{SEDE_BASE}/transparency/79a1a35e-9003-4d01-b51c-385b7885fc3c/"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "BECERRIL DE LA SIERRA"

RE_PREVIEW = re.compile(
    r'href="(https://becerrildelasierra\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://becerrildelasierra\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_JOOMLA_DL = re.compile(
    r'<a[^>]+href="([^"]*(?:download\.send|/descargas/send/)[^"]*)"[^>]*>([^<]{4,300})</a>',
    re.I | re.S,
)
...
RE_PROYECTO = re.compile(
    ...
    r"\b(?:P|UE|UA|AD|AN|AI|PAU|S)-\d+\b)",
)
RE_EXCLUDE = re.compile(
    ...
    r"tasa de agua|cobro de la tasa|barbacoa|incendios forestales|convocatoria pleno)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:P|UE|UA|AD|AN|AI|PAU|S)-\d+[A-Z0-9-]*)\b",
)
...
def _proyecto_tipo(title: str) -> str:
    n = title.lower()
    if re.search(r"\bp-\d+\b", n) or re.search(r"\bua-\d+\b", n):
        return "plan parcial / unidad de actuación"
    ...
    if "desbroce" in n or "parcela" in n:
...
class BecerrilDeLaSierraAyuntamientoAdapter(AyuntamientoAdapter):
    """Joomla web + sede espublico gestiona + ámbitos SITCM WFS (geometría partial)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-becerril-de-la-sierra/1.0")},
        )
    ...
    def _collect_board(self) -> list[dict[str, Any]]:
        ...
        for url, origen in (
            ...
            (f"{self.sede_base}/board/", "tablon_general"),
            (f"{self.sede_base}/info.0", "info_tablon"),
        ):
            try:
    ...
    def _collect_web_urbanismo(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            html = ""
        for m in RE_JOOMLA_DL.finditer(html):
            url = _abs_url(WEB_BASE, m.group(1))
            titulo = _strip_html(m.group(2))
            if url in seen or len(titulo) < 4:
                continue
            seen.add(url)
            rows.append(
                {
        ...
        rows.append(
            {
                "titulo": "Normativa urbanística Becerril de la Sierra (BOCM Comunidad de Madrid)",
                "url": CM_BOCM_PDF,
                "origen": "bocm_cm_pdf",
            }
        )
    ...
    def _collect_transparency_docs(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for url in (TRANSPARENCY_ORDENANZAS, self.transparency_url):
            try:
                html = self._fetch(url)
            except urllib.error.URLError:
                continue
            for m in RE_PREVIEW.finditer(html):
                ...
                if doc_url in seen:
                    continue
                seen.add(doc_url)
                local = html[max(0, m.start() - 200) : m.end() + 200]
                text_m = re.search(r">([^<]{5,300})</a>", local, re.I)
                titulo = unescape(text_m.group(1).strip()) if text_m else doc_url
                rows.append(
                    {
                        "titulo": titulo[:500],
                        "url": doc_url,
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo y licencias — modelos y normativa",
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
                "nota": "Modelos licencia/DR y normativa en web Joomla",
    ...
    def _web_to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        return {
            ...
            "url": row["url"],
            "source": "ayuntamiento",
            "nota": "Modelo o normativa de licencia en web municipal",
            "origen": row.get("origen"),
        }
    ...
    def _web_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        if not RE_PROYECTO.search(row["titulo"]):
            return None
        if RE_LICENCIA.search(row["titulo"]) and not re.search(
            r"(?i)normas|nnss|ordenanza|planeam|urban|bocm",
            row["titulo"],
        ):
            return None
        rec: dict[str, Any] = {
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            "rows": len(rows),
            "status": "ok",
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon", "tablon_general")),
            "tramites": sum(1 for r in rows if r.get("origen") == "catalogo_tramites"),
            "web": sum(1 for r in rows if r.get("origen") in ("web_joomla", "web_urbanismo")),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "web": sum(1 for r in rows if r.get("origen") in ("web_joomla", "bocm_cm_pdf")),
