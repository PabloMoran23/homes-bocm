...
SEDE_BASE = "https://aytobembibre.sedelectronica.es"
...
RE_PREVIEW = re.compile(
    r'href="(https://aytobembibre\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://aytobembibre\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"P[- ]?I[- ]?\d+|PE[- ]?\d+|AA[- ]?\d+[A-Z]?|"
    r"U\s*\d+\s*BIS|U\s*\d+|"
    r"S\.?\s*U\.?\s*\.?\s*R\.?\s*[-.]?\s*\d+|SUR[- ]?\d+|"
...
class BembibreAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=014"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=014"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-bembibre/1.0")},
        )
