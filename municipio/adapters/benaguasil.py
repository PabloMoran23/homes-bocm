...
WEB_BASE = "https://www.benaguasil.es"
SEDE_BASE = "https://benaguasil.sede.dival.es"
MUNICIPIO = "Benaguasil"
ID_PREFIX = "benaguasil"
INE_COD_MUN = "46051"
...
PLANEAMIENTO_URL = f"{WEB_BASE}/es/pagina/planeamiento-municipal"
...
DEFAULT_PLANEAMIENTO_SEEDS: tuple[str, ...] = (
    PLANEAMIENTO_URL,
    f"{WEB_BASE}/sites/www.benaguasil.es/files/wp-content/uploads/2023/01/0-Indice-Planos.pdf",
)
...
RE_PROYECTO = re.compile(
    ...
    r"normas subsidiarias|homologaci[oó]n|moli nou|sur-r10|alineaci|zonificaci)",
)
RE_TABLON_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|nomenament|convocatoria.*empleo|"
    r"polic[ií]a local|nataci[oó]n|jurado|concejal|junta de gobierno|teniente de alcalde|"
    r"portavoz|delegaci[oó]n.*gobierno|comisi[oó]n informativa|bolsa auxiliar)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_DOC_LINK = re.compile(
    r'href="((?:https://benaguasil\.sede\.dival\.es)?/tablondeanuncios/documento\.aspx\?id=\d+[^"]*)"',
    re.I,
)
RE_PDF = re.compile(r'href="([^"]+\.pdf[^"]*)"', re.I)
RE_H1 = re.compile(r"<h1[^>]*>([^<]+)</h1>", re.I)
RE_TRAMITE_URB = re.compile(
    r'href="(https://benaguasil\.sede\.dival\.es/carpetaciudadana/tramite\.aspx\?idtramite=\d+)"[^>]*>\s*(URB_\d+[^<]+)',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    n = (title or "").lower()
    if "plan parcial" in n or "sector" in n or "moli nou" in n or "sur-r10" in n:
        return "plan parcial"
    if "normas subsidiarias" in n:
        return "normas subsidiarias"
    if "homologaci" in n:
        return "homologación planeamiento"
    if "plan general" in n or "pgou" in n or "planeamiento municipal" in n:
        return "PGOU"
    if "zonificaci" in n or "alineaci" in n:
        return "documentación gráfica PGOU"
    if "ordenanza" in n:
...
class BenaguasilAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal www.benaguasil.es + sede Dival (Sedipualba) tablón RSS + ICV GVA WFS partial."""
    ...
    def _fetch(self, url: str, *, timeout: int = 90, retries: int = 3) -> str:
        ua = self.config.get("user_agent", "poc-bocm-benaguasil/1.0")
    ...
    def _fetch_bytes(self, url: str, *, timeout: int = 120) -> bytes:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-benaguasil/1.0")},
        )
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            return resp.read()
    ...
    def _collect_tablon_rss(self) -> list[dict[str, Any]]:
        try:
            raw = self._fetch(self.tablon_rss, timeout=60, retries=2)
    ...
    def _enrich_anuncio_docs(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        if "anuncio.aspx" not in url:
            return row
        try:
            html = self._fetch(url, timeout=60, retries=2)
        except urllib.error.URLError:
            return row
    ...
    def _fetch_geometry(self, titulo: str) -> dict[str, Any] | None:
        ...
        for feat in feats:
            ...
            for k in keys:
                if k in label or k in title_low:
                    score += 1.0
            if "moli nou" in title_low and "moli nou" in label:
                score += 2.0
            if "sur-r10" in title_low or "sur r10" in title_low:
                if "sur-r10" in label or "sur r10" in label:
                    score += 2.0
            if score <= 0:
                continue
        ...
        label_low = (best_feat.get("label") or "").lower()
        if "moli nou" in title_low:
            matched = [f["geom"] for f in feats if "moli nou" in (f.get("label") or "").lower()]
        elif "sur-r10" in title_low or "sur r10" in title_low:
            matched = [
                f["geom"]
                for f in feats
                if "sur-r10" in (f.get("label") or "").lower() or "sur r10" in label_low
            ]
        elif "plan general" in title_low or "pgou" in title_low or "planeamiento" in title_low:
            matched = [f["geom"] for f in feats if f.get("geom")]
        ...
        rows: list[dict[str, Any]] = [
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Tablón de anuncios — sede electrónica Benaguasil",
                "url": self.tablon_url,
                "source": "ayuntamiento",
                "nota": "Edictos y licencias publicados en benaguasil.sede.dival.es",
                "origen": "sede_tablon",
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Catálogo de trámites — urbanismo (URB_*)",
                "url": self.catalogo_url,
                "source": "ayuntamiento",
                "nota": "Licencias mayor/menor, DR obras, primera ocupación vía sede (sin histórico)",
                "origen": "sede_tramite",
            },
            {
                "id": _stable_id("lic", self.planeamiento_url),
                "fecha_concesion": None,
                "tipo": "planeamiento municipal",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Planeamiento municipal — web ayuntamiento",
                "url": self.planeamiento_url,
                "source": "ayuntamiento",
                "nota": "Consulta gratuita circunstancias urbanísticas (URB_013) en web municipal",
                "origen": "web_tramite",
            },
        ]

        try:
            html = self._fetch(self.catalogo_url, timeout=60, retries=2)
        except urllib.error.URLError:
            return rows

        seen_urls: set[str] = set()
        for m in RE_TRAMITE_URB.finditer(html):
            url, label = m.group(1), _strip_html(m.group(2))
            if url in seen_urls:
                continue
            seen_urls.add(url)
            if not RE_LICENCIA.search(label) and "URB_0" not in label.upper():
                continue
            rows.append(
                {
                    "id": _stable_id("lic", url),
                    "fecha_concesion": None,
                    "tipo": _strip_html(label.split("-", 1)[-1] if "-" in label else label)[:120],
                    "distrito": None,
                    "lat": None,
                    "lon": None,
                    "titulo": label[:500],
                    "url": url,
                    "source": "ayuntamiento",
                    "nota": "Trámite informativo sede Dival",
                    "origen": "sede_tramite",
                }
            )
        return rows
    ...
    def _collect_web_planeamiento(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        ...
        for seed in self.planeamiento_seeds:
            ...
            seen.add(seed)
            try:
                html = self._fetch(seed, timeout=45, retries=2)
            except urllib.error.URLError:
                if seed == self.planeamiento_url:
                    rec: dict[str, Any] = {
                        "id": _stable_id("proy", seed),
                        "municipio": MUNICIPIO,
                        "titulo": "Planeamiento municipal — texto consolidado PGOU Benaguasil",
                        "fecha": "2025-03-04",
                        "tipo": "PGOU",
                        "url": seed,
                        "source": "ayuntamiento",
            ...
            titulo = _strip_html(h1.group(1)) if h1 else "Planeamiento municipal"
            fecha = _parse_fecha_dmy(html[:8000]) or "2025-03-04"
            ...
            self._enrich_geometry(page_rec)
            rows.append(page_rec)
            ...
            for m in RE_PDF.finditer(html):
                ...
                if pdf_url in seen:
                    continue
                seen.add(pdf_url)
                pdf_name = unescape(pdf_url.split("/")[-1].replace(".pdf", "").replace("-", " "))
                pdf_rec: dict[str, Any] = {
                    "id": _stable_id("proy", pdf_url),
                    "municipio": MUNICIPIO,
                    "titulo": f"{titulo} — {pdf_name}"[:500],
                    "fecha": fecha,
                    "tipo": _proyecto_tipo(pdf_name),
                    "url": pdf_url,
                    "source": "ayuntamiento",
                    "origen": "web_pdf",
                    "pdf_url": pdf_url,
                }
                self._enrich_geometry(pdf_rec)
                rows.append(pdf_rec)
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for web in self._collect_web_planeamiento():
            add(web)
