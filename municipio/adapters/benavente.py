...
WP_BASE = "https://benavente.es"
SEDE_BASE = "https://aytobenavente.org"
MUNICIPIO = "Benavente"
ID_PREFIX = "benavente"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        f"{WP_BASE}/areas-municipales/urbanismo-infraestructuras-obras-y-servicios/",
        "Urbanismo — planeamiento y documentación PGOU",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=49&municipio=21",
        "Planeamiento en información pública (Junta CYL — Zamora)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=49&municipio=21",
        "Archivo planeamiento urbanístico aprobado (Junta CYL — Zamora)",
    ),
    (f"{WP_BASE}/tramites/", "Trámites municipales"),
    (f"{WP_BASE}/actualidad/anuncios/", "Tablón de anuncios municipal"),
]

RE_PREVIEW = re.compile(
    r'href="((?:https://aytobenavente\.org)?/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban|uso)|obra mayor|obra menor|"
    r"primera ocupaci[oó]n|primera utilizaci[oó]n|memoria.*autorizaci)",
)
RE_PROYECTO = re.compile(
    ...
    r"instrumento|memoria|planos|bocyl|normas urban|delimitaci[oó]n|"
    r"\bS[-.]?\d+[A-Z]*\b|ficha\s+\d+)",
)
...
RE_EXCLUDE = re.compile(
    ...
    r"selecci[oó]n de personal|tribunal|convocatoria pleno|bolsa de empleo|"
    r"beca|becas|ayuda.*luz|ayuda.*gas|bopza|chofer|administrativo|auxiliar|"
    r"electricista|oficial\s*1|fiestas|cine|deporte|empadron)",
)
...
RE_WP_EXCLUDE = re.compile(
    ...
    r"subvenci[oó]n deportiv|violencia de g[eé]nero|turismo activo|bolsa)",
)
...
class BenaventeAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress benavente.es + IDECyL WFS (sectores PGOU) + tablón anuncios Elementor."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.anuncios_url = str(self.config.get("anuncios_url") or f"{WP_BASE}/actualidad/anuncios/")
        self.anuncios_max_pages = int(self.config.get("anuncios_max_pages", 12))
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-benavente/1.0")},
        )
    ...
    def _wfs_sector_geometry(self, sector_code: str) -> tuple[dict[str, Any] | None, str | None]:
        ...
        mun_esc = self.wfs_municipio.replace("'", "''")
        cql = f"n_mun='{mun_esc}' AND n_num_sect ILIKE '%{escaped}%'"
            ...
            rows.append(
                {
                    ...
                    "descripcion": titulo,
                    "fecha": _parse_fecha_dmy(_strip_html(cells[-1])),
                    "url": url,
                    "pdf_url": url,
                    "origen": origen,
                }
    ...
    def _collect_board(self) -> list[dict[str, Any]]:
        ...
        for url, origen in ((self.board_url, "tablon"), (f"{self.sede_base}/info", "info_tablon")):
            try:
                ...
                continue
            if "sede electrónica indeterminada" in html.lower():
                continue
    ...
    def _collect_anuncios(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for page_num in range(1, self.anuncios_max_pages + 1):
            url = self.anuncios_url if page_num == 1 else f"{self.anuncios_url.rstrip('/')}/{page_num}/"
            try:
                html = self._fetch(url)
            except (urllib.error.URLError, OSError, TimeoutError):
                break
            parts = re.split(RE_ELEMENTOR_H1.pattern, html, flags=re.S | re.I)
            found = 0
            for i in range(1, len(parts), 2):
                titulo = _strip_html(parts[i])
                if not titulo or titulo.lower() == "anuncios":
                    continue
                block = parts[i + 1] if i + 1 < len(parts) else ""
                pdfs = [
                    self._abs_url(m)
                    for m in RE_PDF_HREF.findall(block)
                    if "benavente.es" in m or m.startswith("/wp-content")
                ]
                pdf_url = pdfs[0] if pdfs else None
                key = pdf_url or titulo
                if key in seen:
                    continue
                seen.add(key)
                found += 1
                rows.append(
                    {
                        ...
                        "url": pdf_url or url,
                        "pdf_url": pdf_url,
                        "fecha": _fecha_from_blob(f"{titulo} {pdf_url or ''}"),
                        "origen": "wp_anuncios",
                    }
                )
    ...
    def _collect_seed_pdfs(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        urls = [u for u, _ in self.seed_urls if "benavente.es" in u or "jcyl.es" in u]
        for page_url in urls:
            ...
            for m in RE_PDF_HREF.finditer(html):
                pdf = self._abs_url(m.group(1), base=page_url)
                if pdf in seen:
                    continue
                seen.add(pdf)
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            (self.anuncios_url, "tablón de anuncios municipal"),
            (f"{WP_BASE}/areas-municipales/urbanismo-infraestructuras-obras-y-servicios/", "urbanismo — documentación y trámites"),
            (f"{WP_BASE}/tramites/", "trámites municipales"),
            (self.board_url, "sede electrónica — tablón de anuncios"),
        ]
        rows: list[dict[str, Any]] = []
    ...
    def _row_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        blob = " ".join(
            str(row.get(k) or "")
            for k in ("titulo", "procedimiento", "categoria", "descripcion", "pdf_url")
        )
        if RE_EXCLUDE.search(blob) and not RE_PROYECTO.search(blob):
            return None
        ...
        if not RE_PROYECTO.search(blob):
            return None
        key = row.get("pdf_url") or row.get("expediente") or row.get("url") or row["titulo"]
        rec: dict[str, Any] = {
            ...
            "url": row.get("url") or row.get("pdf_url") or f"{WP_BASE}/areas-municipales/urbanismo-infraestructuras-obras-y-servicios/",
            "source": "ayuntamiento",
            "origen": row.get("origen"),
        }
        if row.get("pdf_url"):
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_anuncios():
            rec = self._anuncio_to_licencia(item)
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        ...
        return {
            "rows": len(rows),
            "status": "ok",
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon", "wp_anuncios")),
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_seed_pages():
            rec = self._seed_to_proyecto(item)
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        self._write_jsonl(out_jsonl, rows)
