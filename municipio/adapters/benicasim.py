...
WP_BASE = "https://ayto.benicassim.es"
SEDE_BASE = "https://benicassim.sedelectronica.es"
MUNICIPIO = "Benicasim"
ID_PREFIX = "benicasim"
INE_MUNICIPIO = "12018"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/urbanismo/plan-general-de-ordenacion-urbana/",
    f"{WP_BASE}/planeamiento/",
    f"{WP_BASE}/urbanismo/",
    f"{WP_BASE}/dms/ayuntamiento/urbanisme/PGOU/MapaOS/PGOU2000.html",
    f"{WP_BASE}/dms/ayuntamiento/urbanisme/PGOU/MapaOS/PGOU5000.html",
]
...
RE_BOARD_NON_URBAN = re.compile(
    ...
    r"declaraci[oó]n de bienes|notoriedad|convocatoria.*pleno|juez de paz|bop n)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://benicassim\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
class BenicasimAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress ayto.benicassim.es + sede espublico gestiona + ICV WFS InventarioSuSuz."""
    ...
    def _fetch(self, url: str, *, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-benicasim/1.0")},
        )
    ...
    def _wfs_query_url(self, ambito_id: str) -> str:
        ...
        return f"{self.wfs_base}?{params}#id={ambito_id}"
        ...
        for page_url in self.seed_pages:
            ...
            if page_url.endswith(".html"):
                if page_url not in seen:
                    ...
                    rows.append(
                        {
                            "titulo": f"{page_title} — visor PGOU municipal",
                            "fecha": None,
                            "url": page_url,
                            "page_url": page_url,
                            "blob": f"{page_title} PGOU mapa",
                            "origen": "pgou_mapa",
                        }
                    )
            for m in RE_PDF_HREF.finditer(html):
                ...
                if doc_url in seen or "favicon" in doc_url.lower():
                    continue
                seen.add(doc_url)
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "id": _stable_id("lic", f"{self.sede_base}/dossier.8"),
                "fecha_concesion": None,
                ...
                "url": f"{self.sede_base}/dossier.8",
                "source": "ayuntamiento",
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo — trámites y licencias",
                "url": f"{WP_BASE}/urbanismo/",
                "source": "ayuntamiento",
                "nota": "Información sobre licencias de obra, actividad y consultas urbanísticas",
                "origen": "web_tramite",
            },
        ]
    ...
    def _seed_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        rec = {
            ...
            "url": row["url"],
            "source": "ayuntamiento",
            "origen": row.get("origen") or "wp_pgou",
        }
        self._attach_geometry(rec)
        return rec
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "wp_pgou": sum(1 for r in rows if r.get("origen") in ("wp_pgou", "pgou_mapa")),
            "tablon": sum(1 for r in rows if r.get("origen") == "tablon"),
