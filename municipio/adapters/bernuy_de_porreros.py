...
DIPSEGOVIA_BASE = "https://www.dipsegovia.es/web/ayuntamiento-de-bernuy-de-porreros"
SEDE_BASE = "https://bernuydeporreros.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=031",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=031",
...
RE_PREVIEW = re.compile(
    r'href="(https://bernuydeporreros\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://bernuydeporreros\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class BernuyDePorrerosAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-bernuy-de-porreros/1.0")},
        )
