...
WP_BASE = "https://bonilladelasierra.com"
SEDE_BASE = "https://bonilladelasierra.sedelectronica.es"
...
ID_PREFIX = "bonilla-de-la-sierra"
...
RE_PREVIEW = re.compile(
    r'href="(https://bonilladelasierra\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://bonilladelasierra\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class BonillaDeLaSierraAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress corporativo + sede espublico gestiona + PlanPublica PLAU + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=05&municipio=038"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=05&municipio=038"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-bonilla-de-la-sierra/1.0")},
        )
