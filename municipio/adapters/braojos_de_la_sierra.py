...
JOOMLA_BASE = "https://braojos.org"
SEDE_BASE = "https://braojos.sedelectronica.es"
MUNICIPIO = "Braojos de la Sierra"
ID_PREFIX = "braojos-de-la-sierra"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "BRAOJOS"

AVANCE_PGOU_URL = f"{JOOMLA_BASE}/tu-ayuntamiento/normativa-municipal/avance-plan-general"
PLANEAMIENTO_URL = f"{JOOMLA_BASE}/tu-ayuntamiento/normativa-municipal/planeamiento-urbanistico"
...
LICENCIAS_URL = f"{JOOMLA_BASE}/ciudadanos/tramites-personales/instancias-licencias-y-solicitudes"
TRANSPARENCIA_URL = "http://transparencia.braojos.org/"
VISOR_URL = "https://idem.comunidad.madrid/cartografia/sitcm/html/visor.htm?municipio=024"
BOARD_URL = f"{SEDE_BASE}/board/"
TRANSPARENCY_SEDE_URL = f"{SEDE_BASE}/transparency/"

TABLON_RSS = f"{TABLON_URL}?format=feed&type=rss"
BANDOS_RSS = f"{JOOMLA_BASE}/tu-ayuntamiento/alcaldia/bandos-de-alcaldia?format=feed&type=rss"

RE_PREVIEW = re.compile(
    r'href="(https://braojos\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|avpgou|nnss|normas subsidiarias|"
    ...
    r"urbanizaci[oó]n|suelo|consulta urban|rehabilitaci[oó]n de vivienda|"
...
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?braojos\.org)?/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    n = title.lower()
    if "pgou" in n or "plan general" in n or "avpgou" in n:
        return "plan general"
...
class BraojosDeLaSierraAyuntamientoAdapter(AyuntamientoAdapter):
    """Joomla Helix braojos.org + sede espublico gestiona + SITCM WFS (geometría partial)."""
    ...
    def _collect_pgou_avance(self) -> list[dict[str, Any]]:
        rows = self._collect_page_pdfs(self.avance_pgou_url, "joomla_pgou_pdf")
        if not rows:
            return []
        out = [
            {
                "titulo": "Avance Plan General de Ordenación Urbana — Braojos de la Sierra",
                "fecha": "2023-09-22",
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Planeamiento urbanístico — visor SIT Comunidad de Madrid",
                "url": self.planeamiento_url,
                "source": "ayuntamiento",
                "nota": "Enlace al visor SITCM municipio 024",
                "origen": "joomla_planeamiento",
            },
            {
                "id": _stable_id("lic", self.licencias_url),
                "fecha_concesion": None,
                "tipo": "instancias y modelos licencia",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Instancias, licencias y solicitudes",
                "url": self.licencias_url,
                "source": "ayuntamiento",
                "nota": "Modelos obra mayor/menor, declaración responsable, vallado",
            ...
            },
            {
                ...
                "source": "ayuntamiento",
                "nota": "Documentación administrativa (dominio transparencia.braojos.org)",
    ...
    def _collect_licencia_forms(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for item in self._collect_page_pdfs(self.licencias_url, "joomla_form"):
            titulo = item["titulo"]
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "info": sum(
                ...
                for r in rows
                if r.get("origen")
                in ("sede_tablon", "joomla_planeamiento", "transparencia", "sede_tramite", "joomla_tramite")
            ),
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_pgou_avance():
            add(self._item_to_proyecto(item))
        for item in self._collect_ordenanza_articles():
            add(self._item_to_proyecto(item))
        for item in self._parse_rss(BANDOS_RSS, "joomla_rss_bandos"):
            add(self._item_to_proyecto(item))
        for item in self._parse_rss(TABLON_RSS, "joomla_rss_tablon"):
            add(self._item_to_proyecto(item))
        for item in self._collect_page_pdfs(self.avance_pgou_url, "joomla_pgou_pdf"):
            add(self._item_to_proyecto(item))
        for item in self._parse_board():
