...
WEB_BASE = "https://briviesca.es"
SEDE_BASE = "https://briviesca.sedelectronica.es"
MUNICIPIO = "Briviesca"
ID_PREFIX = "briviesca"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=09&municipio=056",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=056",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/openDocuIndice.do?cDocId=299634",
        "PGOU Revisión — índice documental (Junta CYL)",
    ),
]

DEFAULT_DRUPAL_PAGES: list[tuple[str, str]] = [
    (f"{WEB_BASE}/planeamientourbanistico", "Planeamiento urbanístico vigente"),
    (f"{WEB_BASE}/plangeneral2023", "PGOU 2023 en tramitación"),
    (f"{WEB_BASE}/planespecial", "Plan Especial de Protección del Conjunto Histórico"),
    (f"{WEB_BASE}/inspecciontecnica", "Inspección Técnica de Edificios"),
]

DEFAULT_LICENCIA_PAGES: list[tuple[str, str]] = [
    (f"{SEDE_BASE}/board", "tablón de anuncios — licencias y urbanismo"),
    (f"{SEDE_BASE}/info.0", "catálogo de trámites — sede electrónica"),
]

RE_PREVIEW = re.compile(
    r'href="(https://briviesca\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?briviesca\.es)?/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"ordenanza.*urban|tasa.*urban|instrumento|memoria|planos|bocyl|ite\b|"
    r"protecci[oó]n.*hist[oó]ric|normas urban|\bue[-\s]?\d)",
)
RE_EXCLUDE = re.compile(
    ...
    r"electores|icio\b|ordenanza fiscal|transporte)",
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "plan especial" in n or "conjunto hist" in n:
        return "plan especial"
    if "pgou" in n or "plan general" in n:
        return "PGOU"
    if "proyecto de actuaci" in n:
...
def _abs_url(href: str) -> str:
    if href.startswith("http"):
        return href
    return urllib.parse.urljoin(WEB_BASE, href)


class BriviescaAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal web + sede espublico tablón + IDECyL WFS (Burgos)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        raw_drupal = self.config.get("drupal_pages") or DEFAULT_DRUPAL_PAGES
        self.drupal_pages: list[tuple[str, str]] = []
        for item in raw_drupal:
            if isinstance(item, (list, tuple)) and len(item) >= 2:
                self.drupal_pages.append((str(item[0]), str(item[1])))
            else:
                self.drupal_pages.append((str(item), str(item)))
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-briviesca/1.0")},
        )
    ...
    def _collect_drupal_pdfs(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for page_url, section in self.drupal_pages:
            try:
            ...
            for href in RE_PDF_HREF.findall(html):
                ...
                if pdf_url in seen:
                    continue
                seen.add(pdf_url)
                filename = unescape(pdf_url.rsplit("/", 1)[-1])
                titulo = re.sub(r"[_-]+", " ", filename.replace(".pdf", "")).strip()
                rows.append(
                    {
                        "titulo": f"{section} — {titulo}"[:500],
                        "url": pdf_url,
                        "origen": "drupal_pdf",
                        "section": section,
                    }
