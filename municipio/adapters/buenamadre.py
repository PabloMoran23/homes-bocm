...
SEDE_BASE = "https://buenamadre.sedelectronica.es"
PLAU_BASE = "https://servicios.jcyl.es/PlanPublica"
LASALINA_URBANISMO_URL = (
    "http://www.lasalina.es/Aplicaciones/GestorInter.jsp"
    "?codMunicipio=59&funcion=VerNormasUrbanisticas&nombre=Buenamadre&prestacion=NormasUrbanisticas"
)
WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/wfs"
MUNICIPIO = "Buenamadre"
ID_PREFIX = "buenamadre"
PLAN_PROVINCIA = "37"
PLAN_MUNICIPIO = "59"
INE_CODE = "37059"
...
RE_PREVIEW = re.compile(
    r'href="(https://buenamadre\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://buenamadre\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_LASALINA_ENTRY = re.compile(
    r"<strong>Fecha</strong>:\s*</td>\s*<td>(\d{2}/\d{2}/\d{4})</td>.*?"
    r"<strong>Titulo</strong>:</td><td>([^<]+)</td>.*?"
    r"<strong>Descripci[oó]n</strong>:</td><td>(.*?)</td>.*?"
    r'href="(http://www\.lasalina\.es/documentacion/normasurbanisticas/[^"]+)"',
    re.S | re.I,
)
...
class BuenamadreAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + Diputación La Salina + PlanPublica PLAU + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia={PLAN_PROVINCIA}&municipio={PLAN_MUNICIPIO}"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia={PLAN_PROVINCIA}&municipio={PLAN_MUNICIPIO}"
        )
        self.lasalina_url = str(self.config.get("lasalina_url") or LASALINA_URBANISMO_URL)
        self._sede_session_ready = False
    ...
    def _ensure_sede_session(self) -> None:
        if self._sede_session_ready:
            return
        try:
            self._fetch(self.board_url, sede=True)
        except (urllib.error.URLError, OSError, ConnectionError):
            pass
        self._sede_session_ready = True

    def _fetch(self, url: str, *, sede: bool = False) -> str:
        time.sleep(self.delay_s)
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-buenamadre/1.0")},
        )
    ...
    def _collect_lasalina_urbanismo(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(self.lasalina_url)
        except (urllib.error.URLError, OSError, ConnectionError):
            return []
        rows: list[dict[str, Any]] = []
        for m in RE_LASALINA_ENTRY.finditer(html):
            titulo = unescape(m.group(2).strip())
            descripcion = _strip_html(m.group(3))
            pdf_url = unescape(m.group(4).strip())
            rows.append(
                {
                    "titulo": titulo[:500],
    ...
    def _lasalina_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        key = row["url"]
        blob = f"{row['titulo']} {row.get('descripcion') or ''}"
        rec: dict[str, Any] = {
            "id": _stable_id("proy", f"lasalina:{key}"),
            "municipio": MUNICIPIO,
            "titulo": row["titulo"],
            "fecha": row.get("fecha"),
            "tipo": _proyecto_tipo(blob),
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_plai_seed():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_lasalina_urbanismo():
            rec = self._lasalina_to_proyecto(item)
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_board():
