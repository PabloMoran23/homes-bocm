...
class BurgosAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        seen: set[str] = set()
        rows: list[dict[str, Any]] = []
        for rec in self._collect_catalog_licencias() + self._collect_tablon_licencias():
        ...
        return {
            "rows": len(rows),
            "status": "ok",
            "catalog_tramites": len(self._collect_catalog_licencias()),
            "tablon_licencias": len(self._collect_tablon_licencias()),
        }
