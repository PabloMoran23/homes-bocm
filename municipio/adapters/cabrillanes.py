...
WEB_BASE = "https://www.aytocabrillanes.es"
SEDE_BASE = "https://aytocabrillanes.sedelectronica.es"
MUNICIPIO = "Cabrillanes"
ID_PREFIX = "cabrillanes"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    ...
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=022",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=022",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://aytocabrillanes\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://aytocabrillanes\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
def _normalize_sector_code(raw: str) -> str:
    ...
    code = re.sub(r"S\.U\.N\.C\.", "SNC", code)
    code = re.sub(r"SU-NC", "SNC", code)
    return code
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "licencia" in n:
        return "licencia"
    if "planeam" in n or "pgou" in n or "sector" in n or re.search(r"\b(snc|sued)\b", n):
        return "planeamiento"
    return "urbanismo"


class CabrillanesAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-cabrillanes/1.0")},
        )
