...
WP_BASE = "https://calvarrasadeabajo.es"
SEDE_BASE = "https://calvarrasadeabajo.sedelectronica.es"
...
ID_PREFIX = "calvarrasa-de-abajo"
C_MUN = "37069"

DEFAULT_WP_SEED_PAGES: list[str] = [
    f"{WP_BASE}/area-de-urbanismo/",
    f"{WP_BASE}/exposicion-publica/",
    f"{WP_BASE}/modelos-de-solicitudes/",
]

DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        f"{WP_BASE}/area-de-urbanismo/",
        "Área de urbanismo — archivo planeamiento y modificación NUM SRAI-7",
    ),
    (
        "https://drive.google.com/file/d/1JrCnzVikIn4Cn9p3y83nDFA0hDMW0qpV/view?usp=drive_link",
        "Archivo de planeamiento urbanístico (Google Drive)",
    ),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=069",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=069",
...
RE_PREVIEW = re.compile(
    r'href="(https://calvarrasadeabajo\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://calvarrasadeabajo\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_DIVI_BLURB = re.compile(
    r'et_pb_module_header"><a href="([^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?calvarrasadeabajo\.es)?/wp-content/uploads/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
RE_SECTOR_CODE = re.compile(
    ...
    r"PE[- ]?\d+|PP[- ]?[A-Z0-9-]+|PERI[- ]?[A-Z0-9]+|"
    r"sector\s*(?:sur|norte)?[- ]?\d+"
    r")\b",
)
...
class CalvarrasaDeAbajoAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Divi + sede espublico + PlanPublica PLAU/PLAI + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=069"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=069"
        )
        ...
        self.c_mun = str(self.config.get("c_mun") or C_MUN)
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-calvarrasa-de-abajo/1.0")},
        )
    ...
    def _collect_wp_seed_pages(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.wp_seed_pages:
            try:
                ...
                continue
            for m in RE_DIVI_BLURB.finditer(html):
                href, titulo = m.group(1), unescape(m.group(2).strip())
                url = self._abs_wp(href) if not href.startswith("http") else href
                blob = f"{titulo} {url}"
        ...
        for url, fallback_title in self.seed_urls:
            ...
            try:
                ...
                if title_m:
                    ...
                    titulo = re.sub(r"\s*-\s*Calvarrasa.*$", "", titulo, flags=re.I).strip()
            except (urllib.error.URLError, OSError, ConnectionError):
                pass
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_tramites_catalog():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_wp_seed_pages():
