...
WEB_BASE = "https://www.camporeal.es"
SEDE_BASE = "https://camporeal.sedelectronica.es"
MUNICIPIO = "Campo Real"
ID_PREFIX = "campo-real"

URBANISMO_URL = f"{WEB_BASE}/concejalias/urbanismo-y-obras-publicas"
BOARD_URL = f"{SEDE_BASE}/board/"
SITCM_VISOR = "https://idem.comunidad.madrid/cartografia/sitcm/html/visor.htm?municipio=033"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "CAMPO REAL"

RE_PREVIEW = re.compile(
    r'href="(https://camporeal\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://camporeal\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_LICENCIA = re.compile(
    ...
    r"modelo.*licencia|modelo.*declaraci[oó]n)",
)
RE_PROYECTO = re.compile(
    ...
    r"edicto|bocm|ordenanza|parcela|desbroce.*parcel|bando.*parcel|normativa urban|"
    r"visor|sitcm|suelo urbanizable|unidad de actuaci|"
    r"\b(?:P|UE|UA|SAU|AD|AN|AI|PAU|S)-[\w\d-]+\b)",
)
...
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:P|UE|UA|SAU|AD|AN|AI|PAU|S)-[\w\d-]+)\b",
)
...
class CampoRealAyuntamientoAdapter(AyuntamientoAdapter):
    """Joomla (urbanismo PDFs) + sede espublico gestiona + ámbitos SITCM WFS (geometría partial)."""
    ...
    def _fetch(self, url: str, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-campo-real/1.0")},
        )
    ...
    def _collect_web_urbanismo(self) -> list[dict[str, Any]]:
        ...
        for m in RE_PDF_LINK.finditer(html):
            url = _abs_url(WEB_BASE, m.group(1))
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "fecha_concesion": None,
                "tipo": "trámites urbanismo web",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo y Obras Públicas — modelos y solicitudes",
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
                "nota": "Modelos licencia/DR y formularios PDF en web Joomla",
    ...
    def _web_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if RE_LICENCIA.search(blob) and not re.search(
            r"(?i)ordenanza|planeam|urban|normas|pgou",
            blob,
        ):
            return None
        rec: dict[str, Any] = {
