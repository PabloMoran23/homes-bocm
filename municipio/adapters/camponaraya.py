...
WEB_BASE = "https://camponaraya.es"
SEDE_BASE = "https://camponaraya.sedelectronica.es"
MUNICIPIO = "Camponaraya"
ID_PREFIX = "camponaraya"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (f"{WEB_BASE}/normativa-camponaraya", "Normativa municipal"),
    (f"{WEB_BASE}/tablon-anuncios", "Tablón de anuncios"),
    (
        f"{SEDE_BASE}/transparency/3a2d065a-9774-4800-aa3e-b88b8313a8f2/",
        "Normas Urbanísticas Municipales — transparencia",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=034",
        "Planeamiento en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=034",
        "Archivo planeamiento aprobado (Junta CYL)",
    ),
    (
        f"{WEB_BASE}/noticias-listado/761_bando-informativo",
        "Bando informativo — Normas Urbanísticas Municipales",
    ),
]

DEFAULT_LICENCIA_PAGES: list[tuple[str, str]] = [
    (f"{SEDE_BASE}/dossier", "Trámites — sede electrónica"),
    (f"{SEDE_BASE}/board", "Tablón de anuncios — sede electrónica"),
]

RE_PREVIEW = re.compile(
    r'href="(https://camponaraya\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://camponaraya\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_NORMATIVA_ITEM = re.compile(
    r'<li>\s*<div class="texto">\s*<h3>(.*?)</h3>\s*<p class="fecha">\s*([^<]+)\s*</p>.*?'
    r'href="([^"]+)"[^>]*class="descarga"',
    re.I | re.S,
)
...
RE_PROYECTO = re.compile(
    ...
    r"concentraci[oó]n parcelaria|pol[ií]gono|normas urban|plano)",
)
RE_EXCLUDE = re.compile(
    ...
    r"becas|subvenciones|empadron|certamen|deportiv|escuela)",
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "plan parcial" in n:
        return "plan parcial"
    if "ruina" in n or "demolici" in n:
        return "declaración de ruina"
    if "licencia" in n:
        return "licencia"
    if "planeam" in n or "pgou" in n or "sector" in n:
        return "planeamiento"
    return "urbanismo"
...
class CamponarayaAyuntamientoAdapter(AyuntamientoAdapter):
    """CMS PSI (camponaraya.es) + sede espublico + IDECyL WFS PLAU CyL."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.licencia_pages: list[tuple[str, str]] = [
            (str(a), str(b)) for a, b in raw_lic
        ]
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-camponaraya/1.0")},
        )
    ...
    def _collect_board(self) -> list[dict[str, Any]]:
        try:
            ...
            return []
        return self._parse_board_links(html, "sede_tablon")
    ...
    def _collect_tablon(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        pages = [f"{self.web_base}/tablon-anuncios"] + [
            f"{self.web_base}/tablon-anuncios/{n}" for n in range(2, 7)
        ]
        for page_url in pages:
            ...
            for m in re.finditer(r"<h3[^>]*>(.*?)</h3>", html, re.I | re.S):
                ...
                href = link_m.group(1) if link_m else page_url
                url = _abs_url(self.web_base, href)
                rows.append(
                    {
                        "titulo": titulo[:500],
                        "url": url,
                        "pdf_url": url if href.lower().endswith(".pdf") else None,
                        "fecha": _fecha_from_blob(chunk) or _fecha_from_blob(titulo),
                        "origen": "web_tablon",
                    }
                )
        return rows

    def _collect_normativa(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for page in (f"{self.web_base}/normativa-camponaraya", f"{self.web_base}/normativa-camponaraya/2"):
            try:
                html = self._fetch(page)
            except (urllib.error.URLError, OSError, ConnectionError):
                continue
            for m in RE_NORMATIVA_ITEM.finditer(html):
                titulo = _strip_html(m.group(1))
                ...
                rows.append(
                    {
                        ...
                        "url": url,
                        "pdf_url": url,
                        "fecha": _parse_fecha_dmy(fecha_raw) or _fecha_from_blob(fecha_raw),
    ...
    def _collect_seed_pages(self) -> list[dict[str, Any]]:
        ...
        for url, fallback_title in self.seed_urls:
            ...
            try:
                html = self._fetch(url, sede="sedelectronica.es" in url)
                title_m = re.search(r"<title>([^<]+)</title>", html, re.I)
                if title_m:
                    titulo = _strip_html(title_m.group(1))
                    titulo = re.sub(r"\s*[-|].*Ayuntamiento.*$", "", titulo, flags=re.I).strip()
            except (urllib.error.URLError, OSError, ConnectionError):
                pass
    ...
    def _collect_wfs_proyectos(self) -> list[dict[str, Any]]:
        ...
        for layer, default_tipo in WFS_LAYERS:
            ...
            for feat in data.get("features") or []:
                ...
                titulo = str(props.get("n_titulo") or props.get("n_instrum") or "").strip()
                if sector and num:
                    titulo = f"{sector} ({num})" if not titulo else titulo
                elif sector:
                    titulo = sector
                if not titulo:
                ...
                fecha = None
                for fk in ("f_bocyl", "f_aprob", "f_mapa"):
    ...
    def _tramite_to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not RE_LICENCIA.search(titulo) and "trámite" not in titulo.lower():
            return None
        return {
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_tramites_catalog():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for url, titulo in self.licencia_pages:
            rec = self._tramite_to_licencia({"titulo": titulo, "url": url, "origen": "web_tramite"})
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for source in (
            self._collect_board(),
            self._collect_tablon(),
            self._collect_normativa(),
            self._collect_seed_pages(),
            self._collect_tramites_catalog(),
        ):
            for item in source:
                rec = self._generic_to_proyecto(item)
                if rec and rec["id"] not in seen:
                    seen.add(rec["id"])
                    rows.append(rec)
        self._write_jsonl(out_jsonl, rows)
