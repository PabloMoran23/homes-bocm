...
WEB_BASE = "https://web.canals.es"
SEDE_BASE = "https://canals.sede.dival.es"
MUNICIPIO = "Canals"
ID_PREFIX = "canals"
INE_COD_MUN = "46068"
...
URBANISME_URL = f"{WEB_BASE}/serveis-municipals/urbanisme-i-habitatge"
...
RE_PROYECTO = re.compile(
    ...
    r"les moles|moles|industrial)",
)
RE_TABLON_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"bolsa coordinador|listado provisional bolsa|designaci[oó]n ots)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_ANUNCIO_ID = re.compile(r"anuncio\.aspx\?id=(\d+)", re.I)
RE_DOC_LINK = re.compile(
    r'href="((?:https://canals\.sede\.dival\.es)?/tablondeanuncios/documento\.aspx\?id=\d+[^"]*)"',
    re.I,
)
...
class CanalsAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay web.canals.es + sede Dival (Sedipualba) tablón RSS + ICV GVA WFS partial."""
    ...
    def _fetch(self, url: str, *, timeout: int = 90) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-canals/1.0")},
        )
        ...
        return [
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Tauler d'anuncis — sede electrónica Canals",
                "url": self.tablon_url,
                "source": "ayuntamiento",
                "nota": "Edictos y licencias publicados en canals.sede.dival.es",
                "origen": "sede_tablon",
            },
            {
            ...
            },
            {
                ...
                "fecha_concesion": None,
                "tipo": "urbanismo e habitatge",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Urbanisme i habitatge — web municipal",
                "url": self.urbanisme_url,
                "source": "ayuntamiento",
                "nota": "Sección de servicios municipales (Liferay)",
                "origen": "web_tramite",
            },
        ]
