...
WP_BASE = "https://www.candelario.es"
WP_API = f"{WP_BASE}/wp-json/wp/v2"
SEDE_BASE = "https://candelario.sedelectronica.es"
MUNICIPIO = "Candelario"
ID_PREFIX = "candelario"
URBANISMO_CATEGORY_ID = 59

WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/ows"
WFS_MUNICIPIO = "Candelario"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/urbanismo/",
    f"{WP_BASE}/ayuntamiento/documentacion-administrativa/",
    f"{WP_BASE}/ayuntamiento/informacion-municipal/",
    f"{WP_BASE}/subvenciones/",
]

LICENCIA_PDF = (
    f"{WP_BASE}/wp-content/uploads/2024/03/Solicitud-de-Licencia-o-Autorizacion-Urbanistica.pdf"
)

RE_PREVIEW = re.compile(
    r'href="(https://candelario\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"rehabilitaci[oó]n urbana|sau-|sau |ordenanza.*terrazas|convenio.*rehabilitaci)",
)
RE_WP_EXCLUDE = re.compile(
    ...
    r"senderos|ornitol[oó]gic|museo|gastronom|iluminaci[oó]n navide|audiogu[ií]a|"
    r"feria agroalimentaria|programaci[oó]n fiestas)",
)
RE_EXCLUDE = re.compile(
    ...
    r"empleo p[uú]blico|peju|guardia civil|denuncia telem)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_YM = re.compile(r"/(?:uploads|documents)/(\d{4})/(\d{2})/")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?candelario\.es)?/wp-content/uploads/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "sector" in n or "sau" in n:
        return "sector"
    if "rehabilitaci" in n:
        return "rehabilitación urbana"
    if "ordenanza" in n:
        return "ordenanza urbanística"
    return "urbanismo"


class CandelarioAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Enfold (urbanismo) + sede espublico tablón + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-candelario/1.0")},
        )
    ...
    def _wp_item_to_proyecto(self, item: dict[str, Any], origen: str) -> dict[str, Any] | None:
        ...
        if origen != "wp_urbanismo" and not RE_PROYECTO.search(blob):
            return None
        if origen == "wp_urbanismo" and not RE_PROYECTO.search(blob):
            if not re.search(r"(?i)(obra|proyecto|abastec|alcantarill|paviment|acera|consultorio|aparcamiento)", blob):
                return None
    ...
    def _pdf_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        rec: dict[str, Any] = {
            ...
            "fecha": row.get("fecha"),
            "tipo": _proyecto_tipo(blob),
            "url": row.get("url") or row.get("pdf_url") or f"{self.wp_base}/urbanismo/",
            "source": "ayuntamiento",
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            ...
            (f"{self.wp_base}/urbanismo/", "urbanismo — documentación y trámites"),
            (
                LICENCIA_PDF,
                "solicitud de licencia o autorización urbanística (PDF)",
            ),
            (
                f"{self.wp_base}/ayuntamiento/documentacion-administrativa/",
                "documentación administrativa — formularios",
            ),
        ]
        rows: list[dict[str, Any]] = []
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for post in self._paginate_wp(
            "posts",
            max_pages=2,
            categories=self.urbanismo_category_id,
        ):
            add(self._wp_item_to_proyecto(post, "wp_urbanismo"))
