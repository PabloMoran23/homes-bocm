...
WP_BASE = "https://ayuntamientocandeleda.es"
WP_API = f"{WP_BASE}/wp-json/wp/v2"
SEDE_BASE = "https://ayuntamientocandeleda.sedelectronica.es"
MUNICIPIO = "Candeleda"
ID_PREFIX = "candeleda"

WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/ows"
WFS_MUNICIPIO = "Candeleda"
...
DEFAULT_SEED_PAGES: list[str] = [
    ...
    f"{WP_BASE}/revision-de-las-normas-urbanisticas-municipales-e-informe-de-sostenibilidad-ambiental/",
    f"{WP_BASE}/informacion-publica/",
    f"{WP_BASE}/cartel-licencias-urbanisticas/",
    f"{WP_BASE}/modelo-normalizado-de-licencia-para-construccion-de-nave-agricola/",
    f"{WP_BASE}/descargas-de-modelos-e-impresos-normalizados-en-pdf-2/",
    f"{WP_BASE}/bandos/",
]

RE_PREVIEW = re.compile(
    r'href="(https://ayuntamientocandeleda\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra mayor|obra menor|"
    r"primera ocupaci[oó]n|cartel.*licencia|modelo.*licencia|nave agr[ií]cola)",
)
...
RE_WP_EXCLUDE = re.compile(
    ...
    r"senderos|ornitol[oó]gic|museo|gastronom)",
)
...
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?ayuntamientocandeleda\.es)?/wp-content/uploads/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "sector" in n or "calle" in n:
        return "sector"
    return "urbanismo"


class CandeledaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress (urbanismo/PGOU) + sede espublico tablón + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-candeleda/1.0")},
        )
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            ...
            (f"{WP_BASE}/cartel-licencias-urbanisticas/", "cartel informativo licencias urbanísticas"),
            (
                f"{WP_BASE}/modelo-normalizado-de-licencia-para-construccion-de-nave-agricola/",
                "modelo licencia nave agrícola",
            ),
            (f"{WP_BASE}/urbanismo/", "urbanismo — documentación y trámites"),
            (f"{WP_BASE}/descargas-de-modelos-e-impresos-normalizados-en-pdf-2/", "formularios urbanismo PDF"),
        ]
        rows: list[dict[str, Any]] = []
