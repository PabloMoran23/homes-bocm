...
WFS_MUNICIPIO = "CANENCIA"

PLANEAMIENTO_URL = f"{SITE_BASE}/tu-ayuntamiento/normativa-municipal/planeamiento-urbanistico"
URBANISMO_URL = f"{SITE_BASE}/ciudadanos/tramites-personales/urbanismo"
TABLON_URL = f"{SITE_BASE}/ciudadanos/tablon-municipal"
...
RSS_FEEDS: list[str] = [
    f"{TABLON_URL}?format=feed&type=rss",
    f"{SITE_BASE}/tu-ayuntamiento/normativa-municipal/ordenanzas-municipales?format=feed&type=rss",
]

DEFAULT_PLANEAMIENTO_SEEDS: list[dict[str, str]] = [
    {
        "url": PLANEAMIENTO_URL,
        "titulo": (
            "Aprobación definitiva modificación puntual NNSS Canencia — calificación cementerio "
            "(BOCM 8 nov 2018)"
        ),
        "fecha": "2018-11-08",
        "tipo": "modificación puntual",
        "pdf_url": f"{SITE_BASE}/images/concejalias/urbanismo/BOCM-20181108-10.pdf",
    },
]
...
RE_LICENCIA = re.compile(
    ...
    r"primera ocupaci[oó]n|parcelaci[oó]n|segregaci[oó]n|vado)",
)
RE_PROYECTO = re.compile(
    ...
    r"reparcel|urbanizaci[oó]n|estudio de detalle|bocm|edicto|bando.*(?:parcel|obra|desbroce)|"
    r"exposici[oó]n p[uú]blica|cementerio|rehabilitaci[oó]n de vivienda|"
    r"\b(?:SAU|UG|UE|AD|AN|AI|PAU|S)-[\w\d-]+\b|interfaz urbano)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|empleo|bolsa|igualdad|incendio|emergencias|"
    r"calendario laboral|censo electoral|setas|micol[oó]gic|fiestas|cursos de formaci[oó]n|"
    r"activaci[oó]n profesional|empleo juvenil|ofib[uú]s|caixabank|punto limpio|"
    r"centro de d[ií]a|piscina|campamento|transporte p[uú]blico|itinerario.*autobus|"
    r"ivtm|auto-?taxi|cobranza|oficina m[oó]vil|eclipse|medicamentos)",
)
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?canencia\.es)?/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
RE_BOARD_PREVIEW = re.compile(
    r'href="(https://canencia\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
RE_FECHA_YM = re.compile(r"/images/(?:concejalias|ciudadanos|TablonMunicipal)/(?:[^/]+/)*(\d{4})/(\d{2})/")
RE_BOCM_DATE = re.compile(r"BOCM-(\d{4})(\d{2})(\d{2})")
RE_ARTICLE_LINK = re.compile(
    r'href="((?:https://www\.canencia\.es)?/(?:ciudadanos/tablon-municipal|'
    r"tu-ayuntamiento/normativa-municipal/ordenanzas-municipales)/\d+-[^\"]+)\"",
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "rehabilitaci" in n and "vivienda" in n:
        return "subvención vivienda"
    return "urbanismo"


class CanenciaAyuntamientoAdapter(AyuntamientoAdapter):
    """Joomla (icagenda tablón, planeamiento) + sede espublico + WFS SITCM (geometría partial)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-canencia/1.0")},
        )
    ...
    def _parse_page_pdfs(self, html: str, page_url: str) -> list[dict[str, Any]]:
        ...
        for m in RE_PDF_HREF.finditer(html):
            ...
            rows.append(
                {
                    "titulo": titulo[:500],
                    "fecha": _fecha_from_url(pdf_url) or _fecha_from_blob(titulo),
                    "url": page_url,
                    "pdf_url": pdf_url,
                    "origen": "joomla_pdf",
                }
    ...
    def _collect_joomla(self) -> list[dict[str, Any]]:
        ...
        for base_path in ("/ciudadanos/tablon-municipal",):
    ...
    def _collect_planeamiento_seeds(self) -> list[dict[str, Any]]:
        ...
        for seed in self.planeamiento_seeds:
            rec: dict[str, Any] = {
                ...
                "url": seed["url"],
                "source": "ayuntamiento",
                "origen": "planeamiento_seed",
            }
            if seed.get("pdf_url"):
                rec["pdf_url"] = seed["pdf_url"]
        ...
        rows: list[dict[str, Any]] = [
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Trámites de urbanismo — formularios licencia obra",
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
                "nota": "Formularios PDF obra menor y mayor (Joomla)",
            ...
            },
            {
                ...
                "source": "ayuntamiento",
                "nota": "espublico gestiona; transparencia Wicket",
                "origen": "sede_info",
            },
        ]
        try:
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            "rows": len(rows),
            "status": "ok",
            "info": sum(1 for r in rows if str(r.get("origen", "")).startswith(("joomla", "sede_"))),
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_sit_ambitos():
            add(rec)
        for rec in self._collect_planeamiento_seeds():
            add(rec)
