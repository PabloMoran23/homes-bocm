...
SEDE_BASE = "https://canillasdealbaida.sedelectronica.es"
WEB_BASE = "https://canillasdealbaida.es"
BOARD_URL = f"{SEDE_BASE}/board/"
URBANISMO_URL = f"{WEB_BASE}/urbanismo/"
PARTICIPACION_URL = f"{WEB_BASE}/participacion-ciudadana/"
...
DIP_PLANEAMIENTO_URL = (
    "http://www.malaga.es/fomentoinfraestructuras/planeamiento/ficha.asp?mun=29033"
)
MUNICIPIO = "Canillas de Albaida"
ID_PREFIX = "canillas-de-albaida"

WEB_SEEDS: list[str] = [
    URBANISMO_URL,
    PARTICIPACION_URL,
    f"{WEB_BASE}/ordenanzas/",
]
...
RE_LICENCIA = re.compile(
    ...
    r"obra (?:mayor|menor)|establecimiento hosteler|venta productos|autoliquidaci[oó]n)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|pdsu|pmus|convenio|"
    ...
    r"inventario.*caminos|eae|consulta p[uú]blica|instrumentos de intervenci[oó]n)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"cobranza iae|padrones|bopma.*orientador|cifras electorales|auxiliar administrativo|"
    r"activa-t joven|subvenci[oó]n|programa activa)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://canillasdealbaida\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_PDF_LINK = re.compile(
    r'href="((?:https://canillasdealbaida\.es)?/[^"]+\.(?:pdf|PDF)[^"]*)"[^>]*>([^<]{0,300})',
    re.I | re.S,
)
...
def _proyecto_tipo(blob: str) -> str:
    b = blob.lower()
    if "pdsu" in b or "plan de sostenibilidad" in b:
        return "PDSU"
    if "pmus" in b or "movilidad" in b:
        return "PMUS"
    if "pgou" in b or "planeamiento general" in b:
        return "planeamiento"
    if "inventario" in b and "caminos" in b:
        return "inventario caminos"
    if "ordenanza" in b and "instrumentos" in b:
...
class CanillasDeAlbaidaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Divi + sede espublico gestiona. Urbanismo con PDFs en web propia."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-canillas-de-albaida/1.0")},
        )
    ...
    def _collect_web_pdfs(self) -> list[dict[str, Any]]:
        ...
        for seed in self.web_seeds:
            ...
            for m in RE_PDF_LINK.finditer(html):
                url = _abs_web(m.group(1))
                titulo = _strip_html(m.group(2)) or url.rsplit("/", 1)[-1]
                if url in seen or len(titulo) < 2:
                    continue
                blob = f"{titulo} {url}"
                if not RE_PROYECTO.search(blob) and not RE_LICENCIA.search(blob):
                    if "ordenanza" not in seed.lower() and "urbanismo" not in seed.lower():
                        continue
                seen.add(url)
                rows.append(
                    {
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "fecha_concesion": None,
                "tipo": "autoliquidación de obras",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Autoliquidación de obras — impresos",
                "url": IMPRESOS_URL,
                "source": "ayuntamiento",
                "nota": "Formulario de autoliquidación de obras en web municipal",
                "origen": "web_tramite",
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo — documentación y trámites",
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
                "nota": "Sección urbanismo con documentación de planeamiento",
        ...
        return [
            {
                ...
                "municipio": MUNICIPIO,
                "titulo": "Urbanismo — planeamiento y documentación",
                "fecha": None,
                "tipo": "urbanismo",
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
            ...
            {
                "id": _stable_id("proy", PARTICIPACION_URL),
                "municipio": MUNICIPIO,
                "titulo": (
                    "Consulta pública — Ordenanza reguladora de instrumentos de intervención urbanística"
                ),
                "fecha": None,
                "tipo": "consulta pública",
                "url": PARTICIPACION_URL,
                "source": "ayuntamiento",
                "origen": "web_participacion",
            },
            {
            ...
            },
            {
                ...
                "municipio": MUNICIPIO,
                "titulo": "Planeamiento — ficha Diputación Provincial de Málaga",
                "fecha": None,
                "tipo": "planeamiento",
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "info": sum(1 for r in rows if r.get("origen") in ("sede_tablon", "sede_tramite", "web_tramite", "web_urbanismo")),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "status": "ok",
            "tablon": sum(1 for r in rows if r.get("origen") == "tablon"),
            "web": sum(1 for r in rows if r.get("origen") in ("web_pdf", "web_heading")),
            "static": sum(1 for r in rows if r.get("origen") in ("web_urbanismo", "web_participacion", "situa", "diputacion_planeamiento")),
        }
