...
WEB_BASE = "https://www.cantalejo.es"
SEDE_BASE = "https://cantalejo.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (f"{WEB_BASE}/urbanismo", "Urbanismo — documentos PDF y enlace PLAI"),
    (
...
RE_PREVIEW = re.compile(
    r'href="(https://cantalejo\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://cantalejo\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_LIFERAY_DOC = re.compile(
    r'href="(/documents/[^"]+)"[^>]*title="([^"]+)"[^>]*class="document file-pdf"',
    re.I,
)
...
class CantalejoAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay Segovia8 (cantalejo.es) + sede espublico + PLAI JCYL + geometría parcial IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-cantalejo/1.0")},
        )
    ...
    def _collect_liferay_urb(self) -> list[dict[str, Any]]:
        try:
        ...
        for m in RE_LIFERAY_DOC.finditer(html):
            ...
            url = path if path.startswith("http") else f"{self.web_base}{path}"
            if url in seen:
                continue
            seen.add(url)
            rows.append(
                {
                    ...
                    "url": url,
                    "origen": "cantalejo_liferay",
                }
            )
        return rows

    def _liferay_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        rec: dict[str, Any] = {
    ...
    def _collect_proyectos(self) -> list[dict[str, Any]]:
        ...
        for item in self._collect_wfs():
            ...
            by_id[rec["id"]] = rec
        for item in self._collect_liferay_urb():
            rec = self._liferay_to_proyecto(item)
            by_id[rec["id"]] = rec
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tramites": sum(1 for r in rows if r.get("origen") in ("catalogo_tramites", "cantalejo_tramite")),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
            "semillas": sum(1 for r in rows if r.get("origen") == "cantalejo_semilla"),
            "liferay": sum(1 for r in rows if r.get("origen") == "cantalejo_liferay"),
        }
