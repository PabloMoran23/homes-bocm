...
SEDE_BASE = "https://cantaracillo.sedelectronica.es"
...
RE_PREVIEW = re.compile(
    r'href="((?:https://cantaracillo\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://cantaracillo\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class CantaracilloAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico + PlanPublica JCyL + IDECyL WFS (geometría partial)."""
    ...
    def _bootstrap_session(self) -> None:
        ...
        req = urllib.request.Request(
            self.dossier_url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-cantaracillo/1.0")},
        )
        try:
