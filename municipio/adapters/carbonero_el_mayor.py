...
JOOMLA_BASE = "https://carboneroelmayor.es"
SEDE_BASE = "https://carboneroelmayor.sedelectronica.es"
...
ID_PREFIX = "carbonero-el-mayor"
PLAI_MUNICIPIO = 49
PLAI_PROVINCIA = 40
URBANISMO_URL = (
    f"{JOOMLA_BASE}/index.php?option=com_content&view=section&id=8&layout=blog&Itemid=81"
)
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (URBANISMO_URL, "Urbanismo — NUM, modificaciones puntuales y anuncios"),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=049",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=049",
...
RE_PREVIEW = re.compile(
    r'href="(https://carboneroelmayor\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://carboneroelmayor\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?carboneroelmayor\.es)?/images/stories/[^"]+\.pdf[^"]*)"',
    re.I,
)
RE_ARTICLE_HREF = re.compile(
    r'href="(/index\.php\?option=com_content[^"]+)"[^>]*>(?:\s*<[^>]+>\s*)*([^<]{4,})',
    re.I,
)
...
RE_SECTOR = re.compile(
    r"(?i)\b(?:sector|sect\.?|s\.|s[- ]?)\s*([\"«]?\s*[0-9]+[A-Z0-9\"».\s-]*|"
    r"bustar|ramoal|pol[ií]gono)",
)
...
def _sector_tokens(text: str) -> list[str]:
    ...
    low = (text or "").lower()
    for pat in (
        r"estudio de detalle\s*\d+[-/]\d+",
        r"modificaci[oó]n puntual\s*\d+[-/]\d+",
        r"sector\s*[0-9]+[a-z]?",
        r"\bs\.?\s*[0-9]+[a-z]?\b",
        r"pol[ií]gono",
        r"bustar",
...
class CarboneroElMayorAyuntamientoAdapter(AyuntamientoAdapter):
    """Joomla 1.5 + sede espublico + PLAI JCYL + geometría parcial IDECyL WFS."""

    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        super().__init__(slug, config, base_url or JOOMLA_BASE)
        self.delay_s = float(self.config.get("request_delay_s", 0.35))
        self.web_base = str(self.config.get("web_base") or JOOMLA_BASE).rstrip("/")
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-carbonero-el-mayor/1.0")},
        )
    ...
    def _collect_joomla_urbanismo(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        try:
        ...
        for m in RE_PDF_HREF.finditer(html):
            ...
            if pdf in seen:
                continue
            seen.add(pdf)
            name = unescape(urllib.parse.unquote(pdf.rsplit("/", 1)[-1]))
            rows.append(
                {
                    "titulo": name[:500],
        ...
        for m in RE_ARTICLE_HREF.finditer(html):
            path = unescape(m.group(1).replace("&amp;", "&"))
            title = _strip_html(m.group(2))
            if len(title) < 6 or "leer más" in title.lower():
                continue
    ...
    def _collect_proyectos(self) -> list[dict[str, Any]]:
        ...
        for item in self._collect_tramites():
            ...
            if rec:
                by_id[rec["id"]] = rec
        for item in self._collect_joomla_urbanismo():
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tramites": sum(1 for r in rows if r.get("origen") in ("catalogo_tramites", "joomla_tramite")),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
            "semillas": sum(1 for r in rows if r.get("origen") == "joomla_semilla"),
