...
BASE = "https://carcabuey.es"
URBANISMO_URL = f"{BASE}/urbanismo/"
SEDE_BASE = "https://sede.eprinsa.es/carcabuey"
...
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https://carcabuey\.es)?/wp-content/uploads/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
RE_SKIP_PDF = re.compile(
    r"(?i)(solicitud|rellenable|declaracion-responsable|comunicacion-previa|modelos-dr|"
    r"plan_formacion|indice\.pdf|inv-definitivi)",
)
...
RE_PROYECTO = re.compile(
    ...
    r"clasificaci[oó]n|calificaci[oó]n|protecci[oó]n|alineacion|ordenaci[oó]n|"
    r"zonificaci[oó]n|planimetr|refundido|termino municipal|nucleo)",
)
...
def _proyecto_tipo(blob: str) -> str:
    b = blob.lower()
    if "pgou" in b or "plan general" in b or "plan de ordenacion" in b or "plan de ordenación" in b:
        return "PGOU"
    ...
    if "plano" in b or "clasificaci" in b or "calificaci" in b or "ordenacion" in b or "zonificacion" in b:
...
class CarcabueyAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-carcabuey/1.0")},
        )
    ...
    def _is_proyecto_pdf(self, href: str, anchor: str, section: str | None) -> bool:
        if RE_SKIP_PDF.search(href):
            return False
        blob = f"{anchor} {href} {section or ''}"
    ...
    def _collect_urbanismo_pdfs(self) -> list[dict[str, Any]]:
        ...
        for m in RE_ANCHOR_PDF.finditer(html):
            ...
            if not self._is_proyecto_pdf(href, anchor, section):
                continue
            seen.add(href)
            titulo = anchor[:500]
        ...
        for m in RE_PDF_HREF.finditer(html):
            ...
            section = self._section_at(html, m.start())
            if not self._is_proyecto_pdf(href, name, section):
                continue
            seen.add(href)
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Modelos de solicitud — urbanismo (obras mayores/menores, DR)",
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
                "nota": "Formularios PDF en web municipal; no concesiones publicadas",
                "origen": "web_modelos",
            },
            {
        ...
        add(
            {
                ...
                "municipio": MUNICIPIO,
                "titulo": "PGOU Carcabuey — consulta SITUA (Junta de Andalucía)",
                "fecha": "2023-07-06",
                "tipo": "PGOU",
                "url": SITUA_SEARCH,
                "source": "ayuntamiento",
