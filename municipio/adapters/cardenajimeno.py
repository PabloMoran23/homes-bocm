...
DRUPAL_BASE = "https://cardenajimeno.es"
SEDE_BASE = "https://cardenajimeno.sedelectronica.es"
...
DEFAULT_INFO_PAGES: list[tuple[str, str]] = [
    ...
    (f"{SEDE_BASE}/transparency", "portal de transparencia — urbanismo y obras"),
    (f"{DRUPAL_BASE}/normativa", "normativa municipal"),
    (
        "http://www.jcyl.es/plau/lplanes.plau?municipio=09074",
        "archivo planeamiento JCyL (Cardeñajimeno)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=074",
        "archivo planeamiento aprobado (PlanPublica JCyL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="((?:https://cardenajimeno\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://cardenajimeno\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class CardenajimenoAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-cardenajimeno/1.0")},
        )
    ...
    def _parse_board_table(self, html: str, origen: str) -> list[dict[str, Any]]:
        ...
        for tr in re.findall(r"<tr>(.*?)</tr>", tbody_m.group(1), re.S):
            ...
            link_url = link_m.group(1) if link_m else self.board_url
            if link_m and not link_url.startswith("http"):
                link_url = f"{self.sede_base}{link_url if link_url.startswith('/') else '/' + link_url}"
            rows.append(
                {
                    "titulo": titulo[:500],
                    "doc_label": doc[:500],
                    ...
                    "url": link_url,
                    "pdf_url": link_url if link_m else None,
                    "origen": origen,
                }
            )
        return rows
    ...
    def _parse_board_links(self, html: str, origen: str) -> list[dict[str, Any]]:
        ...
        for m in RE_PREVIEW.finditer(html):
            ...
            if not url.startswith("http"):
                url = f"{self.sede_base}{url if url.startswith('/') else '/' + url}"
    ...
    def _plai_page_url(self, offset: int, *, info_publica: bool = False) -> str:
        ...
        params = {
            ...
            "bInfoPublica": "S" if info_publica else "N",
        }
        ...
        for tr in re.findall(r"<tr[^>]*>(.*?)</tr>", html, re.S | re.I):
            ...
            if doc_id and doc_m:
                ...
                url = (
                    ...
                    f"bInfoPublica=N&provincia={PLAI_PROVINCIA}&municipio={PLAI_MUNICIPIO:03d}"
                )
            rows.append(
                {
                    "title": titulo,
                    ...
                    rows.append(
                        {
                            ...
                            "origen": "plau_jcyl" if not info_publica else "plai_jcyl",
                        }
                    )
