...
DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    (
        f"{SEDE_BASE}/catalog/t/667c2405-5964-48b8-a504-9c64fe6a8836",
        "Declaración Responsable o Comunicación en Materia Urbanística",
    ),
    (
        f"{SEDE_BASE}/catalog/t/3eec8ea1-30dc-4373-a5f8-bfe9e5204172",
...
RE_PREVIEW = re.compile(
    r'href="(https://carracedelo\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://carracedelo\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
def _normalize_sector_code(raw: str) -> str:
    ...
    if re.fullmatch(r"UR\d+", code):
        return f"UR-{code[2:]}"
    if re.fullmatch(r"SUR\d+", code):
        return f"SUR-{code[3:]}"
...
class CarracedeloAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + PlanPublica PLAU + IDECyL WFS (sin web corporativa)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=038"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=038"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-carracedelo/1.0")},
        )
