...
WEB_BASE = "https://carrascaldebarregas.com"
SEDE_BASE = "https://carrascaldebarregas.sedelectronica.es"
...
ID_PREFIX = "carrascal-de-barregas"
WFS_C_MUN = "37087"
PLAN_PROVINCIA = "37"
PLAN_MUNICIPIO = "087"
...
RE_PREVIEW = re.compile(
    r'href="((?:https://carrascaldebarregas\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://carrascaldebarregas\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"UR[- ]?R[- ]?\d+|UrR[- ]?\d+|URR[- ]?\d+|"
    r"SU[- ]?NC\.?\s*(?:N[ºo°]\.?\s*)?\d*(?:[-/]\d+)?|"
    r"SUNC[-\s]?[A-Z0-9]+|"
    r"SECTOR\s+\d+|"
    r"La Alcantarilla|El Monte|Mirador del Montalvo|Las Eras"
    r")\b",
)
...
class CarrascalDeBarregasAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Divi + sede espublico + PlanPublica JCyL + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-carrascal-de-barregas/1.0")},
        )
