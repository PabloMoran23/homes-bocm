...
BASE = "https://www.ayto-carreno.es"
SEDE_BASE = "https://sedeelectronica.ayto-carreno.es"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{BASE}/normativa-urbanistica",
    f"{BASE}/estructura-p.g.o.",
    f"{BASE}/planos-de-informacion",
    f"{BASE}/planos-de-ordenacion-suelo-urbano",
    f"{BASE}/planos-de-ordenacion-suelo-no-urbanizable",
    f"{BASE}/planos-infraestructura-suelo-urbano",
    f"{BASE}/documentos-sometidos-a-informacion-publica",
    f"{BASE}/informacion-ambiental-y-urbanistica",
]

DEFAULT_LICENCIA_PAGES: list[str] = [
    f"{BASE}/normativa-urbanistica",
    f"{SEDE_BASE}/opensiac/action/tramites?method=enter",
    f"{SEDE_BASE}/opensiac/action/eppublicacion?method=enter&te=N",
]

RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|comunicaci[oó]n previa|declaraci[oó]n responsable|"
    r"autorizaci[oó]n (?:previa|urban)|ordenanza|normativa urban|edificaci[oó]n|obra)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|pgo|convenio|"
    r"informaci[oó]n p[uú]blica|expediente|proyecto|modificaci[oó]n|reparcel|"
    r"estudio (?:ac[uú]stico|ambiental|de detalle|de implantaci[oó]n)|normas? (?:provisionales|subsidiarias)|"
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|perlora|abono|cand[aá]s|carreño)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://www\.ayto-carreno\.es)?/c/document_library/get_file[^"]*|'
    r'(?:https://www\.ayto-carreno\.es)?/documents/[^"]+(?:\.pdf|-pdf)[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Carreño.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
class CarrenoAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay ayto-carreno.es + RPGUR Asturias; geometría parcial WFS visorrpgur."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "14")
    ...
    def _load_geom_index(self) -> dict[str, dict[str, Any]]:
        ...
        params = {
            ...
            "srsName": "EPSG:4326",
            "CQL_FILTER": "Instrumento LIKE '%CARREÑO%'",
        }
    ...
    def _crawl_liferay_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in RE_DOC_LINK.finditer(html):
                ...
                if not title or title == "get_file":
                    anchor = re.search(
                    ...
                    if anchor:
                        title = unescape(anchor.group(1).strip())
            ...
            for m in re.finditer(r'href="(https://www\.ayto-carreno\.es/[^"#?]+)"', html, re.I):
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        info_url = f"{BASE}/normativa-urbanistica"
        ...
        rows.append(
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Normativa urbanística y trámites",
                "url": info_url,
                "source": "ayuntamiento",
                "nota": "Sin tablón público scrapeable; sede electrónica con timeout en CI",
            }
        )

        tramites_url = f"{SEDE_BASE}/opensiac/action/tramites?method=enter"
        tramites_id = _stable_id("lic", tramites_url)
        if tramites_id not in seen:
            seen.add(tramites_id)
            rows.append(
                {
                    "id": tramites_id,
                    "fecha_concesion": None,
                    ...
                    "lat": None,
                    "lon": None,
                    "titulo": "Catálogo de procedimientos administrativos (sede)",
                    "url": tramites_url,
                    "source": "ayuntamiento",
                    "nota": "Sede opensiac; sin listado de concesiones público accesible",
                }
            )
                ...
                rows.append(
                    {
                        ...
                        "lat": None,
                        "lon": None,
                        "titulo": title[:500] if title and title != "get_file" else "Documento normativa urbanística",
                        "url": page_url,
                        "pdf_url": link,
                        "source": "ayuntamiento",
