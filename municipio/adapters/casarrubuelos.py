...
WP_BASE = "https://casarrubuelos.es"
SEDE_BASE = "https://casarrubuelos.sedelectronica.es"
MUNICIPIO = "Casarrubuelos"
ID_PREFIX = "casarrubuelos"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "CASARRUBUELOS"

URBANISMO_URL = f"{WP_BASE}/areas-municipales/urbanismo/"
BOARD_URL = f"{SEDE_BASE}/board"
TRANSPARENCY_URL = f"{SEDE_BASE}/transparency"

RE_PREVIEW = re.compile(
    r'href="(https://casarrubuelos\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra menor|obra mayor|vado|"
    r"ocupaci[oó]n de la v[ií]a|venta ambulante|parcelaci[oó]n)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|informaci[oó]n p[uú]blica|"
    r"expediente|proyecto|modificaci[oó]n|aprobaci[oó]n (?:inicial|definitiva)|"
    r"reparcel|bando|edicto|concesi[oó]n|transformaci[oó]n|"
    r"\b(?:UE|UA|AD|AN|AI|PAU|SAU|S|SUS)-[\w\d-]+\b)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|funcionario|empleo|plusvalia|bolsa|"
    r"residuos|vehiculos|notificaci[oó]n expediente|igualdad|"
    r"convocatoria.*plaza|jurado|iae\b|proceso selectivo|tribunal calificador|"
    r"ordenanza fiscal|ivtm|ibi\b)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|UA|AD|AN|AI|PAU|SAU|S|SUS)-[\w\d-]+)\b",
)
...
def _sector_ilike_parts(text: str) -> list[str]:
    ...
    for marker in (" del pgou", " pgou", " bocm", " aprob", " anuncio", " casarrubuelos"):
        if marker in low:
...
class CasarrubuelosAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress urbanismo + sede espublico gestiona + WFS SITCM (geometría partial)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-casarrubuelos/1.0")},
        )
    ...
    def _collect_urbanismo_pdfs(self) -> list[dict[str, Any]]:
        ...
        for m in re.finditer(
            ...
            pdf_url = urllib.parse.urljoin(self.urbanismo_url, m.group(1))
            if pdf_url in seen:
                continue
            seen.add(pdf_url)
            ...
            if not RE_LICENCIA.search(label) and not RE_PROYECTO.search(label):
                if not re.search(r"(?i)urb-|tasa|modelo", label):
                    continue
            rows.append(
                {
                    "titulo": label[:500],
    ...
    def _collect_wp_posts(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        try:
            cats = self._fetch_json(f"{WP_BASE}/wp-json/wp/v2/categories?per_page=100")
        ...
        cat_ids = [
            ...
            and any(x in str(c.get("slug") or "").lower() for x in ("urban", "licencia", "bando"))
        ]
        for cat_id in cat_ids:
            try:
                posts = self._fetch_json(
                    f"{WP_BASE}/wp-json/wp/v2/posts?categories={cat_id}&per_page=50&_fields=id,link,title,date"
                )
            except (urllib.error.URLError, json.JSONDecodeError):
                continue
            ...
            for p in posts:
                ...
                if raw_date:
                    try:
                        fecha = datetime.fromisoformat(raw_date.replace("Z", "+00:00")).strftime("%Y-%m-%d")
                    except ValueError:
                        fecha = raw_date[:10]
                rows.append(
                    {
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Concejalía de Licencias y Urbanismo — formularios",
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
                "nota": "Modelos URB (licencias obra, actividad, ocupación vía pública)",
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Sede electrónica — trámites urbanismo",
                "url": f"{self.sede_base}/info.0",
                "source": "ayuntamiento",
                "nota": "Presentación telemática de licencias y declaraciones responsables",
                "origen": "sede_tramite",
            },
        ]
    ...
    def _collect_sit_ambitos(self) -> list[dict[str, Any]]:
        ...
        for f in feats:
            ...
            same_name_feats = [
                x
                for x in feats
                if str((x.get("properties") or {}).get("DS_NOMB_AMB") or "").strip() == name
            ]
            merged = _merge_geometries(same_name_feats)
            rec: dict[str, Any] = {
    ...
    def _pdf_to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        if not RE_LICENCIA.search(row["titulo"]) and not re.search(r"(?i)urb-", row["titulo"]):
            return None
        return {
            ...
            "source": "ayuntamiento",
            "nota": "Modelo de solicitud/declaración; no concesión publicada",
            "origen": row.get("origen"),
        }
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "formularios": sum(1 for r in rows if r.get("origen") == "urbanismo_pdf"),
            "info": sum(1 for r in rows if r.get("origen") in ("sede_tablon", "urbanismo_web", "sede_tramite")),
        }
