...
WEB_BASE = "https://castellanosdecastro.es"
SEDE_BASE = "https://castellanosdecastro.sedelectronica.es"
...
ID_PREFIX = "castellanos-de-castro"
...
RE_PREVIEW = re.compile(
    r'href="(https://castellanosdecastro\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://castellanosdecastro\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
def _proyecto_tipo(blob: str, instrumento: str = "") -> str:
    ...
    if "sin planeamiento" in n or "spg" in n.split():
...
class CastellanosDeCastroAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=089"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=09&municipio=089"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-castellanos-de-castro/1.0")},
        )
