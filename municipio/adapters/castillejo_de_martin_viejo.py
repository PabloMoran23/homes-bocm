...
SEDE_BASE = "https://castillejodemartinviejo.sedelectronica.es"
...
ID_PREFIX = "castillejo-de-martin-viejo"
WFS_C_MUN = "37096"
PLAN_PROVINCIA = "37"
PLAN_MUNICIPIO = "096"
...
RE_PREVIEW = re.compile(
    r'href="((?:https://castillejodemartinviejo\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://castillejodemartinviejo\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"unidad(?:es)? de ejecuci[oó]n|suelo urbano|dsu|uso excepcional)",
)
...
class CastillejoDeMartinViejoAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico + PlanPublica JCyL + IDECyL WFS (municipio rural CYL/Salamanca)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-castillejo-de-martin-viejo/1.0")},
        )
    ...
    def _attach_geometry(self, rec: dict[str, Any]) -> None:
        ...
        for code in _sector_codes_from_text(blob):
            if code in self._sector_geom_cache:
                hit = self._sector_geom_cache[code]
                if hit:
