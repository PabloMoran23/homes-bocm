...
WEB_BASE = "https://www.castrillodelavega.es"
SEDE_BASE = "https://castrillodelavega.sedelectronica.es"
...
ID_PREFIX = "castrillo-de-la-vega"
DEFAULT_UA = (
    "Mozilla/5.0 (compatible; poc-bocm-castrillo-de-la-vega/1.0; +urbanismo municipal research)"
)
...
DEFAULT_DRUPAL_SEEDS: list[tuple[str, str]] = [
    (f"{WEB_BASE}/normativa", "Normativa municipal — urbanismo"),
    (f"{WEB_BASE}/node/863", "Ordenanza Nº 1: Casco Tradicional CT"),
    (
        f"{WEB_BASE}/node/864",
        "Aprobación inicial modificación puntual NUM",
    ),
    (
        f"{WEB_BASE}/node/865",
        "Aprobación inicial modificaciones puntuales Nº 2 y 3 NUM",
    ),
]
...
DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{SEDE_BASE}/catalog/t/e8594295-30ea-4a16-8f17-60c061a8a147",
        "Solicitud de Recepción de Obras de Urbanización",
    ),
    (
        f"{SEDE_BASE}/catalog/t/e64a8029-2e12-49aa-a002-120e27ca1386",
        "Solicitud de Declaración de Ruina",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://castrillodelavega\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://castrillodelavega\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_DRUPAL_PDF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?castrillodelavega\.es)?/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra menor|"
    r"primera ocupaci[oó]n|primera utilizaci[oó]n|ambiental|ocupaci[oó]n)",
)
RE_PROYECTO = re.compile(
    ...
    r"ordenanza|casco tradicional|\bed[- ]?\d)",
)
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"PE[- ]?\d+|AR[- ]?P\d+|ED[- ]?\d+|UNC\d+|"
...
RE_EXCLUDE = re.compile(
    r"(?i)(juez de paz|vacante juez|proceso selectivo|empleo p[uú]blico|"
    r"subvenci[oó]n deportiv|fiestas patronales|alumbrado p[uú]blico)",
)
...
class CastrilloDeLaVegaAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal Toools + sede espublico + PlanPublica PLAU + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=085"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=09&municipio=085"
        )
        ...
        raw_drupal = self.config.get("drupal_seeds") or DEFAULT_DRUPAL_SEEDS
        self.drupal_seeds: list[tuple[str, str]] = []
        for item in raw_drupal:
            if isinstance(item, dict):
                self.drupal_seeds.append((str(item["url"]), str(item.get("titulo") or item["url"])))
            elif isinstance(item, (list, tuple)) and len(item) >= 2:
                self.drupal_seeds.append((str(item[0]), str(item[1])))
            else:
                self.drupal_seeds.append((str(item), str(item)))
    ...
    def _user_agent(self) -> str:
        return str(self.config.get("user_agent") or DEFAULT_UA)
    ...
    def _collect_drupal_pages(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for page_url, seed_title in self.drupal_seeds:
            try:
                html = self._fetch(page_url)
            except (urllib.error.URLError, OSError, ConnectionError):
                continue
            title_m = re.search(r"<title>([^<|]+)", html, re.I)
            page_title = unescape(title_m.group(1).strip()) if title_m else seed_title
            page_title = re.sub(r"\s*\|\s*Castrillo de la Vega.*$", "", page_title).strip()
            page_title = page_title.strip("“”\"'")
            if not page_title:
                page_title = seed_title
            key = f"page:{page_url}"
            if key not in seen and RE_PROYECTO.search(page_title):
                seen.add(key)
                rows.append(
                    {
            ...
            for pdf_m in RE_DRUPAL_PDF.finditer(html):
                pdf_href = pdf_m.group(1)
                if not pdf_href.startswith("http"):
                    pdf_href = urljoin(self.web_base, pdf_href)
                pdf_name = unescape(pdf_href.rsplit("/", 1)[-1])
                pdf_key = pdf_href
                if pdf_key in seen:
                    continue
                seen.add(pdf_key)
                titulo = f"{page_title} — {pdf_name}"
                rows.append(
                    {
                        "titulo": titulo[:500],
                        "url": pdf_href,
        ...
        for layer, default_tipo in WFS_LAYERS:
            ...
            for feat in data.get("features") or []:
                ...
                if sector and num:
                    titulo = f"{sector} ({num})"
                if not titulo:
                    titulo = str(props.get("c_id_sect") or props.get("c_plan") or props.get("n_titulo") or layer)
    ...
    def _attach_geometry(self, rec: dict[str, Any]) -> None:
        ...
        blob = " ".join(
            str(rec.get(k) or "")
            for k in ("titulo", "descripcion", "expte", "sector_code", "instrumento", "subtipo")
        )
    ...
    def _tramite_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if RE_LICENCIA.search(row["titulo"]) and not re.search(
            r"(?i)planeam|actuaci[oó]n urban|modificaci[oó]n del planeamiento|ruina|urbanizaci",
            row["titulo"],
        ):
            return None
        rec: dict[str, Any] = {
