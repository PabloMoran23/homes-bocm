...
DRUPAL_BASE = "https://www.castrilloderiopisuerga.es"
SEDE_BASE = "https://castrilloderiopisuerga.sedelectronica.es"
...
ID_PREFIX = "castrillo-de-riopisuerga"
...
RE_PREVIEW = re.compile(
    r'href="(https://castrilloderiopisuerga\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://castrilloderiopisuerga\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class CastrilloDeRiopisuergaAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=088"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=09&municipio=088"
        )
        self.seed_pages: list[tuple[str, str]] = [
            (f"{self.drupal_base}/normativa", "Normativa municipal"),
            (self.plau_url, "Archivo planeamiento urbanístico (PLAU JCyL)"),
            (self.plai_url, "Planeamiento en información pública (PLAI JCyL)"),
        ]
    ...
    def _warm_sede_session(self) -> None:
        if self._sede_warmed:
            return
        try:
            req = urllib.request.Request(
                self.info_url,
                headers={"User-Agent": self.config.get("user_agent", "poc-bocm-castrillo-de-riopisuerga/1.0")},
            )
            self._opener.open(req, timeout=60).read()
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        ua = self.config.get("user_agent", "poc-bocm-castrillo-de-riopisuerga/1.0")
        req = urllib.request.Request(url, headers={"User-Agent": ua})
        if sede:
            self._warm_sede_session()
            opener = self._opener
        else:
    ...
    def _collect_seed_pages(self) -> list[dict[str, Any]]:
        ...
        for url, titulo in self.seed_pages:
            rows.append({"titulo": titulo[:500], "url": url, "origen": "drupal_seed"})
        return rows
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_seed_pages():
            rec = self._tramite_to_proyecto(item) or {
