...
SEDE_BASE = "https://cenicientos.sedelectronica.es"
MUNICIPIO = "Cenicientos"
ID_PREFIX = "cenicientos"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "CENICIENTOS"
NNSS_BOE_URL = "https://www.boe.es/diario_boe/txt.php?id=BOE-A-1978-1025"

RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|ocupaci[oó]n.*v[ií]a|terrazas|vado)",
)
RE_PROYECTO = re.compile(
    ...
    r"reparcel|bando|edicto|actuaci[oó]n urban|protecci[oó]n civil|infoma|"
    r"\b(?:UE|UA|AD|AN|AI|PAU|SAU|S)-\d+[A-Z0-9-]*\b)",
)
RE_EXCLUDE = re.compile(
    ...
    r"fiestas patronales|hosteler[ií]a|pirot[eé]cnic|fuego|horario de cierre)",
)
...
def _sector_ilike_parts(text: str) -> list[str]:
    s = text.strip()
    low = s.lower()
    for marker in (" del pgou", " pgou", " bocm", " aprob", " anuncio", " de cenicientos"):
        if marker in low:
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if re.search(r"\bsau-\d+\b", n) or "suelo urbanizable" in n:
        return "suelo urbanizable"
    if "nnss" in n or "normas subsidiarias" in n:
        return "normas subsidiarias"
    if "plan especial" in n or "infoma" in n:
...
class CenicientosAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona (tablón) + WFS SITCM (geometría partial)."""
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "source": "ayuntamiento",
                "nota": "Licencias de obra, declaración responsable, cédula urbanística",
                "origen": "sede_tramite",
            },
        ]
        ...
        return [
            {
                ...
                "source": "ayuntamiento",
                "nota": "5 documentos en sección 7 (Wicket AJAX; consulta manual)",
                "origen": "transparencia",
            },
            {
                "id": _stable_id("proy", NNSS_BOE_URL),
                "municipio": MUNICIPIO,
                "titulo": "Normas Complementarias y Subsidiarias del Planeamiento (NNSS) — BOE 1978",
                "fecha": "1978-01-13",
                "tipo": "normas subsidiarias",
                "url": NNSS_BOE_URL,
                "source": "ayuntamiento",
                "nota": "Aprobación BOE-A-1978-1025; ámbitos en SITCM",
                "origen": "nnss_boe",
            },
        ]
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if RE_EXCLUDE.search(blob) and not re.search(
            r"(?i)exposici[oó]n p[uú]blica|informaci[oó]n p[uú]blica|plan especial|infoma",
            blob,
        ):
            return None
        if RE_LICENCIA.search(blob) and not RE_PROYECTO.search(blob):
            return None
        if row.get("procedimiento", "").lower() == "actuaciones urbanísticas":
            pass
