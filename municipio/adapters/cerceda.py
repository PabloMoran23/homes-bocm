...
RE_EXCLUDE = re.compile(
    ...
    r"gymkhana urbana|autotaxi|incendios forestales|olas de calor|summer sport|"
    r"concierto|mercado medieval|hinchables|fiesta de la espuma|ajedrez|macarronada|"
    r"feria taurina|cirqueando|banda sinf[oó]nica)",
)
...
RE_CERCEDA = re.compile(r"(?i)\bcerceda\b")
...
def _pdf_relevant_for_cerceda(title: str, pdf_url: str) -> bool:
    blob = f"{title} {pdf_url}".lower()
    if "mataelpino" in blob and "cerceda" not in blob:
        return False
    if re.search(r"plano.*el[\-\s]?boalo", blob) and "cerceda" not in blob:
        return False
    if "cerceda" in blob:
        return True
    if any(k in blob for k in ("normas-subsidiarias", "normas subsidiarias", "pgou")):
        return True
...
class CercedaAyuntamientoAdapter(AyuntamientoAdapter):
    """Cerceda (localidad) — WordPress + sede add4u + SITCM WFS (partial)."""
    ...
    def _collect_seed_pdfs(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.seed_pages:
            ...
            for title, pdf_url in self._extract_pdfs(html):
                if not _pdf_relevant_for_cerceda(title, pdf_url):
                    continue
    ...
    def _collect_wp_proyectos(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for search in ("cerceda", None):
            for post in self._paginate_wp_posts(search=search):
    ...
    def _parse_tablon_html(self, html: str, source: str) -> list[dict[str, Any]]:
        ...
        for m in RE_TABLON_LINK.finditer(html):
            ...
            if not title or len(title) < 5:
                continue
            if not RE_CERCEDA.search(title) and not (
                RE_PROYECTO.search(title) or RE_LICENCIA.search(title)
            ):
                continue
        ...
        for m in RE_TABLON_ROW.finditer(html):
            ...
            if len(text) < 15:
                continue
            if not RE_CERCEDA.search(text) and not (
                RE_PROYECTO.search(text) or RE_LICENCIA.search(text)
            ):
                continue
