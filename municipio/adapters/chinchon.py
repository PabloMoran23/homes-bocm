...
WEB_BASE = "https://www.ciudad-chinchon.com"
SEDE_BASE = "https://chinchon.sedelectronica.es"
MUNICIPIO = "Chinchón"
ID_PREFIX = "chinchon"

URBANISMO_URL = f"{WEB_BASE}/ayuntamiento/concejalias/servicios-urbanisticos/presentacion.php"
IMPRESOS_URL = f"{WEB_BASE}/ayuntamiento/ayto/descarga-impresos.php"
LEY_SUELO_URL = f"{WEB_BASE}/ayuntamiento/pdf/ley-suelo-CAM.pdf"
...
WFS_MUNICIPIO = "CHINCHÓN"

URBANISMO_PAGES: list[tuple[str, str]] = [
    (f"{WEB_BASE}/ayuntamiento/concejalias/servicios-urbanisticos/tramitacion-licencias.php", "tramitacion_licencias"),
    (f"{WEB_BASE}/ayuntamiento/concejalias/servicios-urbanisticos/obras-mayores.php", "obras_mayores"),
    (f"{WEB_BASE}/ayuntamiento/concejalias/servicios-urbanisticos/obras-menores.php", "obras_menores"),
    (f"{WEB_BASE}/ayuntamiento/concejalias/servicios-urbanisticos/licencias-ocupacion.php", "licencias_ocupacion"),
]

LICENCIA_PDFS: list[tuple[str, str]] = [
    (f"{WEB_BASE}/ayuntamiento/images-bbdd/impresos/instancia_obras.pdf", "Solicitud instancia de obras"),
    (f"{WEB_BASE}/ayuntamiento/images-bbdd/impresos/licencia_funcionamiento.pdf", "Licencia de funcionamiento"),
    (
        f"{WEB_BASE}/ayuntamiento/images-bbdd/impresos/licencia_apertura_sin_evaluacion_ambiental.pdf",
        "Licencia de apertura sin evaluación ambiental",
    ),
    (
        f"{WEB_BASE}/ayuntamiento/images-bbdd/impresos/licencia_instalacion_con_evaluacion_ambiental.pdf",
        "Licencia de instalación con evaluación ambiental",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://chinchon\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra (?:mayor|menor)|"
    r"primera ocupaci[oó]n|cedula urban|c[eé]dula urban|instancia de obra)",
)
RE_PROYECTO = re.compile(
    ...
    r"ley del suelo|\b(?:UE|UA|AD|AN|AI|PAU|S)-\d+[A-Z0-9-]*\b)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|empleo|jurado|selecci[oó]n de personal|"
    r"activaci[oó]n profesional|igualdad|bolsa|subvenci[oó]n|iae\b|"
    r"cobranza|ampas?|asociaciones agricolas|deportiv)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|UA|AD|AN|AI|PAU|S)-\d+[A-Z0-9-]*)\b",
)
...
def _proyecto_tipo(title: str) -> str:
    n = title.lower()
    if re.search(r"\bua-\d+\b", n) or re.search(r"\bue-\d+\b", n) or "unidad de ejecuci" in n:
...
def _sector_ilike_parts(text: str) -> list[str]:
    low = text.lower()
    for marker in (" bocm", " aprob", " anuncio", " del ", " de chin"):
        if marker in low:
...
class ChinchonAyuntamientoAdapter(AyuntamientoAdapter):
    """Web ciudad-chinchon.com (PHP) + sede espublico + ámbitos SITCM WFS."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-chinchon/1.0")},
        )
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = [
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Servicios Técnicos Urbanísticos — licencias y obras",
        ...
        for page_url, origen in URBANISMO_PAGES:
            rows.append(
                {
                    ...
                    "lat": None,
                    "lon": None,
                    "titulo": f"Trámites urbanísticos — {origen.replace('_', ' ')}",
                    "url": page_url,
                    "source": "ayuntamiento",
                    "nota": "Documentación requerida; no concesión publicada",
                    "origen": origen,
                }
            )
        return rows
    ...
    def _collect_impresos_licencias(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for pdf_url, label in LICENCIA_PDFS:
            rows.append(
                {
    ...
    def _collect_ley_suelo_seed(self) -> list[dict[str, Any]]:
        rec: dict[str, Any] = {
            "id": _stable_id("proy", LEY_SUELO_URL),
            "municipio": MUNICIPIO,
            "titulo": "Ley del Suelo de la Comunidad de Madrid — folleto informativo",
            "fecha": None,
            "tipo": "normativa",
            "url": LEY_SUELO_URL,
            "pdf_url": LEY_SUELO_URL,
            "source": "ayuntamiento",
            "origen": "web_urbanismo",
            "nota": "Documento de referencia en sección urbanismo",
        }
        return [rec]
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_sit_ambitos():
            add(rec)
        for rec in self._collect_ley_suelo_seed():
            add(rec)
        for item in self._collect_board():
