...
SEDE_BASE = "https://chiva.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WEB_BASE = "https://www.chiva.es"
TRANSPARENCIA_NORMAS_URL = f"{WEB_BASE}/transparencia/normas-urbanisticas"
TRANSPARENCIA_TRAMITACION_URL = f"{WEB_BASE}/transparencia/documentos-tramitacion"
URBANISMO_URL = f"{WEB_BASE}/urbanismo"
IMPRESOS_URL = f"{WEB_BASE}/impresos"
URBANIZACIONES_URL = f"{WEB_BASE}/seccion/urbanizaciones"
MUNICIPIO = "Chiva"
ID_PREFIX = "chiva"
COD_INE_MUN = "46100"

DEFAULT_SEED_PAGES: list[str] = [
    TRANSPARENCIA_NORMAS_URL,
    TRANSPARENCIA_TRAMITACION_URL,
    URBANISMO_URL,
    IMPRESOS_URL,
    URBANIZACIONES_URL,
    f"{WEB_BASE}/transparencia/visor",
]
...
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://chiva\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_PDF_HREF = re.compile(
    r'href="((?:https://www\.chiva\.es)?/sites/www\.chiva\.es/files/[^"]+\.pdf)"',
    re.I,
)
RE_WEB_LINK = re.compile(
    r'href="((?:https://www\.chiva\.es)?/(?:transparencia|urbanismo|seccion|impresos)[^\"#?]+)"',
    re.I,
)
...
class ChivaAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.transparencia_normas_url = str(
            self.config.get("transparencia_normas_url") or TRANSPARENCIA_NORMAS_URL
        )
        self.transparencia_tramitacion_url = str(
            self.config.get("transparencia_tramitacion_url") or TRANSPARENCIA_TRAMITACION_URL
        )
    ...
    def _fetch(self, url: str, *, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-chiva/1.0")},
        )
    ...
    def _abs_web_url(self, path: str) -> str:
    ...
    def _extract_pdfs(self, html: str) -> list[str]:
        ...
        for m in RE_PDF_HREF.finditer(html):
            url = self._abs_web_url(m.group(1))
            if url not in seen:
                seen.add(url)
                pdfs.append(url)
        return pdfs
    ...
    def _page_title(self, html: str, fallback: str = "") -> str:
        ...
        if m:
            ...
            if t and "Ayuntamiento de Chiva" not in t[:30]:
                return t.replace("| Ayuntamiento de Chiva", "").strip()
        return fallback
    ...
    def _collect_web_pdfs(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            title = self._page_title(html, url.rsplit("/", 1)[-1])
            pdfs = self._extract_pdfs(html)
            blob = f"{title} {url}"

            if pdfs and (
                RE_PROYECTO.search(blob)
                or RE_LICENCIA.search(blob)
                or "urban" in url.lower()
            ):
                for pdf in pdfs:
                    ...
                    rec: dict[str, Any] = {
                        ...
                        "fecha": _fecha_from_expediente(pdf_name) or _parse_fecha_dmy(html),
            ...
            if len(visited) < self.max_crawl_pages:
                for m in RE_WEB_LINK.finditer(html):
                    link = self._abs_web_url(m.group(1)).rstrip("/")
                    if link in visited or link in queue:
                        continue
                    if any(x in link for x in ("aviso-legal", "politica-privacidad", "contacto")):
                        continue
                    queue.append(link)
    ...
    def _collect_impresos_licencias(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        try:
            html = self._fetch(self.impresos_url)
        except urllib.error.URLError:
            return rows
        for m in re.finditer(
            r'href="((?:https://www\.chiva\.es)?/(?:sites/www\.chiva\.es/files/[^"]+|documentos/impresos/[^"]+))"',
            html,
            re.I,
        ):
            url = self._abs_web_url(m.group(1))
            if not RE_LICENCIA.search(url):
                continue
            ...
            rows.append(
                {
                    ...
                    "url": url,
                    "source": "ayuntamiento",
                    "nota": "Impreso informativo; no concesión publicada",
                    "origen": "impresos",
                }
            )
        return rows
    ...
    def _collect_proyecto_info_pages(self) -> list[dict[str, Any]]:
        return [
            {
                "id": _stable_id("proy", self.transparencia_normas_url),
                "municipio": MUNICIPIO,
                "titulo": "Transparencia — normas urbanísticas",
                "fecha": None,
                "tipo": "normativa",
                "url": self.transparencia_normas_url,
                "source": "ayuntamiento",
                "origen": "transparencia",
                "nota": "PGOU planos ZIP, NNSS, ordenanzas",
            },
            {
                "id": _stable_id("proy", self.transparencia_tramitacion_url),
                "municipio": MUNICIPIO,
                "titulo": "Transparencia — documentos en tramitación",
                "fecha": None,
                "tipo": "tramitación",
                "url": self.transparencia_tramitacion_url,
                "source": "ayuntamiento",
                "origen": "transparencia",
                "nota": "PAM/PTM planos y memorias en tramitación",
            },
            {
                "id": _stable_id("proy", self.urbanismo_url),
                "municipio": MUNICIPIO,
                "titulo": "Urbanismo — Chiva Urban LAB",
                "fecha": None,
                "tipo": "normativa",
            ...
            {
                "id": _stable_id("proy", self.urbanizaciones_url),
                "municipio": MUNICIPIO,
                "titulo": "Urbanizaciones y diseminados",
                "fecha": None,
                "tipo": "urbanización",
                "url": self.urbanizaciones_url,
                "source": "ayuntamiento",
                "origen": "urbanismo",
            },
            {
                "id": _stable_id("proy", "https://visor.gva.es/visor/?capas=spaicv0702_plan_zonificacion"),
                "municipio": MUNICIPIO,
                "titulo": "Visor GVA — zonificación urbanística ICV",
                "fecha": None,
                "tipo": "visor GIS",
                "url": "https://visor.gva.es/visor/?capas=spaicv0702_plan_zonificacion",
                "source": "ayuntamiento",
                "origen": "visor",
            },
            {
