...
SEDE_BASE = "https://cigales.sedelectronica.es"
WP_BASE = "https://www.cigales.es"
...
RE_PREVIEW = re.compile(
    r'href="(https://cigales\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://cigales\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_WP_EXCLUDE = re.compile(
    r"(?i)(fiestas|cine|empleo|concurso|deporte|pij\b|punto de informaci[oó]n juvenil|"
    r"facebook|agenda cultural|biblioteca|conciliamos|subvenci[oó]n escolar)",
)
RE_LICENCIA = re.compile(
    ...
    r"recepci[oó]n de obras)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|convenio|"
    r"informaci[oó]n p[uú]blica|exposici[oó]n p[uú]blica|expediente|proyecto de (?:actuaci|obras)|"
    r"estudio de detalle|modificaci[oó]n|aprobaci[oó]n (?:inicial|definitiva)|reparcel|sector|"
    r"edicto|bando.*parcel|actuaci[oó]n urban|normas urban|num\b|instrumento|parque tecnol[oó]gico|"
    r"desafectaci[oó]n|bocyl|bopva)",
)
RE_NOISE = re.compile(
    ...
    r"pe[oó]n de obra-planiel|piscina municipal|polic[ií]a local)",
)
...
def _iso_date_wp(text: str) -> str | None:
    m = re.search(r"\b((?:19|20)\d{2})-(\d{2})-(\d{2})", text or "")
    if not m:
        return None
...
class CigalesAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + WP municipal + PLAU/PLAI JCYL + geometría parcial IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or (
                f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N"
                f"&provincia={self.plai_provincia}&municipio={self.plai_municipio:03d}"
            )
        )
        ...
        self.wfs_municipio = str(self.config.get("wfs_municipio") or "Cigales")
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-cigales/1.0")},
        )
    ...
    def _collect_wp_posts(self, search: str, max_pages: int = 4) -> list[dict[str, Any]]:
        items: list[dict[str, Any]] = []
        for page in range(1, max_pages + 1):
            params = urllib.parse.urlencode(
                {"search": search, "per_page": "100", "page": str(page), "status": "publish"}
            )
    ...
    def _wp_item_to_licencia(self, item: dict[str, Any]) -> dict[str, Any] | None:
    ...
    def _wp_item_to_proyecto(self, item: dict[str, Any]) -> dict[str, Any] | None:
        title = _strip_html(str((item.get("title") or {}).get("rendered") or ""))
        if not title or RE_WP_EXCLUDE.search(title):
            return None
        content = str((item.get("content") or {}).get("rendered") or "")
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            (f"{WP_BASE}/normativas-municipales/", "Normativas municipales urbanísticas"),
            (
                "https://cigales.es/normativas-municipales/",
                "Normativas municipales (ordenanzas y PGOU)",
            ),
            ("https://uer.diputaciondevalladolid.es/Cigales/", "Normativas municipales urbanísticas"),
        ]
            ...
            rows.append(
                {
                    ...
                    "url": url,
                    "source": "ayuntamiento",
                    "nota": "Página informativa de trámite o visor",
                    "origen": "wp_tramite",
                }
            )
        return rows
        ...
        params = urllib.parse.urlencode(
            {
                ...
                "srsName": "EPSG:4326",
                "count": "200",
                "CQL_FILTER": f"n_mun ILIKE '{self.wfs_municipio}%'",
            }
        )
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if row.get("categoria", "").lower() == "urbanismo":
            ...
            if row.get("categoria") not in ("Órganos de gobierno",):
                return None
            if not re.search(r"(?i)pleno|acuerdo|certificado", blob):
                return None
        key = row.get("expediente") or row["url"]
        rec: dict[str, Any] = {
    ...
    def _plai_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        rec: dict[str, Any] = {
            "id": _stable_id("proy", f"{row.get('origen','plai')}:{key}"),
            "municipio": MUNICIPIO,
            "titulo": row["titulo"],
            "fecha": row.get("fecha"),
        ...
        if row.get("subtipo"):
            rec["subtipo"] = row["subtipo"]
        self._attach_geometry(rec)
        return rec

    def _plau_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        return self._plai_to_proyecto(row)

    def _collect_normativas_seeds(self) -> list[dict[str, Any]]:
        seeds = [
            (
                "https://cigales.es/normativas-municipales/",
                "Normativas municipales urbanísticas",
            ),
            (
                "https://www.dropbox.com/s/xxs58nizlhs6viy/PGOU%20%28Documentaci%C3%B3n%20no%20vinculante.%20En%20tramitacion%29.zip?dl=0",
                "PGOU (documentación no vinculante, en tramitación)",
            ),
        ]
        rows: list[dict[str, Any]] = []
        for url, titulo in seeds:
            rec: dict[str, Any] = {
                "id": _stable_id("proy", url),
                "municipio": MUNICIPIO,
                "titulo": titulo,
                "fecha": "2024-05-29" if "PGOU" in titulo else None,
                "tipo": "PGOU" if "PGOU" in titulo else "normativa urbanística",
                "url": url,
                "source": "ayuntamiento",
                "origen": "wp_normativas",
            }
            self._attach_geometry(rec)
            rows.append(rec)
        return rows
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        for item in self._collect_tramites():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_wp_posts("licencia ambiental"):
            rec = self._wp_item_to_licencia(item)
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
    ...
    def _collect_proyectos(self) -> list[dict[str, Any]]:
        ...
        for item in self._collect_plai():
            ...
            by_id[rec["id"]] = rec
        for rec in self._collect_normativas_seeds():
            by_id[rec["id"]] = rec
        ...
        for item in self._collect_tramites():
            ...
            if rec:
                by_id[rec["id"]] = rec
        for item in self._collect_wp_posts("información pública"):
            rec = self._wp_item_to_proyecto(item)
            if rec:
                by_id[rec["id"]] = rec
        for item in self._collect_wp_posts("plan parcial"):
            rec = self._wp_item_to_proyecto(item)
            if rec:
                by_id[rec["id"]] = rec
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "wp": sum(1 for r in rows if str(r.get("origen") or "").startswith("wp")),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
            "tramites": sum(1 for r in rows if r.get("origen") == "catalogo_tramites"),
            "wp": sum(1 for r in rows if r.get("origen") == "wp_noticias"),
        }
