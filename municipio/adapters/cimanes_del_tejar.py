...
WEB_BASE = "https://www.aytocimanesdeltejar.es"
SEDE_BASE = "https://cimanesdeltejar.sedelectronica.es"
MUNICIPIO = "Cimanes del Tejar"
ID_PREFIX = "cimanes-del-tejar"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    ...
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=055",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=055",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/openDocuIndice.do?cDocId=301592",
        "Normas Urbanísticas Territoriales — índice documental",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://cimanesdeltejar\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://cimanesdeltejar\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class CimanesDelTejarAyuntamientoAdapter(AyuntamientoAdapter):
    """OpenCms web (Diputación León) + sede espublico tablón + IDECyL WFS (CYL)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-cimanes-del-tejar/1.0")},
        )
