...
WEB_BASE = "https://www.aytocolladomediano.es"
SEDE_BASE = "https://aytocolladomediano.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
URBANISMO_URL = f"{WEB_BASE}/areas-municipales/urbanismo-obras-y-vivienda"
PGOU_URL = f"{WEB_BASE}/areas-municipales/urbanismo-obras-y-vivienda/pgou"
LICENCIAS_OBRA_URL = (
    f"{WEB_BASE}/areas-municipales/urbanismo-obras-y-vivienda/licencias/licencias-de-obra"
)
LICENCIAS_ACTIVIDAD_URL = (
    f"{WEB_BASE}/areas-municipales/urbanismo-obras-y-vivienda/licencias/licencias-de-actividad"
)
CONSULTAS_URB_URL = f"{WEB_BASE}/tramites/consultas-urbanismo"
MUNICIPIO = "Collado Mediano"
ID_PREFIX = "collado-mediano"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "COLLADO MEDIANO"

DEFAULT_URBANISMO_URLS = (PGOU_URL,)

AMBITO_KEYWORDS: tuple[tuple[str, str], ...] = (
    ("cerro castillo 1", "SAU-1 CERRO CASTILLO 1"),
    ("cerro castillo 2", "SAU-2 CERRO CASTILLO 2"),
    ("cerro castillo", "SAU-1 CERRO CASTILLO 1"),
    ("serrania de la paloma", "ZONA 13 SERRANIA DE LA PALOMA 1"),
    ("sau-12", "SAU-12 INDUSTRIAL"),
    ("industrial", "SAU-12 INDUSTRIAL"),
    ("peñarrubia", "SAU-11 PEÑARRUBIA"),
    ("penarrubia", "SAU-11 PEÑARRUBIA"),
    ("dehesilla 3", "SAU-9 LA DEHESILLA 3"),
    ("dehesilla 2", "SAU-8 LA DEHESILLA 2"),
    ("dehesilla 1", "SAU-7 LA DEHESILLA 1"),
    ("la dehesilla", "SAU-7 LA DEHESILLA 1"),
    ("arroyo del soto 2", "SAU-6 ARROYO DEL SOTO 2"),
    ("arroyo del soto 1", "SAU-5 ARROYO DEL SOTO 1"),
    ("arroyo del soto", "SAU-5 ARROYO DEL SOTO 1"),
    ("el berrocal", "SAU-10 EL BERROCAL"),
    ("berrocal", "SAU-10 EL BERROCAL"),
    ("la roya", "SAU-4 LA ROYA"),
    ("la hontanilla", "SAU-3 LA HONTANILLA"),
    ("hontanilla", "SAU-3 LA HONTANILLA"),
)
...
RE_LICENCIA = re.compile(
    ...
    r"evaluaci[oó]n ambiental.*actividad|documentaci[oó]n licencia|modelo.*licencia)",
)
RE_PROYECTO = re.compile(
    ...
    r"ordenanza|edtu|sector|urbanizaci[oó]n|calificaci[oó]n urban|sau-|residuos municipal)",
)
...
RE_PREVIEW_LINK = re.compile(
    r'href="(https://aytocolladomediano\.sedelectronica\.es/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_PDF_HREF = re.compile(r'href="([^"]+\.pdf[^"]*)"', re.I)
RE_SKIP_PDF = re.compile(
    r"(?i)(clasificaci[oó]n_tm|usos_del_suelo|ordenaci[oó]n_general|infraest_|carreteras|"
    r"plano_cat[aá]logo|topogr[aá]fico|situacion-y-emplazamiento|colectores|"
    r"afecti[oó]n_|encuadre_general|inventario_snu|instalaciones_en_snu)",
)
...
def _doc_tipo(name: str) -> str:
    ...
    if "resumen ejecutivo" in n:
        return "PGOU"
    if "normas urban" in n:
        return "normas urbanísticas"
    if "certificado" in n:
        return "certificado planeamiento"
...
class ColladoMedianoAyuntamientoAdapter(AyuntamientoAdapter):
    """Joomla (urbanismo/PGOU PDFs) + tablón eHome espublico + SITCM WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.licencia_urls = [
            str(u)
            for u in (
                self.config.get("licencia_urls")
                or (LICENCIAS_OBRA_URL, LICENCIAS_ACTIVIDAD_URL)
            )
        ]
        self.consultas_url = str(self.config.get("consultas_url") or CONSULTAS_URB_URL)
    ...
    def _fetch_json(self, url: str) -> Any:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-collado-mediano/1.0")},
        )
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for page_url in self.licencia_urls:
            try:
                html = self._fetch(page_url)
            except urllib.error.URLError:
                continue
        ...
        return [
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo, Obras y Vivienda",
                "url": URBANISMO_URL,
                "source": "ayuntamiento",
            ...
            },
            {
                ...
                "fecha_concesion": None,
                "tipo": "PGOU y planeamiento",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "PGOU — Plan General de Ordenación Urbana",
                "url": PGOU_URL,
                "source": "ayuntamiento",
                "nota": "Documentación del PGOU en tramitación/aprobación",
                "origen": "urbanismo_web",
            },
            {
                "id": _stable_id("lic", LICENCIAS_OBRA_URL),
                "fecha_concesion": None,
                "tipo": "trámites licencias de obra",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Licencias de obra — modelos y documentación",
                "url": LICENCIAS_OBRA_URL,
                "source": "ayuntamiento",
                "nota": "Modelos de solicitud y declaración responsable",
                "origen": "tramites_web",
            },
            {
                "id": _stable_id("lic", LICENCIAS_ACTIVIDAD_URL),
                "fecha_concesion": None,
                "tipo": "trámites licencias de actividad",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Licencias de actividad — documentación exigible",
                "url": LICENCIAS_ACTIVIDAD_URL,
                "source": "ayuntamiento",
                "nota": "Modelos y guías de trámites de actividad",
                "origen": "tramites_web",
            },
            {
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Sede electrónica — catálogo trámites",
    ...
    def _doc_to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if row.get("origen") == "tramites_web" and not re.search(
            r"(?i)licen|declaraci|documentaci|obra|actividad|urban|solicitud|autoliq|hojacaract",
            titulo,
        ):
            return None
        key = row.get("pdf_url") or row["url"]
