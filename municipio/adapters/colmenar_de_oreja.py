...
DRUPAL_BASE = "https://www.aytocdo.es"
SEDE_BASE = "https://colmenardeoreja.sedelectronica.es"
MUNICIPIO = "Colmenar de Oreja"
ID_PREFIX = "colmenar-de-oreja"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "COLMENAR DE OREJA"

DEFAULT_SEED_PAGES: list[str] = [
    f"{DRUPAL_BASE}/areas/urbanismo",
    f"{DRUPAL_BASE}/areas/urbanismo/noticias",
    f"{DRUPAL_BASE}/documentos",
    f"{DRUPAL_BASE}/descargas-generales",
]

RE_PREVIEW = re.compile(
    r'href="(https://colmenardeoreja\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://colmenardeoreja\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra (?:mayor|menor)|"
    r"primera ocupaci[oó]n|apertura|acometida|renovaci[oó]n licencia)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|bocm|convenio|"
    r"informaci[oó]n p[uú]blica|expediente|proyecto|modificaci[oó]n|aprobaci[oó]n|"
    r"reparcel|sector|parcela|embellecimiento|patrimonio|licitaci[oó]n.*urban|"
    r"\b(?:UA|S)-\d+\b)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|empleo p[uú]blico|selecci[oó]n de personal|"
    r"proceso selectivo|cobranza|iae|igualdad|jurado|nota informativa tribunal|"
    r"convocatoria boe tag|tag oep|reactivaci[oó]n proceso)",
)
RE_AMBIT_CODE = re.compile(r"(?i)\b((?:UA|S)-\d+)\b")
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DRUPAL_PDF = re.compile(
    r'href="((?:https://www\.aytocdo\.es)?/sites/default/files/[^"]+\.pdf)"',
    re.I,
)
RE_LICENCIA_PDF = re.compile(
    r'(?i)(licencia|obra menor|apertura|acometida|renovaci[oó]n)',
)
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "embellec" in n:
        return "embellecimiento urbano"
    if "patrimonio" in n:
        return "patrimonio"
    if "licitaci" in n:
        return "contratación urbanismo"
    if "informaci" in n:
        return "información pública"
    return "urbanismo"


class ColmenarDeOrejaAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal 10 aytocdo.es + sede espublico + ámbitos SITCM WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.urbanismo_url = str(self.config.get("urbanismo_url") or f"{self.drupal_base}/areas/urbanismo")
        self.descargas_url = str(
            self.config.get("descargas_url") or f"{self.drupal_base}/descargas-generales"
        )
    ...
    def _fetch(self, url: str, *, timeout: float = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-colmenar-de-oreja/1.0")},
        )
    ...
    def _collect_tramites(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(f"{self.sede_base}/dossier", timeout=35)
        except (urllib.error.URLError, TimeoutError):
            return []
        if len(html) < 100:
            return []
    ...
    def _collect_drupal_noticias(self) -> list[dict[str, Any]]:
        url = f"{self.drupal_base}/areas/urbanismo/noticias"
        try:
        ...
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for m in re.finditer(r'class="field-content"[^>]*>(.*?)</div>', html, re.S):
            text = _strip_html(m.group(1))
            if len(text) < 40 or text in seen:
                continue
            if not RE_PROYECTO.search(text):
                continue
            if RE_EXCLUDE.search(text) and not RE_PROYECTO.search(text):
                continue
            seen.add(text)
            rows.append(
                {
    ...
    def _collect_drupal_pdfs(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.seed_pages:
            ...
            for m in RE_DRUPAL_PDF.finditer(html):
                ...
                link_text = ""
                ctx = html[max(0, m.start() - 250) : m.end() + 80]
                text_m = re.search(r">([^<]{3,120})</a>", ctx)
                if text_m:
                    link_text = _strip_html(text_m.group(1))
                titulo = link_text or re.sub(r"\.pdf$", "", name, flags=re.I).replace("_", " ")
                ...
                rows.append(
                    {
                        ...
                        "url": page_url,
                        "pdf_url": pdf,
                        "fecha": _parse_fecha_dmy(name + " " + titulo),
                        "origen": "drupal_pdf",
                    }
                )
        return rows

    def _collect_descargas_licencias(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(self.descargas_url)
        except urllib.error.URLError:
            return []
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for m in RE_DRUPAL_PDF.finditer(html):
            ...
            ctx = html[max(0, m.start() - 250) : m.end() + 80]
            text_m = re.search(r">([^<]{3,120})</a>", ctx)
            titulo = _strip_html(text_m.group(1)) if text_m else Path(pdf).name
            if not RE_LICENCIA_PDF.search(titulo):
                continue
            seen.add(pdf)
            rows.append(
                {
                    "id": _stable_id("lic", pdf),
                    "fecha_concesion": _parse_fecha_dmy(titulo),
                    "tipo": titulo[:120] or "formulario licencia",
                    "distrito": None,
                    "lat": None,
                    "lon": None,
                    "titulo": f"Formulario: {titulo}"[:500],
                    "url": self.descargas_url,
                    "pdf_url": pdf,
                    "source": "ayuntamiento",
                    "nota": "Modelo descargable; no concesión publicada",
                    "origen": "descargas_drupal",
                }
            )
        return rows
        ...
        return [
            {
                ...
                "source": "ayuntamiento",
                "nota": "Área municipal de urbanismo (cita previa miércoles)",
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Trámites presenciales — formularios licencia",
                "url": self.descargas_url,
                "source": "ayuntamiento",
                "nota": "Modelos PDF obra menor, apertura, acometida, etc.",
                "origen": "descargas_drupal",
            },
        ]
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if cat in ("urbanismo", "ordenanzas y reglamentos", "medio ambiente"):
            pass
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_licencia_info_pages():
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for rec in self._collect_descargas_licencias():
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        ...
        return {
            ...
            "descargas": sum(1 for r in rows if r.get("origen") == "descargas_drupal"),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tablon": sum(1 for r in rows if r.get("origen") == "tablon"),
            "drupal": sum(
                1 for r in rows if str(r.get("origen", "")).startswith("drupal")
            ),
        }
