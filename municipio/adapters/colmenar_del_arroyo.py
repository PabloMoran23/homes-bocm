...
WEB_BASE = "https://www.colmenardelarroyo.es"
SEDE_BASE = "http://colmenardelarroyo.sedelectronica.es"
MUNICIPIO = "Colmenar del Arroyo"
ID_PREFIX = "colmenar-del-arroyo"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "COLMENAR DEL ARROYO"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WEB_BASE}/urbanismo.php",
    f"{WEB_BASE}/ordenanzas.php",
    f"{WEB_BASE}/planocallejero.php",
    f"{WEB_BASE}/punto_informacion_catastral.php",
    f"{WEB_BASE}/actualidad.php",
]

RE_PREVIEW = re.compile(
    r'href="((?:https?://)?colmenardelarroyo\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra (?:mayor|menor)|"
    r"primera ocupaci[oó]n|apertura|cambio de titularidad|actividad)",
)
RE_PROYECTO = re.compile(
    ...
    r"reparcel|sector|unidad de actuaci|estudio de detalle|"
    r"\b(?:UA|AA|S)-\d+\b)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|empleo p[uú]blico|protecci[oó]n de datos|"
    r"politica de privacidad|cookie|programaci[oó]n de fiestas|igualdad|"
    r"cobranza|iae|nota informativa tribunal)",
)
RE_AMBIT_CODE = re.compile(r"(?i)\b((?:UA|AA|S)-\d+(?:\s+[A-ZÁÉÍÓÚÑ]+)?)\b")
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?colmenardelarroyo\.es)?/?[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
RE_LICENCIA_PDF = re.compile(
    r"(?i)(licencia|obra menor|obra mayor|apertura|titularidad|ocupaci[oó]n)",
)
...
def _proyecto_tipo(title: str) -> str:
    ...
    if re.search(r"\bs-\d+\b", n) or "sector" in n or "plan parcial" in n:
...
class ColmenarDelArroyoAyuntamientoAdapter(AyuntamientoAdapter):
    """Web PHP colmenardelarroyo.es + sede espublico + ámbitos SITCM WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.urbanismo_url = str(self.config.get("urbanismo_url") or f"{self.web_base}/urbanismo.php")
    ...
    def _fetch(self, url: str, *, timeout: float = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-colmenar-del-arroyo/1.0")},
        )
    ...
    def _collect_board(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(self.board_url, timeout=35)
    ...
    def _collect_web_pdfs(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.seed_pages:
            try:
                html = self._fetch(page_url, timeout=35)
    ...
    def _collect_urbanismo_forms(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(self.urbanismo_url, timeout=35)
        except urllib.error.URLError:
            return []
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for m in RE_PDF_HREF.finditer(html):
            ...
            heading_m = re.search(r"<h[2-4][^>]*>([^<]+)", ctx, re.I)
            titulo = _strip_html(heading_m.group(1)) if heading_m else name.replace(".pdf", "")
            if not RE_LICENCIA_PDF.search(titulo + " " + name):
                continue
            rows.append(
                {
        ...
        return [
            {
                ...
                "fecha_concesion": None,
                "tipo": "urbanismo y licencias",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo y licencias — requisitos y trámites",
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
                "nota": "Obra menor/mayor, primera ocupación, apertura (presencial)",
            ...
            },
            {
                ...
                "url": self.board_url,
                "source": "ayuntamiento",
                "nota": "Anuncios y edictos publicados en sede espublico",
                "origen": "sede_tablon",
            },
            {
                ...
                "source": "ayuntamiento",
                "nota": "Transparencia municipal (espublico gestiona)",
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            "rows": len(rows),
            "status": "ok",
            "formularios": sum(1 for r in rows if r.get("origen") == "urbanismo_form"),
