...
DEFAULT_SEED_PAGES: list[str] = [
    f"{BASE}/urbanismo",
    f"{BASE}/instrumentos-urban%C3%ADsticos",
]

DEFAULT_LICENCIA_PAGES: list[str] = [
    f"{BASE}/urbanismo",
    f"{BASE}/tramitaci%C3%B3n-de-solicitudes",
]
...
RE_PROYECTO = re.compile(
    ...
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|peut|instrumento)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://www\.colunga\.es)?/documents/[^"]+\.pdf[^"]*|'
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Colunga.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
class ColungaAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay + RPGUR Asturias; geometría parcial WFS visorrpgur."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "19")
    ...
    def _fetch(self, url: str, *, data: bytes | None = None, encoding: str = "utf-8") -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-colunga/1.0"),
        }
        if data is not None:
    ...
    def _load_geom_index(self) -> dict[str, dict[str, Any]]:
        ...
        params = {
            ...
            "srsName": "EPSG:4326",
            "CQL_FILTER": "Instrumento LIKE '%COLUNGA%'",
        }
    ...
    def _crawl_liferay_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in re.finditer(r'href="(https://www\.colunga\.es/[^"#?]+)"', html, re.I):
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        info_url = f"{BASE}/tramitaci%C3%B3n-de-solicitudes"
        ...
        rows.append(
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Tramitación de solicitudes urbanísticas",
                "url": info_url,
                "source": "ayuntamiento",
                "nota": "Sin tablón público de concesiones; formularios informativos",
            }
        )
                    ...
                    rows.append(
                        {
                            ...
                            "url": page_url,
                            "source": "ayuntamiento",
                            "nota": "Página informativa; sin listado de concesiones",
                        }
                    )
                ...
                rows.append(
                    {
                        ...
                        "pdf_url": link,
                        "source": "ayuntamiento",
                        "nota": "Formulario descargable; no concesión publicada",
                    }
                )
        return rows
