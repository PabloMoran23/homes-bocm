...
WEB_BASE = "https://www.condadodetrevino.es"
SEDE_BASE = "https://condadodetrevino.sedelectronica.es"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=109",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        f"{PLAI_BASE}/openDocuIndice.do?cDocId=298006",
        "Normas Urbanísticas Municipales (NUM) — Condado de Treviño",
    ),
    (
        f"{SEDE_BASE}/transparency/6a9a765c-bc20-4432-a8ac-e03789d26539/",
        "Transparencia — planeamiento urbanístico",
    ),
    (
        f"{SEDE_BASE}/preview-document/1c2cd4d2-f96a-45fe-99d0-c48d75105853",
        "NORMAS URBANÍSTICAS MUNICIPALES CONDADO DE TREVIÑO (PDF sede)",
    ),
    (
        f"{WEB_BASE}/inicio",
        "Web municipal — urbanismo y sede electrónica",
    ),
]

DEFAULT_LICENCIA_PAGES: list[tuple[str, str]] = [
    (
        f"{SEDE_BASE}/citizen-service/b6a90317-cfae-4509-83c3-6ac86af25715",
        "Trámites de obras — sede electrónica",
    ),
...
RE_PREVIEW = re.compile(
    r'href="(https://condadodetrevino\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra (?:mayor|menor)|"
    r"primera ocupaci[oó]n|primera utilizaci[oó]n|ambiental|obras)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general|regional)|pgou|num|"
    ...
    r"normas urban|instrumento|memoria|planos|bocyl|plau|pp\b|entidad urban)",
)
RE_EXCLUDE = re.compile(
    ...
    r"incendios forestales|calendario del contribuyente|circular apertura|"
    r"quema de restos|bando prevenci[oó]n)",
)
...
class CondadoDeTrevinoAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal web + sede espublico + PLAI JCYL + geometría IDECyL WFS (Castilla y León)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-condado-de-trevino/1.0")},
        )
    ...
    def _plai_page_url(self, offset: int) -> str:
        params = {
            ...
            "urlResults": "searchVPubDocMuniPlau.do",
    ...
    def _parse_plai_rows(html: str) -> list[dict[str, str]]:
        ...
        for tr in re.findall(r"<tr[^>]*>(.*?)</tr>", html, re.S | re.I):
            ...
            url = (
                ...
                if doc_id
                else f"{PLAI_BASE}/searchVPubDocMuniPlau.do?provincia={PLAI_PROVINCIA}&municipio={PLAI_MUNICIPIO:03d}"
            )
            rows.append(
                {
                    "title": titulo,
