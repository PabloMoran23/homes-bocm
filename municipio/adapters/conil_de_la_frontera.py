...
SEDE_BASE = "https://conil.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board/"
WEB_BASE = "https://www.conildelafrontera.es"
URBANISMO_URL = f"{WEB_BASE}/areas-y-servicios-municipales/urbanismo"
MUNICIPIO = "Conil de la Frontera"
ID_PREFIX = "conil-de-la-frontera"
...
RE_PROYECTO = re.compile(
    ...
    r"\b(?:AR|AO|SLN|SLV|V\d+[A-Z]?)-\d+[A-Z0-9-]*\b|consulta p[uú]blica|"
    r"documento t[eé]cnico|adaptaci[oó]n.*loua|barrio nuevo|el colorado)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"cobranza iae|padrones|polic[ií]a local|ausencia alcaldesa|"
    r"contrataci[oó]n laboral|orden d[ií]a pleno)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://conil\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
class ConilDeLaFronteraAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-conil-de-la-frontera/1.0")},
        )
    ...
    def _collect_urbanismo_web(self) -> list[dict[str, Any]]:
        ...
        for m in RE_LINK.finditer(html):
            ...
            if RE_EXCLUDE_FORM.search(anchor):
                continue
            doc_url = self._abs_url(href, self.urbanismo_url)
            is_doc = (
                "phocadownload" in doc_url
                or "conil.org/www/Urbanismo" in doc_url
