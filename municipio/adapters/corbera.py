...
SEDE_BASE = "https://corbera.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WEB_BASE = "https://www.corbera.es"
INFO_PUBLICA_CA = f"{WEB_BASE}/ca/transparencia/informacio-publica"
INFO_PUBLICA_ES = f"{WEB_BASE}/es/transparencia/informacion-publica"
ORDENANZAS_URL = f"{WEB_BASE}/ca/pagina/ordenances-urbanistiques"
AGENDA_URBANA_URL = "https://agendaurbanacorbera.com/"
SEDE_INFO_IP_URL = f"{SEDE_BASE}/info.4"
MUNICIPIO = "Corbera"
ID_PREFIX = "corbera"
COD_INE_MUN = "46098"
...
DEFAULT_PAGE_SEEDS: tuple[str, ...] = (
    "/ca/transparencia/informacio-publica",
    "/es/transparencia/informacion-publica",
    "/ca/pagina/ordenances-urbanistiques",
    "/ca/pagina-transparencia/informacio-publica-linici-del-procediment-laprovacio-del-programa-dactuacio-integrada-mitjancant-gestio-directa-subunitat-41-unitat-dexecucio-zona-4-fins-l11",
)

STATIC_PDF_PROYECTOS: tuple[tuple[str, str, str], ...] = (
    (
        "Plan General Estructural (PGE) — normativa inicial",
        f"{WEB_BASE}/sites/www.corbera.es/files/46098_PGE_CORBERA_INICIAL_DN1_NORMATIVA_0.pdf",
        "PGE",
    ),
    (
        "Plan de Ordenación Pormenorizada (POP) — normativa inicial",
        f"{WEB_BASE}/sites/www.corbera.es/files/46098_POP_CORBERA_INICIAL_DN1_NORMATIVA.pdf",
        "POP",
    ),
)
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|pge|pop|puam|convenio|"
    r"informaci[oó]n p[uú]blica|informaci[oó] p[uú]blica|expediente|proyecto|modificaci[oó]n|reparcel|"
    r"programa d.?actuaci|unitat d.?execuci|subunitat|estudio (?:ac[uú]stico|ambiental|de detalle|de integraci[oó]n)|"
    r"memoria|planos|dogv|edicto|aprobaci[oó]n (?:inicial|definitiva|provisional)|parcela|pol[ií]gono|suelo|sector|"
    r"cambio de uso|normas subsidiarias|homologaci[oó]n|ordenanza|e[oó]lic|iate|agenda urbana)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|nomenament|convocatoria.*empleo|"
    r"cobranza iae|anunci cobrament|padrones|auxiliar administrativo|modificaci[oó]n de cr[eé]ditos|"
    r"cr[eé]ditos n|suplemento de cr[eé]dito|presupuest|subvenci[oó]n|empleo p[uú]blico|"
    r"oferta d.?ocupaci|hereus abintestato|matrimonio|delegaci[oó]n de funciones)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://corbera\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
RE_PAGE_LINK = re.compile(
    r'href="(/ca/(?:pagina(?:-transparencia)?|transparencia)/[^"#?]+)"',
    re.I,
)
...
def _clean_title(text: str) -> str:
    t = re.sub(r"\s*\|.*Corbera.*$", "", text or "", flags=re.I).strip()
    return re.sub(r"\s+", " ", t)
...
def _proyecto_tipo(denominaci: str) -> str:
    ...
    if "homologaci" in n or "normas subsidiarias" in n:
        return "homologación planeamiento"
    if "pop" in n or "pormenoriz" in n:
        return "POP"
    return "planeamiento"


class CorberaAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal 10 portalesmunicipales + sede espublico gestiona + ICV WFS (partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.info_publica_ca = str(self.config.get("info_publica_ca") or INFO_PUBLICA_CA)
        self.info_publica_es = str(self.config.get("info_publica_es") or INFO_PUBLICA_ES)
        ...
        self.sede_info_ip_url = str(self.config.get("sede_info_ip_url") or SEDE_INFO_IP_URL)
    ...
    def _fetch(self, url: str, *, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-corbera/1.0")},
        )
    ...
    def _enrich_geometry(self, rec: dict[str, Any], *, licencia: bool = False) -> None:
        ...
        if rec.get("origen") == "tablon" and licencia:
            if not re.search(r"(?i)pol[ií]gono|parcela|sector|diseminad|subunitat|unitat", rec.get("titulo") or ""):
                return
    ...
    def _discover_page_paths(self) -> list[str]:
        paths: set[str] = set(self.page_seeds)
        for lang in ("ca", "es"):
            try:
                html = self._fetch(f"{self.web_base}/{lang}", timeout=30)
            except urllib.error.URLError:
                continue
            for m in RE_PAGE_LINK.finditer(html):
                path = m.group(1)
                if RE_PROYECTO.search(path):
                    paths.add(path)
        return sorted(paths)

    def _parse_drupal_page(self, path: str) -> dict[str, Any] | None:
    ...
    def _collect_drupal_pages(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for path in self._discover_page_paths():
            if path in seen:
                continue
            seen.add(path)
            item = self._parse_drupal_page(path)
            if item:
                rows.append(item)
        return rows

    def _collect_static_pdf_proyectos(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for titulo, url, tipo in STATIC_PDF_PROYECTOS:
            rec: dict[str, Any] = {
        ...
        return [
            {
                ...
                "url": self.board_url,
                "source": "ayuntamiento",
                "nota": "Edictos en sede espublico gestiona (ca/es/val)",
                "origen": "sede_tablon",
            },
            {
            ...
            },
            {
                ...
                "source": "ayuntamiento",
                "nota": "Requiere identificación Cl@ve; sin listado histórico público",
                "origen": "sede_tramite",
            },
        ]
    ...
    def _collect_proyecto_info_pages(self) -> list[dict[str, Any]]:
        return [
            {
                "id": _stable_id("proy", self.info_publica_ca),
                "municipio": MUNICIPIO,
                "titulo": "Transparencia — normativa y proyectos en información pública",
                "fecha": None,
                "tipo": "información pública",
                "url": self.info_publica_ca,
                "source": "ayuntamiento",
                "origen": "transparencia",
            },
            {
                "id": _stable_id("proy", self.ordenanzas_url),
                "municipio": MUNICIPIO,
                "titulo": "Ordenanzas urbanísticas municipales",
                "fecha": None,
                "tipo": "normativa",
            ...
            {
                "id": _stable_id("proy", self.sede_info_ip_url),
                "municipio": MUNICIPIO,
                "titulo": "Sede — normativa y proyectos en información pública",
                "fecha": None,
                "tipo": "información pública",
                "url": self.sede_info_ip_url,
                "source": "ayuntamiento",
                "origen": "sede_info",
            },
            {
                ...
                "municipio": MUNICIPIO,
                "titulo": "Agenda Urbana Corbera",
                "fecha": None,
                "tipo": "agenda urbana",
                "url": self.agenda_urbana_url,
                "source": "ayuntamiento",
                "origen": "agenda_urbana",
            },
            {
    ...
    def _drupal_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        tipo = "información pública"
        if re.search(r"(?i)pge|plan general estructural", blob):
            tipo = "PGE"
        elif re.search(r"(?i)pop|pormenoriz", blob):
            tipo = "POP"
        elif re.search(r"(?i)programa d.?actuaci|subunitat", blob):
            tipo = "programa actuación integrada"

        key = row.get("path") or row["url"]
        rec: dict[str, Any] = {
            ...
            "url": row["url"],
            "source": "ayuntamiento",
            "origen": row.get("origen", "drupal_pagina"),
        }
        self._enrich_geometry(rec, licencia=False)
        return rec
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_proyecto_info_pages():
            add(rec)
        for rec in self._collect_static_pdf_proyectos():
            add(rec)
        for rec in self._collect_icv_proyectos():
            add(rec)
        for item in self._collect_drupal_pages():
