...
DEFAULT_INFO_PAGES: list[tuple[str, str]] = [
    ...
    (f"{SEDE_BASE}/dossier", "catálogo de trámites (licencias y urbanismo)"),
    (f"{DRUPAL_BASE}/informacion-general", "información general — enlace archivo PLAU"),
    (f"{DRUPAL_BASE}/normativa", "normativa municipal"),
    (
        "http://www.jcyl.es/plau/lplanes.plau?municipio=09113",
        "archivo planeamiento JCyL (Covarrubias)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=113",
        "archivo PLAU aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="((?:https://covarrubias\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://covarrubias\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra (?:mayor|menor)|"
    r"primera ocupaci[oó]n|recepci[oó]n de obras|instalaci[oó]n|ocupaci[oó]n)",
)
RE_PROYECTO = re.compile(
    ...
    r"normas urban|num\b|instrumento|pech|porn|conjunto hist[oó]rico|"
    r"ordenaci[oó]n detallada|actuaci[oó]n urban|suelo urbano|su[- ]?\d|ua[- ]?\d)",
)
RE_NOISE = re.compile(
    r"(?i)(acta del pleno|decreto bases|concurso|fiestas|subvenci[oó]n|"
    r"ordenanza fiscal|plusval[ií]a|aprovechamiento maderable|contrataci[oó]n)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_YMD = re.compile(r"\b((?:19|20)\d{2})(\d{2})(\d{2})\b")
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"SU[-\s]?\d+[A-Z]?|UA[-\s]?\d+[A-Z]?(?:\s*\d+)?|"
    r"PECH|PORN|NUM\b|SECTOR\s+SU[-\s]?\d+"
    r")\b",
)
...
def _proyecto_tipo(title: str, instrumento: str = "") -> str:
    ...
    if "plan parcial" in blob:
        return "plan parcial"
    if "conjunto hist" in blob or "pech" in blob:
        return "plan especial conjunto histórico"
    if "porn" in blob or "recursos naturales" in blob:
        return "plan ordenación recursos naturales"
    if "proyecto de urbaniz" in blob or "normalizaci" in blob:
    ...
    if "sector" in blob or "su-" in blob or "ua" in blob:
...
class CovarrubiasAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal Toools + sede espublico + PLAU JCyL + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-covarrubias/1.0")},
        )
    ...
    def _planpublica_page_url(self, offset: int, *, plai: bool = False) -> str:
        endpoint = "searchVPubDocMuniPlai.do" if plai else "searchVPubDocMuniPlau.do"
        params = {
        ...
        return f"{PLAI_BASE}/{endpoint}?{urllib.parse.urlencode(params)}"

    @staticmethod
    def _parse_planpublica_rows(html: str) -> list[dict[str, str]]:
        ...
        for tr in re.findall(r"<tr[^>]*>(.*?)</tr>", html, re.S | re.I):
            ...
            if len(cells) < 5 or cells[0] in {"Libro", "Tipo", "FIGURAS"}:
                continue
            ...
            if doc_id and doc_m:
                ...
                url = (
                    ...
                    f"bInfoPublica=N&provincia={PLAI_PROVINCIA}&municipio={PLAI_MUNICIPIO}"
                )
            rows.append(
                {
                    "title": titulo,
    ...
    def _collect_planpublica(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for plai in (False, True):
            for page in range(self.plai_max_pages):
                offset = page * self.plai_page_size
                try:
                    html = self._fetch(self._planpublica_page_url(offset, plai=plai))
                except urllib.error.URLError:
                    break
                parsed = self._parse_planpublica_rows(html)
                if not parsed:
                    break
                for item in parsed:
                    ...
                    rows.append(
                        {
                            ...
                            "origen": "plau_jcyl" if not plai else "plai_jcyl",
                        }
                    )
