...
WP_BASE = "https://aytocubillosdelsil.com"
WP_API = f"{WP_BASE}/wp-json/wp/v2"
SEDE_BASE = "https://cubillosdelsil.sedelectronica.es"
MUNICIPIO = "Cubillos del Sil"
ID_PREFIX = "cubillos-del-sil"

WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/ows"
WFS_MUNICIPIO = "Cubillos del Sil"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/urbanismo/",
    f"{WP_BASE}/clasificacion-del-suelo-urbano/",
    f"{WP_BASE}/709-2/",
    f"{WP_BASE}/solicitud-de-licencia-urbanistica-de-obras-mayores/",
    f"{WP_BASE}/declaracion-responsable-urbanistica-de-obras-menores/",
    f"{WP_BASE}/prorroga-de-licencia-de-obras/",
    f"{WP_BASE}/ayuntamiento/anuncios/",
    "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=064",
]

RE_PREVIEW = re.compile(
    r'href="(https://cubillosdelsil\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra mayor|obra menor|"
    r"primera ocupaci[oó]n|pr[oó]rroga.*licencia|cartel.*licencia)",
)
RE_PROYECTO = re.compile(
    ...
    r"clasificaci[oó]n del suelo|suelo urban|instrumento|memoria|planos|bocyl|pyto)",
)
RE_WP_EXCLUDE = re.compile(
    ...
    r"senderos|ornitol[oó]gic|museo|gastronom|carnaval)",
)
RE_EXCLUDE = re.compile(
    ...
    r"elector|impuesto de actividades|carnaval)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_YM = re.compile(r"/(?:uploads|documents)/(\d{4})/(\d{2})/")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?aytocubillosdelsil\.com)?/wp-content/uploads/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
class CubillosDelSilAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-cubillos-del-sil/1.0")},
        )
        ...
        for page_url in self.seed_pages:
            ...
            for m in RE_PDF_HREF.finditer(html):
                ...
                if not RE_PROYECTO.search(blob) and not RE_LICENCIA.search(blob):
                    if "urban" not in page_url.lower() and "clasific" not in page_url.lower():
                        continue
                rows.append(
                    {
                        "titulo": titulo[:500],
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            ...
            (f"{WP_BASE}/solicitud-de-licencia-urbanistica-de-obras-mayores/", "solicitud licencia obras mayores"),
            (f"{WP_BASE}/declaracion-responsable-urbanistica-de-obras-menores/", "declaración responsable obras menores"),
            (f"{WP_BASE}/prorroga-de-licencia-de-obras/", "prórroga de licencia de obras"),
            (f"{WP_BASE}/urbanismo/", "urbanismo — documentación y trámites"),
            (f"{WP_BASE}/clasificacion-del-suelo-urbano/", "clasificación del suelo y planos"),
        ]
        rows: list[dict[str, Any]] = []
