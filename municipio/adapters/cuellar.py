...
WP_BASE = "https://www.aytocuellar.es"
WP_API = f"{WP_BASE}/wp-json/wp/v2"
SEDE_BASE = "https://cuellar.sedelectronica.es"
MUNICIPIO = "Cuéllar"
ID_PREFIX = "cuellar"

WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/ows"
WFS_MUNICIPIO = "Cuéllar"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/urbanismo/",
    f"{WP_BASE}/urbanismo/normas-urbanisticas-municipales/",
    f"{WP_BASE}/urbanismo/oficina-tecnica-municipal/",
    f"{WP_BASE}/category/informacion-urbanismo/",
    f"{WP_BASE}/sede-electronica/",
]

RE_PREVIEW = re.compile(
    r'href="(https://cuellar\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra mayor|obra menor|"
    r"primera ocupaci[oó]n|cartel.*licencia|modelo.*licencia|tasa por licencia)",
)
RE_PROYECTO = re.compile(
    ...
    r"convenio urban|actuaci[oó]n urban|reurbaniz|urbanizaci[oó]n|"
    r"uso excepcional|suelo r[uú]stico|instrumento|memoria|planos|bocyl|pas-c)",
)
RE_WP_EXCLUDE = re.compile(
    ...
    r"senderos|ornitol[oó]gic|museo|gastronom|viviendas? (?:niñas? )?hu[eé]rfan|"
    r"feria del libro|eclipse de sol|donaci[oó]n de sangre)",
)
RE_EXCLUDE = re.compile(
    ...
    r"fiestas taurin|incendios forestales|agua, alcantarillado)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_YM = re.compile(r"/(?:uploads|documents)/(\d{4})/(\d{2})/")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?aytocuellar\.es)?/wp-content/uploads/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "sector" in n or "pas-c" in n:
        return "sector"
    return "urbanismo"


class CuellarAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress (urbanismo/NUM) + sede espublico tablón + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-cuellar/1.0")},
        )
    ...
    def _paginate_wp(
        self, endpoint: str, max_pages: int = 5, extra_query: str = ""
    ) -> list[dict[str, Any]]:
        items: list[dict[str, Any]] = []
        for page_num in range(1, max_pages + 1):
            suffix = f"&{extra_query}" if extra_query else ""
            url = f"{WP_API}/{endpoint}?per_page=100&page={page_num}&status=publish{suffix}"
            try:
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            ...
            (f"{WP_BASE}/urbanismo/oficina-tecnica-municipal/", "oficina técnica municipal"),
            (f"{self.sede_base}/", "sede electrónica — trámites urbanísticos"),
        ]
        rows: list[dict[str, Any]] = []
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for post in self._paginate_wp("posts", max_pages=10, extra_query="categories=18"):
            add(self._wp_item_to_proyecto(post, "wp_posts_urbanismo"))
        for post in self._paginate_wp("posts", max_pages=3, extra_query="search=convenio+urban"):
            add(self._wp_item_to_proyecto(post, "wp_posts_search"))
        ...
        return {
            ...
            "wp_posts": sum(
                1
                for r in rows
                if r.get("origen") in ("wp_posts_urbanismo", "wp_posts_search")
            ),
