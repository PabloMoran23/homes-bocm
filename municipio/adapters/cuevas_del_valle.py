...
WEB_BASE = "https://www.cuevasdelvalle.es"
SEDE_BASE = "https://cuevasdelvalle.sedelectronica.es"
PLAU_BASE = "https://servicios.jcyl.es/PlanPublica"
MUNICIPIO = "Cuevas del Valle"
ID_PREFIX = "cuevas-del-valle"
PLAU_URL = (
    "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do"
    "?bInfoPublica=N&pager.sortname=fPublicacion&pager.sortindex=-3&provincia=05&municipio=066"
)
...
WFS_MUNICIPIO = "Cuevas del Valle"
...
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?cuevasdelvalle\.es)?/docus/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"cerquitas|rebollar|llanos|ue[- ]?\d)",
)
...
RE_SECTOR = re.compile(r"(?i)\b(?:ue[- ]?|sector\s*)(\d{1,2})\b|(?:el\s+)?rebollar|cerquitas|llanos")
...
def _proyecto_tipo(title: str, instrumento: str = "") -> str:
    ...
    if "sector" in n or re.search(r"ue[- ]?\d", n):
        return "sector"
    return "urbanismo"


class CuevasDelValleAyuntamientoAdapter(AyuntamientoAdapter):
    """Plantilla DipuÁvila + sede espublico + PLAU JCyL + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-cuevas-del-valle/1.0")},
        )
    ...
    def _enrich_geometry(self, rec: dict[str, Any]) -> None:
        ...
        for wfs_rec in self._collect_wfs_proyectos():
            ...
            if titulo and wfs_title and (titulo in wfs_title or wfs_title in titulo):
                ...
                return
            for token in ("rebollar", "cerquitas", "llanos", "ue-3", "ue-1", "ue-2"):
                if token in titulo and token in wfs_title:
                    self._copy_geometry(rec, wfs_rec)
                    return
        sector_m = RE_SECTOR.search(titulo)
        if sector_m:
            sector_token = sector_m.group(1) if sector_m.lastindex else sector_m.group(0)
            sector_token = str(sector_token).replace(" ", "").lower()
            for wfs_rec in self._collect_wfs_proyectos():
                wfs_title = str(wfs_rec.get("titulo") or "").lower()
