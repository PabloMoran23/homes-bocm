...
SEDE_BASE = "https://doninosdesalamanca.sedelectronica.es"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=117",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=117",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://doninosdesalamanca\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://doninosdesalamanca\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"parcela|suelo|sector|ordenanza|unidad(?:es)? de ejecuci[oó]n|\b(?:UE|UBZ|UNC|SUR|UR?[- ]?R)[- ]?[A-Z0-9]+\b|"
...
class DoninosAyuntamientoAdapter(AyuntamientoAdapter):
    """OpenCMS ayto-doninos.com + sede espublico + IDECyL WFS (geometría partial)."""
    ...
    def _bootstrap_session(self) -> None:
        ...
        req = urllib.request.Request(
            self.board_url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-doninos/1.0")},
        )
        try:
