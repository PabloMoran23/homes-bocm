...
WEB_BASE = "https://duenas.es"
SEDE_BASE = "https://duenas.sedelectronica.es"
MUNICIPIO = "Dueñas"
ID_PREFIX = "duenas"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=34&municipio=117",
        "Planeamiento en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=34&municipio=117",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (f"{WEB_BASE}/urbanismo/plan-general-de-ordenacion-urbana-2/", "Plan General de Ordenación Urbana"),
]

RE_PREVIEW = re.compile(
    r'href="(https://duenas\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://duenas\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class DuenasAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Diputación Palencia + sede espublico + SIUCyL WFS."""
        ...
        for url, fallback_title in self.seed_urls:
            ...
            try:
                ...
                if title_m:
                    ...
                    titulo = re.sub(r"\s*[-|]\s*Dueñas.*$", "", titulo, flags=re.I).strip()
            except (urllib.error.URLError, OSError, ConnectionError):
                pass
