...
WP_BASE = "https://ayuntamientoelalamo.org"
SEDE_BASE = "https://elalamo.eadministracion.es"
MUNICIPIO = "El Álamo"
ID_PREFIX = "el-alamo"

URBANISMO_URL = f"{WP_BASE}/oficina-virtual/urbanismo/"
CONCEJALIA_URL = f"{WP_BASE}/el-ayuntamiento/concejalias/urbanismo-y-obras/"
SOLICITUDES_URL = f"{WP_BASE}/solicitudes/"
...
DEFAULT_LICENCIA_PDFS: list[dict[str, str]] = [
    {
        "path": "/wp-content/uploads/2025/12/Solicitud-de-Licencia-Urbanistica2025.pdf",
        "tipo": "solicitud licencia urbanística",
        "titulo": "Solicitud de licencia urbanística (2025)",
    },
    {
        "path": "/wp-content/uploads/2025/12/declaracion-responsable-urbanismo-2025.pdf",
        "tipo": "declaración responsable urbanismo",
        "titulo": "Declaración responsable urbanismo (2025)",
    },
    {
        "path": "/wp-content/uploads/2023/01/19-Vados-202-1.pdf",
        "tipo": "solicitud de vado",
        "titulo": "Solicitud de vado (licencia entrada vehículos)",
    },
    {
        "path": "/wp-content/uploads/2023/03/16-Subrogacion-de-Tramitacion-de-Expediente-de-Licencia-202-2.pdf",
        "tipo": "subrogación expediente licencia",
        "titulo": "Subrogación de tramitación de expediente de licencia",
    },
    {
        "path": "/wp-content/uploads/2023/01/18-Urbanismo-202-3.pdf",
        "tipo": "solicitud concejalía urbanismo",
        "titulo": "Solicitud concejalía de urbanismo",
    },
]
...
RE_LICENCIA = re.compile(
    ...
    r"primera ocupaci[oó]n|cedula urban|c[eé]dula urban|vado)",
)
RE_PROYECTO = re.compile(
    ...
    r"reparcel|sector|parcela|bando|unidad(?:es)? de ejecuci[oó]n|"
    r"\b(?:UE|S|UA|SUR|SUI|AA|SAU|PE)-\d+[A-Z0-9-]*\b|bocm|estudio (?:de )?detalle|"
    r"infraestructura|ambiental|urbanizaci[oó]n|ficha)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|empleo|jurado|selecci[oó]n de personal|"
    r"activaci[oó]n profesional|igualdad|bolsa|perros peligrosos|armas|"
    r"impuesto|tributo|bonificaci[oó]n|domiciliaci[oó]n bancaria|matrimonio|"
    r"censo animales|actividad calificada|actividad no calificada)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|UA|SUR|SUI|AA|SAU|PE)-\d+[A-Z0-9-]*)\b",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_BOCM_DATE = re.compile(r"BOCM[- ]?(\d{4})[- ]?(\d{2})[- ]?(\d{2})", re.I)
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_WP_PDF = re.compile(
    r'href="(https://ayuntamientoelalamo\.org/wp-content/uploads/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    n = title.lower()
    if re.search(r"\bua-\d", n) or re.search(r"\bsur-\d", n) or re.search(r"\bsui-\d", n):
        return "plan parcial"
    if re.search(r"\baa-\d", n):
    ...
    if "planeamiento" in n or "pgou" in n:
        return "planeamiento"
    if "estudio" in n and ("ambiental" in n or "infraestructura" in n):
        return "estudio técnico"
    if "aprobaci" in n:
        return "aprobación"
    if "plano" in n:
        return "documentación gráfica"
    return "urbanismo"


def _sector_ilike_parts(text: str) -> list[str]:
    low = text.lower()
    for marker in (" bocm", " aprob", " anuncio", " del ", " de el "):
        if marker in low:
...
class ElAlamoAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress ayuntamientoelalamo.org + sede eAdmin informativa + ámbitos SITCM WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.concejalia_url = str(self.config.get("concejalia_url") or CONCEJALIA_URL)
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-el-alamo/1.0")},
        )
        ...
        return [
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo — normas PGOU y trámites",
            ...
            },
            {
                ...
                "fecha_concesion": None,
                "tipo": "formularios licencias",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Solicitudes y trámites — licencias urbanísticas",
                "url": self.solicitudes_url,
                "source": "ayuntamiento",
                "nota": "Formularios descargables de licencias y vados",
                "origen": "wp_solicitudes",
            },
            {
                ...
                "source": "ayuntamiento",
                "nota": "Presentación telemática (Maggioli eAdmin)",
                "origen": "sede",
            },
        ]
            ...
            rows.append(
                {
                    ...
                    "lat": None,
                    "lon": None,
                    "titulo": f"Formulario urbanismo: {title}"[:500],
                    "url": self.solicitudes_url,
                    "pdf_url": pdf_url,
                    "source": "ayuntamiento",
                    "nota": "Modelo descargable; no concesión publicada",
                    "origen": "solicitudes_wp",
                }
            )
        return rows
    ...
    def _collect_pgou_seed(self) -> list[dict[str, Any]]:
        rec: dict[str, Any] = {
            "id": _stable_id("proy", "pgou-normas"),
            "municipio": MUNICIPIO,
            "titulo": "Normas Urbanísticas del Plan General de Ordenación Urbana de El Álamo",
            "fecha": None,
            "tipo": "normas subsidiarias",
            "url": self.urbanismo_url,
            "pdf_url": (
                "https://ayuntamientoelalamo.org/wp-content/uploads/2015/11/"
                "Doc-III.-Vol-1.-Normas-Urban%C3%ADsticas.pdf"
            ),
            "source": "ayuntamiento",
            "origen": "wp_urbanismo",
            "nota": "Documentación consultable en Visor SIT Comunidad de Madrid",
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        pdf_n = sum(1 for r in rows if r.get("origen") == "wp_pdf")
