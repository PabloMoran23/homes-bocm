...
WP_BASE = "https://elatazar.org"
WP_API = f"{WP_BASE}/wp-json/wp/v2"
SEDE_BASE = "https://elatazar.sedelectronica.es"
MUNICIPIO = "El Atazar"
ID_PREFIX = "el-atazar"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "EL ATAZAR"

MSAU_PGOU_URL = "https://www.msau-sierranorte.es/"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/urbanismo/",
    f"{WP_BASE}/impresos-y-tramites/",
    f"{WP_BASE}/normativa/",
]
...
RE_LICENCIA = re.compile(
    ...
    r"primera ocupaci[oó]n|1[aª] ocupaci[oó]n|actuaci[oó]n comunicada)",
)
RE_PROYECTO = re.compile(
    ...
    r"ordenanza.*(?:urban|tr[aá]fico|construcc|licen)|tasa.*licen|"
    r"embalse|ordenaci[oó]n del embalse|msau|mancomunidad.*urban|"
    r"\b(?:UE|AD|AN|AI|PAU|S)-[\w\d-]+\b)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|empleo|bolsa|igualdad|recaudaci[oó]n|calendario fiscal|"
    r"piscina|campamento|deporte|turismo|vacunaci[oó]n|"
    r"residuos urbanos|basura|estacionamiento|parking|"
    r"senda|ruta[s]? tur[ií]stic|camino puebla|desplegable sierra|"
    r"cementerio|animales|veh[ií]culos|matrimonio|uniones civiles|rodaje cinematogr[aá]fico|"
    r"certificaci[oó]n catastral|modelo 901)",
)
...
RE_BOARD_PREVIEW = re.compile(
    r'href="(https://elatazar\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "plan general" in n or "pgou" in n:
        return "planeamiento"
    if "embalse" in n or "ordenaci" in n:
        return "planeamiento"
    if "informaci" in n:
...
def _sector_ilike_parts(text: str) -> list[str]:
    low = text.lower()
    for marker in (" bocm", " aprob", " anuncio", " del pgou", " nnss", " de el atazar"):
        if marker in low:
...
class ElAtazarAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress (urbanismo, normativa, impresos) + sede espublico + SITCM WFS (0 ámbitos)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-el-atazar/1.0")},
        )
    ...
    def _pdf_label(self, html: str, href: str) -> str:
        esc = re.escape(href)
        anchor = re.search(
            rf'<a[^>]+href=["\']{esc}["\'][^>]*>(.*?)</a>',
            html,
        ...
        if anchor:
            label = _strip_html(anchor.group(1))
            if label and label.lower() not in ("pdf", "[pdf]"):
                return label
        return Path(urllib.parse.urlparse(href).path).stem.replace("-", " ")
    ...
    def _parse_page_pdfs(self, html: str, page_url: str) -> list[dict[str, Any]]:
        ...
        for m in RE_PDF_HREF.finditer(html):
            ...
            if pdf_url in seen:
                continue
            seen.add(pdf_url)
            label = self._pdf_label(html, m.group(1)) or Path(urllib.parse.urlparse(pdf_url).path).name
            titulo = label if len(label) > 12 else f"{page_title} — {label}"
        ...
        if RE_PROYECTO.search(intro) and not rows and page_url.endswith("/urbanismo/"):
            rows.append(
                {
                    "titulo": "Plan General de Ordenación Urbana — Mancomunidad MSAU Sierra Norte",
                    "fecha": None,
                    "url": page_url,
                    "origen": "wp_page",
                    "nota": intro[:400],
    ...
    def _collect_wp_pages(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.seed_pages:
            try:
                ...
                continue
            items = self._parse_page_pdfs(html, page_url)
            for rec in items:
                key = rec.get("pdf_url") or rec["url"] + "|" + rec["titulo"]
                by_key[key] = rec
        by_key.setdefault(
            f"{MSAU_PGOU_URL}|pgou",
            {
                "titulo": "Plan General de Ordenación Urbana — Mancomunidad Servicios Arquitectura y Urbanismo Sierra Norte",
                "fecha": None,
                "url": MSAU_PGOU_URL,
                "origen": "msau_pgou",
            },
        ...
        by_key.setdefault(
            "https://www.comunidad.madrid/publicacion/ref/3509|embalse",
            {
                "titulo": "Plan de Ordenación del Embalse de El Atazar (documento resumen)",
                "fecha": "1995-01-01",
                "url": "https://www.comunidad.madrid/publicacion/ref/3509",
                "origen": "comunidad_madrid",
            },
        )
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        forms = [
            (
                f"{self.wp_base}/wp-content/uploads/2023/04/solicitud-de-actuacion-comunicada.pdf",
                "Solicitud de actuación comunicada",
                "actuación comunicada",
            ),
            (
                f"{self.wp_base}/wp-content/uploads/2023/04/Solicitud-licencia-obra-menor.pdf",
                "Solicitud licencia de obra menor",
                "licencia de obra menor",
            ),
            (
                f"{self.wp_base}/wp-content/uploads/2023/04/Solicitud-general.pdf",
                "Solicitud general de licencia urbanística",
                "licencia urbanística",
            ),
        ]
        rows: list[dict[str, Any]] = []
        for pdf_url, titulo, tipo in forms:
            rows.append(
                {
                    ...
                    "lon": None,
                    "titulo": titulo,
                    "url": f"{self.wp_base}/impresos-y-tramites/",
                    "pdf_url": pdf_url,
                    "source": "ayuntamiento",
                    "nota": "Formulario descargable (web municipal)",
                    "origen": "wp_tramite",
                }
            )
        rows.extend(
            [
                {
                    ...
                    "lat": None,
                    "lon": None,
                    "titulo": "Urbanismo — servicios técnicos municipales (MSAU Sierra Norte)",
                    "url": f"{self.wp_base}/urbanismo/",
                    "source": "ayuntamiento",
                    "nota": "Consulta previa cita; gestión vía Mancomunidad MSAU",
                    "origen": "wp_tramite",
                },
                {
