...
WEB_BASE = "https://mail.elbarraco.org"
SEDE_BASE = "https://elbarraco.sedelectronica.es"
MUNICIPIO = "El Barraco"
ID_PREFIX = "el-barraco"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WEB_BASE}/atencion/normativa.html",
    f"{WEB_BASE}/atencion/tablon.html",
    f"{WEB_BASE}/atencion/gestiones.html",
]

DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=05&municipio=022",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=05&municipio=022",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://elbarraco\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://elbarraco\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_LICENCIA = re.compile(
    ...
    r"obra (?:mayor|menor|s de nueva planta)|primera ocupaci[oó]n|"
    r"alineaci[oó]n de parcela|agrupaci[oó]n de parcelas|habitabilidad)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|nnss|normas (?:urban|subsidiarias)|"
    r"informaci[oó]n p[uú]blica|expediente|proyecto|modificaci[oó]n|reparcel|"
    r"estudio (?:de )?detalle|memoria|planos|edicto|aprobaci[oó]n (?:inicial|definitiva|provisional)|"
    r"parcela|suelo|sector|pol[ií]gono|delimitaci[oó]n|edtu|"
    r"unidad(?:es)? de ejecuci[oó]n|\b(?:UE|UBZ|UNC|SUNC|P)[- ]?[A-Z0-9]+\b|"
    r"actuaci[oó]n|autorizaci[oó]n de uso|bocyl)",
)
...
def _normalize_sector_code(raw: str) -> str:
    ...
    code = code.replace("POLÍGONO", "P").replace("POLIGONO", "P")
    code = code.replace("SU-NC", "SUNC")
    return code
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "delimitaci" in n or "edtu" in n:
        return "delimitación tramo urbano"
    if "informaci" in n and "públic" in n:
        return "información pública"
    if "plan parcial" in n:
        return "plan parcial"
    if "normas urban" in n or "nnss" in n or "normas subsidiarias" in n:
        return "normas urbanísticas"
    if "modificaci" in n and "puntual" in n:
        return "modificación puntual"
    if "licencia" in n:
        return "licencia"
    if "planeam" in n or "pgou" in n or "sector" in n or "polígono" in n:
        return "planeamiento"
    return "urbanismo"


class ElBarracoAyuntamientoAdapter(AyuntamientoAdapter):
    """Web legacy HTML (normativa PDFs) + sede espublico + IDECyL WFS (geometría partial)."""
    ...
    def _bootstrap_session(self) -> None:
        ...
        req = urllib.request.Request(
            self.board_url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-el-barraco/1.0")},
        )
        try:
    ...
    def _parse_board_links(self, html: str, origen: str) -> list[dict[str, Any]]:
        ...
        for m in RE_PREVIEW.finditer(html):
            ...
            if len(spans) >= 2:
                useful = [s for s in spans if s]
                if useful:
                    descripcion = useful[0]
                    if len(useful) > 1:
                        categoria = useful[0]
                        procedimiento = useful[0]
            rows.append(
                {
    ...
    def _collect_legacy_pdfs(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            ...
            (f"{self.web_base}/atencion/gestiones.html#urbanismo", "formularios urbanismo (obras, ocupación, calificación)"),
            (f"{self.sede_base}/dossier.0", "catálogo de trámites sede electrónica"),
        ]
        rows: list[dict[str, Any]] = []
    ...
    def _pdf_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        rec: dict[str, Any] = {
            ...
            "url": row.get("url") or row.get("pdf_url") or f"{self.web_base}/atencion/normativa.html",
            "source": "ayuntamiento",
    ...
    def _pdf_to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        return {
            ...
            "source": "ayuntamiento",
            "nota": "Formulario o impreso informativo (no concesión publicada)",
            "origen": row.get("origen"),
        }
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_tramites():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_legacy_pdfs():
        ...
        return {
            ...
            "tramites": sum(1 for r in rows if r.get("origen") in ("catalogo_tramites", "web_tramite")),
            "formularios": sum(1 for r in rows if r.get("origen") == "web_pdf"),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
            "tramites": sum(1 for r in rows if r.get("origen") in ("catalogo_tramites", "jcyl_planeamiento")),
