...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (f"{WEB_BASE}/el-municipio/urbanismo", "Urbanismo — normativa y contacto"),
    (
        f"{WEB_BASE}/el-ayuntamiento/administracion-municipal/normativa-urbanistica",
        "Normativa urbanística municipal",
    ),
    (
        f"{WEB_BASE}/el-ayuntamiento/administracion-municipal/tablon-de-edictos",
        "Tablón de edictos — planeamiento e información pública",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=47&municipio=031",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=47&municipio=031",
        "Planeamiento en información pública (PLAI JCYL)",
    ),
    (f"{SEDE_BASE}/dossier", "Catálogo de trámites — sede electrónica"),
]

DEFAULT_TABLON_PROYECTOS: list[dict[str, str]] = [
    {
        "titulo": (
            "Aprobación inicial de la modificación puntual nº 1 de las normas urbanísticas de El Campillo"
        ),
        "fecha": "2021-02-10",
    ...
    {
        "titulo": (
            "Información pública: autorización de uso excepcional de suelo rústico con protección "
            "natural hábitat y licencia urbanística — parcela 5004 polígono 6"
        ),
        "fecha": None,
...
RE_PREVIEW = re.compile(
    r'href="(https://campillo\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://campillo\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_SECTOR = re.compile(
    r"(?i)\b(?:sector|sect\.?|ue|unidad de ejecuci[oó]n)\s*([\"«]?\s*[0-9]+[A-Z0-9\"»\s-]*|"
    r"los hitales|la escombrera|los hornos|la encina|ladera de las vi[nñ]as)",
)
...
class ElCampilloAyuntamientoAdapter(AyuntamientoAdapter):
    """Portal Dip. Valladolid + sede espublico + PLAI JCYL + geometría parcial IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        raw_tablon = self.config.get("tablon_proyectos") or DEFAULT_TABLON_PROYECTOS
        self.tablon_proyectos: list[dict[str, str]] = [
            dict(item) for item in raw_tablon if isinstance(item, dict)
        ]
    ...
    def _warm_sede(self) -> None:
        ...
        try:
            req = urllib.request.Request(
                self.info_url,
                headers={"User-Agent": self.config.get("user_agent", "poc-bocm-el-campillo/1.0")},
            )
            self._opener.open(req, timeout=45)
    ...
    def _tablon_to_proyecto(self, row: dict[str, str]) -> dict[str, Any]:
        key = f"{row.get('url') or ''}:{row.get('titulo') or ''}"
        rec: dict[str, Any] = {
            "id": _stable_id("proy", key),
            "municipio": MUNICIPIO,
            "titulo": row["titulo"],
    ...
    def _collect_tablon_web(self) -> list[dict[str, str]]:
        return [dict(item) for item in self.tablon_proyectos]
    ...
    def _collect_proyectos(self) -> list[dict[str, Any]]:
        ...
        for url, titulo in self.seed_pages:
            ...
            by_id[rec["id"]] = rec
        for item in self._collect_tablon_web():
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
            "semillas": sum(1 for r in rows if r.get("origen") in ("web_semilla", "tablon_web")),
        }
