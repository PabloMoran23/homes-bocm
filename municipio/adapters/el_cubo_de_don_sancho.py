...
SEDE_BASE = "https://elcubodedonsancho.sedelectronica.es"
PLAU_BASE = "https://servicios.jcyl.es/PlanPublica"
WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/wfs"
MUNICIPIO = "El Cubo de Don Sancho"
ID_PREFIX = "el-cubo-de-don-sancho"
...
DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=113",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=113",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        f"{PLAU_BASE}/openDocuIndice.do?cDocId=278415",
        "Instrumento urbanístico vigente — Sin Planeamiento General (JCYL)",
    ),
    (
        "https://idecyl.jcyl.es/siur/index.html?id=37113",
        "Visor SiUR — planeamiento El Cubo de Don Sancho",
    ),
    (
        f"{SEDE_BASE}/info.0",
        "Sede electrónica municipal — trámites urbanísticos y licencias",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://elcubodedonsancho\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://elcubodedonsancho\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class ElCuboDeDonSanchoAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico (indeterminada) + PlanPublica JCYL + IDECyL WFS (sin web corporativa)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=113"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=113"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-el-cubo-de-don-sancho/1.0")},
        )
