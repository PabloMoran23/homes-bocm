...
WP_BASE = "https://elescorial.es"
SEDE_BASE = "https://elescorial.sedelectronica.es"
MUNICIPIO = "El Escorial"
ID_PREFIX = "el-escorial"

PLANEAMIENTO_URL = f"{WP_BASE}/pt-urbanismo-y-medio-ambiente/"
URBANISMO_URL = f"{WP_BASE}/urbanismo-y-vivienda/"
FORMULARIOS_URL = f"{WP_BASE}/formularios/"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "EL ESCORIAL"
...
RE_PREVIEW = re.compile(
    r'href="(https://elescorial\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://elescorial\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_LICENCIA = re.compile(
    ...
    r"alineaci[oó]n|paneles solares|acometida|actividades e instalaciones)",
)
RE_PROYECTO = re.compile(
    ...
    r"modificaci[oó]n|aprobaci[oó]n (?:inicial|definitiva)|reparcel|sector|peri|"
    r"edicto|bando.*parcel|actuaci[oó]n urban|reurbaniz|"
    r"patrimonio|desbroce.*parcel|suelo|convenio urban|montencinar|tomillar|peralejo|"
    r"\b(?:UE|AD|AN|AI|PAU|S|SAU|UA|PERI)-[\w\d-]+\b)",
)
RE_WP_EXCLUDE = re.compile(
    r"(?i)(empleo|concurso|deporte|fiestas|cine|igualdad|subvenci[oó]n|"
    r"jurado|juez de paz|pe[oó]n|guerra civil)",
)
...
def _fecha_from_url(url: str) -> str | None:
    ...
    if m:
        ...
        try:
            ...
            pass
    return _fecha_from_blob(Path(url).name.replace("-", " "))
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "estudio" in n and ("acust" in n or "ambient" in n):
        return "estudio ambiental"
    if re.search(r"\bsau-\d+\b", n) or re.search(r"\bua-\d+\b", n):
        return "planeamiento"
    return "urbanismo"
...
def _sector_ilike_parts(text: str) -> list[str]:
    s = text.strip()
    low = s.lower()
    for marker in (" del pgou", " pgou", " bocm", " aprob", " anuncio", " nnss", " peri"):
        if marker in low:
...
class ElEscorialAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Elementor (planeamiento PDFs) + sede espublico gestiona + ámbitos SIT WFS."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-el-escorial/1.0")},
        )
    ...
    def _parse_page_pdfs(self, html: str, page_url: str) -> list[dict[str, Any]]:
        ...
        chunks: list[tuple[str, str]] = [(page_title, html)]
        if len(parts) > 1:
            chunks = []
            current_section = page_title
            for i in range(1, len(parts), 2):
                if i + 1 < len(parts):
                    h2_m = RE_H2.search(parts[i] + parts[i + 1])
                    if h2_m:
                        current_section = _strip_html(h2_m.group(1))
                    chunks.append((current_section, parts[i] + parts[i + 1]))
        for section_title, chunk in chunks:
            for m in re.finditer(
                ...
                titulo = f"{section_title} — {label}" if label else section_title
                if len(label) > 8 and section_title.lower() not in label.lower():
                    titulo = f"{section_title} — {label}"
                rows.append(
                    {
                        "titulo": titulo[:500],
    ...
    def _collect_wp_proyectos(self) -> list[dict[str, Any]]:
        ...
        for rec in by_key.values():
            titulo = rec["titulo"]
            if RE_WP_EXCLUDE.search(titulo):
                continue
            if page := rec.get("url", ""):
                if "formularios" in page and not RE_LICENCIA.search(titulo):
                    if not RE_PROYECTO.search(titulo):
                        continue
            if not RE_PROYECTO.search(titulo) and rec.get("origen") == "wp_pdf":
                if "urbanismo" not in rec.get("url", "").lower():
                    continue
            out: dict[str, Any] = {
                "id": _stable_id("proy", rec.get("pdf_url") or rec["titulo"]),
                "municipio": MUNICIPIO,
                "titulo": titulo[:500],
                "fecha": rec.get("fecha"),
                "tipo": _proyecto_tipo(titulo),
                "url": rec.get("pdf_url") or rec["url"],
                "source": "ayuntamiento",
    ...
    def _collect_licencia_forms(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for page_url in (self.urbanismo_url, self.formularios_url):
            try:
                html = self._fetch(page_url)
            except urllib.error.URLError:
                continue
            for m in RE_PDF_HREF.finditer(html):
                ...
                if not RE_LICENCIA.search(name) and "urbanismo" not in page_url.lower():
                    continue
                if "formularios" in page_url and not RE_LICENCIA.search(name):
                    continue
                rows.append(
                    {
                        "titulo": name[:500],
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo y vivienda — solicitudes de licencia",
            ...
            },
            {
                ...
                "source": "ayuntamiento",
                "nota": "Índice legal/administrativo en sede espublico",
    ...
    def _load_wfs_ambitos(self) -> dict[str, dict[str, Any]]:
        ...
        for f in feats:
            ...
            for token in re.split(r"[\s\-/]+", name):
                if len(token) >= 4:
                    cache.setdefault(token.upper(), f)
    ...
    def _collect_sit_ambitos(self) -> list[dict[str, Any]]:
        ...
        for name, group in by_name.items():
            fig = str((group[0].get("properties") or {}).get("DS_FIG_DES") or "").strip()
