...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (f"{WP_BASE}/urbanismo/", "Urbanismo — área municipal"),
    (
        f"{WP_BASE}/aprobacion-definitva-parcial-del-pgou-de-el-espinar/",
        "PGOU — aprobación definitiva parcial",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=135",
        "Archivo planeamiento urbanístico aprobado (PLAU JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=135",
...
RE_PREVIEW = re.compile(
    r'href="(https://elespinar\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://elespinar\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class ElEspinarAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress elespinar.es + sede espublico + PLAU JCYL + geometría parcial IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-el-espinar/1.0")},
        )
    ...
    def _collect_wordpress_links(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        crawl_urls = [
            f"{self.wp_base}/urbanismo/",
            f"{self.wp_base}/aprobacion-definitva-parcial-del-pgou-de-el-espinar/",
        ]
        keywords = (
            "urban",
            "plan",
        ...
        for page_url in crawl_urls:
            ...
            for m in RE_WP_LINK.finditer(html):
                ...
                if not link.startswith("http"):
                    link = urllib.parse.urljoin(self.wp_base + "/", link)
                blob = f"{link} {text}".lower()
    ...
    def _wordpress_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        rec: dict[str, Any] = {
    ...
    def _collect_proyectos(self) -> list[dict[str, Any]]:
        ...
        for url, titulo in self.seed_pages:
            ...
            by_id[rec["id"]] = rec
        for item in self._collect_wordpress_links():
            rec = self._wordpress_to_proyecto(item)
            by_id[rec["id"]] = rec
        for item in self._collect_board():
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
            "semillas": sum(1 for r in rows if r.get("origen") == "wordpress_semilla"),
