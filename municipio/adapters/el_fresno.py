...
WEB_BASE = "https://www.elfresno.es"
SEDE_BASE = "https://elfresno.sedelectronica.es"
...
PLAU_URL = (
    ...
    "?bInfoPublica=N&pager.sortname=fPublicacion&pager.sortindex=-3&provincia=05&municipio=076"
)
...
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?elfresno\.es)?/docus/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
class ElFresnoAyuntamientoAdapter(AyuntamientoAdapter):
    """Plantilla DipuÁvila (normas urbanísticas) + sede espublico + PLAU JCyL + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-el-fresno/1.0")},
        )
    ...
    def _plau_row_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
    ...
    def _plau_proyecto(self) -> dict[str, Any]:
        rec: dict[str, Any] = {
            ...
            "municipio": MUNICIPIO,
            "titulo": "Archivo PLAU CyL — planeamiento vigente El Fresno",
            "fecha": None,
            "tipo": "planeamiento",
            "url": self.plau_url,
            "source": "ayuntamiento",
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for plau_row in self._collect_plau():
            add(self._plau_row_to_proyecto(plau_row))
