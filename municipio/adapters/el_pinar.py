...
WEB_BASE = "https://www.aytoelpinar.org"
SEDE_BASE = "https://elpinardeelhierro.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
ARCHIVO_URL = (
    "https://www3.gobiernodecanarias.org/aplicaciones/archivoplaneamientopt/"
    "pages/consulta/islaMunicipio.jsp?municipio=3&provincia=38"
)
IDECANARIAS_PGO = "https://www.idecanarias.es/resources/PLA_ENP_URB/URB_PLA/EH/Pina/PGO/indice.html"
MUNICIPIO = "El Pinar"
ID_PREFIX = "epinar"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-el-pinar"
GEOBDP_KNOWN_DOCS = {
    "644": "Plan Especial de Rehabilitación Urbana de La Restinga",
    "645": "Revisión Parcial del Plan General de Ordenación de Frontera",
    "1403": "Plan General de Ordenación de El Pinar",
}
...
RE_PROYECTO = re.compile(
    ...
    r"texto refundido|suspensi[oó]n de determinaciones|ordenaci[oó]n|rehabilitaci[oó]n urbana)",
)
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?(?:aytoelpinar\.org|elpinardeelhierro\.(?:es|com)))[^"]+\.pdf[^"]*)"',
    re.I,
)
...
RE_BOARD_PREVIEW = re.compile(
    r'href="(https://elpinardeelhierro\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
def _proyecto_tipo(blob: str) -> str:
    b = blob.lower()
    if "plan especial" in b and "rehabilitaci" in b:
        return "plan especial rehabilitación urbana"
...
class ElPinarAyuntamientoAdapter(AyuntamientoAdapter):
    """Joomla aytoelpinar.org + sede espublico + SITCAN CKAN + GEOBDP Grafcan (Canarias)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-el-pinar/1.0")},
        )
    ...
    def _load_geobdp_index(self) -> dict[str, str]:
        ...
        index: dict[str, str] = {}
        for doc_id, title in GEOBDP_KNOWN_DOCS.items():
    ...
    def _parse_board(self, html: str) -> list[dict[str, Any]]:
        ...
        for m in RE_BOARD_PREVIEW.finditer(html):
            doc_url = m.group(1)
            title_m = re.search(r'title="([^"]+)"', html[m.start() - 200 : m.start() + 200], re.I)
            titulo = unescape(title_m.group(1)) if title_m else _strip_html(html[m.start() : m.start() + 300])
            if not titulo:
                continue
            rows.append(
                {
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        return [
            {
                "id": _stable_id("lic", f"{self.sede_base}/board"),
                "fecha_concesion": None,
                "tipo": "trámite informativo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Sede electrónica — tablón de anuncios y trámites urbanismo",
                "url": self.board_url,
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
            {
                "id": _stable_id("lic", ARCHIVO_URL),
                "fecha_concesion": None,
                "tipo": "trámite informativo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Archivo de planeamiento — Gobierno de Canarias (El Hierro)",
                "url": ARCHIVO_URL,
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {"rows": len(rows), "status": "ok", "source": "sede_archivo_canarias"}
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            "rows": len(rows),
            "status": "ok",
            "source": "sitcan_geobdp_sede_archivo",
