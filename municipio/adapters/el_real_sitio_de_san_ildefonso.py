...
WEB_BASE = "https://www.lagranja-valsain.com"
SEDE_BASE = "https://realsitiodesanildefonso.sedelectronica.es"
PLAU_BASE = "https://servicios.jcyl.es/PlanPublica"
WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/wfs"
MUNICIPIO = "El Real Sitio de San Ildefonso"
WFS_MUNICIPIO = "Real Sitio de San Ildefonso"
ID_PREFIX = "el-real-sitio-de-san-ildefonso"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WEB_BASE}/ayuntamiento/concejalias/concejalia-de-urbanismo/",
    f"{WEB_BASE}/ayuntamiento/concejalias/concejalia-de-urbanismo/declaracion-responsable/",
    f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=165",
    f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=165",
]

DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    (
        f"{SEDE_BASE}/catalog/t/declaracion-responsable-o-comunicacion-en-materia-urbanistica",
        "Declaración Responsable o Comunicación en Materia Urbanística",
    ),
    (
        f"{SEDE_BASE}/catalog/t/solicitud-de-licencia-o-autorizacion-urbanistica",
        "Solicitud de Licencia o Autorización Urbanística",
    ),
    (
        f"{SEDE_BASE}/catalog/t/solicitud-de-actuacion-urbanistica",
        "Solicitud de Actuación Urbanística",
    ),
    (
        f"{SEDE_BASE}/catalog/t/modificacion-del-planeamiento-de-desarrollo",
        "Modificación del Planeamiento de Desarrollo",
    ),
    (
        f"{SEDE_BASE}/catalog/t/planeamiento-general-modificacion",
        "Planeamiento General (Modificación)",
    ),
    (
        f"{SEDE_BASE}/catalog/t/solicitud-de-certificado-o-informe-urbanistico",
        "Solicitud de Certificado o Informe Urbanístico",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://realsitiodesanildefonso\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://realsitiodesanildefonso\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra mayor|obra menor|"
    r"primera ocupaci[oó]n|primera utilizaci[oó]n|vado|grua|gr[uú]a)",
)
RE_PROYECTO = re.compile(
    ...
    r"edicto|actuaci[oó]n urban|urbanizaci[oó]n|normas urban|bocyl|"
    r"su[- ]?nc|s\.u\.n\.c|sur[- ]?\d+|unc[- ]?\d+|unca[- ]?\d+|peri|"
    r"las eras|valsain|valsa[ií]n|guadarrama)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|empleo|proceso selectivo|pleno|tribut|"
    r"jurado|tribunal|selecci[oó]n de personal|elecci[oó]n|censo|decreto error|"
    r"depuraci[oó]n de agua|alcantarillado|plusvalia|fiestas|deporte)",
)
RE_SECTOR_CODE = re.compile(
    ...
    r"UNC[-\s]?A?[-\s]?\d+|"
    r"UNCA[-\s]?\d+|"
    r"S\.?\s*U\.?\s*\.?\s*N\.?\s*C\.?\s*[-.]?\s*(?:UNC)?[-.]?\s*\d+"
    r")\b",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_YMD = re.compile(r"\b((?:19|20)\d{2})(\d{2})(\d{2})\b")
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?lagranja-valsain\.com)?/files/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
def _normalize_sector_code(raw: str) -> str:
    ...
    code = re.sub(r"S\.U\.N\.C\.", "UNC", code)
    return code
...
def _proyecto_tipo(blob: str, instrumento: str = "") -> str:
    ...
    if "pgou" in n or "plan general" in n:
        return "PGOU"
    if "porn" in n or "recursos naturales" in n:
        return "ordenación recursos naturales"
    if "informaci" in n and "públic" in n:
        return "información pública"
    if "plan parcial" in n or "sector" in n or re.search(r"\bsur-\d+\b", n):
...
class ElRealSitioDeSanIldefonsoAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress lagranja-valsain.com + sede espublico + PLAU JCyL + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=165"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=165"
        )
        ...
        req = urllib.request.Request(
            ...
            headers={
                "User-Agent": self.config.get(
                    "user_agent", "poc-bocm-el-real-sitio-de-san-ildefonso/1.0"
                )
            },
        )
    ...
    def _parse_board_table(self, html: str, origen: str) -> list[dict[str, Any]]:
        ...
        for tr in re.findall(r"<tr>(.*?)</tr>", tbody_m.group(1), re.S):
            ...
            blob = f"{titulo} {_strip_html(cells[2])} {_strip_html(cells[3])} {_strip_html(cells[4])}"
    ...
    def _collect_wp_tramites(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.seed_pages:
            if "lagranja-valsain.com" not in page_url:
                continue
            try:
            ...
            for m in RE_PDF_HREF.finditer(html):
                ...
                titulo = _strip_html(text_m.group(1)) if text_m else name
                if not RE_LICENCIA.search(f"{titulo} {pdf}") and "tramites/urbanismo" not in pdf:
                    continue
                rows.append(
                    {
                        "titulo": titulo[:500],
    ...
    def _wp_tramite_to_licencia(self, row: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "source": "ayuntamiento",
            "nota": "Modelo/formulario de trámite en web municipal",
            "origen": row.get("origen"),
        }
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_tramites_catalog():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_wp_tramites():
            rec = self._wp_tramite_to_licencia(item)
