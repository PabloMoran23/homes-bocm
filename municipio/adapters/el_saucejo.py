...
WP_BASE = "https://www.elsaucejo.es"
SEDE_BASE = "https://sedeelsaucejo.dipusevilla.es"
MUNICIPIO = "El Saucejo"
ID_PREFIX = "el-saucejo"
INE_CODE = "41090"
CIF = "P4109000B"

TABLON_URL = f"{SEDE_BASE}/tablon-1.0/do/entradaPublica?ine={INE_CODE}"
PGOU_URL = f"{WP_BASE}/es/ayuntamiento/pgou"
NSS_URL = "https://3web.dipusevilla.es/planeamiento/NSSaucejo/inicio-saucejo.htm"
SITUA_URL = f"https://ws132.juntadeandalucia.es/situadifusion/pages/search.jsf?cid={INE_CODE}"
LICYTAL_URL = f"https://portal.dipusevilla.es/LicytalPub/jsp/pub/index.faces?cif={CIF}"
TRANSPARENCIA_PGOU = (
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|pbom|potaus|convenio|"
    ...
    r"normas subsidiarias|instrumento)",
)
RE_TABLON_NON_URBAN = re.compile(
    ...
    r"listas electorales|jurado|barrendero|pe[oó]n de horticultura|"
    r"cobranza|iae|dominio de la finca)",
)
RE_TABLON_URBAN = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|pbom|potaus|"
    r"informaci[oó]n p[uú]blica|licencia|sector|ordenanza|consulta p[uú]blica|"
    r"avance del plan|nnss|catalogo|catálogo|regularizaci[oó]n|normas subsidiarias)",
)
...
class ElSaucejoAyuntamientoAdapter(AyuntamientoAdapter):
    """OpenCMS INPRO web + GSede Diputación Sevilla + tablón INPRO + NSS Diputación."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.nss_url = str(self.config.get("nss_url") or NSS_URL)
    ...
    def _fetch(self, url: str, encoding: str | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-el-saucejo/1.0")},
        )
    ...
    def _collect_nss_documents(self) -> list[dict[str, Any]]:
        nss_base = self.nss_url.rsplit("/", 1)[0] + "/"
        docs = self._collect_pdf_links(self.nss_url, nss_base)
        out: list[dict[str, Any]] = []
        for doc in docs:
            blob = f"{doc['titulo']} {doc['url']}"
            if RE_PROYECTO.search(blob):
                doc = dict(doc)
                doc["origen"] = "nss_dipusevilla"
                out.append(doc)
        return out
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "fecha_concesion": None,
                "tipo": "consulta licencias urbanísticas (LicytalPub)",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "LicytalPub — consulta pública de licencias Diputación Sevilla",
            ...
            {
                "id": _stable_id("lic", f"{self.wp_base}/es/urbanismo/"),
                "fecha_concesion": None,
                "tipo": "área urbanismo web municipal",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo — web municipal El Saucejo",
                "url": f"{self.wp_base}/es/urbanismo/",
                "source": "ayuntamiento",
                "origen": "web_urb",
            },
        ]
    ...
    def _collect_proyecto_metadata(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "id": _stable_id("proy", self.nss_url),
                "municipio": MUNICIPIO,
                "titulo": "Normas Subsidiarias de El Saucejo — Diputación de Sevilla",
                "fecha": "2000-11-17",
                "tipo": "normas subsidiarias",
                "url": self.nss_url,
                "source": "ayuntamiento",
                "origen": "nss_dipusevilla",
            },
        ]
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "info": sum(
                1
                for r in rows
                if r.get("origen") in ("sede_tablon", "sede_tramite", "web_urb", "dipusevilla")
            ),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for doc in self._collect_nss_documents():
            add(self._pdf_to_proyecto(doc, "nss_dipusevilla"))
        ...
        return {
            ...
            "nss": sum(1 for r in rows if r.get("origen") == "nss_dipusevilla"),
