...
WP_BASE = "https://www.elsauzal.es"
SEDE_BASE = "https://elsauzal.sedelectronica.es"
SITCAN_DATASET = "planeamiento-urbanistico-de-el-sauzal"
MUNICIPIO = "El Sauzal"
ID_PREFIX = "esz"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/oficina-tecnica-municipal/",
    f"{WP_BASE}/plan-general-de-ordenacion-urbanistica-pgou/",
...
RE_LICENCIA = re.compile(
    ...
    r"obra (?:mayor|menor)|licencia apertura|actividad (?:inocua|clasificada)|alineaci[oó]n|"
    r"actuaci[oó]n urban[ií]stica)",
)
RE_PROYECTO = re.compile(
    ...
    r"aprobaci[oó]n (?:inicial|definitiva|provisional)|sector|suelo|suso|"
    r"cambio de uso|ordenanza|rectificaci[oó]n|exposici[oó]n p[uú]blica|"
    r"calificaci[oó]n|instrumento|urbanizaci[oó]n|revisi[oó]n parcial|"
    r"consulta (?:previa|p[uú]blica)|evaluaci[oó]n ambiental)",
)
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?elsauzal\.es)?/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*El Sauzal.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
def _proyecto_tipo(blob: str) -> str:
    b = blob.lower()
    if "plan parcial" in b or "sector" in b or "suso" in b:
        return "plan parcial"
    ...
    if "consulta" in b and ("previa" in b or "p" in b and "blica" in b):
...
def _geobdp_url_from_text(text: str) -> str:
    m = RE_GEOBDP_URL.search(text or "")
    if m:
        return f"https://geobdp.grafcan.es/core/documentos/{m.group(1)}/"
    return ""


class ElSauzalAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Bones + sede espublico + SITCAN/GEOBDP planeamiento Canarias."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.sitcan_dataset = str(self.config.get("sitcan_package") or SITCAN_DATASET)
    ...
    def _fetch(self, url: str, encoding: str = "utf-8") -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-el-sauzal/1.0")},
        )
    ...
    def _geobdp_geometry(self, url: str) -> dict[str, Any] | None:
        ...
        if "geobdp.grafcan.es" not in url:
            ...
            return None
        fetch_url = url if url.endswith(".html") else (url if url.endswith("/") else f"{url}/")
        if not fetch_url.endswith(".html") and fetch_url.endswith("/"):
            fetch_url = fetch_url.rstrip("/") + ".html"
        try:
    ...
    def _attach_geometry(self, rec: dict[str, Any], geobdp_url: str) -> None:
        if not geobdp_url:
            return
        ...
        for res in (data.get("result") or {}).get("resources") or []:
            ...
            if name not in grouped:
                grouped[name] = {
                    ...
                    "url": url or f"https://opendata.sitcan.es/dataset/{self.sitcan_dataset}",
                    "blob": blob,
                    "origen": "sitcan",
                    "formato": fmt,
                    "geobdp_url": "",
            ...
            entry = grouped[name]
            geobdp = _geobdp_url_from_text(url) or _geobdp_url_from_text(desc)
            if geobdp:
                entry["geobdp_url"] = geobdp
                entry["url"] = geobdp
            if not entry["fecha"]:
                entry["fecha"] = _fecha_from_blob(desc) or _fecha_from_blob(name)
            entry["blob"] = f"{entry['blob']} {desc}"
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        info_pages = [
            (f"{self.sede_base}/", "Sede electrónica — trámites y gestiones"),
            (f"{self.wp_base}/sede-electronica/", "Sede electrónica — portal informativo"),
            (f"{self.wp_base}/oficina-tecnica-municipal/", "Oficina Técnica Municipal — urbanismo"),
            ("https://eadmin.elsauzal.es/", "eAdmin — gestión administrativa municipal"),
        ]
    ...
    def _to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        self._attach_geometry(rec, row.get("geobdp_url") or "")
        return rec
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {"rows": len(rows), "status": "ok", "source": "wordpress_sede"}
