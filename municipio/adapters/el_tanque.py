...
WP_BASE = "https://www.eltanque.es"
SEDE_BASE = "https://eltanque.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
MUNICIPIO = "El Tanque"
ID_PREFIX = "etanque"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-el-tanque"
GEOBDP_MUNICIPIO = "https://geobdp.grafcan.es/core/municipios/38044/"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/area-administrativa/oficina-tecnica/",
    f"{WP_BASE}/2026/06/18/consulta-publica-previa-por-plazo-de-1-mes-del-plan-general-de-ordenacion-el-tanque-2026/",
    f"{WP_BASE}/2025/05/16/el-ayuntamiento-de-el-tanque-como-miembro-integrante-de-la-actual-mancomunidad-de-servicios-garachico-el-tanque-anuncia-la-apertura-del-tramite-de-consulta-publica-previa-para-recabar-la-opinion-de/",
    f"{WP_BASE}/2025/10/17/el-ayuntamiento-ha-obtenido-una-subvencion-por-importe-de-187-70599-e-destinada-a-la-redaccion-de-la-primera-y-segunda-fase-de-los-instrumentos-de-ordenacion-urbanistica-un-hito-clave-para-el-futuro/",
    f"{WP_BASE}/2022/12/19/el-ayuntamiento-de-el-tanque-convoca-la-licitacion-de-las-obras-de-la-segunda-fase-de-las-reformas-de-la-urbanizacion-el-lance-por-importe-de-559-000e/",
    f"{SEDE_BASE}/catalog",
]
...
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?eltanque\.es)?/[^"]+\.pdf[^"]*)"',
    re.I,
)
RE_BOARD_PREVIEW = re.compile(
    r'href="(https://eltanque\.sedelectronica\.es/preview-document/[a-f0-9-]+)"[^>]*title="([^"]+)"',
    re.I,
)
...
class ElTanqueAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Elementor + sede espublico + SITCAN CKAN + GEOBDP Grafcan (Canarias)."""
    ...
    def _fetch(self, url: str, *, use_sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-el-tanque/1.0")},
        )
        if use_sede or self.sede_base in url:
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = [
            {
                "id": _stable_id("lic", f"{self.sede_base}/catalog"),
            ...
            {
                "id": _stable_id("lic", f"{self.wp_base}/area-administrativa/oficina-tecnica/"),
                "fecha_concesion": None,
                "tipo": "trámite informativo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Oficina Técnica — licencias de obra, segregación, actividades",
                "url": f"{self.wp_base}/area-administrativa/oficina-tecnica/",
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
        ]
