...
SEDE_BASE = "https://elverger.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WEB_BASE = "https://www.elverger.es"
MUNICIPIO = "El Verger"
ID_PREFIX = "elverger"
COD_INE_MUN = "03082"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WEB_BASE}/ajuntament/urbanisme/",
    f"{WEB_BASE}/ajuntament/urbanisme/planejament-municipal-vigent/",
    f"{WEB_BASE}/ajuntament/urbanisme/pai-sector-c-partida-barranquets/",
    f"{WEB_BASE}/ajuntament/urbanisme/registre-de-programes-i-agrupacions-dinteres-urbanistic/",
]
...
RE_LICENCIA = re.compile(
    ...
    r"modificaci[oó]n sustancial|urbanizaci[oó]n|reparcel)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|pai|convenio|"
    r"informaci[oó]n p[uú]blica|expediente|proyecto|modificaci[oó]n|reparcel|"
    r"estudio (?:ac[uú]stico|ambiental|de detalle|de integraci[oó]n)|memoria|planos|dogv|edicto|bop|"
    r"aprobaci[oó]n (?:inicial|definitiva|provisional)|parcela|pol[ií]gono|suelo|sector|"
    r"cambio de uso|normas subsidiarias|homologaci[oó]n|ordenanza|e[oó]lic|iate|urbanizaci[oó]n|"
    r"registre|programa|agrupaci)",
)
RE_BOARD_NON_URBAN = re.compile(
    ...
    r"matrimonio|delegaci[oó]n de funciones|hereus abintestat|jurado|col[oò]nies felines|"
    r"escoles esportives|incendis forestals|igualtat d.oportunitats)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://elverger\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_SECTOR = re.compile(r"(?i)sector\s*([0-9A-Z\-]+)")
RE_PDF = re.compile(r'href="([^"]+\.(?:pdf|PDF))"', re.I)
...
def _fecha_from_upload(url: str) -> str | None:
...
def _proyecto_tipo(denominaci: str) -> str:
    ...
    if "reparcel" in n or "urbanizaci" in n:
        return "reparcelación / urbanización"
    if "registre" in n or "programa" in n:
        return "registro urbanístico"
    return "planeamiento"


class ElVergerAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress elverger.es + sede espublico gestiona + ICV WFS zonificación (partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.visor_url = str(
            self.config.get("visor_url") or "https://el-verger-atlas.urbanimpacte.com/"
        )
    ...
    def _fetch(self, url: str, *, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-elverger/1.0")},
        )
    ...
    def _collect_wordpress_seed_pages(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        ...
        for page_url in self.seed_pages:
            ...
            page_title = re.sub(r"\s*>\s*Ajuntament.*$", "", page_title, flags=re.I).strip()
            ...
            for pdf_m in RE_PDF.finditer(html):
                ...
                rec_pdf: dict[str, Any] = {
                    "id": _stable_id("proy", pdf_url),
                    "municipio": MUNICIPIO,
                    "titulo": titulo,
                    "fecha": _fecha_from_upload(pdf_url),
                ...
                self._enrich_geometry(rec_pdf, licencia=False)
                rows.append(rec_pdf)
        ...
        return [
            {
                ...
                "url": self.board_url,
                "source": "ayuntamiento",
                "nota": "Edictos de licencias y actividades en sede espublico gestiona",
                "origen": "sede_tablon",
            },
            {
            ...
            {
                "id": _stable_id("lic", f"{self.web_base}/ajuntament/urbanisme/"),
                "fecha_concesion": None,
                ...
                "url": f"{self.web_base}/ajuntament/urbanisme/",
                "source": "ayuntamiento",
                "nota": "Planeamiento vigente y documentación urbanística",
        ...
        return [
            {
                ...
                "municipio": MUNICIPIO,
                "titulo": "Atlas urbanístico — UrbanImpacte",
                "fecha": None,
                "tipo": "visor GIS",
                "url": self.visor_url,
                "source": "ayuntamiento",
                "origen": "visor",
                "nota": "Visor cartográfico municipal (SPA UrbanImpacte)",
            },
            {
                "id": _stable_id(
                    "proy",
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        elif re.search(r"(?i)ordenan", blob):
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_icv_proyectos():
            add(rec)
        for rec in self._collect_wordpress_seed_pages():
            add(rec)
        for item in self._collect_board():
        ...
        return {
            ...
            "wordpress": sum(1 for r in rows if r.get("origen") in ("wordpress_seed", "wordpress_pdf")),
        }
