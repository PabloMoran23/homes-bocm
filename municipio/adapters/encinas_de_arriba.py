...
SEDE_BASE = "https://encinasdearriba.sedelectronica.es"
...
DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{SEDE_BASE}/catalog/t/23c80d53-bdad-47a5-b731-f786c411e08d",
        "Solicitud de Aprobación de Planeamiento de Desarrollo",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://encinasdearriba\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://encinasdearriba\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"edicto|bando.*parcel|actuaci[oó]n urban|delimitaci[oó]n|suelo urbano|"
    r"unidad(?:es)? de ejecuci[oó]n|normas urban|dsu)",
)
...
def _normalize_sector_code(raw: str) -> str:
    return unescape(raw or "").strip().upper()
...
def _proyecto_tipo(blob: str, instrumento: str = "") -> str:
    ...
    if "delimitaci" in n or n.strip() == "dsu":
...
class EncinasDeArribaAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + PlanPublica PLAU + IDECyL WFS (sin web corporativa activa)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=122"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=122"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-encinas-de-arriba/1.0")},
        )
    ...
    def _parse_plau_rows(html: str, fallback_url: str) -> list[dict[str, str]]:
        ...
        for tr in re.findall(r"<tr[^>]*>(.*?)</tr>", html, re.S | re.I):
            ...
            cells = [c for c in cells if c != "\xa0"]
            if len(cells) < 4 or cells[0] in {"Libro", "Tipo"}:
                continue
            titulo = next(
                (c for c in reversed(cells) if c and not re.fullmatch(r"\d{1,2}/\d{1,2}/\d{4}", c)),
                "",
            )
            if not titulo:
                continue
            fecha = next((c for c in cells if re.fullmatch(r"\d{1,2}/\d{1,2}/\d{4}", c)), "")
            doc_m = (
            ...
            url = (
                f"{PLAU_BASE}/openDocuIndice.do?cDocId={doc_id}"
                if doc_id
                else fallback_url
            )
            rows.append(
                {
                    ...
                    "url": url,
                    "fecha": fecha,
                    "instrumento": f"{cells[0]} {cells[1]}".strip() if len(cells) > 1 else cells[0],
        ...
        for layer, default_tipo in WFS_LAYERS:
            ...
            for feat in data.get("features") or []:
                ...
                if sector and num:
                    titulo = f"{sector} ({num})"
                if not titulo:
                    titulo = str(props.get("n_instrum") or props.get("c_plan") or layer)
    ...
    def _attach_instrument_geometry(self, rec: dict[str, Any]) -> None:
    ...
    def _attach_geometry(self, rec: dict[str, Any]) -> None:
        ...
        for code in _sector_codes_from_text(blob):
            norm = _normalize_sector_code(code)
            if norm in self._sector_geom_cache:
                hit = self._sector_geom_cache[norm]
                if hit:
                    rec["geom_geojson"] = hit["geom_geojson"]
                    rec["geometry_source"] = "portal_wfs"
                    rec["geometry_source_url"] = hit["geometry_source_url"]
                    rec["coord_source"] = "portal_geometry_centroid"
        ...
        if subtipo in {"DSU", "NUM", "NS", "PGOU", "SPG"}:
            self._attach_instrument_geometry(rec)
