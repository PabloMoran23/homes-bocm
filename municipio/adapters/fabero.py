...
WEB_BASE = "https://www.aytofabero.com"
SEDE_BASE = "https://fabero.sedelectronica.es"
...
DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    (f"{WEB_BASE}/tramite_medio_ambientes6.php", "Solicitud de Licencia Urbanística"),
    (f"{WEB_BASE}/tramite_medio_ambientes5.php", "Solicitud de Licencia de Vados"),
    (f"{WEB_BASE}/tramite_medio_ambientes7.php", "Solicitud de Ocupación de Vía Pública"),
]

DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (f"{WEB_BASE}/plan_ordenacion.php", "Plan de Ordenación Urbanística"),
    (f"{WEB_BASE}/urbanismo.php", "Urbanismo"),
    (
...
RE_PREVIEW = re.compile(
    r'href="((?:https://fabero\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra menor|"
    r"primera ocupaci[oó]n|vado|v[ií]a p[uú]blica)",
)
RE_PROYECTO = re.compile(
    ...
    r"edicto|bando.*parcel|actuaci[oó]n urban|subsidiarias|escombrera|"
    r"unidad(?:es)? de ejecuci[oó]n|suelo urbano|su[- ]?nc|s\.u\.n\.c|pau)",
)
...
RE_PDF_HREF = re.compile(r'href="([^"]*(?:\.pdf|plaupdf)[^"]*)"', re.I)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "pau" in n or "actuaci" in n:
        return "proyecto de actuación urbanística"
...
class FaberoAyuntamientoAdapter(AyuntamientoAdapter):
    """Web PHP municipal + sede espublico tablón + PlanPublica JCyL + IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-fabero/1.0")},
        )
    ...
    def _collect_plan_page_pdfs(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(f"{self.web_base}/plan_ordenacion.php")
        except (urllib.error.URLError, OSError, ConnectionError):
            return []
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for block in re.split(r'<div class="separador_descarga_gris"></div>', html, flags=re.I):
            pdfs = RE_PDF_HREF.findall(block)
            if not pdfs:
                continue
    ...
    def _attach_geometry(self, rec: dict[str, Any]) -> None:
        if record_geometry(rec):
            return
        blob = " ".join(str(rec.get(k) or "") for k in ("titulo", "descripcion", "instrumento", "sector_code"))
    ...
    def _collect_tramite_pages(self) -> list[dict[str, Any]]:
        return [{"titulo": title, "url": url, "origen": "tramite_informativo"} for url, title in self.tramite_pages]
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_plan_page_pdfs():
            add(self._generic_to_proyecto(item))
        for item in self._collect_board():
