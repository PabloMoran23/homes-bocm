...
CORPORATE_BASE = "https://www.fresnodecantespino.es"
DIPSEGOVIA_BASE = "https://www.dipsegovia.es/web/ayuntamiento-de-fresno-de-cantespino"
SEDE_BASE = "https://fresnodecantespino.sedelectronica.es"
...
ID_PREFIX = "fresno-de-cantespino"
PLAI_MUNICIPIO = 89
PLAI_PROVINCIA = 40
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (f"{CORPORATE_BASE}/urbanismo", "Urbanismo — web corporativa Liferay"),
    (f"{DIPSEGOVIA_BASE}/urbanismo", "Urbanismo — DipSegovia (archivo PLAI)"),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=089",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=089",
...
RE_PREVIEW = re.compile(
    r'href="(https://fresnodecantespino\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://fresnodecantespino\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class FresnoDeCantespinoAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay fresnodecantespino.es + DipSegovia + sede espublico + PLAI JCYL + IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-fresno-de-cantespino/1.0")},
        )
