...
SEDE_BASE = "https://fresnodelaribera.sedelectronica.es"
MUNICIPIO = "Fresno de la Ribera"
ID_PREFIX = "fresno-de-la-ribera"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=49&municipio=076",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=49&municipio=076",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/openDocuIndice.do?cDocId=282499",
        "Normas Urbanísticas Municipales (NUM)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/openDocuIndice.do?cDocId=282625",
        "Documentación urbanística complementaria (Junta CYL)",
    ),
]
...
RE_PREVIEW = re.compile(
    r'href="(https://fresnodelaribera\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"ribera|surd\.so|normas urban|ordenanza.*urban|tasa.*urban|"
...
class FresnoDeLaRiberaAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico tablón + PlanPublica PLAU + IDECyL WFS (Zamora)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-fresno-de-la-ribera/1.0")},
        )
