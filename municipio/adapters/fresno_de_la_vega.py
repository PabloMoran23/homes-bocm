...
WEB_BASE = "https://www.fresnodelavega.org"
SEDE_BASE = "https://fresnodelavega.sedelectronica.es"
MUNICIPIO = "Fresno de la Vega"
ID_PREFIX = "fresno-de-la-vega"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        f"{WEB_BASE}/pages/documents/",
        "Normativa urbanística municipal",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=073",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=073",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://fresnodelavega\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://fresnodelavega\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class FresnoDeLaVegaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress fresnodelavega.org + sede espublico tablón + IDECyL WFS sectores."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-fresno-de-la-vega/1.0")},
        )
