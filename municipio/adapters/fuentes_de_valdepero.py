...
SEDE_BASE = "https://fuentesdevaldepero.sedelectronica.es"
...
ID_PREFIX = "fuentes-de-valdepero"
...
RE_PREVIEW = re.compile(
    r'href="(https://fuentesdevaldepero\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://fuentesdevaldepero\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class FuentesDeValdeperoAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=34&municipio=086"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=34&municipio=086"
        )
        ...
        req = urllib.request.Request(
            ...
            headers={
                "User-Agent": self.config.get(
                    "user_agent", "poc-bocm-fuentes-de-valdepero/1.0"
                )
            },
        )
        if dossier:
