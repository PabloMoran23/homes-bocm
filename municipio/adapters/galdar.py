...
WP_BASE = "https://www.galdar.es"
SEDE_BASE = "https://galdar.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
MUNICIPIO = "Gáldar"
ID_PREFIX = "gld"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-galdar"
GEOBDP_MUNICIPIO = "https://geobdp.grafcan.es/core/municipios/38009/"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/urbanismo/",
    f"{WP_BASE}/pgo/",
    f"{WP_BASE}/sede-electronica-del-ayuntamiento-de-galdar/",
...
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?galdar\.es)?/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Gáldar.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
class GaldarAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-galdar/1.0")},
        )
    ...
    def _discover_wp_posts(self) -> list[str]:
        urls: list[str] = []
        seen: set[str] = set()
        sitemaps = [f"{self.wp_base}/wp-sitemap-posts-page-1.xml"]
        sitemaps.extend(f"{self.wp_base}/wp-sitemap-posts-post-{i}.xml" for i in range(1, 8))
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo — licencias, certificados e informes",
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            "rows": len(rows),
            "status": "ok",
            "source": "sitcan_geobdp_wordpress_sede",
