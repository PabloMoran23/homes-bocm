...
WEB_BASE = "https://aytogalende.net"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        f"{WEB_BASE}/index.php/home/informacion-publica",
        "Información pública — Normativa Municipal Urbanismo (web Joomla)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=49&municipio=085",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=49&municipio=085",
...
RE_WEB_URB_LINK = re.compile(
    r'class="ui-link"[^>]*href="([^"]+)"[^>]*>\s*<p>([^<]+)</p>',
    re.I,
)
RE_EXPD = re.compile(r"(?i)expediente\s+([0-9]+/[0-9]{2,4})")
...
RE_PROYECTO = re.compile(
    ...
    r"nnumm|sanabria|pol[ií]gono|parcela|ordenanza.*urban|tasa.*urban|"
    r"normas urban|instrumento|memoria|planos|bocyl|ribiella|ilanes|moncabril)",
)
...
def _proyecto_tipo(blob: str) -> str:
    n = blob.lower()
    if "nnumm" in n or "normas urban" in n or re.search(r"\bnum\b", n):
        return "normas urbanísticas"
    if "plan parcial" in n or re.search(r"\bpol\s*\d+", n):
        return "plan parcial"
...
class GalendeAyuntamientoAdapter(AyuntamientoAdapter):
    """Web Joomla + sede espublico + PlanPublica CYL + IDECyL WFS (Sanabria)."""

    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        super().__init__(slug, config, base_url or WEB_BASE)
        self.delay_s = float(self.config.get("request_delay_s", 0.35))
        self.web_base = str(self.config.get("web_base") or WEB_BASE).rstrip("/")
        self.urbanismo_url = str(
            self.config.get("urbanismo_url")
            or f"{self.web_base}/index.php/home/informacion-publica"
        )
    ...
    @staticmethod
    def _normalize_portal_url(raw: str) -> str:
        url = unescape((raw or "").strip())
        if "chrome-extension://" in url:
            m = re.search(r"chrome-extension://[^/]+/(https?://[^\"]+)", url, re.I)
            if m:
                url = m.group(1)
        if url.startswith("/"):
            url = f"{WEB_BASE}{url}"
    ...
    def _collect_web_urbanismo(self) -> list[dict[str, Any]]:
        try:
            ...
            return []
        rows: list[dict[str, Any]] = []
        for m in RE_WEB_URB_LINK.finditer(html):
            url = self._normalize_portal_url(m.group(1))
            titulo = _strip_html(m.group(2))
            if not titulo or "farmacia" in titulo.lower():
                continue
            if not RE_PROYECTO.search(titulo) and "expediente" not in titulo.lower():
                continue
            exp_m = RE_EXPD.search(titulo)
            rows.append(
                {
                    "titulo": titulo[:500],
                    "doc_label": titulo[:500],
