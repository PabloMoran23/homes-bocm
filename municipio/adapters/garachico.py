...
WP_BASE = "https://www.garachico.es"
SEDE_BASE = "https://garachico.sedelectronica.es"
EADMIN_BASE = "https://eadmin.garachico.es"
MUNICIPIO = "Garachico"
ID_PREFIX = "garachico"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-garachico"
GEOBDP_MUNICIPIO = "https://geobdp.grafcan.es/core/municipios/38015/"
BOARD_URL = f"{SEDE_BASE}/board"
CANARIAS_LICENCIAS_URL = "https://sede.gobiernodecanarias.org/sede/tramites/1186"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/?page_id=1108",
    f"{WP_BASE}/?page_id=2016",
    f"{EADMIN_BASE}/transparencia",
    BOARD_URL,
    f"{SEDE_BASE}/dossier",
    CANARIAS_LICENCIAS_URL,
]
...
RE_PROYECTO = re.compile(
    ...
    r"calificaci[oó]n|supletorio|normas subsidiarias|actuaci[oó]n en medio urbano|"
    r"revisi[oó]n parcial|texto refundido|suspensi[oó]n de determinaciones)",
)
RE_WP_LINK = re.compile(r'<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', re.I | re.S)
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?garachico\.es)?/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
RE_BOARD_NON_URBAN = re.compile(
    ...
    r"cobranza|padrones|modificaciones presupuestarias|modif creditos|"
    r"junta de gobierno|residuos urbanos)",
)
...
class GarachicoAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress + sede espublico + SITCAN CKAN + GEOBDP Grafcan (Canarias)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-garachico/1.0")},
        )
    ...
    def _load_geobdp_index(self) -> dict[str, str]:
        ...
        for m in RE_GEOBDP_DOC.finditer(html):
            doc_id, title = _normalize_doc_id(m.group(1)), _strip_html(m.group(2))
    ...
    def _match_geobdp_doc(self, title: str, urls: list[str]) -> str | None:
        for url in urls:
            ...
            if m:
                return _normalize_doc_id(m.group(1))
    ...
    def _collect_geobdp_docs(self) -> list[dict[str, Any]]:
    ...
    def _parse_board(self, html: str) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for row_m in RE_BOARD_ROW.finditer(html):
            ...
            if not cells or not cells.get("Documento"):
                continue
            titulo = cells.get("Descripción") or cells.get("Documento") or ""
            blob = (
                f"{titulo} {cells.get('Procedimiento', '')} {cells.get('Categoría', '')} "
                f"{cells.get('Expediente', '')}"
            )
        ...
        return [
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Tablón de anuncios — sede electrónica Garachico",
                "url": self.board_url,
                "source": "ayuntamiento",
            ...
            {
                "id": _stable_id("lic", CANARIAS_LICENCIAS_URL),
                "fecha_concesion": None,
                "tipo": "trámite licencia obra (Gob. Canarias)",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Sede Gobierno de Canarias — licencia de obra",
                "url": CANARIAS_LICENCIAS_URL,
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
        ]
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for row in [*self._collect_wp_pages(), *self._collect_board()]:
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for collector in (
            self._collect_sitcan,
            self._collect_geobdp_docs,
            self._collect_wp_pages,
            self._collect_board,
        ):
