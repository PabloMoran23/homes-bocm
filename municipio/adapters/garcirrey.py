...
SEDE_BASE = "https://garcirrey.sedelectronica.es"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=149",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=149",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        f"{PLAU_BASE}/openDocuIndice.do?cDocId=278452",
        "Sin Planeamiento General (SPG) — Garcirrey",
    ),
]
...
RE_PREVIEW = re.compile(
    r'href="(https://garcirrey\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://garcirrey\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general|regional)|pgou|spg|"
    ...
    r"sin planeamiento|normas urban|instrumento|memoria|planos|bocyl)",
)
...
class GarcirreyAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico tablón + dossier trámites + IDECyL WFS (SPG, sin web corporativa)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-garcirrey/1.0")},
        )
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_tramites_catalog():
            if RE_LICENCIA.search(item["titulo"]):
                rec = self._tramite_to_licencia(item["url"], item["titulo"])
                if rec["id"] not in seen:
                    seen.add(rec["id"])
                    rows.append(rec)
