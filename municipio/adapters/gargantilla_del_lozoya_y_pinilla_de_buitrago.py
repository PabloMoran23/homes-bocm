...
ID_PREFIX = "gargantilla-del-lozoya-y-pinilla-de-buitrago"
...
DEFAULT_SEED_PAGES: list[str] = [
    ...
    f"{WP_BASE}/informacion-general-y-municipal/",
]
...
RE_PROYECTO = re.compile(
    ...
    r"ordenanza|pol[ií]gono|plusval[ií]a|tasa.*licencia|ocupaci[oó]n de suelo|"
    r"cat[aá]logo de bienes|documento ambiental|aprobaci[oó]n (?:inicial|provisional)|"
    r"\b(?:UE|AD|AN|AI|PAU|S)-[\w\d-]+\b|pinilla de buitrago|gargantilla)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|empleo|bolsa|igualdad|guerra civil|naturaleza viva|"
    r"mirador|nevera|finca riosequillo|bolet[ií]n informativo|fiestas|turismo|"
    r"campamento|fisioterapia|cementerio|sepultura|estacionamiento|basura|ibi)",
)
...
RE_BOARD_PREVIEW = re.compile(
    r'href="(https://gargantillaypinilla\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
def _sector_ilike_parts(text: str) -> list[str]:
    low = text.lower()
    for marker in (" bocm", " aprob", " anuncio", " del pgou", " nnss", " de gargantilla", " de pinilla"):
        if marker in low:
...
class GargantillaDelLozoyaYPinillaDeBuitragoAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Elementor (PGOU/ordenanzas) + sede espublico + WFS SITCM (geometría partial)."""
        ...
        req = urllib.request.Request(
            ...
            headers={
                "User-Agent": self.config.get(
                    "user_agent", "poc-bocm-gargantilla-del-lozoya-y-pinilla-de-buitrago/1.0"
                )
            },
        )
    ...
    def _abs_wp(self, href: str) -> str:
        if href.startswith("http://") or href.startswith("https://"):
            return href.replace("://www.gargantillaypinilla.madrid", "://gargantillaypinilla.madrid")
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        lic_obra = f"{self.wp_base}/wp-content/uploads/2025/06/solicitud-licencia-obra.pdf"
        lic_dr = f"{self.wp_base}/wp-content/uploads/2025/06/IMPRESO-DECLARACION-RESPONSABLE-OBRAS.pdf"
        return [
            {
                "id": _stable_id("lic", lic_obra),
                ...
                "url": f"{self.wp_base}/documentos-municipales/",
                "pdf_url": lic_obra,
                "source": "ayuntamiento",
                "nota": "Impreso descargable en documentos municipales",
                "origen": "wp_tramite",
            },
            {
                "id": _stable_id("lic", lic_dr),
                "fecha_concesion": None,
                "tipo": "declaración responsable de obras",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Impreso declaración responsable de obras",
                "url": f"{self.wp_base}/documentos-municipales/",
                "pdf_url": lic_dr,
                "source": "ayuntamiento",
                "nota": "Formulario DR urbanística (web municipal)",
                "origen": "wp_tramite",
            },
            {
