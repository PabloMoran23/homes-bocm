...
WEB_BASE = "https://www.aytogarrafedetorio.es"
WEB_WP = "https://www.ayuntamientogarrafedetorio.com"
SEDE_BASE = "https://garrafedetorio.sedelectronica.es"
MUNICIPIO = "Garrafe de Torío"
ID_PREFIX = "garrafe-de-torio"
...
DEFAULT_TRAMITE_PAGES: list[str] = [
    f"{WEB_BASE}/ayuntamiento/tramites-solicitudes/licencias-urbanisticas-ambientales-apertura/licencia-urbanistica.html",
    f"{WEB_BASE}/ayuntamiento/tramites-solicitudes/licencias-urbanisticas-ambientales-apertura/licencia-primera-ocupacion-utilizacion.html",
    f"{WEB_BASE}/ayuntamiento/tramites-solicitudes/licencias-urbanisticas-ambientales-apertura/comunicacion-ambiental.html",
    f"{WEB_BASE}/ayuntamiento/tramites-solicitudes/licencias-urbanisticas-ambientales-apertura/licencia-de-apertura.html",
]
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    ...
    (
        f"{WEB_WP}/ayuntamiento/#normativa",
        "Normas urbanísticas — planos por localidad (WordPress)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=076",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=076",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://garrafedetorio\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://garrafedetorio\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_WP_NORMATIVA_PDF = re.compile(
    r'href="(/documentos/[^"]+\.pdf)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class GarrafeDeTorioAyuntamientoAdapter(AyuntamientoAdapter):
    """OpenCms (Diputación León) + WordPress normativa PDFs + sede espublico + IDECyL WFS."""

    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        super().__init__(slug, config, base_url or WEB_BASE)
        self.delay_s = float(self.config.get("request_delay_s", 0.35))
        self.web_base = str(self.config.get("web_base") or WEB_BASE).rstrip("/")
        self.web_wp = str(self.config.get("web_wp") or WEB_WP).rstrip("/")
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-garrafe-de-torio/1.0")},
        )
    ...
    def _collect_wp_normativa_pdfs(self) -> list[dict[str, Any]]:
        url = f"{self.web_wp}/ayuntamiento/"
        try:
            html = self._fetch(url)
        except (urllib.error.URLError, OSError, ConnectionError):
            return []
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for m in RE_WP_NORMATIVA_PDF.finditer(html):
            rel, titulo = m.group(1), unescape(_strip_html(m.group(2)))
            if rel in seen:
                continue
            seen.add(rel)
            full_url = rel if rel.startswith("http") else f"{self.web_wp}{rel}"
            rows.append(
                {
                    "titulo": f"Normativa urbanística — {titulo}"[:500],
                    "url": full_url,
                    "pdf_url": full_url,
                    "origen": "web_normativa_pdf",
                }
            )
        return rows
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_seed_pages():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_wp_normativa_pdfs():
            key = item["url"]
            rec: dict[str, Any] = {
                "id": _stable_id("proy", key),
                "municipio": MUNICIPIO,
