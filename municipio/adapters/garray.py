...
DRUPAL_BASE = "https://www.garray.es"
SEDE_BASE = "https://garray.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{DRUPAL_BASE}/archivos-de-planeamiento-urbanistico",
    f"{DRUPAL_BASE}/modificacion-puntual-no-4-num",
    f"{DRUPAL_BASE}/modificacion-puntual-no-3-num-garray",
    f"{DRUPAL_BASE}/estudio-de-detalle-calle-olmillos-21-23-25-27-de-dombellas",
    f"{DRUPAL_BASE}/estudio-de-detalle-parcela-calle-real-30-de-canredondo-de-la-sierra",
    f"{DRUPAL_BASE}/informacion-urbanistica",
    f"{DRUPAL_BASE}/normas-urbanisticas-municipales-documentacion-normativa",
    f"{DRUPAL_BASE}/normas-urbanisticas-municipales-documentacion-informativa-y-listado-de-planos-y-documentos",
    f"{DRUPAL_BASE}/autorizacion-uso-provisional-en-suelo-urbano-no-consolidado-obras-de-escasa-entidad-constructiva-en",
]

RE_PREVIEW = re.compile(
    r'href="(https://garray\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://garray\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_DRUPAL_PDF = re.compile(
    r'href="((?:https://www\.garray\.es)?/sites/garray\.es/files/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"concentraci[oó]n parcelaria|dinosaurio|dombellas|numancia|num\b|"
    r"canredondo|santerv[aá]s|tardesillas|chavaler|pend[oó]n|negredos)",
)
RE_NOISE = re.compile(
...
class GarrayAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal 7 garray.es + sede espublico + PLAI JCYL + IDECyL WFS (partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-garray/1.0")},
        )
