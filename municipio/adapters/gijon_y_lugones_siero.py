...
SEDE_BASE = "https://sedeelectronica.ayto-siero.es"
RPGUR_BASE = "https://www54.asturias.es/rpgur/action/publico"
MUNICIPIO = "Gijón y Lugones (Siero)"
ID_PREFIX = "gijon-y-lugones-siero"
...
WFS_MUNICIPIO_ID = "33066"

DEFAULT_SEED_PAGES: list[str] = [
    f"{BASE}/portal-de-urbanismo/",
    f"{BASE}/plan-general-vigente-catalogo-urbanistico-vigente/",
    f"{BASE}/desarrollos-de-planeamiento-planes-parciales/",
    f"{BASE}/desarrollos-de-planeamiento-planes-especiales/",
    f"{BASE}/desarrollos-de-planeamiento-estudios-de-detalle/",
    f"{BASE}/agenda-urbana-de-siero/",
    f"{SEDE_BASE}/siac/Publicaciones.aspx?t=ET&viewall=Y",
]

DEFAULT_LICENCIA_PAGES: list[str] = [
    f"{BASE}/portal-de-urbanismo/",
    f"{SEDE_BASE}/siac/Procedimiento_ver_doc.aspx?sol=43",
    f"{SEDE_BASE}/siac/Procedimiento_ver_doc.aspx?sol=161",
    f"{SEDE_BASE}/siac/Procedimiento_ver_doc.aspx?sol=179",
    f"{SEDE_BASE}/siac/Tablon.aspx",
]
...
RE_PROYECTO = re.compile(
    ...
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|agenda urbana|gij[oó]n|lugones)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://www\.ayto-siero\.es)?/(?:descarga|wp-content)[^"]+\.(?:pdf|docx?)[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Siero.*$", "", t, flags=re.I).strip()
...
class GijonYLugonesSieroAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress ayto-siero.es + sede SIAC + RPGUR Asturias (concejo SIERO id=66); geometría parcial WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "66")
    ...
    def _fetch(self, url: str, *, data: bytes | None = None, encoding: str = "utf-8") -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-gijon-y-lugones-siero/1.0"),
        }
        if data is not None:
    ...
    def _crawl_portal_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in RE_DOC_LINK.finditer(html):
                ...
                title = re.sub(r"\.(pdf|docx?)$", "", title, flags=re.I).replace("+", " ").strip()
            ...
            for m in re.finditer(r'href="(https://www\.ayto-siero\.es/[^"#?]+)"', html, re.I):
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        info_pages = [
            (f"{BASE}/portal-de-urbanismo/", "Portal de urbanismo"),
            (f"{SEDE_BASE}/siac/Tablon.aspx", "Tablón de anuncios — sede electrónica"),
            (f"{SEDE_BASE}/siac/Publicaciones.aspx?t=ET&viewall=Y", "Expedientes en información pública"),
        ]
