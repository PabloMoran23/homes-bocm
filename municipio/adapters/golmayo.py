...
DRUPAL_BASE = "https://www.golmayo.es"
SEDE_BASE = "https://golmayo.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{DRUPAL_BASE}/e-l-m-fuenteboa",
    f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=42&municipio=095",
    f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=42&municipio=095",
    f"{PLAI_BASE}/lmuni_plau.do?provincia=42",
    f"{SEDE_BASE}/transparency/286da81e-ea9f-4163-808d-47f0130242df/",
]

RE_PREVIEW = re.compile(
    r'href="(https://golmayo\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://golmayo\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_DRUPAL_PDF = re.compile(
    r'href="((?:https://www\.golmayo\.es)?/sites/golmayo\.es/files/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"concentraci[oó]n parcelaria|fuentetoba|camaretas|campara[nñ][oó]n|"
    r"carbonera|reordenaci[oó]n|sector\s*[0-9pP]|modificaci[oó]n puntual)",
)
RE_NOISE = re.compile(
    ...
    r"electores|cobranza|iae|fiestas|verbena|orquesta|eclipse|gafas|"
    r"partidos del c\.?d\.?|biblioteca|ocio y tiempo libre)",
)
...
def _proyecto_tipo(title: str, instrumento: str = "") -> str:
    ...
    if "pgou" in blob or "plan general" in blob:
        return "PGOU"
    if "sector" in blob or "fuentetoba" in blob or "camaretas" in blob:
        return "sector urbanístico"
    if "reordenaci" in blob:
        return "reordenación urbanística"
    return "planeamiento"
...
class GolmayoAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal 7 golmayo.es + sede espublico + PLAI JCYL + IDECyL WFS (partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-golmayo/1.0")},
        )
