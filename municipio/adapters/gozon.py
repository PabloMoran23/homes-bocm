...
WFS_MUNICIPIO_ID = "33025"

DEFAULT_SEED_PAGES: list[str] = [
    f"{BASE}/urbanismo",
    f"{BASE}/obra-mayor",
    f"{BASE}/declaraci%C3%B3n-responsable",
    f"{BASE}/condiciones-de-edificacion",
    f"{BASE}/prorroga-licencia",
]

DEFAULT_LICENCIA_PAGES: list[str] = [
    f"{BASE}/urbanismo",
    f"{BASE}/obra-mayor",
    f"{BASE}/obra-menor-ordinaria",
    f"{BASE}/declaraci%C3%B3n-responsable",
    f"{BASE}/prorroga-licencia",
    f"{BASE}/primera-ocupacion",
    f"{BASE}/certificado-inexistencia-expediente-infracci%C3%B3n-urban%C3%ADsitica",
]
...
RE_PROYECTO = re.compile(
    ...
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|conjunto hist[oó]rico|luanco|bañugues|xag[oó])",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://www\.ayto-gozon\.org)?/documents/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Goz[oó]n.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3 and "navegaci" not in t.lower():
                return t[:500]
    return fallback
...
def _tipo_proyecto(denominacion: str, clasificacion: str = "") -> str:
    ...
    if "convenio" in blob or "auc" in blob:
        return "actuación urbanística concertada"
...
class GozonAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay ayto-gozon.org + RPGUR Asturias; geometría parcial WFS visorrpgur (id_municipio=33025)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "25")
    ...
    def _fetch(self, url: str, *, data: bytes | None = None, encoding: str = "utf-8") -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-gozon/1.0"),
        }
        if data is not None:
    ...
    def _crawl_liferay_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in re.finditer(r'href="(https://www\.ayto-gozon\.org/[^"#?]+)"', html, re.I):
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
                    ...
                    rows.append(
                        {
                            ...
                            "url": page_url,
                            "source": "ayuntamiento",
                            "nota": "Formulario/trámite informativo; sin listado público de concesiones",
                        }
                    )
        ...
        rpgur_url = f"{self.rpgur_base}/busquedaConsulta?method=listPublico&idConcejo={self.rpgur_concejo_id}&estado=V"
        info_id = _stable_id("lic", rpgur_url)
            ...
            rows.append(
                {
                    ...
                    "lat": None,
                    "lon": None,
                    "titulo": "Urbanismo — RPGUR y trámites municipales",
                    "url": rpgur_url,
                    "source": "ayuntamiento",
                    "nota": "Instrumentos planeamiento en RPGUR; licencias solo vía formularios/sede",
                }
            )
        return rows
