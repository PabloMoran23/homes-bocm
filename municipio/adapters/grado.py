...
BASE = "https://www.ayto-grado.es"
SEDE_BASE = "https://sedeelectronica.ayto-grado.es"
...
WFS_FILTER = "Instrumento LIKE '%GRADO%'"

DEFAULT_SEED_PAGES: list[str] = [
    f"{BASE}/urbanismo",
    f"{BASE}/revision-pgou",
    f"{BASE}/arearehabilitacion",
]
...
RE_LICENCIA = re.compile(
    ...
    r"autorizaci[oó]n (?:previa|urban)|ordenanza|normativa urban|edificaci[oó]n|obra menor)",
)
RE_PROYECTO = re.compile(
    ...
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|urbanizaci|revisi[oó]n|bopa)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://www\.ayto-grado\.es)?/documents/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Grado.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
class GradoAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay ayto-grado.es + RPGUR Asturias; geometría parcial WFS visorrpgur."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "26")
    ...
    def _fetch(self, url: str, *, data: bytes | None = None, encoding: str = "utf-8") -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-grado/1.0"),
        }
        if data is not None:
    ...
    def _crawl_liferay_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in re.finditer(r'href="(https://www\.ayto-grado\.es/[^"#?]+)"', html, re.I):
        ...
        rows.append(
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo y trámites de licencias",
                "url": info_url,
                "source": "ayuntamiento",
                "nota": "Sin tablón público de concesiones; sede electrónica con timeout en CI",
            }
        )

        sede_url = f"{SEDE_BASE}/"
            ...
            rows.append(
                {
                    ...
                    "url": sede_url,
                    "source": "ayuntamiento",
                    "nota": "Tablón de anuncios en sede; no scrapeable por timeout/red",
                }
            )
                    ...
                    rows.append(
                        {
                            ...
                            "url": page_url,
                            "source": "ayuntamiento",
                            "nota": "Página informativa; modelo declaración responsable obra menor",
                        }
                    )
