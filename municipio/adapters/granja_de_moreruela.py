...
SEDE_BASE = "https://granjademoreruela.sedelectronica.es"
MUNICIPIO = "Granja de Moreruela"
ID_PREFIX = "granja-de-moreruela"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        f"https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia={PLAN_PROVINCIA}&municipio={PLAN_MUNICIPIO}",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        f"https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia={PLAN_PROVINCIA}&municipio={PLAN_MUNICIPIO}",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/openDocuIndice.do?cDocId=280531",
        "Delimitación de Suelo Urbano (DSU) — Granja de Moreruela",
    ),
]

DEFAULT_LICENCIA_PAGES: list[tuple[str, str]] = [
    (f"{SEDE_BASE}/board", "tablón de anuncios — licencias y urbanismo"),
    (f"{SEDE_BASE}/transparency", "transparencia — urbanismo y obras públicas"),
]

RE_PREVIEW = re.compile(
    r'href="(https://granjademoreruela\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"delimitaci[oó]n.*suelo|normas urban|instrumento|dsu\b|memoria|planos|bocyl)",
)
RE_EXCLUDE = re.compile(
    ...
    r"electores|icio\b|ordenanza fiscal|teleclub|albergue|peregrin)",
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "delimitaci" in n and "suelo" in n:
        return "delimitación de suelo urbano"
...
class GranjaDeMoreruelaAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico (tablón vacío) + PlanPublica JCyL + IDECyL WFS (DSU 1988)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-granja-de-moreruela/1.0")},
        )
