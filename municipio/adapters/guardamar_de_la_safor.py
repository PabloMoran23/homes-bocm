...
SEDE_BASE = "https://guardamardelasafor.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WEB_BASE = "https://guardamardelasafor.org"
URBANISMO_URL = f"{WEB_BASE}/es/serveis-municipals/urbanisme/"
TRANSPARENCY_URL = f"{SEDE_BASE}/transparency"
REG_PLANEAMIENTO_URL = (
    "https://mediambient.gva.es/auto/urbanismo/reg-planeamiento/4%20VALENCIA/"
    "46140%20GUARDAMAR%20DE%20LA%20SAFOR/"
)
MUNICIPIO = "Guardamar de la Safor"
ID_PREFIX = "guardamar-de-la-safor"
COD_INE_MUN = "46140"
...
ICV_ZONES: list[dict[str, Any]] = [
    {
        "fids": ["945"],
        "denominaci": "Plan General — zona nuevo desarrollo industrial (ZND-IN)",
        "expediente": "20011225",
        "tipo": "plan parcial",
        "match_tokens": ["industrial"],
    },
    {
        "fids": ["4170", "4136", "4250", "4252"],
        "denominaci": "Plan General — zona nuevo desarrollo residencial (ZND-RE)",
        "expediente": "20011225",
        "tipo": "PGOU",
        "match_tokens": ["residencial", "casco", "playa", "ensanche", "pueblo", "normativa", "pgou"],
    },
]

URBANISMO_PDFS: list[dict[str, str]] = [
    {
        "titulo": "VER NORMATIVA (NNSS/PGOU)",
        "url": f"{WEB_BASE}/wp-content/uploads/2024/09/ayuntamiento-479592587.pdf",
        "tipo": "PGOU",
    },
    {
        "titulo": "SOLO INDUSTRIAL — plan parcial sector industrial",
        "url": f"{WEB_BASE}/wp-content/uploads/2024/09/ayuntamiento-864083815.pdf",
        "tipo": "plan parcial",
    },
    {
        "titulo": "CASCO URBANO",
        "url": f"{WEB_BASE}/wp-content/uploads/2024/09/ayuntamiento-1104848687.pdf",
        "tipo": "plan parcial",
    },
    {
        "titulo": "ENSANCHE CASCO URBANO",
        "url": f"{WEB_BASE}/wp-content/uploads/2024/09/ayuntamiento-447295644_compressed.pdf",
        "tipo": "plan parcial",
    },
    {
        "titulo": "RESIDENCIAL BAJA DENSIDAD PUEBLO",
        "url": f"{WEB_BASE}/wp-content/uploads/2024/09/ayuntamiento-2024645708.pdf",
        "tipo": "plan parcial",
    },
    {
        "titulo": "SECTOR BAJA DENSIDAD PLAYA",
        "url": f"{WEB_BASE}/wp-content/uploads/2024/09/ayuntamiento-1400070421_compressed.pdf",
        "tipo": "plan parcial",
    },
    {
        "titulo": "Sector residencial PLAYA",
        "url": f"{WEB_BASE}/wp-content/uploads/2024/09/ayuntamiento-415328093_compressed.pdf",
        "tipo": "plan parcial",
    },
    {
        "titulo": "SUELO NO URBANIZABLE PROTECCIÓN AGRÍCOLA",
        "url": f"{WEB_BASE}/wp-content/uploads/2024/09/ayuntamiento-1233469931.pdf",
        "tipo": "planeamiento",
    },
]
...
RE_LICENCIA = re.compile(
    ...
    r"obra (?:mayor|menor)|primera ocupaci[oó]n|segunda ocupaci[oó]n)",
)
RE_PROYECTO = re.compile(
    ...
    r"cambio de uso|normas subsidiarias|homologaci[oó]n|ordenanza|nnss|pgmod)",
)
RE_BOARD_NON_URBAN = re.compile(
    ...
    r"matrimonio|delegaci[oó]n de funciones)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://guardamardelasafor\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
class GuardamarDeLaSaforAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.reg_planeamiento_url = str(
            self.config.get("reg_planeamiento_url") or REG_PLANEAMIENTO_URL
        )
    ...
    def _fetch(self, url: str, *, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-guardamar-de-la-safor/1.0")},
        )
    ...
    def _fetch_icv_feature(self, fid: str) -> dict[str, Any] | None:
        ...
        if str(props.get("cod_ine_mun") or "") != self.cod_ine_mun:
            self._icv_geom_cache[fid] = None
            return None
        self._icv_geom_cache[fid] = feats[0]
        return feats[0]

    def _fetch_icv_zone_geometry(self, fids: list[str]) -> dict[str, Any] | None:
        feats: list[dict[str, Any]] = []
        urls: list[str] = []
        for fid in fids:
            feat = self._fetch_icv_feature(fid)
            if feat:
                feats.append(feat)
                urls.append(self._icv_geometry_url(fid))
        ...
        return {
            ...
            "geometry_source_url": urls[0] if len(urls) == 1 else ";".join(urls[:3]),
    ...
    def _match_icv_zone(self, titulo: str) -> dict[str, Any] | None:
        ...
        for zone in ICV_ZONES:
            score = 0.0
            for token in zone.get("match_tokens") or []:
    ...
    def _should_enrich_board_geometry(self, rec: dict[str, Any], *, licencia: bool) -> bool:
        ...
        if licencia:
            return bool(re.search(r"(?i)pol[ií]gono|parcela|diseminad|sector", titulo))
        return rec.get("tipo") in {"planeamiento", "información pública", "PGOU", "plan parcial"}
    ...
    def _enrich_geometry(self, rec: dict[str, Any], *, licencia: bool = False) -> None:
        ...
        if not zone:
            return
        geom = self._fetch_icv_zone_geometry(zone["fids"])
        if geom:
            rec.update(geom)
    ...
    def _collect_web_pdf_proyectos(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        pdfs = list(URBANISMO_PDFS)
        try:
            html = self._fetch(self.urbanismo_url)
            seen_urls = {p["url"] for p in pdfs}
            for m in RE_PDF_LINK.finditer(html):
                href = m.group(1)
                if href.startswith("/"):
                    href = f"{self.web_base}{href}"
                if href in seen_urls or "wp-content/uploads" not in href:
                    continue
                seen_urls.add(href)
                pdfs.append(
                    {
                        "titulo": f"Documento urbanismo — {href.rsplit('/', 1)[-1]}",
                        "url": href,
                        "tipo": "planeamiento",
                    }
                )
        ...
        for pdf in pdfs:
            titulo = pdf["titulo"]
            rec: dict[str, Any] = {
                "id": _stable_id("proy", pdf["url"]),
                "municipio": MUNICIPIO,
                "titulo": titulo[:500],
                "fecha": "2001-12-25" if "pgou" in titulo.lower() or "normativa" in titulo.lower() else None,
                "tipo": pdf.get("tipo") or "planeamiento",
                "url": pdf["url"],
                "source": "ayuntamiento",
                "origen": "web_pdf",
            }
        ...
        return [
            {
                ...
                "url": self.board_url,
                "source": "ayuntamiento",
                "nota": "Edictos de licencias publicados en sede espublico",
                "origen": "sede_tablon",
            },
            {
                ...
                "source": "ayuntamiento",
                "nota": "Licencias 1ª/2ª ocupación, DR obras reforma",
                "origen": "sede_tramite",
            },
            {
        ...
        return [
            {
                ...
                "municipio": MUNICIPIO,
                "titulo": "Urbanismo — planes y normativa (web municipal)",
                "fecha": None,
                "tipo": "planeamiento",
            ...
            },
            {
                ...
                "municipio": MUNICIPIO,
                "titulo": "Portal transparencia — carpeta urbanismo (sede)",
                "fecha": None,
                "tipo": "transparencia",
                "url": self.transparency_url,
                "source": "ayuntamiento",
                "origen": "transparencia",
                "nota": "Carpeta LGT 7 — instrumentos planeamiento (Wicket AJAX)",
            },
            {
                "id": _stable_id("proy", self.reg_planeamiento_url),
                "municipio": MUNICIPIO,
                "titulo": "Registro planeamiento GVA — Guardamar de la Safor",
                "fecha": None,
                "tipo": "planeamiento",
                "url": self.reg_planeamiento_url,
                "source": "ayuntamiento",
                "origen": "gva_registro",
                "nota": "PG 2001/1225, PGMOD nº7",
            },
            {
                "id": _stable_id(
                    "proy",
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_icv_proyectos():
            add(rec)
        for rec in self._collect_web_pdf_proyectos():
            add(rec)
        for item in self._collect_board():
