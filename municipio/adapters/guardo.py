...
WEB_BASE = "https://guardo.org"
SEDE_BASE = "https://guardo.sedelectronica.es"
MUNICIPIO = "Guardo"
ID_PREFIX = "guardo"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=34&municipio=80",
        "Planeamiento en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=34&municipio=80",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/openDocuIndice.do?cDocId=282619",
        "PGOU revisión (PlanPublica Junta CYL)",
    ),
    (f"{WEB_BASE}/anuncios/", "Anuncios municipales (normativa urbanística)"),
]

RE_PREVIEW = re.compile(
    r'href="(https://guardo\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://guardo\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra menor|"
    r"primera ocupaci[oó]n|recepci[oó]n de obra)",
)
RE_PROYECTO = re.compile(
    ...
    r"sector|edicto|actuaci[oó]n urban|normalizaci[oó]n|urbanizaci[oó]n|"
    r"unidad(?:es)? de (?:actuaci[oó]n|ejecuci[oó]n)|suelo urban|nuf[- ]?\d|"
    r"camponecha|el cercado|casco hist[oó]rico|conjunto hist[oó]rico)",
)
...
def _fecha_from_blob(text: str) -> str | None:
    return _parse_fecha_dmy(text) or _parse_fecha_iso(text) or _parse_fecha_iso(
        RE_FECHA_YMD.sub(lambda m: f"{m.group(1)}-{m.group(2)}-{m.group(3)}", text or "")
    )
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "plan parcial" in n or "camponecha" in n:
        return "plan parcial"
    if "plan especial" in n or "casco hist" in n or "conjunto hist" in n:
        return "plan especial"
    if "modificaci" in n and "pgou" in n:
        return "modificación PGOU"
    if "normalizaci" in n or "urbanizaci" in n:
...
class GuardoAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress guardo.org + sede espublico + SIUCyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, dossier: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-guardo/1.0")},
        )
        if dossier:
    ...
    def _collect_tramites_catalog(self) -> list[dict[str, Any]]:
        ...
        # Guardo sede requiere cookie de sesión del tablón antes de abrir el catálogo.
        for warm_url in (self.board_url, self.info_url):
            try:
                self._fetch(warm_url, sede=True)
            except (urllib.error.URLError, OSError, ConnectionError):
                continue
        html = ""
        dossier_urls = (self.dossier_url, f"{self.sede_base}/dossier")
        for attempt in range(int(self.config.get("dossier_retries", 3))):
            url = dossier_urls[attempt % len(dossier_urls)]
            try:
                html = self._fetch(url, sede=True)
                if "catalog/t/" in html:
                    break
            except (urllib.error.URLError, OSError, ConnectionError):
                if attempt + 1 < int(self.config.get("dossier_retries", 3)):
                    time.sleep(self.delay_s * (attempt + 2))
                    try:
                        self._fetch(self.board_url, sede=True)
                    except (urllib.error.URLError, OSError, ConnectionError):
                        pass
                continue
        if not html or "catalog/t/" not in html:
            return []
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
    ...
    def _collect_wp_proyectos(self) -> list[dict[str, Any]]:
        ...
        for page in range(1, 12):
            url = f"{self.web_base}/urbanismo/" if page == 1 else f"{self.web_base}/urbanismo/page/{page}/"
            try:
                ...
                break
            posts: list[tuple[str | None, str, str]] = []
            for m in re.finditer(r"<article[^>]*>(.*?)</article>", html, re.S):
                art = m.group(1)
                link_m = re.search(r'href="([^"]+)"', art)
                title_m = re.search(r"<h2[^>]*>.*?<a[^>]*>([^<]+)</a>", art, re.S)
                date_m = re.search(r'datetime="([^"]+)"', art)
                if link_m and title_m:
                    posts.append(
                        (
                            date_m.group(1)[:10] if date_m else None,
                            _strip_html(title_m.group(1)),
                            link_m.group(1),
                        )
                    )
            if not posts:
                break
            for fecha, titulo, post_url in posts:
                if post_url in seen:
                    continue
                seen.add(post_url)
                fecha_val = fecha or None
                if not fecha_val:
                    try:
                        detail = self._fetch(post_url)
                        fecha_val = _fecha_from_blob(detail[:8000])
                    except urllib.error.URLError:
                        pass
                rows.append(
                    {
        ...
        for url, fallback_title in self.seed_urls:
            ...
            try:
                ...
                if title_m:
                    ...
                    titulo = re.sub(r"\s*[-|]\s*Guardo.*$", "", titulo, flags=re.I).strip()
            except (urllib.error.URLError, OSError, ConnectionError):
                pass
    ...
    def _wfs_sector_geometry(self, sector_code: str) -> tuple[dict[str, Any] | None, str | None]:
        ...
        cql = f"n_mun='{self.wfs_municipio.replace(chr(39), chr(39)+chr(39))}' AND (n_num_sect ILIKE '%{escaped}%' OR n_sector ILIKE '%{escaped}%')"
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_wfs_proyectos():
            add(item)
        for item in self._collect_wp_proyectos():
        ...
        return {
            ...
            "wordpress": sum(1 for r in rows if r.get("origen") == "wordpress_urbanismo"),
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
        }
