...
SEDE_BASE = "https://guarroman.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WEB_BASE = "https://www.guarroman.es"
ORDENANZAS_URL = (
    f"{WEB_BASE}/transparencia-ayuntamiento-guarroman/"
    "informacion-institucional/normativa/ordenanzas-urbanisiticas/"
)
URBANISMO_URL = f"{WEB_BASE}/urbanismo/"
REGISTRO_PLANEAMIENTO_URL = f"{WEB_BASE}/registro-municipal-instrumentos-planeamiento/"
CITIZEN_URBANISMO_URL = f"{SEDE_BASE}/citizen-service/4b04d1fc-d511-4ec8-9239-4c3d2c05a3cc"
MUNICIPIO = "Guarromán"
ID_PREFIX = "guarroman"
...
RE_PROYECTO = re.compile(
    ...
    r"normas urban|edificaci[oó]n|sancionador|denuncia|calificaci[oó]n ambiental|gasolinera)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"cobranza iae|padrones|modificaciones presupuestarias|modif creditos|"
    r"oferta de empleo p[uú]blico|n[oó]minas|arbitrios|censo electoral|"
    r"convocatoria de el pleno)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://guarroman\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_ORDENANZA_LINK = re.compile(
    r'<a href="(https?://(?:www\.)?guarroman\.es/wp-content/uploads/[^"]+\.pdf)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_WP_PDF = re.compile(
    r'href=["\'](https?://(?:www\.)?guarroman\.es/wp-content/uploads/[^"\']+\.pdf)["\']',
    re.I,
)
RE_ORDENANZA_URBAN = re.compile(
    r"(?i)(normas urban|pgou|pbom|planeamiento|edificaci[oó]n|suelo|subsu|"
    r"protecci[oó]n del espacio urbano|habitabilidad|fuera de ordenaci[oó]n|"
    r"entidades urban[ií]sticas|licencia urban|residuos.*construcci[oó]n|pol[ií]gono)",
)
...
def _proyecto_tipo(title: str, procedimiento: str = "") -> str:
    ...
    if "plan parcial" in blob or "polígono" in blob or "poligono" in blob:
        return "plan parcial"
...
class GuarromanAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + WordPress (urbanismo, ordenanzas)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.registro_planeamiento_url = str(
            self.config.get("registro_planeamiento_url") or REGISTRO_PLANEAMIENTO_URL
        )
    ...
    def _fetch(self, url: str, *, insecure: bool | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-guarroman/1.0")},
        )
    ...
    def _collect_urbanismo_pdfs(self) -> list[dict[str, Any]]:
        ...
        for m in RE_WP_PDF.finditer(html):
            ...
            title = fname.replace("-", " ").replace(".pdf", "")
            if "reguladora" in fname.lower():
                title = "Ordenanza reguladora de polígono industrial"
            elif re.fullmatch(r"\d+\.pdf", fname, re.I):
                title = f"Plano urbanístico nº {fname.replace('.pdf', '')}"
            rows.append(
                {
                    "titulo": title[:500],
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Licencias urbanísticas — sede electrónica",
                "url": self.citizen_urbanismo_url,
                "source": "ayuntamiento",
                "nota": "Página informativa sobre declaraciones responsables y licencias",
                "origen": "sede_tramite",
            },
            {
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Ordenanzas urbanísticas — normativa edificación",
                "url": self.ordenanzas_url,
                "source": "ayuntamiento",
                "nota": "Normas subsidiarias y ordenanzas de edificación publicadas en web municipal",
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_ordenanzas_urbanismo():
            add(self._doc_to_proyecto(item, "ordenanzas"))
        for item in self._collect_urbanismo_pdfs():
            add(self._doc_to_proyecto(item, "web_urbanismo"))
