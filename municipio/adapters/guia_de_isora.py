...
WP_BASE = "https://www.guiadeisora.org/corp"
SEDE_BASE = "http://guiadeisora.sedelectronica.es"
MUNICIPIO = "Guía de Isora"
ID_PREFIX = "gdi"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-guia-de-isora"
GEOBDP_MUNICIPIO = "https://geobdp.grafcan.es/core/municipios/38019/"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/areas-municipales/urbanismo/",
    f"{WP_BASE}/areas-municipales/urbanismo/plan-general-de-ordenacion-urbana/",
    f"{WP_BASE}/areas-municipales/urbanismo/otras-formas-de-ordenacion/",
    f"{WP_BASE}/areas-municipales/urbanismo/tramites/",
    f"{WP_BASE}/administracion/tramites-municipales-modelos-de-solicitud/",
    f"{SEDE_BASE}/board",
]
...
RE_PROYECTO = re.compile(
    ...
    r"texto refundido|suspensi[oó]n de determinaciones|compensaci[oó]n)",
)
RE_WP_LINK = re.compile(r'<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', re.I | re.S)
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?guiadeisora\.org)?/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https?://guiadeisora\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"subvenci[oó]n|becas?|padrones?|impuesto|tasa municipal)",
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Gu[ií]a de Isora.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
class GuiaDeIsoraAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress FoundationPress + sede espublico + SITCAN CKAN + GEOBDP Grafcan."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-guia-de-isora/1.0")},
        )
    ...
    def _discover_wp_posts(self) -> list[str]:
        ...
        seen: set[str] = set()
        try:
            index_xml = self._fetch(f"{self.wp_base}/sitemap.xml")
        except urllib.error.URLError:
            return urls
        sitemaps = [
            m.group(1).strip()
            for m in RE_SITEMAP_LOC.finditer(index_xml)
            if "sitemap-pt-post" in m.group(1)
        ]
        for sitemap in sitemaps[:24]:
            try:
    ...
    def _collect_wp_pages(self) -> list[dict[str, Any]]:
        ...
        for page_url in page_urls:
            ...
            for m in RE_WP_LINK.finditer(html):
                ...
                if not re.search(r"(?i)(?:wp-content|/download/).*\.pdf", href):
                    continue
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Catálogo de trámites — sede espublico",
                "url": f"{self.sede_base}/dossier",
                "source": "ayuntamiento",
                "origen": "sede_tramite",
            },
            {
                "id": _stable_id("lic", f"{self.wp_base}/areas-municipales/urbanismo/tramites/"),
                "fecha_concesion": None,
                "tipo": "modelos solicitud licencias",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Trámites urbanísticos — modelos de solicitud (licencias, DR, informes)",
                "url": f"{self.wp_base}/areas-municipales/urbanismo/tramites/",
                "source": "ayuntamiento",
    ...
    def _to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        if row.get("origen") in {"sede_tablon", "sede_tramite", "tramite_informativo"} and row.get("id"):
            return row
        blob = row.get("blob") or ""
        if row.get("origen") == "sede_tablon":
            if not self._board_is_urban(row) or not RE_LICENCIA.search(blob):
                return None
