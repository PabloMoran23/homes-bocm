...
WP_BASE = "https://guijuelo.es"
SEDE_BASE = "https://guijuelo.sedelectronica.es"
MUNICIPIO = "Guijuelo"
ID_PREFIX = "guijuelo"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (f"{WP_BASE}/urbanismo/", "Urbanismo — web municipal (WordPress)"),
    (
        f"{SEDE_BASE}/transparency/3db63381-f67e-44fa-89ba-039537fb040a/",
        "Transparencia — planeamiento y gestión urbanística (sede)",
    ),
    (
        f"https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia={PLAI_PROVINCIA}&municipio={PLAI_MUNICIPIO:03d}",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        f"https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia={PLAI_PROVINCIA}&municipio={PLAI_MUNICIPIO:03d}",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://guijuelo\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://guijuelo\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"parcela|suelo|sector|unidad(?:es)? de ejecuci[oó]n|\b(?:UE|UBZ|UBN|UNC|SUR)[- ]?[A-Z0-9]+\b|"
    r"actuaci[oó]n|autorizaci[oó]n de uso|ruina|desafectaci[oó]n|enajenaci[oó]n|porcino|ganado)",
)
...
class GuijueloAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress guijuelo.es + sede espublico + PLAI JCYL + IDECyL WFS (geometría partial)."""
    ...
    def _bootstrap_session(self) -> None:
        ...
        req = urllib.request.Request(
            self.board_url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-guijuelo/1.0")},
        )
        try:
