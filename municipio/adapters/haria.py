...
WP_BASE = "https://www.ayuntamientodeharia.com/haria"
SEDE_BASE = "https://sede.ayuntamientodeharia.com"
MUNICIPIO = "Haría"
ID_PREFIX = "haria"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-haria"
PLAN_GENERAL_URL = f"{WP_BASE}/ayuntamiento/normativa/plan-general/"
FORMULARIOS_URL = f"{WP_BASE}/formularios-y-bases-de-concursos-y-subvenciones/"
PLAN_GENERAL_WP_API = f"{WP_BASE}/wp-json/wp/v2/pages/134"
...
RE_LICENCIA = re.compile(
    ...
    r"cedula urban|certificaci[oó]n.*urban)",
)
RE_PROYECTO = re.compile(
    ...
    r"texto refundido|suspensi[oó]n de determinaciones|ordenaci[oó]n|cat[aá]logo|"
    r"normas urban)",
)
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?ayuntamientodeharia\.com)?/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "ordenaci" in b and "estructural" in b:
        return "ordenación estructural"
    if "ordenaci" in b and "pormenorizada" in b:
...
class HariaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress ayuntamientodeharia.com + sede Galileo + SITCAN CKAN + GEOBDP Grafcan."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-haria/1.0")},
        )
    ...
    def _load_geobdp_index(self) -> dict[str, str]:
        ...
        try:
            ...
            for res in (payload.get("result") or {}).get("resources") or []:
                url = (res.get("url") or "").strip()
                name = (res.get("name") or "").strip()
                m = RE_GEOBDP_URL.search(url)
                if m and name:
                    doc_id = m.group(1)
                    index[_norm_title(name)] = doc_id
                    short = _norm_title(re.sub(r"\(.*\)$", "", name))
                    if short:
    ...
    def _collect_edictos_rss(self) -> list[dict[str, Any]]:
        ...
        for item in root.findall("./channel/item"):
            ...
            fecha = None
            try:
                fecha = datetime.strptime(pub[:25], "%a, %d %b %Y %H:%M:%S").strftime("%Y-%m-%d")
            except ValueError:
    ...
    def _collect_wp_plan_general_api(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        try:
            page = self._fetch_json(PLAN_GENERAL_WP_API)
        except (urllib.error.URLError, json.JSONDecodeError):
            return rows
        title = (page.get("title") or {}).get("rendered") or "Plan General"
        content = (page.get("content") or {}).get("rendered") or ""
        fecha = (page.get("modified") or page.get("date") or "")[:10] or None
        rows.append(
            {
                "titulo": _strip_html(title)[:500],
                "fecha": fecha,
                "url": PLAN_GENERAL_URL,
                "blob": f"{title} plan general ordenación haría",
                "origen": "wordpress_api_page",
                "urls": [PLAN_GENERAL_URL],
            }
        )
        for m in RE_WP_LINK.finditer(content):
            ...
            name = unescape(urllib.parse.unquote(Path(pdf.split("?")[0]).name))
            pdf_blob = f"{title} {anchor} {name} {pdf}"
            if not RE_PROYECTO.search(pdf_blob) and not RE_LICENCIA.search(pdf_blob):
                continue
            rows.append(
                {
                    "titulo": (anchor if len(anchor) > 3 else name)[:500],
                    "fecha": _fecha_from_blob(f"{name} {pdf}"),
                    "url": PLAN_GENERAL_URL,
                    "pdf_url": pdf,
                    "blob": pdf_blob,
                    "origen": "wordpress_api_pdf",
                    "urls": [pdf, PLAN_GENERAL_URL],
                }
            )
        return rows
    ...
    def _collect_wp_pages(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        rows.extend(self._collect_wp_plan_general_api())
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "id": _stable_id("lic", FORMULARIOS_URL),
                "fecha_concesion": None,
                "tipo": "trámite informativo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Formularios oficina técnica — licencias y comunicaciones previas",
                "url": FORMULARIOS_URL,
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
            {
                "id": _stable_id("lic", f"{self.sede_base}/publico/territorio/informeurbanistico"),
                "fecha_concesion": None,
                "tipo": "trámite informativo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Informe urbanístico — sede electrónica",
                "url": f"{self.sede_base}/publico/territorio/informeurbanistico",
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
        ]
