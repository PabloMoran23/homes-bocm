...
WEB_BASE = "https://herreradepisuerga.es"
SEDE_BASE = "https://herreradepisuerga.sedelectronica.es"
MUNICIPIO = "Herrera de Pisuerga"
ID_PREFIX = "herrera-de-pisuerga"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=34&municipio=83",
        "Planeamiento en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=34&municipio=83",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        "http://servicios.jcyl.es/PlanPublica/lplanes.plau?municipio=3408380001701",
        "Planeamiento urbanístico aprobado (PLAU Herrera de Pisuerga)",
    ),
    (f"{WEB_BASE}/ayuntamiento/urbanismo/", "Urbanismo municipal (galería documentos)"),
]

RE_PREVIEW = re.compile(
    r'href="(https://herreradepisuerga\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://herreradepisuerga\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"normas urban|memoria|planos|pol[ií]gono industrial|sunc|sue[- ]?\d|"
    r"protecci[oó]n|arqueol[oó]gic|catalogo|catalago)",
)
...
RE_GALLERY_SKIP = re.compile(
    r"(?i)(feed|oembed|json|rsd|seccion|seguir|ir a|herrera de pisuerga\s*$|comentarios)",
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "modificaci" in n and ("normat" in n or "pgou" in n):
        return "modificación planeamiento"
    if "normalizaci" in n or "urbanizaci" in n:
        return "actuación urbanística"
    if "memoria" in n or "planos" in n:
...
class HerreraDePisuergaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress + sede espublico + PLAU/PLAI Junta CYL + IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, dossier: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-herrera-de-pisuerga/1.0")},
        )
        if dossier or sede:
            opener = self._opener
            retries = int(self.config.get("dossier_retries", 3)) if dossier else 1
        else:
    ...
    def _collect_gallery_proyectos(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        base = f"{self.web_base}/ayuntamiento/urbanismo/"
        for st in range(0, 12):
            url = base if st == 0 else f"{base}?g=10&st={st}"
            try:
                html = self._fetch(url)
            except (urllib.error.HTTPError, urllib.error.URLError):
                break
            titles: list[str] = []
            for m in re.finditer(r'title="([^"]{8,})"', html):
                titulo = unescape(m.group(1).strip())
                if RE_GALLERY_SKIP.search(titulo):
                    continue
                if RE_PROYECTO.search(titulo):
                    titles.append(titulo)
            if not titles and st > 0:
                break
            for titulo in titles:
                key = f"gallery:{st}:{titulo}"
                if key in seen:
                    continue
                seen.add(key)
                rows.append(
        ...
        for url, fallback_title in self.seed_urls:
            ...
            try:
                ...
                if title_m:
                    ...
                    titulo = re.sub(r"\s*[-|]\s*Herrera de Pisuerga.*$", "", titulo, flags=re.I).strip()
            except (urllib.error.URLError, OSError, ConnectionError):
                pass
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_wfs_proyectos():
            add(item)
        for item in self._collect_gallery_proyectos():
        ...
        return {
            ...
            "wordpress": sum(1 for r in rows if r.get("origen") == "wordpress_galeria"),
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
        }
