...
WEB_BASE = "https://www.aytohospitaldeorbigo.es"
WEB_JOOMLA = "https://www.hospitaldeorbigo.com"
SEDE_BASE = "https://hospitaldeorbigo.sedelectronica.es"
PLAI_BASE = "https://servicios.jcyl.es/PlanPublica"
MUNICIPIO = "Hospital de Órbigo"
ID_PREFIX = "hospital-de-orbigo"
...
DEFAULT_TRAMITE_PAGES: list[str] = [
    f"{SEDE_BASE}/citizen-service/d6fcc5b2-3497-419a-9add-f8e5a5e1ac1c",
    f"{SEDE_BASE}/catalog/t/urbanismo",
    f"{WEB_BASE}/ayuntamiento/normativa-municipal/urbanismo/",
]

DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        f"{WEB_BASE}/ayuntamiento/normativa-municipal/urbanismo/",
        "Normativa urbanística municipal (OpenCms)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia={PLAI_PROVINCIA}&municipio={PLAI_MUNICIPIO}",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia={PLAI_PROVINCIA}&municipio={PLAI_MUNICIPIO}",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        f"{WEB_JOOMLA}/index.php/ayuntamiento/urbanismo/plan-urbanistico",
        "Plan Urbanístico 2015 (Joomla)",
    ),
    (
        f"{WEB_JOOMLA}/index.php/ayuntamiento/urbanismo/anuncios",
        "Anuncios urbanismo (Joomla)",
    ),
]

DEFAULT_JOOMLA_PAGES: list[str] = [
    f"{WEB_JOOMLA}/index.php/ayuntamiento/urbanismo/plan-urbanistico",
    f"{WEB_JOOMLA}/index.php/ayuntamiento/urbanismo/anuncios",
]

RE_PREVIEW = re.compile(
    r'href="((?:https://hospitaldeorbigo\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://hospitaldeorbigo\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_JOOMLA_PDF = re.compile(
    r'href="((?:https://www\.hospitaldeorbigo\.com)?/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "normas urban" in n or blob.strip() == "num":
...
class HospitalDeOrbigoAyuntamientoAdapter(AyuntamientoAdapter):
    """OpenCms + Joomla web + sede espublico + PLAI JCYL + IDECyL WFS (c_mun 24107)."""

    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        super().__init__(slug, config, base_url or WEB_BASE)
        self.delay_s = float(self.config.get("request_delay_s", 0.35))
        self.web_base = str(self.config.get("web_base") or WEB_BASE).rstrip("/")
        self.joomla_base = str(self.config.get("joomla_base") or WEB_JOOMLA).rstrip("/")
        ...
        self.joomla_pages = list(self.config.get("joomla_pages") or DEFAULT_JOOMLA_PAGES)
    ...
    def _abs_joomla(self, href: str) -> str:
        if href.startswith("http"):
            return href
        return urllib.parse.urljoin(f"{self.joomla_base}/", href.lstrip("/"))
    ...
    def _collect_joomla_seeds(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for page_url in self.joomla_pages:
            try:
    ...
    def _joomla_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        rec: dict[str, Any] = {
            ...
            "url": row.get("url") or row.get("pdf_url") or self.joomla_base,
            "source": "ayuntamiento",
    ...
    def _plai_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        rec: dict[str, Any] = {
            ...
            "titulo": row["titulo"],
            "fecha": row.get("fecha"),
            "tipo": _proyecto_tipo(f"{row['titulo']} {row.get('instrumento', '')}"),
            "url": row["url"],
            "source": "ayuntamiento",
            "origen": row.get("origen"),
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-hospital-de-orbigo/1.0")},
        )
            ...
            rows.append(
                {
                    ...
                    "url": self._abs_sede(link_m.group(1)) if link_m else self.board_url,
                    "pdf_url": self._abs_sede(link_m.group(1)) if link_m else None,
                    "origen": origen,
                }
            )
        return rows
    ...
    def _collect_tramites_catalog(self) -> list[dict[str, Any]]:
        ...
        for m in RE_CATALOG.finditer(html):
            url, titulo = self._abs_sede(m.group(1)), unescape(m.group(2).strip())
            if url in seen:
                continue
            seen.add(url)
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_plai():
            ...
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_joomla_seeds():
            rec = self._joomla_to_proyecto(item)
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
