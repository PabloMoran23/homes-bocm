...
DRUPAL_BASE = "https://www.huermeces.es"
SEDE_BASE = "https://huermeces.sedelectronica.es"
...
DEFAULT_INFO_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=9&municipio=140",
        "archivo planeamiento JCyL (Huérmeces)",
    ),
    (
        "https://idecyl.jcyl.es/siur/index.html?id=09140",
        "visor SiUR planeamiento (JCyL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="((?:https://huermeces\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://huermeces\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"ordenaci[oó]n detallada|actuaci[oó]n urban|obras?|conjunto hist|limpieza de solares|"
    r"bando.*obra)",
)
RE_NOISE = re.compile(
...
class HuermecesAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal Toools + sede espublico + PLAU/PLAI JCyL + IDECyL WFS (geometría partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=9&municipio=140"
        )
    ...
    def _warm_sede(self) -> None:
        try:
            req = urllib.request.Request(
                self.board_url,
                headers={"User-Agent": self.config.get("user_agent", "poc-bocm-huermeces/1.0")},
            )
    ...
    def _collect_plau(self) -> list[dict[str, Any]]:
        ...
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for item in self._parse_plai_rows(html):
