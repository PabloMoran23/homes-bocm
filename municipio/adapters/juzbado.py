...
WP_BASE = "https://juzbado.es/nuevaweb"
SEDE_BASE = "https://juzbado.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/agenda-urbana-de-juzbado/",
    f"{WP_BASE}/ordenanzas/",
    "https://juzbado.blogspot.com/p/urbanismo.html",
    f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=167",
    f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=167",
]
...
RE_PREVIEW = re.compile(
    r'href="(https://juzbado\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://juzbado\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_DRIVE_HREF = re.compile(
    r'href="(https://drive\.google\.com/file/d/[^"]+)"',
    re.I,
)
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://juzbado\.es)?/nuevaweb/wp-content/uploads/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
class JuzbadoAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Kubio + sede espublico + PlanPublica PLAU + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=167"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=167"
        )
        ...
        self._use_opener_for_all = bool(self.config.get("insecure_ssl", True))
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-juzbado/1.0")},
        )
        opener = self._opener if (sede or self._use_opener_for_all) else urllib.request.build_opener()
    ...
    def _collect_seed_pages(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.seed_pages:
            try:
                ...
                continue
            page_title = _strip_html(re.search(r"<title>([^<]+)</title>", html, re.I).group(1)) if re.search(r"<title>", html, re.I) else page_url
            for m in RE_DRIVE_HREF.finditer(html):
                url = m.group(1)
                if url in seen:
                    continue
                seen.add(url)
                ctx = html[max(0, m.start() - 400) : m.start()]
                ctx_text = _strip_html(ctx)
                titulo = ctx_text[-180:].strip() if ctx_text else page_title
                if not titulo or len(titulo) < 8:
                    titulo = f"Modelo urbanismo ({page_title})"
                rows.append(
                    {
                        "titulo": titulo[:500],
                        "url": url,
                        "origen": "blogger_formularios",
                        "fecha": None,
                    }
                )
            for m in RE_PDF_HREF.finditer(html):
                href = m.group(1)
                url = href if href.startswith("http") else f"https://juzbado.es{href}"
                if url in seen:
                    continue
                blob = url.lower()
            ...
            if "agenda-urbana" in page_url.lower() or "urbanismo" in page_url.lower():
                key = f"seed:{page_url}"
                if key not in seen:
                    seen.add(key)
                    rows.append(
                        {
                            "titulo": page_title[:500] or "Agenda Urbana de Juzbado",
                            "url": page_url,
                            "origen": "wp_agenda_urbana",
                            "fecha": None,
                        }
                    )
        return rows
    ...
    def _tramite_to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        if row.get("origen") == "blogger_formularios":
            pass
        elif not RE_LICENCIA.search(row["titulo"]):
            return None
        return {
