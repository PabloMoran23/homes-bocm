...
WP_BASE = "https://laalberca.com"
SEDE_BASE = "https://laalberca.sedelectronica.es"
MUNICIPIO = "La Alberca"
ID_PREFIX = "la-alberca"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=010",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=010",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        f"{WP_BASE}/servicios-municipales/",
        "Servicios municipales — trámites y contacto",
    ),
    (
        f"{WP_BASE}/descargas/",
        "Descargas — documentación municipal (callejero, PMBD)",
    ),
]
...
DEFAULT_LICENCIA_PAGES: list[tuple[str, str]] = [
    ...
    (f"{WP_BASE}/servicios-municipales/", "servicios municipales — licencias urbanísticas"),
]

RE_PREVIEW = re.compile(
    r'href="(https://laalberca\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"conjunto hist[oó]rico|dehesa|eras|ordenanza.*urban|tasa.*urban|"
    r"normas urban|instrumento|memoria|planos|bocyl|pmbd|protecci[oó]n)",
)
...
def _proyecto_tipo(blob: str) -> str:
    n = blob.lower()
    if "conjunto hist" in n or "pmbd" in n:
        return "protección conjunto histórico"
...
class LaAlbercaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress turismo + sede espublico tablón + IDECyL WFS PLAU CyL (Sierra de Francia)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-la-alberca/1.0")},
        )
