...
DRUPAL_BASE = "https://www.lacisterniga.es"
SEDE_BASE = "https://cisterniga.sedelectronica.es"
...
ID_PREFIX = "la-cisterniga"
PLAI_MUNICIPIO = 114
PLAI_PROVINCIA = 47

DOCUMENTACION_URBANISMO_URL = (
    f"{DRUPAL_BASE}/documentacion?field_category_target_id=75"
)
...
DEFAULT_INFO_PAGES: list[tuple[str, str]] = [
    ...
    (f"{DRUPAL_BASE}/urbanismo", "urbanismo municipal"),
    (DOCUMENTACION_URBANISMO_URL, "documentos descargables — urbanismo"),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=47&municipio=114",
        "planeamiento en información pública (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(?:https://cisterniga\.sedelectronica\.es)?/preview-document/([a-f0-9-]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://cisterniga\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra (?:mayor|menor)|"
    r"primera ocupaci[oó]n|ocupaci[oó]n (?:de |en )?v[ií]a|recepci[oó]n de obras|"
    r"instalaci[oó]n|cementerio)",
)
RE_PROYECTO = re.compile(
    ...
    r"desafectaci[oó]n|bocyl|bopva|ruina|peri\b|normalizaci[oó]n de fincas|"
    r"ordenaci[oó]n detallada|clasificaci[oó]n del suelo)",
)
RE_NOISE = re.compile(
    ...
    r"retirada de veh[ií]culos|emplazamiento|actuaciones judiciales)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_YMD = re.compile(r"\b((?:19|20)\d{2})(\d{2})(\d{2})\b")
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"SECTOR\s*(?:N[ºo°\.\s]*)?\d+[A-Z0-9]*|"
    r"S\.?\s*U\.?\s*D\.?|"
    r"PP\s*S\s*\d+|"
    r"SUNC[-\s]?\d+"
    r")\b",
...
def _fecha_from_filename(path: str) -> str | None:
    m = re.search(r"(20\d{6})", path or "")
    if m:
        return _fecha_from_blob(m.group(1))
    return None
...
def _proyecto_tipo(title: str, instrumento: str = "") -> str:
    ...
    if "plan parcial" in blob:
        return "plan parcial"
    if "proyecto de urbaniz" in blob or "proyecto de actuaci" in blob:
...
def _sector_codes_from_text(text: str) -> list[str]:
    ...
    for m in RE_SECTOR_CODE.finditer(text or ""):
        ...
        if code and code not in seen:
            seen.add(code)
            out.append(code)
    for m in re.finditer(r"(?i)sector\s+(\d+)", text or ""):
        code = f"SECTOR {m.group(1)}"
        if code not in seen:
            seen.add(code)
            out.append(code)
    return out
...
class LaCisternigaAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal mipueblo + sede espublico + PLAI JCyL + IDECyL WFS (geometría partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.documentacion_url = str(
            self.config.get("documentacion_url") or DOCUMENTACION_URBANISMO_URL
        )
        ...
        self.plai_max_pages = int(self.config.get("plai_max_pages", 6))
        ...
        self.wfs_municipio = str(self.config.get("wfs_municipio") or "Cistérniga")
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-la-cisterniga/1.0")},
        )
    ...
    def _collect_drupal_documentacion(self) -> list[dict[str, Any]]:
        try:
        ...
        seen: set[str] = set()
        title_blocks = list(re.finditer(r"views-field-title[^>]*>(.*?)</div>", html, re.S | re.I))
        for idx, title_m in enumerate(title_blocks):
            titulo = _strip_html(title_m.group(1))
            if not titulo or len(titulo) < 8:
                continue
            chunk = html[title_m.start() : min(len(html), title_m.end() + 2500)]
            pdfs = re.findall(r'href="(/sites/default/files/documents/[^"]+)"', chunk)
            url = (
                f"{self.drupal_base}{pdfs[0]}"
                if pdfs
                else f"{self.documentacion_url}#{hashlib.sha256(titulo.encode()).hexdigest()[:10]}"
            )
            if url in seen:
                continue
            seen.add(url)
            fecha = _fecha_from_filename(pdfs[0] if pdfs else titulo)
            rows.append(
                {
                    "titulo": titulo[:500],
                    "fecha": fecha,
                    "url": url,
                    "pdf_url": f"{self.drupal_base}{pdfs[0]}" if pdfs else None,
                    "origen": "drupal_documentacion",
                }
            )
        return rows
    ...
    def _parse_board_table(self, html: str, origen: str) -> list[dict[str, Any]]:
        ...
        for tr in re.findall(r"<tr>(.*?)</tr>", tbody_m.group(1), re.S):
            ...
            pdf_url = None
            if link_m:
                pdf_url = f"{self.sede_base}/preview-document/{link_m.group(1)}"
            rows.append(
                {
                    "titulo": titulo[:500],
                    "doc_label": doc[:500],
    ...
    def _parse_board_links(self, html: str, origen: str) -> list[dict[str, Any]]:
        ...
        for m in RE_PREVIEW.finditer(html):
            url = f"{self.sede_base}/preview-document/{m.group(1)}"
            local = html[max(0, m.start() - 400) : m.end() + 200]
            title_m = re.search(r'title="([^"]*)"', local, re.I)
            text_m = re.search(rf'href="[^"]*preview-document/{re.escape(m.group(1))}"[^>]*>([^<]+)<', local, re.I)
            titulo = ""
            if title_m:
    ...
    def _attach_geometry(self, rec: dict[str, Any]) -> None:
        ...
        for code in _sector_codes_from_text(blob):
            ...
            if not geom:
                # try numeric sector only
                m = re.search(r"(\d+)", code)
                if m:
                    geom, source_url = self._wfs_sector_geometry(f"SECTOR {m.group(1)}")
            if not geom:
                continue
            rec["geom_geojson"] = geom
            rec["geometry_source"] = "portal_wfs"
        ...
        for wfs_rec in self._collect_wfs():
            ...
            sector_num = re.search(r"sector\s*(\d+)", titulo)
            wfs_sector = re.search(r"sector\s*(\d+)", wfs_title)
            if sector_num and wfs_sector and sector_num.group(1) == wfs_sector.group(1):
                for key in (
                    "geom_geojson",
                    "geometry_source",
                    "geometry_source_url",
    ...
    def _drupal_to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        return {
            ...
            "url": row["url"],
            "source": "ayuntamiento",
            "nota": "Formulario o guía de trámite (Drupal documentación)",
            "origen": row.get("origen"),
            **({"pdf_url": row["pdf_url"]} if row.get("pdf_url") else {}),
        }
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        for item in self._collect_board():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_drupal_documentacion():
            rec = self._drupal_to_licencia(item)
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "drupal": sum(1 for r in rows if r.get("origen") == "drupal_documentacion"),
            "tramites": sum(
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "drupal_documentacion": sum(1 for r in rows if r.get("origen") == "drupal_documentacion"),
