...
SEDE_BASE = "https://lafregeneda.sedelectronica.es"
MUNICIPIO = "La Fregeneda"
ID_PREFIX = "la-fregeneda"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=132",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=132",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        "https://idecyl.jcyl.es/siur/index.html?id=37132",
        "Visor SIUR — instrumentos urbanísticos La Fregeneda (IDECyL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://lafregeneda\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://lafregeneda\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class LaFregenedaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress lafregeneda.com + sede espublico + PlanPublica JCyL + IDECyL WFS."""
    ...
    def _bootstrap_session(self) -> None:
        ...
        req = urllib.request.Request(
            self.board_url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-la-fregeneda/1.0")},
        )
        try:
