...
WP_BASE = "https://www.laguancha.es"
SEDE_BASE = "https://laguancha.sedelectronica.es"
MUNICIPIO = "La Guancha"
ID_PREFIX = "la-guancha"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-la-guancha"

BOARD_TABLON = f"{SEDE_BASE}/board/975040b6-f59b-11de-b600-00237da12c6a/"
BOARD_BANDOS = f"{SEDE_BASE}/board/974e6d5e-f59b-11de-b600-00237da12c6a/"
TRANSPARENCY_ORDENANZAS = f"{SEDE_BASE}/transparency/aec0d65b-fe1c-41d4-b280-21d7bc835c3a/"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/ayuntamiento-2/departamentos/d-urbanismo/",
    f"{WP_BASE}/category/urbanismo/",
    f"{BOARD_TABLON}",
    f"{BOARD_BANDOS}",
    TRANSPARENCY_ORDENANZAS,
    "https://www.idecanarias.es/resources/PLA_ENP_URB/URB_PLA/TF/Guan/PGO/indice.html",
    "https://www.idecanarias.es/resources/PLA_ENP_URB/URB_PLA/TF/Guan/ed_vial/indice.html",
    "https://www.idecanarias.es/resources/PLA_ENP_URB/URB_PLA/TF/Guan/ome-lgu/indice.html",
]
...
RE_PROYECTO = re.compile(
    ...
    r"estudio (?:ac[uú]stico|ambiental|de detalle)|memoria|planos|planimetr|bop|boc|edicto|"
    r"aprobaci[oó]n (?:inicial|definitiva|provisional)|parcela|suelo|sector|"
    r"cambio de uso|ordenanza|rectificaci[oó]n|exposici[oó]n p[uú]blica|"
    r"calificaci[oó]n|legalidad urban|normativa urban|adaptaci[oó]n)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"cobranza iae|padrones|modificaci[oó]n de cr[eé]dito|bolsa de empleo|"
    r"bando.*fiesta|trail|quema de rastrojos|tribunal|jurado)",
)
RE_WP_LINK = re.compile(r'<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', re.I | re.S)
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?laguancha\.es)?/[^"]+\.pdf[^"]*)"',
    re.I,
)
RE_BOARD_ROW = re.compile(r'<tr[^>]*>\s*<td class="class_name".*?</tr>', re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://laguancha\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*(?:Guancha|Ayuntamiento).*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
class LaGuanchaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Bones + sede espublico gestiona + SITCAN CKAN + GEOBDP Grafcan (Canarias)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.board_urls = [
            str(u)
            for u in (
                self.config.get("board_urls")
                or [BOARD_TABLON, BOARD_BANDOS]
            )
        ]
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-la-guancha/1.0")},
        )
    ...
    def _fetch_geobdp_geometry(self, doc_id: str) -> dict[str, Any] | None:
        ...
        for scheme in ("https", "http"):
            url = f"{scheme}://geobdp.grafcan.es/core/documentos/{doc_id}.html"
            try:
    ...
    def _collect_board(self, board_url: str) -> list[dict[str, Any]]:
        try:
    ...
    def _collect_all_boards(self) -> list[dict[str, Any]]:
        ...
        for board_url in self.board_urls:
            rows.extend(self._collect_board(board_url))
        return rows
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            {
                "id": _stable_id("lic", BOARD_TABLON),
    ...
    def _board_is_urban(self, row: dict[str, Any]) -> bool:
        ...
        if any(
            k in proc
            for k in ("planeamiento", "licencia", "urban", "actividad", "obra", "legalidad", "desarrollo")
        ):
            return True
        if "urbanismo" in cat or "bop" in cat:
            return True
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if RE_LICENCIA.search(blob) and not RE_PROYECTO.search(blob):
            return None
        proc = (row.get("procedimiento") or "").lower()
        if not RE_PROYECTO.search(blob) and "planeamiento" not in proc and "legalidad" not in proc:
            return None
        key = row.get("expediente") or row["url"]
        rec: dict[str, Any] = {
    ...
    def _to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        if row.get("origen") in {"sede_tablon", "sede_tramite"}:
            return row
        blob = row.get("blob") or ""
        if not RE_LICENCIA.search(blob):
            return None
