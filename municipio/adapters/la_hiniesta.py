...
SEDE_BASE = "https://lahiniesta.sedelectronica.es"
MUNICIPIO = "La Hiniesta"
ID_PREFIX = "la-hiniesta"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=49&municipio=095",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=49&municipio=095",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/openDocuIndice.do?cDocId=293548",
...
RE_PREVIEW = re.compile(
    r'href="(https://lahiniesta\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"ordenanza.*urban|tasa.*urban|normas urban|instrumento|memoria|planos|bocyl)",
)
RE_EXCLUDE = re.compile(
    ...
    r"electores|icio\b|ordenanza fiscal|agua potable|corte de agua)",
)
...
class LaHiniestaAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico tablón + PlanPublica + IDECyL WFS (Zamora)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-la-hiniesta/1.0")},
        )
