...
WP_BASE = "https://www.lahorra.es"
SEDE_BASE = "https://lahorra.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/pagina-basica/informacion-general",
    f"{SEDE_BASE}/board",
    f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=168",
    "https://idecyl.jcyl.es/siur/index.html?id=09168",
]

RE_PREVIEW = re.compile(
    r'href="(https://lahorra\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"edicto|actuaci[oó]n urban|normas urban|instrumento|evaluaci[oó]n de impacto|edar)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(pleno|convocatoria|empleo|subvenci[oó]n deportiv|cert\.|ordenanza fiscal|"
    r"perfil contratante|sede electronica|padr[oó]n fiscal|proteccion perimetral|pista de p[aá]del|"
    r"carga de agua|tasa por)",
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "pgou" in n or "plan general" in n:
        return "PGOU"
    if "normas urban" in n or "normas subsidiarias" in n:
        return "normas urbanísticas"
    if "sector" in n or "u.a." in n:
        return "sector"
...
class LaHorraAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal 10 + sede espublico + PlanPublica JCyL + IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-la-horra/1.0")},
        )
    ...
    def _parse_plau_rows(self, html: str) -> list[dict[str, Any]]:
        ...
        for tr in re.findall(r"<tr[^>]*>(.*?)</tr>", html, re.S | re.I):
            ...
            boletin_m = re.search(r"doGoBoletin\('(\d+)',\s*'([^']+)'\)", tr)
            if not doc_m and not boletin_m:
                continue
            cells = [
                _strip_html(c)
            ...
            title_span = re.search(
                r'<span[^>]*font-weight:bold[^>]*font-size:1\.2em[^>]*>([^<]+)</span>',
                tr,
                re.I,
            )
            titulo = (
                unescape(title_span.group(1)).strip()
                if title_span
                else (cells[4] if len(cells) > 4 else (cells[-1] if cells else ""))
            )
            if not titulo:
                continue
            if doc_m:
                ...
                doc_id, codigo = boletin_m.group(1), boletin_m.group(2)
                url = f"{PLAU_BASE}/openBoletin.do?cDocId={doc_id}"
    ...
    def _parse_board_table(self, html: str, origen: str) -> list[dict[str, Any]]:
        ...
        for tr in re.findall(r"<tr>(.*?)</tr>", tbody_m.group(1), re.S):
            ...
            if not url.startswith("http"):
                url = f"{self.sede_base}/{url.lstrip('/')}"
            rows.append(
                {
                    "titulo": titulo[:500],
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            (
                f"{self.wp_base}/pagina-basica/informacion-general",
                "información general — planeamiento urbanístico",
            ),
            ...
            (
                ...
                f"bInfoPublica=N&provincia=09&municipio=168",
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if row.get("categoria", "").lower() in ("urbanismo", "ordenanzas y reglamentos"):
            pass
        elif row.get("procedimiento", "").lower() == "actuaciones urbanísticas":
            pass
    ...
    def _wp_urbanismo_proyecto(self) -> dict[str, Any] | None:
        url = f"{self.wp_base}/pagina-basica/informacion-general"
        return {
            "id": _stable_id("proy", url),
            "municipio": MUNICIPIO,
            "titulo": "Archivo de Planeamiento Urbanístico y Ordenación del territorio vigente",
            "fecha": None,
            "tipo": "planeamiento",
            "url": url,
            "source": "ayuntamiento",
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        add(self._wp_urbanismo_proyecto())
        for item in self._collect_board():
