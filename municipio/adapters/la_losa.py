...
DIPSEGOVIA_BASE = "https://www.dipsegovia.es/web/ayuntamiento-de-la-losa"
WP_BASE = "https://www.lalosa.es"
SEDE_BASE = "https://lalosa.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (f"{DIPSEGOVIA_BASE}/normativa-municipal", "Normativa municipal — Diputación Segovia"),
    (f"{WP_BASE}/", "Web municipal WordPress"),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=113",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=113",
...
RE_PREVIEW = re.compile(
    r'href="(https://lalosa\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://lalosa\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class LaLosaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress + Liferay DipSegovia + sede espublico + PLAI JCYL + geometría parcial IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-la-losa/1.0")},
        )
