...
WP_BASE = "https://www.matanceros.es"
SEDE_BASE = "https://matanceros.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board/"
MUNICIPIO = "La Matanza de Acentejo"
ID_PREFIX = "lma"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-la-matanza-de-acentejo"
GEOBDP_MUNICIPIO = "https://geobdp.grafcan.es/core/municipios/38025/"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/tablon-de-anuncios/",
    f"{WP_BASE}/transparencia/",
    f"{WP_BASE}/areas-municipales/oficina-tecnica/",
    f"{WP_BASE}/vivienda/",
    f"{WP_BASE}/bandos-municipales/",
    BOARD_URL,
]
...
RE_PROYECTO = re.compile(
    ...
    r"texto refundido|suspensi[oó]n de determinaciones|ordenaci[oó]n|retranqueo)",
)
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?matanceros\.es)?/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Matanza.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
class LaMatanzaDeAcentejoAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress matanceros.es + sede espublico + SITCAN CKAN + GEOBDP Grafcan (Canarias)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-la-matanza-de-acentejo/1.0")},
        )
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "id": _stable_id("lic", f"{self.wp_base}/tablon-de-anuncios/"),
                "fecha_concesion": None,
                "tipo": "trámite informativo",
                "distrito": None,
                "lat": None,
                ...
                "url": f"{self.wp_base}/tablon-de-anuncios/",
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
            {
                "id": _stable_id("lic", f"{self.wp_base}/areas-municipales/oficina-tecnica/"),
                "fecha_concesion": None,
                "tipo": "trámite informativo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Oficina técnica — urbanismo y licencias",
                "url": f"{self.wp_base}/areas-municipales/oficina-tecnica/",
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
        ]
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            "rows": len(rows),
            "status": "ok",
            "source": "wordpress_sede_sitcan_geobdp",
