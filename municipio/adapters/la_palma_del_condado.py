...
SEDE_BASE = "https://lapalmadelcondado.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board/"
MUNICIPIO = "La Palma del Condado"
ID_PREFIX = "la-palma-del-condado"
...
RE_PROYECTO = re.compile(
    ...
    r"cambio de uso|ordenanza|urbanizaci[oó]n|calidad ambiental)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"cobranza iae|padrones|polic[ií]a local|subvenci[oó]n|natalidad|barrendero|"
    r"contrataci[oó]n laboral|orden d[ií]a pleno|baremaci[oó]n)",
)
RE_BOARD_ROW = re.compile(r'<tr[^>]*>\s*<td class="class_name".*?</tr>', re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://lapalmadelcondado\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
class LaPalmaDelCondadoAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona (tablón). Web corporativa inaccesible (SSL)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-la-palma-del-condado/1.0")},
        )
    ...
    def _board_is_urban(self, row: dict[str, Any]) -> bool:
        ...
        if any(k in proc for k in ("planeamiento", "licencia", "urban", "actividad", "obra", "genérico", "ambiental")):
            return True
        if "urbanismo" in cat:
            return True
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not RE_PROYECTO.search(blob) and not any(
            k in proc for k in ("planeamiento", "urban", "ambiental", "genérico")
        ):
            return None
        key = row.get("expediente") or row["url"]
        return {
