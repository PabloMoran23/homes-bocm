...
WEB_BASE = "https://www.lapobladefarnals.es"
SEDE_BASE = "https://lapobladefarnals.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
URBANISMO_URL = f"{WEB_BASE}/es/pagina/urbanismo"
TRANSPARENCIA_URL = f"{SEDE_BASE}/transparency/75c01293-0f52-4348-958d-85c0dda000f5/"
MUNICIPIO = "La Pobla de Farnals"
ID_PREFIX = "la-pobla-de-farnals"
COD_INE_MUN = "46139"
...
RE_LICENCIA = re.compile(
    ...
    r"obra (?:mayor|menor)|concesi[oó]n.*costas|ocupaci[oó]n)",
)
RE_PROYECTO = re.compile(
    ...
    r"cambio de uso|normas subsidiarias|homologaci[oó]n|ordenanza|e[oó]lic|iate|dripalia|nnss)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"subvenci[oó]n|empleo p[uú]blico|ordenanza general de subvenciones|"
    r"ayudas comedor|ayudas deportistas|ayudas extraescolares|bando.*subven)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://lapobladefarnals\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_PDF_HREF = re.compile(
    r'href="((?:https://www\.lapobladefarnals\.es)?/sites/www\.lapobladefarnals\.es/files/[^"]+\.(?:pdf|PDF))"',
    re.I,
)
...
def _abs_web_url(path: str) -> str:
    if path.startswith("http"):
        return path
    return f"{WEB_BASE}{path if path.startswith('/') else '/' + path}"
...
class LaPoblaDeFarnalsAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-la-pobla-de-farnals/1.0")},
        )
    ...
    def _collect_board(self) -> list[dict[str, Any]]:
        ...
        for m in RE_BOARD_ROW.finditer(html):
            ...
            categoria = cells.get("class_folderCategory", "")
    ...
    def _collect_drupal_urbanismo(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(self.urbanismo_url, timeout=90)
        except urllib.error.URLError:
            return []
        ...
        for m in RE_PDF_HREF.finditer(html):
            pdf_url = _abs_web_url(unescape(m.group(1)))
            if pdf_url in seen:
                continue
            seen.add(pdf_url)
            name = unescape(pdf_url.split("/")[-1]).replace("%20", " ")
            titulo = name
            if "dripalia" in name.lower():
                titulo = "Información pública DRIPALIA (edicto 18-16)"
            rows.append(
                {
                    "titulo": titulo[:500],
                    "url": pdf_url,
                    "fecha": "2016-11-11" if "dripalia" in name.lower() else None,
        ...
        if "DRIPALIA" in text.upper() and not any("dripalia" in r["titulo"].lower() for r in rows):
            rows.append(
                {
                    "titulo": "Información pública DRIPALIA (11.11.2016)",
    ...
    def _drupal_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        rec: dict[str, Any] = {
            ...
            "url": row["url"],
            "source": "ayuntamiento",
            "origen": row.get("origen", "drupal_urbanismo"),
        }
        ...
        return [
            {
                ...
                "municipio": MUNICIPIO,
                "titulo": "Urbanismo — PGOU, NNSS y ordenanzas",
                "fecha": None,
                "tipo": "urbanismo",
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_icv_proyectos():
            add(rec)
        for item in self._collect_drupal_urbanismo():
        ...
        return {
            ...
            "drupal": sum(1 for r in rows if r.get("origen") == "drupal_urbanismo"),
        }
