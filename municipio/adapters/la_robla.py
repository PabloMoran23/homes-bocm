...
WEB_BASE = "https://www.aytolarobla.es"
SEDE_BASE = "https://aytolarobla.sedelectronica.es"
MUNICIPIO = "La Robla"
ID_PREFIX = "la-robla"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    ...
    (
        f"{WEB_BASE}/ayuntamiento/normativa-municipal/urbanismo/DOCUMENTACION_MODIFICACION_PUNTUAL_Nx6_xEL_CRISPINx.html",
        "Modificación puntual nº6 El Crispín — documentación",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=142",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=142",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://aytolarobla\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://aytolarobla\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class LaRoblaAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-la-robla/1.0")},
        )
