...
WEB_BASE = "https://www.latorre.es"
SEDE_BASE = "https://latorre.sedelectronica.es"
...
PLAU_URL = (
    ...
    f"?bInfoPublica=N&provincia=05&municipio=247"
)
...
DEFAULT_SEED_PAGES: list[str] = [
    ...
    PLAU_URL,
    "https://idecyl.jcyl.es/siur/index.html?id=05247",
]

WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/ows"
WFS_MUNICIPIO = "La Torre"
...
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?latorre\.es)?/docus/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
class LaTorreAyuntamientoAdapter(AyuntamientoAdapter):
    """Plantilla DipuÁvila + sede espublico + PlanPublica JCyL + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-la-torre/1.0")},
        )
    ...
    def _parse_plau_rows(self, html: str) -> list[dict[str, Any]]:
        ...
        for tr in re.findall(r"<tr[^>]*>(.*?)</tr>", html, re.S | re.I):
            ...
            titulo = cells[2] if len(cells) > 2 else (cells[-1] if cells else "")
            if not titulo:
                continue
    ...
    def _plau_page_proyecto(self) -> dict[str, Any]:
        url = f"{self.web_base}/ayuntamiento/normas-urbanisticas/plau-junta-de-castilla-y-leon.html"
        rec: dict[str, Any] = {
            "id": _stable_id("proy", url),
            "municipio": MUNICIPIO,
            "titulo": "Archivo PLAU CyL — planeamiento vigente La Torre",
            "fecha": "2020-04-11",
            "tipo": "planeamiento",
            "url": url,
            "source": "ayuntamiento",
            "origen": "web_plau_page",
        }
        self._enrich_geometry(rec)
        return rec
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            ...
            (self.plau_url, "archivo planeamiento urbanístico (Junta CYL)"),
        ]
        rows: list[dict[str, Any]] = []
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "web_plau_page": sum(1 for r in rows if r.get("origen") == "web_plau_page"),
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
