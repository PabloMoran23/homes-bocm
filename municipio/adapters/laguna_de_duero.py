...
SEDE_BASE = "https://lagunadeduero.sedelectronica.es"
WP_BASE = "https://www.lagunadeduero.org"
...
RE_PREVIEW = re.compile(
    r'href="(https://lagunadeduero\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://lagunadeduero\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
def _sector_tokens(text: str) -> list[str]:
    ...
    low = (text or "").lower()
    for pat in (
        r"sector\s*2c?\s*[\"«]?\s*el molino\s*3?[\"»]?",
        r"sector\s*2\s*[\"«]?\s*el molino[\"»]?",
        r"el molino\s*3?",
        r"la guarnicionera",
        r"el canal",
    ):
...
class LagunaDeDueroAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + PLAI JCYL + geometría parcial SIUCyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-laguna-de-duero/1.0")},
        )
