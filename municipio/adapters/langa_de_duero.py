...
DRUPAL_BASE = "https://www.langadeduero.es"
SEDE_BASE = "https://langadeduero.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{DRUPAL_BASE}/actualidad/ya-esta-disponible-la-nueva-normativa-urbanistica-de-langa-de-duero",
    f"{DRUPAL_BASE}/actualidad/concentracion-parcelaria-de-langa-de-duero",
    f"{DRUPAL_BASE}/actualidad/concentracion-parcelaria-de-langa-de-duero-ii-regadio",
    f"{DRUPAL_BASE}/ordenanzas",
    f"{DRUPAL_BASE}/anuncios-particulares",
]

RE_PREVIEW = re.compile(
    r'href="(https://langadeduero\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://langadeduero\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_DRUPAL_PDF = re.compile(
    r'href="((?:https://www\.langadeduero\.es)?/sites/langadeduero\.es/files/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"concentraci[oó]n parcelaria|regad[ií]o|gatopardo|vega de alcozar)",
)
RE_NOISE = re.compile(
...
def _sector_tokens(text: str) -> list[str]:
    ...
    low = (text or "").lower()
    for pat in (
        r"vega de alcozar\s*(?:i{1,3}|ii|iii|1|2|3)?",
        r"gatopardo",
        r"[áa]rea ferroviaria",
        r"concentraci[oó]n parcelaria",
    ):
...
class LangaDeDueroAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal 7 langadeduero.es + sede espublico + PLAI JCYL + IDECyL WFS (partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-langa-de-duero/1.0")},
        )
