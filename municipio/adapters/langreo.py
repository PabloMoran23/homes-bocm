...
BASE = "https://tramites.ayto-langreo.es"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{BASE}/sede/catalogoTramites.do?ent_id=2&idioma=1",
    f"{BASE}/sede/tramites.do?ent_id=2&idioma=1",
]

DEFAULT_LICENCIA_PAGES: list[str] = [
    f"{BASE}/sede/catalogoTramites.do?ent_id=2&idioma=1",
    f"{BASE}/sede/fichaInformativa.do?asu_cod=287&asu_mod_cod=52",
    "https://sede.langreo.as/tramites/catalogo-general-de-tramites/tramite-urbanismo",
]
...
RE_LICENCIA = re.compile(
    ...
    r"cierre de fincas|certificaci[oó]n urban)",
)
RE_PROYECTO = re.compile(
    ...
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|sunc|reforma interior)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://(?:tramites\.)?ayto-langreo\.es)?/[^"]+\.(?:pdf|docx?)[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Langreo.*$", "", t, flags=re.I).strip()
...
class LangreoAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede tramites.ayto-langreo.es + RPGUR Asturias; geometría parcial WFS visorrpgur."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "31")
        self.wfs_url = str(self.config.get("wfs_url") or WFS_BASE)
        self.wfs_layer = str(self.config.get("wfs_layer") or WFS_LAYER)
        self.wfs_filter = str(self.config.get("wfs_filter") or "Instrumento LIKE '%LANGREO%'")
    ...
    def _fetch(self, url: str, *, data: bytes | None = None, encoding: str = "utf-8") -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-langreo/1.0"),
        }
        if data is not None:
    ...
    def _crawl_sede_pages(self) -> list[dict[str, Any]]:
        visited: set[str] = set()
                ...
                rows.append(
                    {
                        ...
                        "pdf_url": link,
                        "source": "ayuntamiento",
                        "origen": "sede_document",
                    }
                )

            for m in re.finditer(r'href="(https://tramites\.ayto-langreo\.es/[^"#?]+)"', html, re.I):
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        info_pages = [
            (f"{BASE}/sede/catalogoTramites.do?ent_id=2&idioma=1", "Catálogo de trámites — urbanismo y licencias"),
            (
                f"{BASE}/sede/fichaInformativa.do?asu_cod=287&asu_mod_cod=52",
                "Cierre de fincas / licencia de obra menor",
            ),
            (
                "https://sede.langreo.as/tramites/catalogo-general-de-tramites/tramite-urbanismo",
                "Trámite urbanismo — sede.langreo.as",
            ),
        ]
            ...
            rows.append(
                {
                    ...
                    "url": page_url,
                    "source": "ayuntamiento",
                    "nota": "Sin listado público de concesiones; página informativa de trámites",
                }
            )
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_rpgur_proyectos():
            add(rec)
        for rec in self._crawl_sede_pages():
            add(rec)
        ...
        return {
            ...
            "sede": sum(1 for r in rows if str(r.get("origen", "")).startswith("sede")),
