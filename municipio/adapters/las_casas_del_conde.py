...
SEDE_BASE = "https://lascasasdelconde.sedelectronica.es"
MUNICIPIO = "Las Casas del Conde"
ID_PREFIX = "las-casas-del-conde"
PLAU_URL = (
    "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do"
    "?bInfoPublica=N&pager.sortname=fPublicacion&pager.sortindex=-3&provincia=37&municipio=090"
)

WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/ows"
WFS_MUNICIPIO = "Las Casas del Conde"
...
RE_PROYECTO = re.compile(
    ...
    r"edicto|bando.*parcel|actuaci[oó]n urban|reurbaniz|urbanizaci[oó]n|plau|bocyl|ruina|"
    r"desafectaci[oó]n|delimitaci[oó]n)",
)
RE_EXCLUDE = re.compile(
    ...
    r"horario|fiesta|felina|colonia|indemnizaci[oó]n|dedicaci[oó]n del alcalde)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_ISO = re.compile(r"\b((?:19|20)\d{2})-(\d{2})-(\d{2})\b")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_SECTOR = re.compile(r"(?i)\b(?:sector\s*)?(s[- ]?\d{1,2}|ue[- ]?\d{1,2})\b")
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "nnss" in n or "normas urban" in n or "normas subsidiarias" in n:
    ...
    if "sector" in n or "unidad de ejecuci" in n:
        return "sector"
    if "ruina" in n:
...
class LasCasasDelCondeAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + PLAU JCyL + IDECyL WFS (web corporativa mínima)."""
    ...
    def _fetch(self, url: str, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-las-casas-del-conde/1.0")},
        )
    ...
    def _plau_proyecto(self) -> dict[str, Any]:
        rec: dict[str, Any] = {
            ...
            "municipio": MUNICIPIO,
            "titulo": "Archivo PLAU CyL — planeamiento vigente Las Casas del Conde",
            "fecha": None,
            "tipo": "planeamiento",
            "url": self.plau_url,
            "source": "ayuntamiento",
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            ...
            (f"{self.sede_base}/transparency", "portal de transparencia — urbanismo"),
        ]
        rows: list[dict[str, Any]] = []
