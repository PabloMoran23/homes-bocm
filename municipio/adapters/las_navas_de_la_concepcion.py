...
WP_BASE = "https://www.lasnavasdelaconcepcion.es"
SEDE_BASE = "https://sedenavasdelaconcepcion.dipusevilla.es"
MUNICIPIO = "Las Navas de la Concepción"
ID_PREFIX = "las-navas-de-la-concepcion"
INE_CODE = "41066"

TABLON_URL = f"{SEDE_BASE}/tablon-1.0/do/entradaPublica?ine={INE_CODE}"
PGOU_URL = f"{WP_BASE}/es/ayuntamiento/pgou-"
NORMATIVA_URL = f"{WP_BASE}/es/ayuntamiento/normativa"
TRANSPARENCIA_PGOU = (
    f"{WP_BASE}/es/transparencia/indicadores-de-transparencia/indicador/"
    "Plan-General-de-Ordenacion-Urbana-PGOU-y-los-mapas-y-planos-que-lo-detallan-00048/"
)
NNSS_3WEB_URL = "http://3web.dipusevilla.es/planeamiento/66PG%20Las%20Navas/66NSLasNavas.html"
LICYTAL_URL = "https://portalag.dipusevilla.es/LicytalPub/jsp/pub/index.faces?cif=P4106600A"
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general|b[aá]sico)|pgou|pbom|potaus|convenio|"
    r"informaci[oó]n p[uú]blica|expediente|proyecto|modificaci[oó]n|reparcel|"
    r"estudio (?:ac[uú]stico|ambiental|de detalle)|memoria|planos|bop|boja|edicto|"
    r"aprobaci[oó]n (?:inicial|definitiva|provisional)|parcela|suelo|sector|"
    r"ordenanza|regularizaci[oó]n|nnss|normas subsidiarias|catalogo|catálogo|consulta p[uú]blica|avance|"
    r"adaptaci[oó]n|ordenaci[oó]n|calificaci[oó]n)",
)
RE_TABLON_NON_URBAN = re.compile(
    ...
    r"activat joven|limpiador|auxiliar administrativo|oferta de empleo|"
    r"cr[eé]dito extraordinario|monitor|enfermer|bando|programa activa)",
)
RE_TABLON_URBAN = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general|b[aá]sico)|pgou|pbom|potaus|"
    r"informaci[oó]n p[uú]blica|licencia|sector|ordenanza|consulta p[uú]blica|"
    r"avance del plan|nnss|catalogo|catálogo|regularizaci[oó]n|edicto pgou)",
)
...
class LasNavasDeLaConcepcionAyuntamientoAdapter(AyuntamientoAdapter):
    """OpenCMS INPRO web + GSede Diputación Sevilla + 3web planeamiento NNSS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.nnss_3web_url = str(self.config.get("nnss_3web_url") or NNSS_3WEB_URL)
        ...
        req = urllib.request.Request(
            ...
            headers={
                "User-Agent": self.config.get(
                    "user_agent", "poc-bocm-las-navas-de-la-concepcion/1.0"
                )
            },
        )
    ...
    def _collect_pdf_links(self, page_url: str, base: str) -> list[dict[str, Any]]:
        ...
        for href, inner in RE_LINK.findall(html):
            ...
            if not title or title.lower() in ("leer más", "leer mas", "&nbsp;"):
                continue
            low_href = href.lower()
            if not any(ext in low_href for ext in (".pdf", ".tif", "documentos/", "galleries/")):
                continue
    ...
    def _collect_3web_nnss(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(self.nnss_3web_url, encoding="latin-1")
        except urllib.error.URLError:
            return []
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        base = self.nnss_3web_url.rsplit("/", 1)[0] + "/"
        for href, inner in RE_LINK.findall(html):
            title = _strip_html(inner)
            if not title:
                continue
            low = href.lower()
            if not any(ext in low for ext in (".pdf", ".tif", ".tiff")):
                continue
            url = _abs_url(href, base)
            if url in seen:
                continue
            ...
            rows.append(
                {
                    "titulo": f"NNSS Las Navas — {title}"[:500],
                    "url": url,
                    "page": self.nnss_3web_url,
                    "origen": "nnss_3web",
                }
            )
        return rows
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Sede electrónica GSede — área Urbanismo",
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Portal LicytalPub — licencias de obra Diputación Sevilla",
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "info": sum(
                1
                for r in rows
                if r.get("origen") in ("sede_tablon", "sede_tramite", "licytal", "transparencia")
            ),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for doc in self._collect_pgou_documents():
            add(self._doc_to_proyecto(doc, "pgou_web"))
        for doc in self._collect_3web_nnss():
            add(self._doc_to_proyecto(doc, "nnss_3web"))
        ...
        return {
            ...
            "nnss_3web": sum(1 for r in rows if r.get("origen") == "nnss_3web"),
