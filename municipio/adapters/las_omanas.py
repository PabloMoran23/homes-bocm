...
WEB_BASE = "https://www.aytolasomanas.es"
SEDE_BASE = "https://aytolasomanas.sedelectronica.es"
MUNICIPIO = "Las Omañas"
ID_PREFIX = "las-omanas"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    ...
    (
        "https://servicios.jcyl.es/PlanPublica/openDocumento.do?cDocId=301592",
        "NORMAS URBANÍSTICAS TERRITORIALES (PlanPublica CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=104",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=104",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://aytolasomanas\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://aytolasomanas\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class LasOmanasAyuntamientoAdapter(AyuntamientoAdapter):
    """OpenCms web (Diputación León) + sede espublico tablón + IDECyL WFS instrumentos."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-las-omanas/1.0")},
        )
