...
SEDE_BASE = "https://ledesma.sedelectronica.es"
...
RE_PREVIEW = re.compile(
    r'href="(https://ledesma\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://ledesma\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"UBZ[- ]?\d+|SNC[- ]?\d+|PE[- ]?\d+|AR[- ]?P\d+|UNC\d+|"
...
class LedesmaAyuntamientoAdapter(AyuntamientoAdapter):
    """Web Angular + sede espublico gestiona + PlanPublica PLAU + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=170"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=170"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-ledesma/1.0")},
        )
