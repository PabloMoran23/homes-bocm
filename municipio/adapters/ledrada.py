...
SEDE_BASE = "https://ledrada.sedelectronica.es"
...
RE_PREVIEW = re.compile(
    r'href="(https://ledrada\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://ledrada\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"pol[ií]gono industrial|prat)",
)
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"PE[- ]?\d+|AR[- ]?P\d+|UNC[- ]?[A-Z0-9]+|"
    r"S\.?\s*U\.?\s*\.?\s*N\.?\s*C\.?\s*[-.]?\s*(?:UNC)?[A-Z0-9-]+|"
...
def _normalize_sector_code(raw: str) -> str:
    ...
    if re.fullmatch(r"SECTOR\s*[\"']?([A-Z0-9]+)[\"']?", code, re.I):
        return f"SECTOR {re.search(r'([A-Z0-9]+)', code).group(1)}"
    code = re.sub(r"\s+", "", code)
    if re.fullmatch(r"LASERAS\d+", code):
        return code.replace("LASERAS", "LAS ERAS ")
    return code
...
def _proyecto_tipo(blob: str, instrumento: str = "") -> str:
    ...
    if "plan parcial" in n or "sector" in n or "pau" in n:
        return "plan parcial"
    if "prat" in n or "polígono industrial" in n or "poligono industrial" in n:
...
class LedradaAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + PlanPublica PLAU + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=171"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=171"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-ledrada/1.0")},
        )
        ...
        for layer, default_tipo in WFS_LAYERS:
            ...
            for feat in data.get("features") or []:
                ...
                if isinstance(geom, dict) and geom.get("type"):
                    ...
                    sector_name = str(props.get("n_sector") or "").strip()
                    if sector_name:
                        self._sector_geom_cache[_normalize_sector_code(sector_name)] = {
                            "geom_geojson": geom,
                            "geometry_source_url": url,
                        }
                rows.append(rec)
                ...
                if wfs_rec.get("wfs_layer") == "urbanismo:plau_cyl_instrumentos_ambito":
                    for key in (
                        ...
                        if wfs_rec.get(key) is not None:
                            rec[key] = wfs_rec[key]
                    return
        if "LAS ERAS" in blob.upper() or "PRAT" in blob.upper():
            for wfs_rec in self._collect_wfs_proyectos():
                title = str(wfs_rec.get("titulo") or "").upper()
                if "ERAS" in title and wfs_rec.get("geom_geojson"):
                    for key in (
                        "geom_geojson",
                        "geometry_source",
                        "geometry_source_url",
