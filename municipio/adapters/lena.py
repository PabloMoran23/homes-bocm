...
WFS_MUNICIPIO_ID = "33033"

DEFAULT_SEED_PAGES: list[str] = [
    f"{BASE}/urbanismo",
    f"{BASE}/es/pgol",
    f"{BASE}/es/normativa-urbanistica1",
    f"{BASE}/es/catalogo",
    f"{BASE}/es/modificado-plan-parcial-villayana",
    f"{BASE}/es/suelo-urbanizable",
    f"{BASE}/es/suelo-no-urbanizable",
    f"{BASE}/es/nucleos-rurales",
]

DEFAULT_LICENCIA_PAGES: list[str] = [
    f"{BASE}/tramitacion-de-licencias",
    f"{BASE}/urbanismo",
    f"{BASE}/es/normativa-urbanistica1",
]
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|pgol|convenio|"
    r"informaci[oó]n p[uú]blica|expediente|proyecto|modificaci[oó]n|reparcel|"
    r"estudio (?:ac[uú]stico|ambiental|de detalle|de implantaci[oó]n)|normas? (?:provisionales|subsidiarias)|"
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|expropiaci[oó]n|villayana|masgain|sotiello)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://www\.aytolena\.es)?/documents/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Lena.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
def _tipo_proyecto(denominacion: str, clasificacion: str = "") -> str:
    ...
    if "plan general" in blob or "pgou" in blob or "pgo " in blob or "pgol" in blob:
        return "PGOU"
...
class LenaAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay aytolena.es + RPGUR Asturias; geometría parcial WFS visorrpgur."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "33")
    ...
    def _fetch(self, url: str, *, data: bytes | None = None, encoding: str = "utf-8") -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-lena/1.0"),
        }
        if data is not None:
    ...
    def _crawl_liferay_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in re.finditer(r'href="(https://www\.aytolena\.es/[^"#?]+)"', html, re.I):
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        info_url = f"{BASE}/tramitacion-de-licencias"
        ...
        rows.append(
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Tramitación de licencias urbanísticas",
                "url": info_url,
                "source": "ayuntamiento",
                "nota": "Sin tablón público de concesiones; solo trámites informativos",
            }
        )
