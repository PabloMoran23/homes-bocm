...
BASE = "https://www.llanera.es"
SEDE_BASE = "https://sede.llanera.es"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{BASE}/urbanismo-y-vivienda",
    f"{BASE}/plan-general-ordenaci%C3%B3n-y-cat%C3%A1logo-urban%C3%ADstico-2023",
    f"{BASE}/normas-subsidiarias",
    f"{BASE}/pe%C3%B1a-corada",
    f"{BASE}/caleras-de-san-cucao",
    f"{BASE}/planta-de-aglomerado-asf%C3%A1ltico-en-abarr%C3%ADo",
    f"{BASE}/pol%C3%ADgono-industrial-silvota",
    f"{BASE}/pol%C3%ADgono-industrial-asipo",
    f"{BASE}/mercasturias",
]

DEFAULT_LICENCIA_PAGES: list[str] = [
    f"{BASE}/urbanismo-y-vivienda",
    f"{SEDE_BASE}/tablondeanuncios/",
]
...
RE_PROYECTO = re.compile(
    ...
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|silvota|asipo|mercasturias|pe[nñ]a corada|"
    r"caleras|aglomerado|solar|instalaci[oó]n)",
)
RE_TABLON_SKIP = re.compile(
    r"(?i)(padr[oó]n|teleasistencia|ayuda a domicilio|matr[ií]cula|m[uú]sica|"
    r"premio llanerense|jurado|protecci[oó]n de datos|selecci[oó]n de personal)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://www\.llanera\.es)?/documents/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Llanera.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
def _tipo_proyecto(denominacion: str, clasificacion: str = "") -> str:
    ...
    if "normas" in blob and ("subsidiaria" in blob or "provisional" in blob):
...
class LlaneraAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay llanera.es + sede tablón RSS + RPGUR Asturias; geometría parcial WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "35")
    ...
    def _load_geom_index(self) -> dict[str, dict[str, Any]]:
        ...
        params = {
            ...
            "srsName": "EPSG:4326",
            "CQL_FILTER": "Instrumento LIKE '%LLANERA%'",
        }
    ...
    def _crawl_liferay_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in re.finditer(r'href="(https://www\.llanera\.es/[^"#?]+)"', html, re.I):
    ...
    def _collect_tablon_proyectos(self) -> list[dict[str, Any]]:
        ...
        for item in self._collect_tablon_rss():
            ...
            if RE_LICENCIA.search(blob) and not re.search(
                r"(?i)(informaci[oó]n p[uú]blica|exposici[oó]n|anuncio|estudio|proyecto)",
                blob,
            ):
                continue
            rows.append(
                {
                    ...
                    "tipo": _tipo_proyecto(item["titulo"]),
                    "url": item["url"],
                    "source": "ayuntamiento",
                    "origen": "sede_tablon_rss",
                }
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        info_url = f"{BASE}/urbanismo-y-vivienda"
        ...
        rows.append(
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo y vivienda — trámites",
                "url": info_url,
                "source": "ayuntamiento",
        ...
        for item in self._collect_tablon_rss():
            ...
            if not RE_LICENCIA.search(blob):
                continue
            rec_id = _stable_id("lic", item["url"])
            if rec_id in seen:
                continue
            seen.add(rec_id)
            rows.append(
                {
                    ...
                    "tipo": "licencia / anuncio urbanismo",
                    "distrito": None,
                    "lat": None,
                    "lon": None,
                    "titulo": item["titulo"],
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            "rows": len(rows),
            "status": "ok",
            "tablon": sum(1 for r in rows if r.get("origen") == "sede_tablon_rss"),
        }
