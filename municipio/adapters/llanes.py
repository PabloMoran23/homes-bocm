...
DEFAULT_SEED_PAGES: list[str] = [
    ...
    f"{BASE}/documentos-en-informaci%C3%B3n-p%C3%BAblica1",
...
RE_DOC_LINK = re.compile(
    r'href="((?:https://www\.ayuntamientodellanes\.com)?/documents/[^"]+\.pdf[^"]*|'
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Llanes.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
class LlanesAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "36")
    ...
    def _fetch(self, url: str, *, data: bytes | None = None, encoding: str = "utf-8") -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-llanes/1.0"),
        }
        if data is not None:
    ...
    def _load_geom_index(self) -> dict[str, dict[str, Any]]:
        ...
        params = {
            ...
            "srsName": "EPSG:4326",
            "CQL_FILTER": "Instrumento LIKE '%LLANES%'",
        }
    ...
    def _crawl_liferay_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in re.finditer(r'href="(https://www\.ayuntamientodellanes\.com/[^"#?]+)"', html, re.I):
