...
PGOU_PAGE = f"{WP_BASE}/plan-general-de-ordenacion-urbana/"
URBANISMO_PAGE = f"{WP_BASE}/urbanismo/"
MUNICIPIO = "Loeches"
ID_PREFIX = "loeches"
...
DEFAULT_WP_CATEGORIES = (34,)

RE_PREVIEW = re.compile(
    r'href="(https://loeches\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "pgou" in n or "plan general" in n:
        return "PGOU"
    if "sector industrial" in n or "ronda" in n:
        return "sector industrial"
    if "informaci" in n or "bocm" in n:
        return "información pública"
    if "aprobaci" in n:
        return "aprobación"
    if "plano" in n or "memoria" in n:
        return "documento planeamiento"
    if "licencia" in n:
...
def _sector_ilike_parts(text: str) -> list[str]:
    low = text.lower()
    for marker in (" bocm", " aprob", " anuncio", " del ", " en loeches", " municipio"):
        if marker in low:
...
class LoechesAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress REST (urbanismo) + tablón espublico sede + WFS SITCM (geometría partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.pgou_page = str(self.config.get("pgou_page") or PGOU_PAGE)
    ...
    def _fetch(self, url: str, *, insecure: bool | None = None) -> str:
        ...
        if use_insecure and url.startswith("https://loeches.sedelectronica.es"):
    ...
    def _collect_seed_pages(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for page_url in (self.pgou_page, self.urbanismo_page):
            try:
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for seed in self._collect_seed_pages():
            add(self._seed_to_proyecto(seed))
        for board in self._collect_board():
