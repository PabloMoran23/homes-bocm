...
WEB_BASE = "https://www.loshuertos.es"
SEDE_BASE = "https://loshuertos.sedelectronica.es"
PLAI_BASE = "https://servicios.jcyl.es/PlanPublica"
WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/ows"
MUNICIPIO = "Los Huertos"
ID_PREFIX = "los-huertos"
PLAI_MUNICIPIO = 103
PLAI_PROVINCIA = 40
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (f"{WEB_BASE}/urbanismo", "Urbanismo — sede electrónica y contacto"),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=103",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=103",
...
RE_PREVIEW = re.compile(
    r'href="(https://loshuertos\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://loshuertos\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_SECTOR = re.compile(
    r"(?i)\b(?:sector|sect\.?|ua|ue|unidad de ejecuci[oó]n)\s*([\"«]?\s*[0-9]+[a-z]?[A-Z0-9\"»\s-]*|"
    r"ua[- ]?1)",
)
...
class LosHuertosAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay DipSegovia (www.loshuertos.es) + sede espublico + PLAI JCYL + geometría parcial IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-los-huertos/1.0")},
        )
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tramites": sum(1 for r in rows if r.get("origen") in ("catalogo_tramites", "liferay_tramite")),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
            "semillas": sum(1 for r in rows if r.get("origen") == "liferay_semilla"),
        }
