...
RE_LICENCIA = re.compile(
    ...
    r"comunicaci[oó]n previa|autorizaci[oó]n (?:previa|urban)|instancia obra|orcc)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|convenio|"
    r"informaci[oó]n p[uú]blica|expediente|reparcel|aprobaci[oó]n (?:inicial|definitiva)|"
    r"orden de ejecuci|modificaci[oó]n|estudio (?:ac[uú]stico|ambiental|de detalle)|"
    r"ua-\d|ue-\d|avance plan|bocm|normas subsidiarias|normativa urban|orcc|"
    r"proyecto de|iniciativa urban|finca)",
)
...
RE_SKIP_PDF = re.compile(
    r"(?i)(cita_previa|notif-tele|modelo-de-instancia|alteracion-titular-catastral|"
    r"cartel_de_cita|plusvalia\.pdf)",
)
...
def _proyecto_tipo(title: str, default: str = "urbanismo") -> str:
    n = title.lower()
    if "avance plan" in n or "pgou" in n:
        return "planeamiento"
    ...
    if "orcc" in n or "ordenanza" in n:
        return "ordenanza urbanística"
    if "normas subsidiarias" in n or "normativa urban" in n:
        return "normas urbanísticas"
    if "memoria" in n:
        return "memoria planeamiento"
    if "planos" in n or re.search(r"\bp\d+\.pdf", n):
        return "planos ordenación"
    return default


class LosMolinosAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress urbanismo + sede eAdmin (tablón + trámites) + transparencia planeamiento."""
    ...
    def _abs_wp(self, href: str) -> str:
        return urllib.parse.urljoin(self.wp_base + "/", href)
    ...
    def _collect_wp_urbanismo_pdfs(self) -> list[dict[str, Any]]:
        try:
        ...
        for m in RE_WP_PDF.finditer(html):
            ...
            titulo = f"Urbanismo Los Molinos: {re.sub(r'\.pdf$', '', name, flags=re.I).replace('_', ' ')}"
            rows.append(
                {
    ...
    def _title_to_licencia(self, rec: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not RE_LICENCIA.search(title):
            return None
        if RE_PROYECTO.search(title) and not re.search(r"(?i)orcc|licencia", title):
            return None
        tipo_m = re.search(r"(?i)(licencia[^,]{0,80}|orcc|ordenanza)", title)
        out: dict[str, Any] = {
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_transparencia_proyectos():
            add(rec)
        for rec in self._collect_wp_urbanismo_pdfs():
            add(rec)
        tablon = self._collect_tablon()
        for rec in tablon.values():
        ...
        self._write_jsonl(out_jsonl, rows)
        wp_count = sum(1 for r in rows if r.get("origen") == "urbanismo_web")
        transp = sum(1 for r in rows if r.get("origen") == "planeamiento")
        tablon_count = sum(1 for r in rows if r.get("origen") == "tablon")
        return {
            "rows": len(rows),
            "status": "ok",
            "urbanismo_web_pdfs": wp_count,
            "transparencia_docs": transp,
            "tablon_items": tablon_count,
        }
