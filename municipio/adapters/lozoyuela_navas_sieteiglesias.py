...
WP_BASE = "https://www.lozoyuela.com"
SEDE_BASE = "https://lozoyuela.sedelectronica.es"
MUNICIPIO = "Lozoyuela-Navas-Sieteiglesias"
ID_PREFIX = "lozoyuela"
WFS_MUNICIPIO = "LOZOYUELA-NAVAS-SIETEIGLESIAS"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/108-2/normativa-municipal/normas-subsidiarias/",
    f"{WP_BASE}/108-2/normativa-municipal/plan-de-ordenacion-del-nucleo-de-poblacion-de-mazacorta/",
    f"{WP_BASE}/108-2/bandos/",
    f"{WP_BASE}/tramites-personales/",
]
...
RE_PREVIEW_LINK = re.compile(
    r'href="(https://lozoyuela\.sedelectronica\.es/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_WP_PDF = re.compile(
    r'href="((?:https://www\.lozoyuela\.com)?/wp-content/uploads/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "normas subsidiarias" in n or "nnss" in n:
        return "normas subsidiarias"
    if "ponp" in n or "mazacorta" in n:
        return "PONP"
    if "sector" in n:
        return "sector urbanístico"
    if "subasta" in n and "parcela" in n:
...
class LozoyuelaNavasSieteiglesiasAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress + tablón eHome (espublico) + PDFs planeamiento + SITCM WFS (geometría partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.tramites_url = str(
            self.config.get("tramites_url") or f"{WP_BASE}/tramites-personales/"
        )
    ...
    def _collect_wp_posts(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[int] = set()
        for term in ("pgou", "planeamiento", "normas subsidiarias", "exposicion publica"):
        ...
        return [
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Trámites personales — licencias y declaraciones urbanísticas",
                "url": self.tramites_url,
                "source": "ayuntamiento",
                "nota": "Formularios de licencia, declaración responsable y autoliquidación",
                "origen": "tramites_web",
            },
            {
