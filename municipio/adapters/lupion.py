...
SEDE_BASE = "https://lupion.sedelectronica.es"
WEB_BASE = "https://www.lupion.es"
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|pbom|convenio|"
    r"informaci[oó]n p[uú]blica|expediente|proyecto|modificaci[oó]n|reparcel|"
    r"estudio (?:ac[uú]stico|ambiental|de detalle)|memoria|planos|boja|edicto|"
    r"aprobaci[oó]n (?:inicial|definitiva|provisional)|parcela|suelo|sector|"
    r"cambio de uso|ordenanza|rectificaci[oó]n descriptiva|divisi[oó]n|segregaci[oó]n)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"cobranza iae|padrones|bolsa de empleo|subvenci[oó]n|ayudas y subvenciones|"
    r"material escolar|cooperaci[oó]n por f[eé]n[oó]menos meteorol[oó]gicos|"
    r"plan especial de cooperaci[oó]n|energ[ií]as renovables.*polideportivo|"
    r"resoluci[oó]n estimatoria.*subvenc|concesi[oó]n de subvenciones)",
)
RE_DOSSIER_LINK = re.compile(
    r'href="((?:https://lupion\.sedelectronica\.es)?/catalog/t/[a-f0-9-]+)"[^>]*>([^<]{5,200})</a>',
    re.I,
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://lupion\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "rectificaci" in b and "descriptiva" in b:
...
class LupionAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress www.lupion.es (turismo) + sede espublico gestiona (tablón, trámites) + SITUADIFUSION."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.situa_search_url = str(self.config.get("situa_search_url") or SITUA_SEARCH)
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-lupion/1.0")},
        )
    ...
    def _collect_situa_proyecto(self) -> dict[str, Any]:
        return {
            "id": _stable_id("proy", self.situa_search_url),
            "municipio": MUNICIPIO,
            "titulo": "Consulta del planeamiento urbanístico — SITUADIFUSION (Lupión, Jaén)",
            "fecha": None,
            "tipo": "planeamiento",
            "url": self.situa_search_url,
            "source": "ayuntamiento",
            "origen": "situa",
            "nota": "Visor regional Junta de Andalucía (municipio INE 23062)",
        }
    ...
    def _tramite_to_licencia(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        if re.search(r"(?i)actividad", blob):
            tipo = "licencia de actividad"
        elif re.search(r"(?i)ocupaci[oó]n", blob):
            tipo = "licencia de ocupación"
        elif re.search(r"(?i)obra|urban", blob):
            tipo = "licencia urbanística"
        elif re.search(r"(?i)certificado|informe", blob):
            tipo = "certificado urbanístico"
        return {
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        add(self._collect_situa_proyecto())
        for item in self._collect_board():
