...
WEB_BASE = "https://www.madrigalejodelmonte.es"
SEDE_BASE = "https://madrigalejodelmonte.sedelectronica.es"
...
ID_PREFIX = "madrigalejo-del-monte"
PLAU_PROVINCIA = 9
PLAU_MUNICIPIO = 197
...
DEFAULT_INFO_PAGES: list[tuple[str, str]] = [
    ...
    (f"{SEDE_BASE}/dossier", "catálogo de trámites sede"),
    (f"{SEDE_BASE}/transparency", "portal de transparencia"),
    (f"{WEB_BASE}/pueblos/madrigalejo-del-monte", "planeamiento urbanístico — enlace PLAU JCyL"),
    (
        "http://www.jcyl.es/plau/lplanes.plau?municipio=09197",
        "archivo planeamiento JCyL (Madrigalejo del Monte)",
    ),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia={PLAU_PROVINCIA}&municipio={PLAU_MUNICIPIO}",
        "PlanPublica PLAU — documentos aprobados",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://madrigalejodelmonte\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://madrigalejodelmonte\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"modificaci[oó]n|aprobaci[oó]n (?:inicial|definitiva|parcial)|reparcel|sector|"
    r"normas urban|num\b|instrumento|suz|zdr|suelo r[uú]stico|ordenaci[oó]n|"
    r"actuaci[oó]n urban|bocyl|plau)",
)
RE_NOISE = re.compile(
    r"(?i)(impuesto.*veh[ií]culo|liquidaci[oó]n|anuncio de cobranza|"
    r"acta del pleno|decreto bases|concurso|fiestas|subvenci[oó]n|"
    r"ordenanza fiscal|plusval[ií]a|tasa de piscinas)",
)
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"SUZ\s*[A-Z]?\d+|Z\.?D\.?R\.?\s*\d+|ZDR\s*\d+|"
    r"S\.?\s*U\.?\s*\.?\s*Z\.?\s*[-.]?\s*[A-Z]?\d+|"
    r"SECTOR\s+SUZ\s*[A-Z]?\d+"
    r")\b",
)
...
def _normalize_sector_code(raw: str) -> str:
    ...
    code = re.sub(r"Z\.?\s*D\.?\s*R\.?", "ZDR", code)
    return code
...
def _proyecto_tipo(title: str, instrumento: str = "") -> str:
    ...
    if "reclasific" in blob:
        return "reclasificación suelo"
    if "informaci" in blob:
        return "información pública"
    if "sector" in blob or "suz" in blob or "zdr" in blob:
        return "sector urbanístico"
    if "planeam" in blob or "plau" in blob:
        return "planeamiento"
    return "urbanismo"


class MadrigalejoDelMonteAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal Toools + sede espublico + PlanPublica PLAU + IDECyL WFS (geometría partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or (
                f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N"
                f"&provincia={PLAU_PROVINCIA}&municipio={PLAU_MUNICIPIO}"
            )
        )
        ...
        req = urllib.request.Request(
            ...
            headers={
                "User-Agent": self.config.get(
                    "user_agent", "poc-bocm-madrigalejo-del-monte/1.0"
                )
            },
        )
        opener = self._opener if sede else urllib.request.build_opener()
        timeout = 90 if sede else 60
    ...
    def _parse_preview_links(self, html: str, origen: str) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
            ...
            rows.append(
                {
                    ...
                    "expediente": "",
                    "procedimiento": "",
                    "categoria": "urbanismo" if origen == "transparencia" else "",
                    "descripcion": titulo,
    ...
    def _collect_board(self) -> list[dict[str, Any]]:
        ...
        for url, origen in ((self.board_url, "tablon"), (self.info_url, "info_tablon")):
            ...
            if not items:
                items = self._parse_preview_links(html, origen)
            for rec in items:
                by_url[rec["url"]] = rec
    ...
    def _collect_transparency(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(self.transparency_url, sede=True)
        except (urllib.error.URLError, OSError, ConnectionError):
            return []
        rows = self._parse_preview_links(html, "transparencia")
    ...
    def _parse_plau_rows(html: str, fallback_url: str) -> list[dict[str, str]]:
        ...
        for tr in re.findall(r"<tr[^>]*>(.*?)</tr>", html, re.S | re.I):
            ...
            if len(cells) < 5 or cells[0] in {"Libro", "Tipo", "Instrumento"}:
                continue
    ...
    def _wfs_sector_geometry(self, sector_code: str) -> tuple[dict[str, Any] | None, str | None]:
        ...
        cql = (
            f"n_mun='{escaped_mun}' AND "
            f"(n_num_sect ILIKE '%{escaped_code}%' OR n_sector ILIKE '%{escaped_code}%')"
        )
    ...
    def _attach_geometry(self, rec: dict[str, Any]) -> None:
        ...
        blob = " ".join(
            str(rec.get(k) or "")
            for k in ("titulo", "instrumento", "sector_code", "subtipo")
        )
    ...
    def _tramite_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if RE_LICENCIA.search(row["titulo"]) and not re.search(
            r"(?i)planeam|actuaci[oó]n urban|modificaci[oó]n|archivo|plau",
            row["titulo"],
        ):
            return None
        rec: dict[str, Any] = {
