...
WP_BASE = "https://magazdepisuerga.es"
SEDE_BASE = "https://magazdepisuerga.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/urbanismo/",
    f"{WP_BASE}/urbanismo/parcela-san-pedro-no4-estudio-de-detalle-modificacion-alineaciones/",
    f"{WP_BASE}/urbanismo/plan-parcial-del-poligono-industrial-de-magaz-de-pisuerga-modificacion-puntual/",
    f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=34&municipio=098",
    f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=34&municipio=098",
]

DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    (
        f"{SEDE_BASE}/catalog",
        "Declaración Responsable, Solicitud de Licencia o Comunicación en Materia Urbanística",
    ),
    (
        f"{SEDE_BASE}/dossier/.0",
        "Catálogo de trámites — Urbanismo y Vivienda",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://magazdepisuerga\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://magazdepisuerga\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"pol[ií]gono industrial|prau|num\b|alineacion)",
)
RE_EXCLUDE = re.compile(
    ...
    r"selecci[oó]n de personal|tribunal|convocatoria pleno|matrimonio)",
)
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"Sector\s+[IVXLC\d]+|"
    r"PRAU\s+MAGAZ(?:\s+NORTE)?|"
    r"CASTILLO\s+DE\s+MAGAZ|"
    r"PE[- ]?\d+|"
...
RE_FECHA_WP = re.compile(r"(\d{1,2})\s+([A-Za-z]{3})\s+(\d{4})")
RE_WP_ARTICLE = re.compile(
    r'<article[^>]*class="[^"]*ic_urbanismo[^"]*"[^>]*>(.*?)</article>',
    re.S | re.I,
)
RE_WP_TITLE = re.compile(r'class="entry-title"[^>]*>\s*<a[^>]+href="([^"]+)"[^>]*>([^<]+)</a>', re.I)
RE_WP_DATE = re.compile(r'class="published">([^<]+)</span>', re.I)
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?magazdepisuerga\.es)?(?:/download/|/files/)[^"\']+)["\']',
    re.I,
)
...
def _parse_fecha_wp(text: str) -> str | None:
    m = RE_FECHA_WP.search(text or "")
    if not m:
        return None
...
class MagazDePisuergaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Divi (ic_urbanismo) + sede espublico + PlanPublica + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=34&municipio=098"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=34&municipio=098"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-magaz-de-pisuerga/1.0")},
        )
    ...
    def _collect_wp_urbanismo(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.seed_pages:
            if "magazdepisuerga.es" not in page_url:
                continue
            try:
            ...
            for article in RE_WP_ARTICLE.findall(html):
                ...
                if not title_m:
                    continue
                url, title = title_m.group(1), _strip_html(title_m.group(2))
                if url in seen:
                    continue
                seen.add(url)
                date_m = RE_WP_DATE.search(article)
                fecha = _parse_fecha_wp(date_m.group(1)) if date_m else None
                rows.append(
                    {
                        "titulo": title[:500],
                        "url": url,
                        "fecha": fecha,
                        "origen": "wp_urbanismo",
                        "excerpt": _strip_html(article)[:300],
                    }
                )
            if page_url.endswith("/urbanismo/") or "/urbanismo/" in page_url:
                title_m = re.search(r"<title>([^<|]+)", html, re.I)
                page_title = _strip_html(title_m.group(1)) if title_m else ""
                if RE_PROYECTO.search(page_title) and page_url not in seen:
                    seen.add(page_url)
                    rows.append(
                        {
    ...
    def _attach_geometry(self, rec: dict[str, Any]) -> None:
        ...
        blob = " ".join(
            str(rec.get(k) or "")
            for k in ("titulo", "descripcion", "expte", "sector_code", "instrumento", "excerpt")
        )
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            ...
            (f"{WP_BASE}/urbanismo/", "urbanismo — documentación y planeamiento"),
            (f"{self.sede_base}/catalog", "trámites de licencia urbanística (sede electrónica)"),
        ]
        rows: list[dict[str, Any]] = []
    ...
    def _wp_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        blob = f"{row.get('titulo', '')} {row.get('excerpt', '')}"
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_plau():
            ...
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_wp_urbanismo():
            rec = self._wp_to_proyecto(item)
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
