...
BASE = "https://www.manises.es"
API_BASE = "https://api.digitalvalue.es/manises/collections"
SEDE = "https://manises.sedipualba.es"
...
DEFAULT_DRUPAL_SEEDS: tuple[str, ...] = (
    "/es/urbanisme/pagina/planeamiento-estructural",
    "/es/urbanisme/pagina/planeamiento-pormenorizado",
    "/es/urbanisme/pagina/revision-planeamiento-vigente-informacion-publica",
    "/es/transparencia/informacion-urbanistica",
)

HUB_SLUGS = {
    "comunicaciones",
    "comunicacions",
    "prevencion-de-incendios-recordatorio-a-propietarios",
    "planeamiento",
    "planejament",
...
RE_PROYECTO = re.compile(
    ...
    r"homologaci[oó]n|catalogo de protecc|cat[aà]leg de protecc|unidad de actuaci|"
    r"reurbaniz|participaci[oó]n p[uú]blica|plan de reforma|pri\b|pe \d)",
)
RE_NOISE = re.compile(
    ...
    r"prevenci[oó]n de incendios|recordatorio a propietarios|arte urbano|butoni fest|"
    r"empiezan las obras|obras de la|obras del|rehabilitaci[oó]n de la fachada|"
    r"piscina de verano|rotonda del|accesos peatonales|renovaci[oó]n del vallado|"
    r"plaza \d de mayo|oficina de turismo|dana)",
)
RE_PLANEAMIENTO_STRICT = re.compile(
    r"(?i)(planeam|pgou|pge|plan (?:parcial|especial|general)|informaci[oó]n p[uú]blica|"
    r"modificaci[oó]n.*pgou|plan de reforma|estudio de detalle|cat[aà]logo de protecc|"
    r"homologaci[oó]n|reparcel|sector \d|unidad de actuaci|plan de accesibilidad|"
    r"encuesta de participaci[oó]n|integraci[oó]n paisaj)",
)
RE_TRAMITE = re.compile(
    r'href="(https://manises\.sedipualba\.es/carpetaciudadana/tramite\.aspx\?idtramite=\d+)"[^>]*>\s*([^<]+)',
    re.I,
)
RE_SECTOR_TOKEN = re.compile(
    r"(?i)\b((?:UE|SD|PP|SECTOR|SECTOR|UA|UNIDAD)[\s\-]?(?:IND\s*)?[\dA-ZÁÉÍÓÚ'._\-]+(?:[\s,\-yY/]+[\dA-ZÁÉÍÓÚ'._\-]+)*)\b",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_A2A = re.compile(
    r'data-a2a-url="([^"]+)"[^>]*data-a2a-title="([^"]+)"',
    re.I,
)
RE_H1 = re.compile(r"<h1[^>]*>([^<]+)</h1>", re.I)
RE_PDF = re.compile(r'href="(/sites/www\.manises\.es/files/[^"]+\.pdf[^"]*)"', re.I)
...
def _article_url(item: dict[str, Any]) -> str:
    ...
    if not slug:
        return f"{BASE}/es/articulo/{item.get('_id', '')}"
    cats = [str(c).lower() for c in (item.get("categories") or [])]
    if "urbanisme" in cats or "urbanismo" in cats:
        return f"{BASE}/es/urbanisme/pagina/{slug}"
    return f"{BASE}/es/pagina/{slug}"
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "reforma interior" in n or "pri" in n:
        return "plan de reforma interior"
    if "unidad de actuaci" in n or "ua " in n:
...
class ManisesAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal Portales + Digital Value API + sedipualba + ICV WFS (partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.drupal_seeds = tuple(self.config.get("drupal_seeds") or DEFAULT_DRUPAL_SEEDS)
    ...
    def _fetch(self, url: str, *, timeout: int = 60, retries: int = 3) -> str:
        ...
        for attempt in range(retries):
            ...
            req = urllib.request.Request(
                url,
                headers={"User-Agent": self.config.get("user_agent", "poc-bocm-manises/1.0")},
            )
            try:
    ...
    def _is_proyecto_article(self, item: dict[str, Any]) -> bool:
        ...
        title = _title(item)
        cats = [str(c).lower() for c in (item.get("categories") or [])]
        if "urbanisme" not in cats and "urbanismo" not in cats:
            return False
        blob = f"{title} {slug} {' '.join(cats)}"
        if RE_NOISE.search(blob):
            return False
        if RE_PLANEAMIENTO_STRICT.search(blob):
            return True
        if item.get("filesGroup") and any(
            k in slug for k in ("informacion-publica", "pgou", "planeam", "plan-parcial", "modificacion")
        ):
            return True
        return False
    ...
    def _collect_drupal_seeds(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for path in self.drupal_seeds:
            url = path if path.startswith("http") else f"{BASE}{path}"
            try:
                html = self._fetch(url)
            except urllib.error.URLError:
                continue
            page_title = _clean(RE_H1.search(html).group(1)) if RE_H1.search(html) else path
            pdfs = [f"{BASE}{m.group(1)}" for m in RE_PDF.finditer(html)]
            for a2a_url, a2a_title in RE_A2A.findall(html):
                titulo = _clean(a2a_title)
                if not titulo or titulo.lower() in {"planes parciales y modificaciones", "planes especiales"}:
                    continue
                key = a2a_url.lower()
                if key in seen:
                    continue
                ...
                rec: dict[str, Any] = {
                    ...
                    "origen": "drupal_seed",
                    "pagina_padre": page_title,
                }
                if pdfs:
                    rec["documentos"] = pdfs[:10]
                rows.append(rec)
            if not RE_A2A.search(html) and pdfs:
                key = url.lower()
                if key not in seen:
                    seen.add(key)
                    rows.append(
                        {
                            "titulo": page_title,
                            "fecha": _parse_fecha_dmy(page_title),
                            "url": url,
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "drupal_seed": sum(1 for r in rows if r.get("origen") == "drupal_seed"),
