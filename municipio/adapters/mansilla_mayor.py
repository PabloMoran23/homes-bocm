...
WEB_BASE = "https://www.aytomansillamayor.es"
SEDE_BASE = "https://aytomansillamayor.sedelectronica.es"
MUNICIPIO = "Mansilla Mayor"
ID_PREFIX = "mansilla-mayor"
...
DEFAULT_TRAMITE_PAGES: list[str] = [
    f"{WEB_BASE}/ayuntamiento/tramites-solicitudes/",
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    ...
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=095",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=095",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://aytomansillamayor\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://aytomansillamayor\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"unidad(?:es)? de ejecuci[oó]n|suelo urbano|su[- ]?nc|s\.u\.n\.c|plau|bocyl)",
)
...
class MansillaMayorAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-mansilla-mayor/1.0")},
        )
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            ...
            (self.transparency_url, "portal transparencia — urbanismo (90 docs)"),
            (f"{self.web_base}/ayuntamiento/tramites-solicitudes/", "trámites y solicitudes"),
            (f"{self.web_base}/ayuntamiento/normativa-municipal/urbanismo/", "normativa urbanística"),
        ]
        rows: list[dict[str, Any]] = []
