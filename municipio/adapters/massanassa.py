...
SEDE_BASE = "https://massanassa.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WEB_BASE = "https://www.massanassa.es"
NORMATIVA_URL = f"{WEB_BASE}/es/pagina/urbanismo-medio-ambiente"
VISOR_GVA_URL = "https://visor.gva.es/visor/?capas=spaicv0702_plan_zonificacion"
MUNICIPIO = "Massanassa"
ID_PREFIX = "massanassa"
COD_INE_MUN = "46165"
...
RE_PROYECTO = re.compile(
    ...
    r"cambio de uso|normas subsidiarias|homologaci[oó]n|ordenanza|e[oó]lic|iate|divendres|maestrat)",
)
...
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://massanassa\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_SECTOR = re.compile(r"(?i)sector\s*([0-9IVXLC]+)")
RE_NORMATIVA_LINK = re.compile(
    r'href="([^"]+)"[^>]*>([^<]{8,200})</a>',
    re.I | re.S,
)
RE_BOP_DATE = re.compile(r"BOP[-\s]*(\d+)\s+de\s+(\d{1,2})/(\d{1,2})/(\d{4})", re.I)
...
def _fecha_from_bop(text: str) -> str | None:
    m = RE_BOP_DATE.search(text or "")
    if not m:
        return None
    try:
        return datetime(int(m.group(4)), int(m.group(3)), int(m.group(2))).strftime("%Y-%m-%d")
    except ValueError:
        return None
...
def _proyecto_tipo(denominaci: str) -> str:
    ...
    if "ordenanza" in n or "licencia" in n:
        return "normativa"
    return "planeamiento"


class MassanassaAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-massanassa/1.0")},
        )
    ...
    def _match_icv_zone(self, titulo: str) -> dict[str, Any] | None:
        ...
        for zone in self._collect_icv_zones():
            ...
            if den and den in norm:
                ...
                score = 80.0
            elif "DIVENDRES" in norm and "DIVENDRES" in den:
                score = 75.0
            elif "INDUSTRIAL" in norm and "INDUSTRIAL" in den:
                score = 70.0
            else:
    ...
    def _collect_normativa_web(self) -> list[dict[str, Any]]:
        try:
        ...
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for href, raw_title in RE_NORMATIVA_LINK.findall(html):
            titulo = _strip_html(raw_title)
            if not titulo or titulo.lower() in seen:
                continue
            blob = f"{titulo} {href}".lower()
            if not any(
                k in blob
                for k in (
                    "plan general",
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "fecha_concesion": None,
                "tipo": "normativa licencias edificación",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Normativa licencias de edificación y DR (web municipal)",
                "url": self.normativa_url,
                "source": "ayuntamiento",
                "nota": "Ordenanzas URB-035/038 PDF en urbanismo-medio-ambiente",
                "origen": "web_normativa",
            },
        ]
        ...
        return [
            {
                ...
                "municipio": MUNICIPIO,
                "titulo": "Urbanismo y medio ambiente — normativa municipal",
                "fecha": None,
                "tipo": "normativa",
                "url": self.normativa_url,
                "source": "ayuntamiento",
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "info": sum(1 for r in rows if r.get("origen") in ("sede_tablon", "sede_tramite", "web_normativa")),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_icv_proyectos():
            add(rec)
        for rec in self._collect_normativa_web():
            add(rec)
        for item in self._collect_board():
        ...
        return {
            ...
            "normativa": sum(1 for r in rows if r.get("origen") == "web_normativa"),
        }
