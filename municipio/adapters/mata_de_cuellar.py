...
WEB_BASE = "https://www.matadecuellar.es"
SEDE_BASE = "https://matadecuellar.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (f"{WEB_BASE}/urbanismo", "Urbanismo — contacto y normativa"),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=124",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=124",
...
RE_PREVIEW = re.compile(
    r'href="(https://matadecuellar\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://matadecuellar\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
def _proyecto_tipo(title: str, instrumento: str = "") -> str:
    ...
    if "normas urban" in blob or "nut" in blob or blob.strip() == "num":
        return "normas urbanísticas"
    if "sin planeamiento general" in blob or "spg" in blob:
...
class MataDeCuellarAyuntamientoAdapter(AyuntamientoAdapter):
    """Web Liferay DipSegovia (matadecuellar.es) + sede espublico + PLAI JCYL + IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-mata-de-cuellar/1.0")},
        )
        ...
        for layer, default_tipo in WFS_LAYERS:
            ...
            for feat in data.get("features") or []:
                ...
                if not titulo:
                    titulo = str(
                        props.get("n_titulo")
                        or props.get("n_instrum")
                        or props.get("c_id_sect")
                        or props.get("c_plan")
                        or layer
                    )
