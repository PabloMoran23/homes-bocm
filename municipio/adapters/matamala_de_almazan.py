...
DRUPAL_BASE = "https://www.matamaladealmazan.es"
SEDE_BASE = "https://matamaladealmazan.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{DRUPAL_BASE}/normas-urbanisticas",
    f"{DRUPAL_BASE}/informacion-urbanistica",
    f"{DRUPAL_BASE}/modificacion-puntual-no5-de-las-normas-urbanisticas-municipales",
    f"{DRUPAL_BASE}/modificacion-puntual-no-6-de-las-normas-urbanisticas-municipales",
    f"{DRUPAL_BASE}/plan-especial-de-ordenacion-de-la-finca-de-la-concepcion",
    f"{DRUPAL_BASE}/ordenanzas-y-reglamentos",
    f"{DRUPAL_BASE}/bandos",
]

RE_PREVIEW = re.compile(
    r'href="(https://matamaladealmazan\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://matamaladealmazan\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_DRUPAL_PDF = re.compile(
    r'href="((?:https://www\.matamaladealmazan\.es)?/sites/matamaladealmazan\.es/files/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"concentraci[oó]n parcelaria|regad[ií]o|finca de la concepci[oó]n|"
    r"modificaci[oó]n puntual|st-n[oº°]\s*\d)",
)
RE_NOISE = re.compile(
...
def _proyecto_tipo(title: str, instrumento: str = "") -> str:
    ...
    if "pgou" in blob or "plan general" in blob:
        return "PGOU"
    if "plan especial" in blob or "finca de la concepci" in blob:
        return "plan especial"
    if "modificaci" in blob and "puntual" in blob:
        return "modificación puntual NUM"
    if "sector" in blob or re.search(r"st-n[oº°]\s*\d", blob):
...
def _sector_tokens(text: str) -> list[str]:
    ...
    low = (text or "").lower()
    for pat in (
        r"finca de la concepci[oó]n",
        r"st-n[oº°]\s*\d+",
        r"modificaci[oó]n puntual\s*n[oº°]?\s*\d+",
        r"sector\s*\d+",
    ):
...
class MatamalaDeAlmazanAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal 7 matamaladealmazan.es + sede espublico + PLAI JCYL + IDECyL WFS (partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-matamala-de-almazan/1.0")},
        )
