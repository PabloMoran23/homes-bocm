...
DRUPAL_BASE = "https://melgardefernamental.es"
SEDE_BASE = "https://melgardefernamental.sedelectronica.es"
...
ID_PREFIX = "melgar-de-fernamental"
PLAI_MUNICIPIO = 211
PLAI_PROVINCIA = 9
...
DEFAULT_INFO_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{SEDE_BASE}/citizen-service/c44d8302-4327-4ea9-8679-1810f910fa84",
        "servicio en línea OBRAS — licencias y urbanismo",
    ),
    (f"{DRUPAL_BASE}/arquitecto-municipal", "arquitecto municipal — urbanismo"),
    (f"{DRUPAL_BASE}/normativa", "normativa municipal"),
    (
        "http://www.jcyl.es/plau/lplanes.plau?municipio=09211",
        "archivo planeamiento JCyL (Melgar de Fernamental)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://melgardefernamental\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://melgardefernamental\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class MelgarDeFernamentalAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-melgar-de-fernamental/1.0")},
        )
