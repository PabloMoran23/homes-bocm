...
SEDE_BASE = "https://merindaddecuestaurrea.sedelectronica.es"
...
WFS_MUNICIPIO = "Merindad de Cuesta-Urria"
ID_PREFIX = "merindad-de-cuesta-urria"
PLAU_PROVINCIA = 9
PLAU_MUNICIPIO = 213
INE_CODE = "09213"
...
DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    (
        ...
        "Archivo de planeamiento urbanístico (PlanPublica JCyL)",
    ),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia={PLAU_PROVINCIA}&municipio={PLAU_MUNICIPIO}",
        "Información pública de planeamiento (PlanPublica)",
    ),
    (
        f"https://idecyl.jcyl.es/siur/index.html?id={INE_CODE}",
        "Visor SiUR — planeamiento urbanístico municipal",
    ),
    (
        f"http://www.jcyl.es/plau/lplanes.plau?municipio={INE_CODE}",
        "Catálogo JCyL de documentos de planeamiento",
    ),
    (
        f"{PLAU_BASE}/openDocuIndice.do?cDocId=284321",
        "NUM — Normas Urbanísticas Municipales (instrumento vigente)",
    ),
    (
        f"{SEDE_BASE}/catalog/",
        "Solicitud de licencia o autorización urbanística (sede indeterminada)",
    ),
    (
        f"{SEDE_BASE}/catalog/#declaracion",
        "Declaración responsable o comunicación en materia urbanística (sede indeterminada)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://merindaddecuestaurrea\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://merindaddecuestaurrea\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class MerindadDeCuestaUrriaAyuntamientoAdapter(AyuntamientoAdapter):
    """PlanPublica PLAU + IDECyL WFS (sede espublico indeterminada; sin web corporativa)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        plau_prov = int(self.config.get("plau_provincia") or self.config.get("planpublica_provincia") or PLAU_PROVINCIA)
        plau_mun = int(self.config.get("plau_municipio") or self.config.get("planpublica_municipio") or PLAU_MUNICIPIO)
        self.plau_url = str(
            self.config.get("plau_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia={plau_prov}&municipio={plau_mun}"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia={plau_prov}&municipio={plau_mun}"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-merindad-de-cuesta-urria/1.0")},
        )
