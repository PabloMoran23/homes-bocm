...
DRUPAL_BASE = "https://www.merindadderioubierna.es"
SEDE_BASE = "https://merindadderioubierna.sedelectronica.es"
...
ID_PREFIX = "merindad-de-rio-ubierna"
PLAI_MUNICIPIO = 229
PLAI_PROVINCIA = 9
...
DEFAULT_INFO_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{DRUPAL_BASE}/node/1302",
        "revisión provisional NORMAS URBANÍSTICAS MUNICIPALES (Drupal)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=229",
        "archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=09&municipio=229",
        "planeamiento en información pública (Junta CYL)",
    ),
    (
        "https://ovc.diputaciondeburgos.es/",
        "oficina virtual ciudadana Diputación de Burgos",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://merindadderioubierna\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://merindadderioubierna\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class MerindadDeRioUbiernaAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-merindad-de-rio-ubierna/1.0")},
        )
