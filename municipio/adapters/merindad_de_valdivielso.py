...
DRUPAL_BASE = "https://www.merindaddevaldivielso.es"
SEDE_BASE = "https://merindaddevaldivielso.sedelectronica.es"
...
ID_PREFIX = "merindad-de-valdivielso"
PLAI_MUNICIPIO = 217
PLAI_PROVINCIA = 9
...
DEFAULT_INFO_PAGES: list[tuple[str, str]] = [
    ...
    (f"{SEDE_BASE}/transparency", "transparencia municipal"),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=217",
        "archivo planeamiento urbanístico aprobado (JCyL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=09&municipio=217",
        "planeamiento en información pública (JCyL)",
    ),
    (
        "http://www.jcyl.es/plau/lplanes.plau?municipio=09217",
        "archivo planeamiento JCyL (Merindad de Valdivielso)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://merindaddevaldivielso\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://merindaddevaldivielso\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"informaci[oó]n p[uú]blica|expediente|dominio|inmatricul|proyecto de (?:urbaniz|actuaci)|"
    r"estudio de detalle|modificaci[oó]n|aprobaci[oó]n (?:inicial|definitiva)|reparcel|sector|"
    r"normas urban|num\b|instrumento|un-\d|su-\d|prat|pau\b|"
...
class MerindadDeValdivielsoAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal Toools + sede espublico + PLAI JCyL + IDECyL WFS (geometría partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.info_url = str(self.config.get("info_url") or f"{self.sede_base}/board")
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-merindad-de-valdivielso/1.0")},
        )
