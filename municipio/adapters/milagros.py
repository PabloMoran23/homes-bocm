...
WEB_BASE = "https://www.milagros.es"
SEDE_BASE = "https://milagros.sedelectronica.es"
...
RE_PREVIEW = re.compile(
    r'href="((?:https://milagros\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://milagros\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_SECTOR_CODE = re.compile(
    ...
    r"AP[- ]?UR[- ]?\d+|AP[- ]?RR[- ]?\d+|"
    r"SU[- ]?I[- ]?\d+|SUI[- ]?\d+|"
    r"SU[- ]?NC(?:\.?\s*ST\s*N[ºo°]?\s*\d+\s*SOD)?|"
    r"SU[- ]?NC\.?\s*(?:N[ºo°]\.?\s*)?\d+(?:[-/]\d+)?|"
    r"SUR\s*ST\s*N[ºo°]?\s*\d+\s*SOD|"
    r"SUNC[-\s]?\d+(?:[-/]\d+)?|"
    r"SECTOR\s+[A-Z0-9-]+|"
    r"\b[IU]-\d+\b|\bRIUC\b"
    r")\b",
...
class MilagrosAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-milagros/1.0")},
        )
    ...
    def _collect_noticia_links(self) -> list[str]:
        ...
        return [urllib.parse.urljoin(f"{self.web_base}/", path) for path in links]
    ...
    def _collect_noticias(self) -> list[dict[str, Any]]:
        ...
        for url in self._collect_noticia_links():
            ...
            if not rec:
                continue
            if not RE_PROYECTO.search(rec["titulo"]) and not RE_LICENCIA.search(rec["titulo"]):
                continue
            rows.append(rec)
        return rows
