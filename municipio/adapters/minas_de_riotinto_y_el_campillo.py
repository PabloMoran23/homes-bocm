...
SEDE_BASE = "https://minasderiotinto.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WEB_BASE = "https://www.aytoriotinto.es"
MUNICIPIO = "Minas de Riotinto y El Campillo"
ID_PREFIX = "minas-de-riotinto-y-el-campillo"

SITUA_SEARCH = "https://ws132.juntadeandalucia.es/situadifusion/pages/search.jsf"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WEB_BASE}/es/areas-tematicas/urbanismo/index.html",
...
RE_LICENCIA = re.compile(
    ...
    r"obra (?:mayor|menor)|establecimiento hosteler|certificado.*informe urban)",
)
RE_PROYECTO = re.compile(
    ...
    r"cambio de uso|ordenanza|normativa urban|gesti[oó]n integral|parques|jardines|"
    r"rat\b|registro.*actividades|tratamiento)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"cobranza iae|padrones|censo jurado|subvencion|plan normativo anual|"
    r"lista definitiva candidatos|monitor|empleo)",
)
RE_BOARD_ROW = re.compile(r'<tr[^>]*>\s*<td class="class_name".*?</tr>', re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://minasderiotinto\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
RE_EXPORT_HREF = re.compile(
    r'href=["\']((?:https://www\.aytoriotinto\.es)?/export/sites/riotinto/[^"\']+)["\']',
    re.I,
)
RE_SKIP_URL = re.compile(
    r"(?i)(file://|javascript:|mailto:|#|style-custom\.css|favicon|apple-touch)",
)
...
def _pdf_title(url: str) -> str:
    ...
    name = Path(path).stem.replace("_", " ").replace("%20", " ").replace("-", " ")
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "parques" in b or "jardines" in b:
        return "parques y jardines"
    if "rat" in b or "registro de actividades" in b:
...
class MinasDeRiotintoYElCampilloAyuntamientoAdapter(AyuntamientoAdapter):
    """Saga Suite aytoriotinto.es + sede espublico gestiona (tablón, transparencia)."""
    ...
    def _collect_web_documents(self) -> list[dict[str, Any]]:
        ...
        def add(url: str, titulo: str, blob: str, origen: str = "web_saga") -> None:
            if RE_SKIP_URL.search(url) or url in seen:
                return
            if url.startswith("/"):
                url = self._abs_web(url)
            ...
            rows.append(
                {
                    "url": url,
                    "titulo": titulo[:500],
                    "fecha": _fecha_from_blob(f"{titulo} {blob}"),
                    "blob": blob[:2000],
                    "origen": origen,
                }
            )
        ...
        for page_url in self.seed_pages:
            ...
            for m in RE_PDF_HREF.finditer(html):
                href = m.group(1)
                if RE_SKIP_URL.search(href):
                    continue
                abs_url = self._abs_web(href) if not href.startswith("http") else href
                title = _pdf_title(abs_url)
                blob = f"{title} {abs_url} urbanismo Minas de Riotinto"
                if not RE_PROYECTO.search(blob):
                    continue
                add(abs_url, title, blob)

            for m in RE_EXPORT_HREF.finditer(html):
                href = m.group(1)
        ...
        add(
            f"{self.web_base}/es/areas-tematicas/urbanismo/pgou/",
            "P.G.O.U — Plan General de Ordenación Urbana",
            "PGOU planeamiento Minas de Riotinto",
            origen="web_pgou",
        )
        add(
            SITUA_SEARCH,
            "PGOU Minas de Riotinto — consulta SITUADIFUSION (Junta de Andalucía)",
            "planeamiento general SITUA INE 21052",
            origen="situa",
        )
        add(
            f"{self.sede_base}/transparency",
            "Portal de transparencia — Urbanismo, Obras Públicas y Medio Ambiente",
            "transparencia urbanismo obras públicas medio ambiente",
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Catálogo de trámites — sede electrónica (URBANISMO)",
                "url": f"{self.sede_base}/dossier",
                "source": "ayuntamiento",
                "nota": "Incluye certificado/informe urbanístico y licencias vía sede",
                "origen": "sede_tramite",
            },
            {
            ...
            },
            {
                ...
                "source": "ayuntamiento",
                "nota": "Información urbanística y PGOU en portal Saga Suite",
    ...
    def _board_is_urban(self, row: dict[str, Any]) -> bool:
        ...
        if any(k in proc for k in ("planeamiento", "licencia", "urban", "actividad", "obra", "genérico")):
            return True
        if "urbanismo" in cat or "ordenanza" in cat:
            return True
    ...
    def _doc_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "url": row["url"],
            "source": "ayuntamiento",
            "origen": row.get("origen", "web_saga"),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "status": "ok",
            "tablon": sum(1 for r in rows if r.get("origen") == "tablon"),
            "web": sum(1 for r in rows if r.get("origen", "").startswith("web")),
