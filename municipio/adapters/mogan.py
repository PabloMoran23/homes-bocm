...
WEB_BASE = "https://www.mogan.es"
OAT_BASE = "https://oat.mogan.es:8448/ventanilla"
SEDE_BASE = "https://mogan.sedelectronica.es"
PLANEAMIENTO_URL = f"{WEB_BASE}/40-urbanismo/50-planeamiento"
URBANISMO_URL = f"{WEB_BASE}/40-urbanismo"
MUNICIPIO = "Mogán"
ID_PREFIX = "mogan"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-mogan"
SITCAN_DATASET_URL = f"https://opendata.sitcan.es/dataset/{SITCAN_PACKAGE}"
GEOBDP_MUNICIPIO = "https://geobdp.grafcan.es/core/municipios/35012/"

DEFAULT_SEED_PAGES: list[str] = [
    PLANEAMIENTO_URL,
    URBANISMO_URL,
    f"{WEB_BASE}/40-urbanismo/60-licencias",
]
...
class MoganAyuntamientoAdapter(AyuntamientoAdapter):
    """SITCAN + GEOBDP Grafcan; portal mogan.es (WAF) y sede OAT/espublico con acceso limitado en CI."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.oat_base = str(self.config.get("oat_base") or OAT_BASE).rstrip("/")
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-mogan/1.0")},
        )
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        oat_inicio = f"{self.oat_base}/web/inicioWebc.do?opcion=noreg"
        return [
            {
                "id": _stable_id("lic", oat_inicio),
                "fecha_concesion": None,
                "tipo": "trámite informativo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "OAT — sede electrónica (urbanismo, tablón de edictos)",
                "url": oat_inicio,
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
            {
                "id": _stable_id("lic", PLANEAMIENTO_URL),
                "fecha_concesion": None,
                "tipo": "trámite informativo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Portal municipal — planeamiento y licencias urbanísticas",
                "url": PLANEAMIENTO_URL,
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
            {
                "id": _stable_id("lic", SITCAN_DATASET_URL),
                "fecha_concesion": None,
                "tipo": "trámite informativo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "SITCAN — planeamiento urbanístico sistematizado de Mogán",
                "url": SITCAN_DATASET_URL,
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
        ]
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {"rows": len(rows), "status": "ok", "source": "sitcan_oat_portal"}
