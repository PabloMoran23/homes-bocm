...
WP_BASE = "https://www.mogarraz.es"
SEDE_BASE = "https://mogarraz.sedelectronica.es"
...
WORDPRESS_SEED_PAGES: list[tuple[str, str]] = [
    (f"{WP_BASE}/normas-urbanisticas/", "Normas Urbanísticas (web municipal)"),
    (
        f"{WP_BASE}/solicitud-de-licencia-de-obras-y-declaracion-responsable/",
        "Solicitud de Licencia de Obras y Declaración Responsable",
    ),
]

RE_WP_PDF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?mogarraz\.es)?[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
RE_PREVIEW = re.compile(
    r'href="(https://mogarraz\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://mogarraz\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class MogarrazAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress (normas PDF) + PlanPublica PLAU + IDECyL WFS; sede espublico sin DNS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio={PLANPUBLICA_MUN}"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio={PLANPUBLICA_MUN}"
        )
        self.wp_base = str(self.config.get("wp_base") or WP_BASE).rstrip("/")
        raw_wp = self.config.get("wordpress_seed_pages") or WORDPRESS_SEED_PAGES
        self.wordpress_seed_pages: list[tuple[str, str]] = [
            (str(a), str(b)) for a, b in raw_wp
        ]
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-mogarraz/1.0")},
        )
    ...
    def _absolutize_wp_url(self, href: str) -> str:
        href = unescape(href.strip())
        if href.startswith("//"):
            return f"https:{href}"
        if href.startswith("/"):
            return f"{self.wp_base}{href}"
        if href.startswith("http"):
            return href
        return f"{self.wp_base}/{href.lstrip('/')}"

    def _collect_wordpress_pdfs(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for page_url, page_title in self.wordpress_seed_pages:
            try:
                html = self._fetch(page_url)
            except (urllib.error.URLError, OSError, ConnectionError):
                continue
            for m in RE_WP_PDF.finditer(html):
                pdf_url = self._absolutize_wp_url(m.group(1))
                if pdf_url in seen:
                    continue
                seen.add(pdf_url)
                name = unescape(pdf_url.rsplit("/", 1)[-1])
    ...
    def _collect_wordpress_pages(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for url, titulo in self.wordpress_seed_pages:
            rows.append({"titulo": titulo, "url": url, "origen": "wordpress_page"})
        return rows

    def _wp_pdf_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        rec: dict[str, Any] = {
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_plai_seed():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_wordpress_pdfs():
            rec = self._wp_pdf_to_proyecto(item)
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_board():
