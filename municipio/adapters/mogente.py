...
WEB_BASE = "https://www.moixent.es"
SEDE_BASE = "https://moixent.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
MUNICIPIO = "Mogente"
ID_PREFIX = "mogente"

URBANISMO_URL = f"{WEB_BASE}/es/pagina/urbanismo"
IMPRESOS_URL = f"{WEB_BASE}/es/pagina/impresos-urbanismo"
TRANSPARENCY_URBANISMO_URL = (
    "https://moixent.sedelectronica.es/transparency/991457ff-f304-40d5-8815-2b9820cb5a4c/"
)

DEFAULT_AVISO_SEEDS: tuple[str, ...] = (
    "/es/noticia-aviso/plan-urbano-actuacion-municipal-puam",
    "/es/noticia-aviso/plan-movilidad-urbana-sostenible-pmus",
    "/es/noticia-aviso/alegaciones-contra-proyecto-central-fotovoltaica-para-estacion-bombeo-moixent",
)
...
RE_LICENCIA = re.compile(
    ...
    r"obra (?:mayor|menor)|compatibilidad urban|ambiental funcionamiento)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general|urba)|pgou|puam|pmus|convenio|"
    r"informaci[oó]n p[uú]blica|consulta (?:p[uú]blica|pr[eè]via)|expedient|proyecto|modificaci[oó]n|"
    r"reparcel|estudio (?:ac[uú]stico|ambiental|de detalle)|memoria|planos|dogv|edicto|"
    r"aprobaci[oó]n (?:inicial|definitiva|provisional)|parcela|suelo|sector|"
    r"cambio de uso|normas subsidiarias|homologaci[oó]n|ordenanza|fotovolt|planta solar|"
    r"central fotovoltaica|disposici[oó]n normativa|pla urb[aà]|mobilitat|movilidad)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|planificaci[oó]n i ordenaci[oó]n de personal|"
    r"nombramiento|nomenament|convocatoria.*empleo|empleo p[uú]blico|"
    r"cobrança|padro fiscal|padr[oó] fiscal|subvenci[oó]n|concessi[oó] de subvencions|"
    r"calendari fiscal|festiu local|premis del|beques del)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://moixent\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_AVISO_LINK = re.compile(r'href="(/es/noticia-aviso/[^"#?]+)"', re.I)
...
def _proyecto_tipo(blob: str) -> str:
    n = blob.lower()
    if "puam" in n or "pla urb" in n:
        return "plan urbanístico de actuación"
    if "pmus" in n or "mobilitat" in n or "movilidad" in n:
        return "plan de movilidad urbana"
    if "fotovolt" in n or "planta solar" in n:
        return "instalación fotovoltaica"
    if "modificaci" in n and "pgou" in n:
        return "modificación PGOU"
    if "pgou" in n or "planeam" in n:
        return "planeamiento"
    if "informaci" in n and "pública" in n:
        return "información pública"
    return "urbanismo"


class MogenteAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal 10 Portales (moixent.es) + sede espublico gestiona (tablón / transparencia)."""
    ...
    def _fetch(self, url: str, retries: int = 3) -> str:
        ua = self.config.get("user_agent", "poc-bocm-mogente/1.0")
    ...
    def _discover_aviso_paths(self) -> list[str]:
        ...
        for page in self.seed_pages:
            try:
                html = self._fetch(page if page.startswith("http") else self._abs_web(page))
            except urllib.error.URLError:
                continue
            for m in RE_AVISO_LINK.finditer(html):
                paths.add(m.group(1))
    ...
    def _parse_aviso_page(self, path: str) -> dict[str, Any] | None:
        ...
        titulo = re.sub(r"\s*\|.*Moixent.*$", "", titulo, flags=re.I).strip()
        ...
        return [
            {
                ...
                "municipio": MUNICIPIO,
                "titulo": "Urbanismo — PGOU y subvención digitalización planeamiento",
                "fecha": "2025-09-22",
                "tipo": "planeamiento",
                "url": URBANISMO_URL,
                "source": "ayuntamiento",
                "origen": "web_urbanismo",
                "nota": "Texto refundido PGOU financiado DOGV 10010/2024",
            },
            {
                "id": _stable_id("proy", self.transparency_url),
                "municipio": MUNICIPIO,
                "titulo": "Portal transparencia — documentación planeamiento urbanístico",
                "fecha": None,
                "tipo": "planeamiento",
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Impresos urbanismo — licencias de obra y actividad",
                "url": IMPRESOS_URL,
                "source": "ayuntamiento",
                "nota": "Modelos PDF (licencia obra, DR, segregación, compatibilidad urbanística)",
                "origen": "web_tramite",
            },
            {
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        proc = (row.get("procedimiento") or "").lower()
        cat = (row.get("categoria") or "").lower()
        if not RE_PROYECTO.search(blob) and "planeamiento" not in proc and "urban" not in cat:
            if "disposici" not in proc and "normativa" not in proc:
                return None
        key = row.get("expediente") or row["url"]
        return {
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tablon": sum(1 for r in rows if r.get("origen") == "tablon"),
            "drupal": sum(1 for r in rows if r.get("origen") == "drupal_aviso"),
            "static": sum(1 for r in rows if r.get("origen") in ("web_urbanismo", "transparencia")),
        }
