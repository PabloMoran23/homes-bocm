...
SEDE_BASE = "https://moguer.sedelectronica.es"
BOARD_URL = (
    f"{SEDE_BASE}/board/9753e838-f59b-11de-b600-00237da12c6a/"
)
WEB_BASE = "https://www.aytomoguer.es"
URBANISMO_URL = f"{WEB_BASE}/es/servicios/urbanismo/"
PLANEAMIENTO_URL = f"{WEB_BASE}/es/planeamiento-urbanistico-de-moguer/"
PLANEAMIENTO_MUNICIPAL_URL = (
    f"{WEB_BASE}/es/planeamiento-urbanistico-de-moguer/planeamiento-municipal/"
)
CITIZEN_SERVICE_URL = (
    f"{SEDE_BASE}/citizen-service/17e16d13-06d2-420a-acf8-9e27134736b4"
)
...
RE_PROYECTO = re.compile(
    ...
    r"aprobaci[oó]n (?:inicial|definitiva|provisional)|parcela|suelo|sector|mazagon|"
    ...
    r"adaptaci[oó]n parcial|texto refundido|ordenaci[oó]n|alturas|aparcamiento)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"cobranza iae|padrones|modificaci[oó]n.*presupuest|bolsa de empleo|"
    r"activa-t joven|punto vuela|formador|protecci[oó]n civil|barrendero|"
    r"arquitecto|administrativo)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://moguer\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https://www\.aytomoguer\.es)?/export/sites/moguer[^"\']+\.(?:pdf|PDF)[^"\']*)["\']',
    re.I,
)
RE_PAGE_HREF = re.compile(
    r'href=["\']((?:https://www\.aytomoguer\.es)?/es/(?:planeamiento|servicios/urbanismo)[^"\']*)["\']',
    re.I,
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "nnss" in b or "normas subsidiarias" in b:
        return "NNSS"
    if "adaptaci" in b and "parcial" in b:
        return "adaptación parcial planeamiento"
    if "modificaci" in b and ("nnss" in b or "mazagon" in b):
    ...
    if "red viaria" in b or "aparcamiento" in b:
        return "red viaria"
    if "abastecimiento" in b or "saneamiento" in b:
        return "infraestructuras urbanas"
    if "ordenanza" in b or "normativa" in b or "texto refundido" in b:
...
class MoguerAyuntamientoAdapter(AyuntamientoAdapter):
    """SAGA Diputación Huelva (galerías PDF) + sede espublico gestiona (tablón)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-moguer/1.0")},
        )
    ...
    def _collect_web_pdfs(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited_pages) < 20:
            ...
            page_url = self._abs_web(page_url)
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "id": _stable_id("lic", CITIZEN_SERVICE_URL),
                "fecha_concesion": None,
                "tipo": "información obras y urbanismo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Obras y Urbanismo — declaraciones responsables y licencias",
                "url": CITIZEN_SERVICE_URL,
                "source": "ayuntamiento",
                "nota": "Página informativa sede: licencias, comunicaciones previas, actuaciones en suelo rústico",
                "origen": "sede_tramite",
            },
            {
    ...
    def _situa_row(self) -> dict[str, Any]:
        return {
            ...
            "source": "ayuntamiento",
            "origen": "situa",
            "nota": "Visor regional SituaDIFusión enlazado desde web municipal",
        }
