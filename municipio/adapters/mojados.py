...
SEDE_BASE = "https://mojados.sedelectronica.es"
...
DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    (
        f"{SEDE_BASE}/catalog/t/1406ed04-c9ea-4840-acc2-d8acfc94a2e6",
        "Declaración Responsable de Obras y Usos",
    ),
    (
        f"{SEDE_BASE}/catalog/t/2b88dac8-2367-4794-baea-f4e843217131",
    ...
    (
        f"{SEDE_BASE}/catalog/t/bf5e9be9-4ba9-4bbd-86cd-2f915f8b3c78",
...
RE_PREVIEW = re.compile(
    r'href="(https://mojados\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://mojados\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class MojadosAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=47&municipio=090"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=47&municipio=090"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-mojados/1.0")},
        )
