...
SEDE_BASE = "https://molacillos.sedelectronica.es"
MUNICIPIO = "Molacillos"
ID_PREFIX = "molacillos"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=49&municipio=119",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=49&municipio=119",
        "Archivo planeamiento urbanístico aprobado (Junta CYL) — NUM",
    ),
]
...
RE_PREVIEW = re.compile(
    r'href="(https://molacillos\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
class MolacillosAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico tablón + PlanPublica JCYL + IDECyL WFS sectores (Zamora)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-molacillos/1.0")},
        )
