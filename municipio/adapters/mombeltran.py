...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WEB_BASE}/planeamientourbanistico.shtml",
    f"{WEB_BASE}/tramites.shtml",
    f"{SEDE_BASE}/board",
    f"{PLAU_BASE}/searchVPubDocMuniPlauPrint.do?provincia=05&municipio=132&bInfoPublica=N",
    "https://idecyl.jcyl.es/siur/index.html?id=05132",
]

RE_PREVIEW = re.compile(
    r'href="(https://mombeltran\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra mayor|obra menor|"
    r"primera ocupaci[oó]n|vado)",
)
RE_PROYECTO = re.compile(
    ...
    r"edicto|actuaci[oó]n urban|normas urban|instrumento|evaluaci[oó]n de impacto|num\b|"
    r"regularizaci[oó]n de suelo)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(pleno|convocatoria|empleo|subvenci[oó]n deportiv|cert\.|ordenanza fiscal|"
    r"perfil contratante|fiestas|carnaval|deporte|turismo|biblioteca|guarder[ií]a)",
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "sector" in n:
        return "sector"
    if "regularizaci" in n and "suelo" in n:
        return "regularización suelo urbano"
    if "impacto ambiental" in n:
        return "evaluación ambiental"
    return "planeamiento"


class MombeltranAyuntamientoAdapter(AyuntamientoAdapter):
    """Web estática mombeltran.es + sede espublico + PlanPublica JCyL + IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-mombeltran/1.0")},
        )
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            (f"{self.web_base}/tramites.shtml", "trámites — licencia municipal de obras"),
            (
                f"{self.web_base}/pdf/tramites/3_%20solicitud_de_licencia_de_obra.pdf",
                "formulario — solicitud licencia municipal de obras",
            ),
            (
                f"{self.web_base}/pdf/tramites/solicitud_de_licencia_de_alta_de_vados.pdf",
                "formulario — licencia alta de vados",
            ),
            (self.board_url, "tablón de anuncios — sede electrónica"),
            (f"{self.sede_base}/dossier", "trámites — sede electrónica"),
            (self._plau_print_url(), "archivo planeamiento urbanístico (Junta CYL)"),
        ]
        rows: list[dict[str, Any]] = []
    ...
    def _planeamiento_page_proyecto(self) -> dict[str, Any]:
        url = f"{self.web_base}/planeamientourbanistico.shtml"
        return {
            "id": _stable_id("proy", url),
            "municipio": MUNICIPIO,
            "titulo": "Planeamiento urbanístico — enlaces SiuCyL y PlanPublica",
            "fecha": None,
            "tipo": "planeamiento",
            "url": url,
            "source": "ayuntamiento",
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        add(self._planeamiento_page_proyecto())
        for item in self._collect_board():
        ...
        return {
            ...
            "web_page": sum(1 for r in rows if r.get("origen") == "web_page"),
            "tablon": sum(1 for r in rows if r.get("origen") == "tablon"),
        }
