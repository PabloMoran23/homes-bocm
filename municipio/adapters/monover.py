...
SEDE_BASE = "https://monovar.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WP_BASE = "https://www.monovar.es"
MUNICIPIO = "Monòver"
ID_PREFIX = "monover"
INE_MUNICIPIO = "03090"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/urbanismo/",
    f"{WP_BASE}/plan-general-de-ordenacion-urbana-de-monovar/",
    f"{WP_BASE}/propuesta-plan-general-estructural-2019-2a-exposicion-publica/",
    f"{WP_BASE}/plan-especial-vertedero-de-inertes/",
]
...
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://monovar\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
class MonoverAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress (PGOU/urbanismo) + sede espublico gestiona; ICV InventarioSuSuz sin ámbitos INE 03090."""
    ...
    def _fetch(self, url: str, *, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-monover/1.0")},
        )
    ...
    def _load_wfs_ambitos(self) -> list[dict[str, Any]]:
        ...
        if (self.config.get("geometry") or {}).get("skip_wfs_scan"):
            self._wfs_cache = []
            return self._wfs_cache
    ...
    def _collect_seed_docs(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.seed_pages:
            ...
            for m in re.finditer(
                r'href="((?:https?://(?:www\.)?monovar\.es)?/wp-content/uploads/[^"]+\.(?:pdf|zip))"',
                html,
                re.I,
            ):
                href = m.group(1)
                if href.startswith("http"):
                    doc_url = unescape(href)
                else:
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo — modelos de licencias y declaraciones responsables",
                "url": f"{WP_BASE}/urbanismo/",
                "source": "ayuntamiento",
                "nota": "PDFs de solicitud de licencia, DR y trámites (sin registro histórico público)",
                "origen": "web_tramite",
            },
            {
    ...
    def _seed_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        rec = {
            ...
            "url": row["url"],
            "source": "ayuntamiento",
            "origen": row.get("origen") or "wordpress_pgou",
        }
        self._attach_geometry(rec)
        return rec
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "wordpress_pgou": sum(1 for r in rows if r.get("origen") == "wordpress_pgou"),
            "tablon": sum(1 for r in rows if r.get("origen") == "tablon"),
