...
SEDE_BASE = "https://ayto-moraleja.sedelectronica.es"
WP_BASE = "https://ayto-moraleja.es"
MUNICIPIO = "Moraleja de Enmedio"
ID_PREFIX = "moraleja-de-enmedio"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "MORALEJA DE ENMEDIO"

URBANISMO_URL = (
    f"{SEDE_BASE}/citizen-service/2c9b5472-33f1-48d9-af01-ae5ae82bd19d"
)

RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|edicto.*licencia|vado)",
)
RE_PROYECTO = re.compile(
    ...
    r"edicto|bocm|convocatoria.*pleno|acuerdo plenario|obra p[uú]blica|"
    r"unidad(?:es)? de (?:actuaci[oó]n|ejecuci[oó]n)|"
    r"\b(?:UE|UA|SUNP|SNUP|S)-[\w\d.-]+\b)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|iae\b|plusval[ií]a|calendario fiscal|jurado|presupuest|"
    r"selecci[oó]n de personal|funcionario|bii\b|residuos|ivtm|mercadillo|cajero)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|UA|SUNP|SNUP|S)-[\w\d.-]+)\b",
)
...
def _proyecto_tipo(blob: str) -> str:
    n = blob.lower()
    if re.search(r"(?i)\bua-\d+\b", blob) or "unidad de actuaci" in n:
        return "unidad de actuación"
    if re.search(r"(?i)\bs-\d", blob) or "sector" in n:
        return "sector PGOU"
    if "informaci" in n or "exposici" in n:
        return "información pública"
    if re.search(r"(?i)convocatoria.*pleno|acuerdo plenario", blob):
...
class MoralejaDeEnmedioAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona (tablón) + WFS SIT CM (ámbitos PGOU con geometría)."""
    ...
    def _geometry_payload(self, name: str, merged: dict[str, Any]) -> dict[str, Any]:
        cql = (
    ...
    def _fetch_geometry(self, titulo: str) -> dict[str, Any] | None:
        ...
        if not merged:
            return None
        return self._geometry_payload(best_name, merged)
    ...
    def _collect_sit_ambitos(self) -> list[dict[str, Any]]:
        ...
        for f in feats:
            ...
            if merged:
                rec.update(self._geometry_payload(name, merged))
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
                "nota": "Catálogo de procedimientos licencias y declaraciones responsables",
                "origen": "urbanismo_sede",
            },
            {
            ...
            },
            {
                ...
                "fecha_concesion": None,
                "tipo": "trámites municipales",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Trámites municipales (web corporativa)",
                "url": f"{self.wp_base}/tramites",
                "source": "ayuntamiento",
                "nota": "Información general de trámites; sin listado de licencias concedidas",
                "origen": "wp_tramites",
            },
        ]
    ...
    def _collect_transparency_info(self) -> dict[str, Any]:
        return {
            ...
            "source": "ayuntamiento",
            "nota": "41 documentos en sección 7 (Wicket AJAX; consulta manual)",
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "info": sum(
                ...
                for r in rows
                if r.get("origen")
                in ("sede_tablon", "sede_tramite", "wp_tramites", "urbanismo_sede")
            ),
        }
