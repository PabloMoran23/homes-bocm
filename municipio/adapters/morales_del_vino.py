...
WEB_BASE = "https://www.moralesdelvino.es"
SEDE_BASE = "https://aytomoralesdelvino.sedelectronica.es"
PLAU_BASE = "https://servicios.jcyl.es/PlanPublica"
MUNICIPIO = "Morales del Vino"
ID_PREFIX = "morales-del-vino"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=49&municipio=138",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=49&municipio=138",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        f"{WEB_BASE}/instancias-de-solicitudes/",
        "Instancias de solicitudes (declaración responsable de obras)",
    ),
]
...
RE_PREVIEW = re.compile(
    r'href="(https://aytomoralesdelvino\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://aytomoralesdelvino\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"ordenanza.*urban|tasa.*urban|pgou|normas urban|"
...
class MoralesDelVinoAyuntamientoAdapter(AyuntamientoAdapter):
    """Web WordPress + sede espublico + PlanPublica JCYL + IDECyL WFS (Tierra del Vino)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=49&municipio=138"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=49&municipio=138"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-morales-del-vino/1.0")},
        )
    ...
    def _tramite_to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        titulo = row.get("titulo") or row.get("tipo") or ""
        url = row.get("url") or ""
        ...
        return {
            ...
            "origen": row.get("origen") if isinstance(row, dict) else "sede_tramite",
        }
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for url, tipo in self.licencia_pages:
            rec = self._tramite_to_licencia({"url": url, "titulo": tipo, "origen": "sede_tramite"})
