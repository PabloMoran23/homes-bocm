...
WP_BASE = "https://www.moralzarzal.es"
SEDE_BASE = "https://carpeta.moralzarzal.es/eAdmin"
TRANSP_BASE = "https://transparencia.moralzarzal.es"
TRAMITES_BASE = "https://tramites.moralzarzal.es"
MUNICIPIO = "Moralzarzal"
ID_PREFIX = "moralzarzal"

TABLON_ALL = f"{SEDE_BASE}/Tablon.do?action=verAnuncios"
URBANISMO_URL = f"{WP_BASE}/urbanismo/"
PLANEAMIENTO_URL = f"{TRANSP_BASE}/urbanismo-y-medio-ambiente/planeamiento/"
URBANISMO_TRAMITES = f"{TRAMITES_BASE}/urbanismo-vivienda-y-medio-ambiente/"
...
DEFAULT_TRAMITE_URLS: list[str] = [
    f"{TRAMITES_BASE}/urbanismo-vivienda-y-medio-ambiente/licencia-urbanistica-mayor/",
    f"{TRAMITES_BASE}/urbanismo-vivienda-y-medio-ambiente/licencias-urbanisticas/",
    f"{TRAMITES_BASE}/urbanismo-vivienda-y-medio-ambiente/declaracion-responsable:-Primera-ocupacion-y-funcionamiento/",
    f"{TRAMITES_BASE}/urbanismo-vivienda-y-medio-ambiente/reapertura-de-piscinas-comunitarias/",
    f"{TRAMITES_BASE}/urbanismo-vivienda-y-medio-ambiente/contenedores/",
]
...
RE_EXPTE = re.compile(r"(?i)(?:MC\s+)?([0-9]+-[0-9]+/[0-9]{4})")
RE_PERIOD = re.compile(r"(\d{2}/\d{2}/\d{4})\s*-\s*(\d{2}/\d{2}/\d{4})")
RE_LICENCIA = re.compile(
    r"(?i)(solicitud de licencia|licencia (?:de |urban|municipal|instalaci|funcionamiento|apertura)|"
    r"notificaci[oó]n.*licencia|edicto.*licencia|declaraci[oó]n responsable|comunicaci[oó]n previa|"
    r"viabilidad urban|titulo habilitante|cambio de titularidad|primera ocupaci[oó]n)",
)
RE_PROYECTO = re.compile(
    ...
    r"orden de ejecuci|modificaci[oó]n|estudio (?:ac[uú]stico|ambiental)|api-|z\d+-p|"
    r"proyecto de|catalogo|memoria|normas urban|bando|obra[s]? berrocal|gesti[oó]n urban)",
)
RE_SKIP = re.compile(
    r"(?i)(presupuesto|modificaci[oó]n presupuestaria|cuenta general|calendario fiscal|"
    r"iae\b|infofma|igualdad|convocatoria.*plaza|jurado|padr[oó]n|incendios forestales)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:API|UE|UA|AD|AN|AI|PAU|Z\d+)-[\w./-]+)\b",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_PDF_HREF = re.compile(r'href="([^"]+\.pdf[^"]*)"', re.I)
RE_TRAMITE_LINK = re.compile(
    r'<a[^>]+href="(https://tramites\.moralzarzal\.es/[^"]+)"[^>]*>([^<]{5,200})</a>',
    re.I,
)
...
def _proyecto_tipo(title: str, default: str = "urbanismo") -> str:
    n = title.lower()
    if re.search(r"\bapi-\d", n) or re.search(r"\bz\d+-p", n):
        return "planeamiento"
    if "informaci" in n:
...
def _sector_ilike_parts(text: str) -> list[str]:
    s = text.strip()
    low = s.lower()
    for marker in (" del pgou", " pgou", " bocm", " aprob", " anuncio", " nnss", " moralzarzal"):
        if marker in low:
...
class MoralzarzalAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress + eAdmin sede (tablón) + trámites WP + transparencia + SITCM WFS (partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.tramites_base = str(self.config.get("tramites_base") or TRAMITES_BASE).rstrip("/")
        self.search_terms = list(self.config.get("search_terms") or DEFAULT_SEARCH_TERMS)
        self.seed_pages = [str(u) for u in (self.config.get("seed_pages") or DEFAULT_SEED_PAGES)]
        self.tramite_urls = [str(u) for u in (self.config.get("tramite_urls") or DEFAULT_TRAMITE_URLS)]
    ...
    def _fetch(self, url: str, data: bytes | None = None) -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-moralzarzal/1.0"),
        }
        if data is not None:
        ...
        with urllib.request.urlopen(req, timeout=60) as resp:
            ...
            if "Tablon.do" in url or "carpeta.moralzarzal.es" in url:
                charset = "iso-8859-1"
    ...
    def _extract_seed_proyectos(self, html: str, page_url: str) -> list[dict[str, Any]]:
        ...
        for m in re.finditer(r'href="([^"]+\.zip)"', html, re.I):
            zip_url = self._abs_url(m.group(1), page_url)
            if zip_url in seen:
                continue
            if "nnss" not in zip_url.lower():
                continue
            name = Path(zip_url).stem
            title = f"NNSS Moralzarzal — {name}"
            seen.add(zip_url)
            rows.append(
                {
    ...
    def _collect_tramites_licencias(self) -> list[dict[str, Any]]:
        ...
        seen: set[str] = set()
        try:
            html = self._fetch(URBANISMO_TRAMITES)
        except urllib.error.URLError:
            html = ""
        for m in RE_TRAMITE_LINK.finditer(html):
            url, title = m.group(1), _clean_title(m.group(2))
            if "urbanismo-vivienda-y-medio-ambiente" in url and url.rstrip("/").endswith(
                "urbanismo-vivienda-y-medio-ambiente"
            ):
                continue
            if "ver tr" in title.lower():
                continue
            if not RE_LICENCIA.search(title) and "licencia" not in url.lower():
                continue
            if url in seen:
                continue
            seen.add(url)
            rows.append(
                {
                    ...
                    "tipo": title[:120] or "trámite urbanístico",
                    "distrito": None,
                    "lat": None,
                    "lon": None,
                    "titulo": title or url.split("/")[-2].replace("-", " "),
                    "url": url,
                    "source": "ayuntamiento",
        ...
        rows.append(
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo — información y contacto",
                "url": URBANISMO_URL,
                "source": "ayuntamiento",
