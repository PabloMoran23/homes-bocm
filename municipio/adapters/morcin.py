...
DEFAULT_SEED_PAGES: list[str] = [
    ...
    f"{BASE}/plan-parcial-de-argame",
    f"{BASE}/clasificacion-zonificacion",
]

DEFAULT_LICENCIA_PAGES: list[str] = [
    f"{BASE}/normativa-urbanistica1",
    f"{BASE}/normativa-municipal",
]
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgo[u]?|pgou|convenio|"
    r"informaci[oó]n p[uú]blica|expediente|proyecto|modificaci[oó]n|reparcel|"
    r"estudio (?:ac[uú]stico|ambiental|de detalle|de implantaci[oó]n)|normas? (?:provisionales|subsidiarias)|"
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|peri\b|parteayer|argame|zonific|clasificaci|"
    r"instrumento|gesti[oó]n urban|n[uú]cleo rural|memoria|plano)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://www\.morcin\.es)?/documents/[^"]+)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Morc[ií]n.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3 and t.lower() not in ("navegación", "navigation"):
                return t[:500]
    return fallback
...
class MorcinAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay morcin.es + RPGUR Asturias (concejo 38); geometría parcial WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "38")
    ...
    def _fetch(self, url: str, *, data: bytes | None = None, encoding: str = "utf-8") -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-morcin/1.0"),
        }
        if data is not None:
    ...
    def _load_geom_index(self) -> dict[str, dict[str, Any]]:
        ...
        params = {
            ...
            "srsName": "EPSG:4326",
            "CQL_FILTER": "Instrumento LIKE '%MORC%'",
        }
    ...
    def _crawl_liferay_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in RE_DOC_LINK.finditer(html):
                ...
                if re.fullmatch(r"[0-9a-f-]{30,}", title, re.I):
                    parts = urllib.parse.unquote(href).split("/documents/")[-1].split("/")
                    for part in parts[1:-1]:
                        if part and not re.fullmatch(r"[0-9a-f-]{30,}", part, re.I):
            ...
            for m in re.finditer(r'href="(https://www\.morcin\.es/[^"#?]+)"', html, re.I):
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        info_url = f"{BASE}/normativa-urbanistica1"
        ...
        rows.append(
            {
                ...
                "url": info_url,
                "source": "ayuntamiento",
                "nota": "Sin tablón público; sede electrónica inaccesible (TLS)",
            }
        )
                    ...
                    rows.append(
                        {
                            ...
                            "url": page_url,
                            "source": "ayuntamiento",
                            "nota": "Página informativa; sede electrónica inaccesible (TLS)",
                        }
                    )
