...
SEDE_BASE = "https://mucientes.sedelectronica.es"
WP_BASE = "https://www.mucientes.es"
...
RE_PREVIEW = re.compile(
    r'href="(https://mucientes\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://mucientes\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class MucientesAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Divi + sede espublico gestiona + PlanPublica PLAU + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.wp_planeamiento_url = str(
            self.config.get("wp_planeamiento_url")
            or f"{WP_BASE}/archivo-de-planeamiento-urbanistico-y-ordenacion-del-territorio-de-mucientes/"
        )
        self.plau_url = str(
            self.config.get("plau_url")
            or (
                f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N"
                f"&provincia={PLAU_PROVINCIA}&municipio={PLAU_MUNICIPIO:03d}"
            )
        )
        self.plai_url = str(
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-mucientes/1.0")},
        )
    ...
    def _collect_wp_seed(self) -> list[dict[str, Any]]:
        return [
            {
                "titulo": "Archivo de Planeamiento Urbanístico y Ordenación del Territorio de Mucientes",
                "url": self.wp_planeamiento_url,
                "fecha": "2020-12-16",
                "origen": "web_wordpress",
            }
        ]
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_plai_seed():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_wp_seed():
            rec = self._tramite_to_proyecto(item)
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
