...
SEDE_BASE = "https://ayto-muelasdelpan.sedelectronica.es"
MUNICIPIO = "Muelas del Pan"
ID_PREFIX = "muelas-del-pan"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=49&municipio=135",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=49&municipio=135",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/openDocuIndice.do?cDocId=281831",
        "Normas Urbanísticas Municipales (NUM)",
    ),
    (
        "https://www.ayto-muelasdelpan.com/tablonurbanismo.htm",
        "Tablón de urbanismo histórico (web municipal; desde 2018 en sede)",
    ),
    (
        "https://ayto-muelasdelpan.sedelectronica.es/transparency/c7a77dba-7233-4ce0-8f01-30c136dc94d7/",
        "Transparencia — normas urbanísticas (sede)",
    ),
]
...
RE_PREVIEW = re.compile(
    r'href="(https://ayto-muelasdelpan\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"ordenanza.*urban|tasa.*urban|suelo urbano|s\.u\.n\.c|"
...
class MuelasDelPanAyuntamientoAdapter(AyuntamientoAdapter):
    """Web municipal + sede espublico + PlanPublica JCyL + IDECyL WFS (Zamora)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-muelas-del-pan/1.0")},
        )
