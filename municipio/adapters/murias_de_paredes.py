...
WEB_BASE = "https://www.aytomuriasdeparedes.es"
SEDE_BASE = "https://aytomuriasdeparedes.sedelectronica.es"
MUNICIPIO = "Murias de Paredes"
ID_PREFIX = "murias-de-paredes"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    ...
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=102",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=102",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://aytomuriasdeparedes\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://aytomuriasdeparedes\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class MuriasDeParedesAyuntamientoAdapter(AyuntamientoAdapter):
    """OpenCms web (Diputación León) + sede espublico tablón + IDECyL WFS instrumentos/planes."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-murias-de-paredes/1.0")},
        )
