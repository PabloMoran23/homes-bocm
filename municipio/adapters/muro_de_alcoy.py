...
SEDE_BASE = "https://vilademuro.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WP_BASE = "https://www.vilademuro.net"
MUNICIPIO = "Muro de Alcoy"
ID_PREFIX = "muro-de-alcoy"
INE_MUNICIPIO = "03092"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/el-ayuntamiento/concejalias/urbanisme/planejament/",
    f"{WP_BASE}/es/el-ayuntamiento/concejalias/urbanisme/planejament/",
    f"{WP_BASE}/portal-de-transparencia/urbanisme-i-medi-ambient/",
    f"{WP_BASE}/es/catalogo-de-procedimientos/",
]
...
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://vilademuro\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
class MuroDeAlcoyAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress vilademuro.net (PGOU) + sede espublico gestiona + ICV WFS InventarioSuSuz (geometría partial)."""
    ...
    def _fetch(self, url: str, *, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-muro-de-alcoy/1.0")},
        )
    ...
    def _collect_seed_docs(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.seed_pages:
            ...
            for m in re.finditer(
                r'href="((?:https://www\.vilademuro\.net)?/wp-content/uploads/[^"]+\.(?:pdf|zip))"',
                html,
                re.I,
            ):
                href = m.group(1)
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "id": _stable_id("lic", f"{WP_BASE}/es/catalogo-de-procedimientos/"),
                "fecha_concesion": None,
                "tipo": "catálogo trámites (web municipal)",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Catálogo de procedimientos — Urbanismo y licencias",
                "url": f"{WP_BASE}/es/catalogo-de-procedimientos/",
                "source": "ayuntamiento",
                "nota": "Trámites de licencias y comunicaciones previas vía sede electrónica",
                "origen": "web_tramite",
            },
            {
    ...
    def _board_is_urban(self, row: dict[str, Any]) -> bool:
        ...
        if any(
            ...
            return True
        if any(k in cat for k in ("urbanisme", "urbanismo")):
            return True
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not RE_PROYECTO.search(blob) and not any(
            k in proc for k in ("planeamiento", "planejament", "genérico", "genèric")
        ):
            return None
        key = row.get("expediente") or row["url"]
        rec = {
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "web_pgou": sum(1 for r in rows if r.get("origen") == "web_pgou"),
            "tablon": sum(1 for r in rows if r.get("origen") == "tablon"),
