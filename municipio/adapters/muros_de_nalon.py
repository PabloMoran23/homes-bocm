...
BASE = "https://www.murosdenalon.es"
SEDE_BASE = "https://murosdenalon.sedelectronica.e-ayuntamiento.es"
RPGUR_BASE = "https://www54.asturias.es/rpgur/action/publico"
MUNICIPIO = "Muros de Nalón"
ID_PREFIX = "muros-de-nalon"
...
WFS_FILTER = "Instrumento LIKE '%MUROS%'"

DEFAULT_SEED_PAGES: list[str] = [
    f"{BASE}/urbanismo-y-vivienda",
    f"{BASE}/normativa-urbanistica",
    f"{BASE}/pgou",
    f"{BASE}/catalogo-urbanistico",
    f"{BASE}/revision-parcial",
    f"{BASE}/inventario-de-caminos-aprobacion-inicial",
]

DEFAULT_LICENCIA_PAGES: list[str] = [
    f"{BASE}/urbanismo-y-vivienda",
    f"{BASE}/normativa-urbanistica",
    f"{SEDE_BASE}/tablondeanuncios/default.aspx",
]
...
RE_PROYECTO = re.compile(
    ...
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|inventario|caminos|revisi[oó]n parcial)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://www\.murosdenalon\.es)?/documents/[^"]+\.pdf[^"]*|'
    r'https?://(?:www\.)?(?:asturias\.es|sede\.asturias\.es)/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Muros.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
def _tipo_proyecto(denominacion: str, clasificacion: str = "") -> str:
    ...
    if "inventario" in blob and "camin" in blob:
...
class MurosDeNalonAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay murosdenalon.es + RPGUR Asturias + sede tablón RSS; geometría parcial WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "39")
    ...
    def _fetch(self, url: str, *, data: bytes | None = None, encoding: str = "utf-8") -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-muros-de-nalon/1.0"),
        }
        if data is not None:
    ...
    def _crawl_liferay_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in re.finditer(r'href="(https://www\.murosdenalon\.es/[^"#?]+)"', html, re.I):
    ...
    def _collect_tablon_rss(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        url = f"{SEDE_BASE}/tablondeanuncios/tablon_rss.aspx"
        try:
            xml = self._fetch(url, encoding="iso-8859-1")
        except urllib.error.URLError:
            return rows
        for m in RE_RSS_ITEM.finditer(xml):
            title = unescape(m.group(1).strip())
            link = m.group(2).strip()
            fecha = _parse_rss_date(m.group(3).strip())
            if not RE_LICENCIA.search(title) and not RE_PROYECTO.search(title):
                continue
            kind = "lic" if RE_LICENCIA.search(title) else "proy"
            rows.append(
                {
                    "id": _stable_id(kind, link),
                    "fecha_concesion": fecha if kind == "lic" else None,
                    "tipo": "anuncio tablón",
                    "distrito": None,
                    "lat": None,
                    "lon": None,
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        for rec in self._collect_tablon_rss():
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)

        info_pages = [
            (f"{BASE}/urbanismo-y-vivienda", "Urbanismo y vivienda"),
            (f"{BASE}/normativa-urbanistica", "Normativa urbanística"),
            (f"{SEDE_BASE}/tablondeanuncios/default.aspx", "Tablón de anuncios — sede electrónica"),
        ]
