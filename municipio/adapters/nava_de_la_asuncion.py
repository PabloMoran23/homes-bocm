...
WEB_BASE = "https://navadelaasuncion.org"
SEDE_BASE = "https://navadelasuncion.sedelectronica.es"
PLANPUBLICA_BASE = "https://servicios.jcyl.es/PlanPublica"
WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/wfs"
SIUR_VISOR = "https://idecyl.jcyl.es/siur/index.html?id=40138"
MUNICIPIO = "Nava de la Asunción"
ID_PREFIX = "nava-de-la-asuncion"
WFS_C_MUN = "40138"
PLAN_PROVINCIA = "40"
PLAN_MUNICIPIO = "138"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        f"{WEB_BASE}/ayuntamiento/oficina-virtual/sede-electronica/",
    ...
    (SIUR_VISOR, "Visor urbanístico SIUR (IDECyL)"),
]

RE_PREVIEW = re.compile(
    r'href="((?:https://navadelasuncion\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://navadelasuncion\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"unidad(?:es)? de ejecuci[oó]n|suelo urbano|su[- ]?nc|s\.u\.n\.c|nn\.?ss)",
)
...
def _normalize_sector_code(raw: str) -> str:
    ...
    code = re.sub(r"AP[- ]?UR[- ]?", "AP-UR-", code)
    code = re.sub(r"AP[- ]?RR[- ]?", "AP-RR-", code)
    return code
...
class NavaDeLaAsuncionAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress navadelaasuncion.org + sede espublico + PlanPublica JCyL + IDECyL WFS (partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-nava-de-la-asuncion/1.0")},
        )
    ...
    def _sede_available(self, html: str) -> bool:
        return "Sede Electrónica Indeterminada" not in html and len(html) > 2000
    ...
    def _collect_board(self) -> list[dict[str, Any]]:
        ...
        for url, origen in ((self.board_url, "tablon"), (self.info_url, "info_tablon")):
            try:
                ...
                continue
            if not self._sede_available(html):
                continue
    ...
    def _collect_wp_posts(self) -> list[dict[str, Any]]:
        ...
        page = 1
        while page <= 5:
            url = f"{self.web_base}/wp-json/wp/v2/posts?per_page=100&page={page}"
            try:
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for url, titulo in self.seed_urls:
            ...
            rec = {
                ...
                "url": url,
                "source": "ayuntamiento",
                "nota": "Página informativa de sede; tablón no accesible (sede indeterminada)",
                "origen": "web_tramite",
            }
