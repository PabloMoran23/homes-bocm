...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=145",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=145",
...
RE_PREVIEW = re.compile(
    r'href="((?:https://navasdeoro\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://navasdeoro\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class NavasDeOroAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay DipSegovia (Segovia11) + sede espublico + PLAI JCYL + geometría parcial IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-navas-de-oro/1.0")},
        )
