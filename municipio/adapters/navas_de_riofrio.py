...
WEB_BASE = "https://www.navasderiofrio.es"
SEDE_BASE = "https://navasderiofrio.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (f"{WEB_BASE}/urbanismo", "Urbanismo — documentación NUM y planos"),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=146",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=146",
...
RE_PREVIEW = re.compile(
    r'href="((?:https://navasderiofrio\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://navasderiofrio\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_URBANISMO_DOC = re.compile(
    r'(?:title="([^"]+)"[^>]*href="(/documents/[^"]+)"|href="(/documents/[^"]+)"[^>]*title="([^"]+)")',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"unidad(?:es)? de ejecuci[oó]n|ue\s*\d|subasta|parcela|fotovolta|caminos rurales|"
    r"ordenanza municipal|memoria informativa|planos de ordenaci)",
)
RE_NOISE = re.compile(
    ...
    r"censo de candidatos|tribunal del jurado|iae|impuesto sobre actividades|"
    r"notificaci[oó]n colectiva|cobranza del periodo|ibir|tasas 202)",
)
...
RE_SECTOR = re.compile(
    ...
    r"la f[aá]brica|prados|camino de aguilafuente|piscinas|eras|ermita|"
    r"virgen de la estrella|el pinar|eras arriba)",
)
...
def _proyecto_tipo(title: str, instrumento: str = "") -> str:
    ...
    if "informaci" in blob or "consulta p" in blob:
    ...
    if "caminos rurales" in blob or "ordenanza" in blob:
        return "ordenanza"
    if "memoria informativa" in blob or "planos de ordenaci" in blob:
        return "planeamiento"
    return "planeamiento"
...
def _sector_tokens(text: str) -> list[str]:
    ...
    for pat in (
        r"la f[aá]brica",
        r"prados",
        r"camino de aguilafuente",
        r"piscinas",
        r"eras(?:\s+i)?",
        r"ermita",
        r"virgen de la estrella",
        r"el pinar",
        r"eras arriba",
        r"u[1-6]",
        r"d[4-5]",
...
class NavasDeRiofrioAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay Segovia8 + sede espublico + PLAI JCYL + geometría parcial IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-navas-de-riofrio/1.0")},
        )
    ...
    def _collect_urbanismo_docs(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for path in ("/es/urbanismo", "/urbanismo"):
            url = f"{self.web_base}{path}"
            try:
                html = self._fetch(url)
            except urllib.error.URLError:
                continue
            for m in RE_URBANISMO_DOC.finditer(html):
                if m.group(1):
                    titulo, path = unescape(m.group(1).strip()), m.group(2)
                else:
                    path, titulo = m.group(3), unescape(m.group(4).strip())
                if path in seen:
                    continue
                seen.add(path)
                doc_url = path if path.startswith("http") else f"{self.web_base}{path}"
                rows.append(
                    {
                        "titulo": titulo[:500],
                        "url": doc_url,
    ...
    def _generic_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        rec: dict[str, Any] = {
            ...
            "titulo": row["titulo"],
            "fecha": row.get("fecha"),
            "tipo": _proyecto_tipo(blob, row.get("instrumento", "")),
            "url": row["url"],
            "source": "ayuntamiento",
            "origen": row.get("origen"),
        }
