...
WEB_BASE = "https://www.novelda.es"
SEDE_BASE = "https://sede.novelda.es"
MUNICIPIO = "Novelda"
ID_PREFIX = "novelda"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WEB_BASE}/el-ayuntamiento/concejalias/urbanismo/",
    f"{WEB_BASE}/agenda-urbana-novelda-2030/",
]

LICENCIA_TRAMITE_PATTERNS: tuple[str, ...] = (
    "licencia urbanística para obras mayores",
    "licencia de obra",
    "declaración responsable para obras",
    "comunicación previa",
    "comunicación de actividades",
    "autorización municipal para realizar obras",
    "ocupación vía pública con obras",
    "licencia para la apertura de establecimientos",
    "autorización de suministros",
    "licencia para obras y construcciones",
)
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|pge|convenio urban|"
    r"informaci[oó]n p[uú]blica|exposici[oó]n p[uú]blica|expediente|edicto|"
    r"reparcel|estudio de detalle|modificaci[oó]n|sector|compatibilidad urban|"
    r"integraci[oó]n paisag|participaci[oó]n p[uú]blica|fotovolta|salinetes|"
    r"uzi|uzo|uze|ubo|nnss|v[ií]as ciclist)",
)
RE_NOISE = re.compile(
    r"(?i)(proceso selectivo|oposici[oó]n|plaza de|empleo p[uú]blico|"
    r"presupuest|subvenci[oó]n|huertos urbanos|cobranza|iae|bolsa de|"
    r"calificaciones provisionales|t[eé]cnico omac|integraci[oó]n social)",
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "plan parcial" in n or "sector" in n or re.search(r"\buz[ioe]\b", n):
        return "plan parcial"
    if "estudio de detalle" in n or "integraci" in n:
        return "estudio de integración"
...
class NoveldaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress web + sede STA TAO (tablón/catálogo) + ICV InventarioSuSuz WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.tablon_url = str(
            self.config.get("tablon_url")
            or f"{self.sede_base}/sta/CarpetaPublic/doEvent?APP_CODE=STA&PAGE_CODE=PTS_TABLON&lang=ES"
        )
        self.catalogo_url = str(
            self.config.get("catalogo_url")
            or f"{self.sede_base}/sta/CarpetaPublic/doEvent?APP_CODE=STA&PAGE_CODE=PTS_CATSERV&lang=ES"
        )
        ...
        self.ine_mun_alt = str(geom_cfg.get("cod_ine_mun_alt") or INE_MUN_ALT)
        self.mun_name = str(geom_cfg.get("mun_name") or MUNICIPIO)
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-novelda/1.0")},
        )
    ...
    def _tablon_rows(self) -> list[dict[str, Any]]:
        ...
        for item in self._extract_sta_dataset(html, "PTS_TABLON"):
    ...
    def _wfs_matches_municipio(self, props: dict[str, Any]) -> bool:
        cod = props.get("cod_ine_mun")
        name = str(props.get("noms_mun") or "").strip().lower()
        if name == self.mun_name.lower():
            return True
        return cod in {self.ine_mun, self.ine_mun_alt}
    ...
    def _parse_wfs_feature(self, feat_el: ET.Element) -> dict[str, Any] | None:
        ...
        if not self._wfs_matches_municipio(props):
            return None
        ...
        rec: dict[str, Any] = {
            ...
            "url": f"{self.web_base}/el-ayuntamiento/concejalias/urbanismo/",
            "source": "ayuntamiento",
            "origen": "icv_wfs",
