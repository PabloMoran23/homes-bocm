...
WP_BASE = "https://ayto-nuevobaztan.es"
SEDE_BASE = "https://nuevobaztan.sedelectronica.es"
MUNICIPIO = "Nuevo Baztán"
ID_PREFIX = "nuevo-baztan"

DOCUMENTACION_URL = f"{WP_BASE}/documentacion-y-tramites/"
PLANEAMIENTO_URL = f"{WP_BASE}/urbanismo/"
TRANSPARENCIA_URL = (
    "https://transparenciaayto-nuevobaztan.eadministracion.es/"
    "transparencia/funcionamiento/urbanismo/planes-urbanisticos-y-estudios-de-impacto-ambiental"
)

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "NUEVO BAZTÁN"

RE_PREVIEW = re.compile(
    r'href="(https://nuevobaztan\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://nuevobaztan\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_SITEMAP_LOC = re.compile(r"<loc>(https://ayto-nuevobaztan\.es/[^<]+)</loc>", re.I)
...
RE_WP_EXCLUDE = re.compile(
    r"(?i)(campamento urbano|arte urbano|autob[uú]s|mapa concesional|"
    r"mercadillo|cine|fiestas|cabalga|empleo|concurso|deporte|movilidad urbana|"
    r"iluminaci[oó]n urbana|mobiliario urbano|vandalismo.*pistas)",
)
...
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|AD|AN|AI|PAU|S|SAU)-\d+[A-Z0-9-]*)\b",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_YMD = re.compile(r"\b((?:19|20)\d{2})(\d{2})(\d{2})\b")
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?ayto-nuevobaztan\.es)?/(?:download|wp-content)[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
class NuevoBaztanAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Divi (documentación + noticias) + sede espublico gestiona + ámbitos SIT WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.transparency_url = str(
            self.config.get("transparency_url")
            or f"{self.sede_base}/citizen-service/ef8a54f8-49a6-43c3-af0a-d9b64b4142d4"
        )
        self.transparencia_portal_url = str(
            self.config.get("transparencia_portal_url") or TRANSPARENCIA_URL
        )
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-nuevo-baztan/1.0")},
        )
    ...
    def _post_url_to_proyecto(self, post_url: str) -> dict[str, Any] | None:
        slug = post_url.rstrip("/").rsplit("/", 1)[-1]
        if RE_WP_EXCLUDE.search(slug):
            return None
        ...
        title = _strip_html(title_m.group(1)) if title_m else slug.replace("-", " ")
        if title.lower().startswith("herramientas de accesibilidad"):
            title = slug.replace("-", " ")
        if RE_WP_EXCLUDE.search(title):
            return None
        pub_m = RE_PUBLISHED.search(html)
        fecha = _iso_date_wp(pub_m.group(1)) if pub_m else _fecha_from_blob(slug)
        rec: dict[str, Any] = {
            "id": _stable_id("proy", post_url),
            "municipio": MUNICIPIO,
            "titulo": title[:500],
            "fecha": fecha,
    ...
    def _wpfd_url_to_proyecto(self, wpfd_url: str) -> dict[str, Any] | None:
        slug = wpfd_url.rstrip("/").rsplit("/", 1)[-1].replace("-", " ")
        if not RE_LICENCIA.search(slug) and not RE_PROYECTO.search(slug):
            return None
        if RE_LICENCIA.search(slug) and not RE_PROYECTO.search(slug):
            return None
        titulo = slug[:500]
        rec: dict[str, Any] = {
            "id": _stable_id("proy", wpfd_url),
            "municipio": MUNICIPIO,
            "titulo": titulo,
            "fecha": _fecha_from_blob(wpfd_url),
    ...
    def _collect_wp_proyectos(self) -> list[dict[str, Any]]:
        ...
        post_urls: list[str] = []
        for sm in ("post-sitemap.xml", "post-sitemap2.xml"):
            post_urls.extend(self._collect_sitemap_urls(sm))
        for post_url in post_urls:
            rec = self._post_url_to_proyecto(post_url)
            if rec:
                by_id[rec["id"]] = rec
        for wpfd_url in self._collect_sitemap_urls("wpfd_file-sitemap.xml"):
            rec = self._wpfd_url_to_proyecto(wpfd_url)
            if rec:
                by_id[rec["id"]] = rec
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "fecha_concesion": None,
                "tipo": "portal transparencia urbanismo sede",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Sede electrónica — URBANISMO y ACTIVIDADES",
            ...
            {
                "id": _stable_id("lic", self.transparencia_portal_url),
                "fecha_concesion": None,
                "tipo": "portal transparencia planeamiento",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Portal transparencia — planes urbanísticos",
                "url": self.transparencia_portal_url,
                "source": "ayuntamiento",
                "nota": "Sección planes urbanísticos en portal eadministracion",
                "origen": "transparencia_portal",
            },
        ]
    ...
    def _load_wfs_ambitos(self) -> dict[str, dict[str, Any]]:
        ...
        feats = self._wfs_query(f"DS_MUNICIPIO LIKE '{muni[:10]}%'", count=120)
    ...
    def _fetch_geometry(self, titulo: str) -> dict[str, Any] | None:
        ...
        if parts:
            ...
            feats = self._wfs_query(
                f"DS_MUNICIPIO LIKE '{muni[:10]}%' AND DS_NOMB_AMB ILIKE '{pattern}'",
                count=10,
            )
        ...
        cql = (
            f"DS_MUNICIPIO LIKE '{self.wfs_municipio[:10].replace(chr(39), chr(39)*2)}%' "
