...
SEDE_BASE = "https://olivaresdeduero.sedelectronica.es"
...
DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    ...
    (f"{SEDE_BASE}/transparency", "Transparencia — Urbanismo, obras públicas y medio ambiente"),
]

RE_PREVIEW = re.compile(
    r'href="(https://olivaresdeduero\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://olivaresdeduero\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class OlivaresDeDueroAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + PlanPublica PLAU + IDECyL WFS (web Diputación inaccesible)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=47&municipio=107"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=47&municipio=107"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-olivares-de-duero/1.0")},
        )
    ...
    def _collect_transparency(self) -> list[dict[str, Any]]:
        ...
        for block in re.split(r"<tr", html)[1:]:
            if "preview-document" not in block:
                continue
            link_m = re.search(
                r"(https://olivaresdeduero\.sedelectronica\.es)?/preview-document/[a-f0-9-]+",
                block,
                re.I,
            )
            if not link_m:
            ...
            titulo = cells[0] if cells else url
            rows.append(
                {
                    "titulo": titulo[:500],
                    "url": url,
    ...
    def _transparency_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        blob = row.get("titulo", "")
        if not RE_PROYECTO.search(blob) and not re.search(
            r"(?i)obra|paviment|urbaniz|plan\b", blob
        ):
            return None
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_transparency():
            rec = self._transparency_to_proyecto(item)
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_board():
