...
DRUPAL_BASE = "https://www.olvega.es"
SEDE_BASE = "https://olvega.sedelectronica.es"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{DRUPAL_BASE}/tablon-de-edictos",
    f"{DRUPAL_BASE}/modificacion-puntual-v-de-las-normas-urbanisticas-municipales",
    f"{DRUPAL_BASE}/modificacion-puntual-vi-normas-urbanisticas-municipales",
    f"{DRUPAL_BASE}/edicto-modificacion-puntual-vii-de-las-normas-urbanisticas",
    f"{DRUPAL_BASE}/edicto-estudio-de-detalle-s-l-1",
    f"{DRUPAL_BASE}/aprobacion-leyes-urbanisticas-2-de-diciembre-de-2016",
    f"{DRUPAL_BASE}/solicitud-licencia-de-obras-con-proyecto",
    f"{DRUPAL_BASE}/solicitud-licencia-de-obras-sin-proyecto",
    f"{DRUPAL_BASE}/actualidad/anuncio-de-informacion-publica-sobre-solicitud-de-autorizacion-administrativa-previa-y",
    f"{DRUPAL_BASE}/actualidad/anuncio-de-informacion-publica-de-la-modificacion-puntual-del-plan-parcial",
    f"{DRUPAL_BASE}/actualidad/anuncio-de-informacion-publica-de-la-modificacion-puntual-del-estudio-de-detalle",
    f"{DRUPAL_BASE}/actualidad/aprobacion-inicial-modificacion-puntual-xi",
    f"{DRUPAL_BASE}/actualidad/anuncio-boletin-oficial-provincial-de-soria-sobre-concesion-de-licencia-ambiental",
]

RE_PREVIEW = re.compile(
    r'href="(https://olvega\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://olvega\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_DRUPAL_PDF = re.compile(
    r'href="((?:https://www\.olvega\.es)?/sites/olvega\.es/files/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"concentraci[oó]n parcelaria|regad[ií]o|sector s[- ]?[0-9]+|si[- ]?[0-9]+|"
    r"modificaci[oó]n puntual|estudio de detalle|plan parcial|num\b|augustobriga|"
    r"fotovoltaica|planta solar)",
)
RE_NOISE = re.compile(
...
class OlvegaAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal 7 olvega.es + sede espublico + PLAI JCYL + IDECyL WFS (partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-olvega/1.0")},
        )
