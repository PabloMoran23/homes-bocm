...
SEDE_BASE = "https://sede.oviedo.es"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{TRANSPARENCIA_BASE}/urbanismo-y-medio-ambiente/pgo",
    f"{TRANSPARENCIA_BASE}/urbanismo-y-medio-ambiente/planeamiento-de-desarrollo-planes-parciales-planes-especiales-estudios-de-detalle-y-estudios-de-implantacion/planes-parciales",
    f"{TRANSPARENCIA_BASE}/urbanismo-y-medio-ambiente/planeamiento-de-desarrollo-planes-parciales-planes-especiales-estudios-de-detalle-y-estudios-de-implantacion/planes-especiales",
    f"{TRANSPARENCIA_BASE}/urbanismo-y-medio-ambiente/planeamiento-de-desarrollo-planes-parciales-planes-especiales-estudios-de-detalle-y-estudios-de-implantacion/estudios-de-detalle",
    f"{TRANSPARENCIA_BASE}/urbanismo-y-medio-ambiente/actuaciones-urbanisticas-en-tramitacion",
    f"{TRANSPARENCIA_BASE}/urbanismo-y-medio-ambiente/convenios-urbanisticos",
    f"{WEB_BASE}/urbanismo",
    f"{WEB_BASE}/vive/urbanismo-e-infraestructuras/pgo",
]

DEFAULT_LICENCIA_PAGES: list[str] = [
    f"{SEDE_BASE}/tramites/licencias",
    f"{TRANSPARENCIA_BASE}/urbanismo-y-medio-ambiente/licencias-urbanisticas",
    f"{WEB_BASE}/urbanismo",
]
...
RE_PROYECTO = re.compile(
    ...
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|aus[_ ]|peri )",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_EXPEDIENTE = re.compile(r"\b(1191[-/]\d{6}|\d{3,4}[-/]\d{6})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://(?:www\.)?oviedo\.es|https://transparencia\.oviedo\.es)?'
    r'/documents/[^"]+(?:\.pdf|-pdf)[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Oviedo.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
def _tipo_proyecto(denominacion: str, clasificacion: str = "") -> str:
    ...
    if "pgo" in blob or "plan general" in blob or "pgou" in blob:
        return "PGO"
    if "plan especial" in blob or "pe " in blob:
        return "plan especial"
    if "plan parcial" in blob or "aus" in blob or "peri" in blob:
        return "plan parcial"
...
class OviedoAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay oviedo.es + transparencia + RPGUR Asturias; geometría parcial WFS."""

    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        super().__init__(slug, config, base_url or WEB_BASE)
        self.delay_s = float(self.config.get("request_delay_s", 0.35))
        self.rpgur_base = str(self.config.get("rpgur_base") or RPGUR_BASE).rstrip("/")
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "44")
        ...
        self.max_crawl_pages = int(self.config.get("max_crawl_pages", 25))
        self.max_rpgur_pages = int(self.config.get("max_rpgur_pages", 30))
    ...
    def _fetch(self, url: str, *, data: bytes | None = None, encoding: str = "utf-8") -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-oviedo/1.0"),
        }
        if data is not None:
    ...
    def _abs_url(self, href: str, base: str = TRANSPARENCIA_BASE) -> str:
    ...
    def _load_geom_index(self) -> dict[str, dict[str, Any]]:
        ...
        params = {
            ...
            "srsName": "EPSG:4326",
            "CQL_FILTER": "Instrumento LIKE '%OVIEDO%'",
        }
    ...
    def _best_fecha(self, detail: dict[str, str], denominacion: str, expediente: str = "") -> str | None:
        for key in (
            ...
            if parsed:
                return parsed
        return _iso_from_year(f"{denominacion} {expediente}")
    ...
    def _doc_title_from_href(self, href: str) -> str:
    ...
    def _crawl_liferay_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in RE_DOC_LINK.finditer(html):
                ...
                base = TRANSPARENCIA_BASE if "transparencia" in page_url else WEB_BASE
                link = self._abs_url(href, base) if href.startswith("/") else href
                key = link.split("?")[0]
                if key in seen_docs:
                    continue
                seen_docs.add(key)
                title = self._doc_title_from_href(link)
                ...
                if exp_m:
                    rec["expediente"] = exp_m.group(1).replace("/", "-")
                rows.append(rec)
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        tramite_pages = [
            (f"{SEDE_BASE}/tramites/licencias", "Licencias urbanísticas y planeamiento (sede)"),
            (
                f"{SEDE_BASE}/tramites/licencias/08-licencia-obra-mayor-y-otras-actuaciones",
                "Solicitud de obra mayor y otras actuaciones",
            ),
            (
                f"{TRANSPARENCIA_BASE}/urbanismo-y-medio-ambiente/licencias-urbanisticas",
                "Licencias urbanísticas (transparencia)",
            ),
        ]
        for page_url, titulo in tramite_pages:
            ...
            rows.append(
                {
                    ...
                    "url": page_url,
                    "source": "ayuntamiento",
                    "nota": "Sin tablón público de concesiones; página informativa de trámites",
                }
            )
                    ...
                    rows.append(
                        {
                            ...
                            "url": page_url,
                            "source": "ayuntamiento",
                            "nota": "Página informativa; sin listado de licencias concedidas",
                        }
                    )
