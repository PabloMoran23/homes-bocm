...
WP_BASE = "https://pedrezuela.info"
WP_API = f"{WP_BASE}/wp-json/wp/v2"
SEDE_BASE = "https://pedrezuela.sedelectronica.es"
MUNICIPIO = "Pedrezuela"
ID_PREFIX = "pedrezuela"

DOCUMENTACION_URL = f"{WP_BASE}/tramites/"
PLANEAMIENTO_URL = f"{WP_BASE}/areas/urbanismo/"
URBANISMO_SEED_PAGES: list[str] = [
    PLANEAMIENTO_URL,
    f"{WP_BASE}/areas/urbanismo/licencias-de-obras/",
    f"{WP_BASE}/areas/urbanismo/actuaciones-municipales/",
    f"{WP_BASE}/areas/urbanismo/plan-general-de-ordenacion-urbana/",
    f"{WP_BASE}/areas/urbanismo/normas-subsidiarias/",
    f"{WP_BASE}/areas/urbanismo/eficiencia-energetica/actuaciones/",
    f"{WP_BASE}/areas/urbanizaciones/",
]

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "PEDREZUELA"

RE_PREVIEW = re.compile(
    r'href="(https://pedrezuela\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://pedrezuela\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?pedrezuela\.info)?/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
class PedrezuelaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Genesis (urbanismo + trámites) + sede espublico gestiona + ámbitos SIT WFS."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-pedrezuela/1.0")},
        )
