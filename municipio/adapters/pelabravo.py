...
SEDE_BASE = "https://pelabravo.sedelectronica.es"

RE_PREVIEW = re.compile(
    r'href="(https://pelabravo\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://pelabravo\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class PelabravoAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona: tablón HTML + catálogo trámites urbanismo."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-pelabravo/1.0")},
        )
    ...
    def _parse_board_links(self, html: str, origen: str) -> list[dict[str, Any]]:
        """Fallback para extractos sin tabla completa (p. ej. /info)."""
        rows: list[dict[str, Any]] = []
        for m in RE_PREVIEW.finditer(html):
            ...
            if len(spans) >= 3:
                # heurística: últimos spans antes del enlace
    ...
    def _tramite_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if RE_LICENCIA.search(row["titulo"]) and not re.search(
            ...
            return None
        tipo = "planeamiento" if re.search(r"(?i)planeam|actuaci[oó]n urban", row["titulo"]) else "trámite urbanismo"
        return {
