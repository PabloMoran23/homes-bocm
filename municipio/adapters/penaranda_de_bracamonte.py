...
SEDE_BASE = "https://penarandadebracamonte.sedelectronica.es"
MUNICIPIO = "Peñaranda de Bracamonte"
ID_PREFIX = "penaranda-de-bracamonte"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=37&municipio=224",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=37&municipio=224",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
    (
        "https://www.bracamonte.es/areas-municipales/urbanismo/",
        "Urbanismo — Ayuntamiento de Peñaranda de Bracamonte",
    ),
    (
        "https://www.bracamonte.es/documentos/normas-urbanisticas/",
        "Normas urbanísticas municipales",
    ),
    (
        "https://www.bracamonte.es/documentos/plan-especial-del-conjunto-historico/",
        "Plan especial del conjunto histórico",
    ),
]

RE_PREVIEW = re.compile(
    r'href="((?:https://penarandadebracamonte\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://penarandadebracamonte\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class PenarandaDeBracamonteAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress bracamonte.es (PDFs) + sede espublico + PlanPublica + IDECyL WFS (geometría partial)."""
    ...
    def _bootstrap_session(self) -> None:
        ...
        req = urllib.request.Request(
            self.board_url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-penaranda-de-bracamonte/1.0")},
        )
        try:
