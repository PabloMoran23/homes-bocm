...
WEB_BASE = "https://www.ayto-peralestajuna.org"
SEDE_BASE = "https://ayto-peralestajuna.sedelectronica.es"
MUNICIPIO = "Perales de Tajuña"
ID_PREFIX = "perales-de-tajuna"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "PERALES DE TAJUÑA"

URBANISMO_URL = f"{WEB_BASE}/paginas/urbanismo"

DEFAULT_LICENCIA_TRAMITES: list[dict[str, str]] = [
    {"nombre": "Declaración responsable", "url": f"{SEDE_BASE}/info.0"},
    {"nombre": "Licencia de obras", "url": f"{SEDE_BASE}/info.0"},
    {"nombre": "Primera ocupación", "url": f"{SEDE_BASE}/info.0"},
    {"nombre": "Cédula urbanística", "url": f"{SEDE_BASE}/info.0"},
    {"nombre": "Certificado urbanístico", "url": f"{SEDE_BASE}/info.0"},
    {"nombre": "Informe urbanístico", "url": f"{SEDE_BASE}/info.0"},
]
...
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|ponp|nnss|normas subsidiarias|"
    r"informaci[oó]n p[uú]blica|exposici[oó]n p[uú]blica|expediente|proyecto|"
    r"modificaci[oó]n|aprobaci[oó]n|reparcel|sector|edicto|bocm|valdeperales|"
    r"estaci[oó]n de recarga|autov[ií]a)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|iae\b|plusval[ií]a|calendario fiscal|jurado|presupuest|"
    r"selecci[oó]n de personal|funcionario|canon de regularizaci[oó]n|orve\b|"
    r"cobranza.*iae|baja del padr[oó]n)",
)
...
def _sector_ilike_parts(text: str) -> list[str]:
    s = text.strip()
    low = s.lower()
    for marker in (" del pgou", " pgou", " bocm", " aprob", " anuncio", " perales"):
        if marker in low:
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "ponp" in n or "valdeperales" in n:
        return "plan parcial"
...
class PeralesDeTajunaAyuntamientoAdapter(AyuntamientoAdapter):
    """Neosoft web (planeamiento PDFs) + sede espublico gestiona (tablón) + WFS SITCM."""
    ...
    def _collect_planeamiento_docs(self) -> list[dict[str, Any]]:
        ...
        for href, label_raw in RE_PDF_LINK.findall(html):
            ...
            pdf_url = urljoin(f"{self.web_base}/", unescape(href))
            if pdf_url in seen:
                continue
            seen.add(pdf_url)
            titulo = label if len(label) > 5 else Path(unescape(href)).name
            rows.append(
                {
                    "titulo": titulo[:500],
        ...
        if "Documento Avance del Plan General" in html:
            key = "avance-pgou"
            if key not in seen:
                rows.append(
                    {
                        "titulo": "Documento Avance del Plan General de Ordenación Urbana",
                        "fecha": None,
                        "tipo": "planeamiento",
                        "url": self.urbanismo_url,
                        "origen": "planeamiento_web",
