...
SEDE_BASE = "https://pesqueradeduero.sedelectronica.es"
...
RE_PREVIEW = re.compile(
    r'href="(https://pesqueradeduero\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://pesqueradeduero\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"SUNC\s*\d+|S\.?\s*U\.?\s*\.?\s*N\.?\s*C\.?\s*\d+|"
    r"S\.?\s*U\.?R?B?L?E?\s*\d+|S\d+|"
    r"PE[- ]?\d+|AR[- ]?P\d+|UNC\d+|"
...
class PesqueraDeDueroAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + PlanPublica PLAI/PLAU + IDECyL WFS (sin web corporativa)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or (
                ...
                f"bInfoPublica=N&provincia={self.plai_provincia}&municipio={self.plai_municipio:03d}"
            )
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or (
                f"{PLAU_BASE}/searchVPubDocMuniPlai.do?"
                f"bInfoPublica=S&provincia={self.plai_provincia}&municipio={self.plai_municipio:03d}"
            )
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-pesquera-de-duero/1.0")},
        )
    ...
    def _plai_page_url(self, offset: int) -> str:
        ...
        return f"{PLAU_BASE}/searchVPubDocMuniPlai.do?{urllib.parse.urlencode(params)}"
    ...
    def _collect_plai_seed(self) -> list[dict[str, Any]]:
        ...
        for page in range(self.plai_max_pages):
            ...
            url = self._plai_page_url(offset) if page else self.plai_url
            try:
                html = self._fetch(url)
            except (urllib.error.URLError, OSError, ConnectionError):
                break
            parsed = self._parse_plau_rows(html, url)
            if not parsed:
                break
            for item in parsed:
                key = (item.get("doc_id") or "") + item["title"]
