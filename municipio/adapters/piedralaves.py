...
WEB_BASE = "https://www.piedralaves.es"
SEDE_BASE = "https://piedralaves.sedelectronica.es"
MUNICIPIO = "Piedralaves"
ID_PREFIX = "piedralaves"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WEB_BASE}/normas-urbanisticas-de-piedralaves",
    f"{WEB_BASE}/ayuntamiento/tramites-solicitudes",
    f"{SEDE_BASE}/board",
    "https://idecyl.jcyl.es/siur/index.html?id=05187",
]

RE_PREVIEW = re.compile(
    r'href="((?:https://piedralaves\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?piedralaves\.es)?/sites/default/files/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"edicto|bando.*parcel|actuaci[oó]n urban|urbanizaci[oó]n|instrumento|memoria|planos|"
    r"ruina|declaraci[oó]n de ruina|bocyl)",
)
RE_EXCLUDE = re.compile(
    ...
    r"forest|limpieza viaria|prevenci[oó]n de riesgos)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_YM = re.compile(r"/(?:uploads|documents|files)/(\d{4})/(\d{2})/")
...
def _proyecto_tipo(title: str) -> str:
    n = title.lower()
    if "nnss" in n or "normas urban" in n or "nnuu" in n:
    ...
    if "sector" in n or re.search(r"s\.u\.", n):
        return "sector"
    return "urbanismo"


class PiedralavesAyuntamientoAdapter(AyuntamientoAdapter):
    """Urbimedia/Drupal web + sede espublico tablón + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-piedralaves/1.0")},
        )
    ...
    def _enrich_geometry(self, rec: dict[str, Any]) -> None:
        ...
        wfs_normas = next(
            (
                r
                for r in self._collect_wfs_proyectos()
                if r.get("wfs_layer") == "urbanismo:plau_cyl_instrumentos_ambito"
            ),
            None,
        )
        if wfs_normas and RE_PROYECTO.search(titulo):
            for key in (
                "geom_geojson",
                "geometry_source",
                "geometry_source_url",
                ...
                if wfs_normas.get(key) is not None:
                    rec[key] = wfs_normas[key]
    ...
    def _collect_seed_pdfs(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.seed_pages:
            if page_url.startswith(self.sede_base):
                continue
            try:
            ...
            for m in RE_PDF_HREF.finditer(html):
                ...
                if not RE_PROYECTO.search(blob) and not RE_LICENCIA.search(blob):
                    if "normas" not in page_url.lower() and "tramite" not in page_url.lower():
                        continue
                rows.append(
                    {
                        "titulo": titulo[:500],
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            ...
            (
                f"{self.web_base}/ayuntamiento/tramites-solicitudes",
                "trámites y solicitudes — instancia general y modelos",
            ),
            (
                f"{SEDE_BASE}/info",
                "catálogo de trámites sede (licencia urbanística destacada)",
            ),
            (
                f"{self.web_base}/normas-urbanisticas-de-piedralaves",
                "normas urbanísticas municipales (textos y planos PDF)",
            ),
        ]
        rows: list[dict[str, Any]] = []
    ...
    def _pdf_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        rec: dict[str, Any] = {
            ...
            "url": row.get("url") or row.get("pdf_url") or f"{self.web_base}/normas-urbanisticas-de-piedralaves",
            "source": "ayuntamiento",
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_board():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_licencia_info_pages():
