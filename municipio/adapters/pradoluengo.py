...
WEB_BASE = "https://www.pradoluengo.es"
SEDE_BASE = "https://pradoluengo.sedelectronica.es"
MUNICIPIO = "Pradoluengo"
ID_PREFIX = "pradoluengo"
WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/ows"
PLAI_BASE = "https://servicios.jcyl.es/PlanPublica"
PLAU_URL = f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=274"
PLAI_URL = f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=09&municipio=274"
PLAU_DOC_URL = f"{PLAI_BASE}/openDocuIndice.do?cDocId=291831"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WEB_BASE}/normativa",
    f"{WEB_BASE}/informacion-general",
    f"{WEB_BASE}/instalaciones-y-servicios-publicos",
    f"{WEB_BASE}/noticias",
    PLAU_URL,
    PLAI_URL,
    PLAU_DOC_URL,
...
RE_PREVIEW = re.compile(
    r'href="(https://pradoluengo\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra mayor|obra menor|"
    r"primera ocupaci[oó]n|arquitecto t[eé]cnico|tasa.*licencia|ordenanza.*licencia)",
)
RE_PROYECTO = re.compile(
    ...
    r"instrumento|memoria|planos|bocyl|plau|ordenanza reglament|alumbrado)",
)
RE_DRUPAL_EXCLUDE = re.compile(
    r"(?i)(fiestas|cine|empleo|concurso|deporte|violencia de g[eé]nero|"
    r"subvenci[oó]n deportiv|empadron|tribut|matrimonio|carnaval|biblioteca|"
    r"piscina|polideportivo|albergue|misa homenaje|calendario agosto)",
)
RE_EXCLUDE = re.compile(
    ...
    r"selecci[oó]n de personal|tribunal|convocatoria pleno|baremaci[oó]n|"
    r"oficial 1[aª]|interino)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_YM = re.compile(r"/(?:uploads|files)/(\d{4})/(\d{2})/")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?pradoluengo\.es)?/sites/pradoluengo/files/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "ordenanza reglament" in n:
        return "ordenanza reglamentaria"
    if "alumbrado" in n:
...
class PradoluengoAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal pradoluengo.es + sede espublico + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-pradoluengo/1.0")},
        )
    ...
    def _abs_web(self, href: str) -> str:
        return urljoin(f"{self.web_base}/", href)
    ...
    def _collect_wfs_proyectos(self) -> list[dict[str, Any]]:
        ...
        for layer, default_tipo in WFS_LAYERS:
            ...
            for feat in data.get("features") or []:
                ...
                doc_url = str(props.get("url_doc_info") or "").strip() or PLAU_DOC_URL
    ...
    def _collect_drupal_noticias(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return rows
        for href in RE_NOTICIA_HREF.findall(html):
            if href in seen:
                continue
            seen.add(href)
            ...
            try:
                ...
                continue
            h1_m = RE_H1.search(page)
            title_m = RE_TITLE.search(page)
            title = _strip_html(h1_m.group(1) if h1_m else title_m.group(1) if title_m else href)
            if RE_DRUPAL_EXCLUDE.search(title) and not RE_PROYECTO.search(title):
                continue
            if not RE_PROYECTO.search(title) and not RE_LICENCIA.search(title):
                continue
            body_m = re.search(r"field--name-body[^>]*>(.*?)</div>", page, re.S | re.I)
        ...
        for page_url in self.seed_pages:
            ...
            for m in RE_PDF_HREF.finditer(html):
                ...
                if not RE_PROYECTO.search(blob) and not RE_LICENCIA.search(blob):
                    if "normativa" not in page_url.lower() and "plau" not in page_url.lower():
                        continue
                rows.append(
                    {
                        "titulo": titulo[:500],
    ...
    def _plau_proyectos(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for url, titulo, tipo in (
            (PLAU_URL, "Archivo planeamiento urbanístico aprobado — PLAU CyL (Pradoluengo)", "planeamiento"),
            (PLAI_URL, "Planeamiento en información pública — PLAI CyL (Pradoluengo)", "información pública"),
            (
                PLAU_DOC_URL,
                "Normas Urbanísticas Municipales — índice documentación PLAU CyL",
                "normas urbanísticas",
            ),
        ):
            rec: dict[str, Any] = {
    ...
    def _drupal_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        blob = f"{row.get('titulo', '')} {row.get('body', '')} {row.get('pdf_url', '')}"
        if RE_EXCLUDE.search(blob) and not RE_PROYECTO.search(blob):
            return None
        ...
        rec: dict[str, Any] = {
            ...
            "url": row.get("url") or row.get("pdf_url") or f"{self.web_base}/normativa",
            "source": "ayuntamiento",
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            ...
            (f"{self.web_base}/normativa", "normativa municipal"),
            (f"{self.web_base}/informacion-general", "información general — archivo PLAU CyL"),
            (f"{self.sede_base}/dossier", "catálogo de trámites — sede electrónica"),
            (f"{self.sede_base}/transparency", "portal transparencia — urbanismo y obras públicas"),
            (PLAU_URL, "archivo planeamiento urbanístico aprobado (PLAU CyL)"),
        ]
        rows: list[dict[str, Any]] = []
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_drupal_noticias():
            if RE_LICENCIA.search(item.get("titulo", "")):
                rec = {
                    "id": _stable_id("lic", item["url"]),
                    "fecha_concesion": item.get("fecha"),
                    "tipo": "arquitecto técnico municipal",
                    "distrito": None,
                    "lat": None,
                    "lon": None,
                    "titulo": item["titulo"],
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._plau_proyectos():
            add(rec)
        for item in self._collect_drupal_noticias():
        ...
        return {
            ...
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
            "plau_cyl": sum(1 for r in rows if r.get("origen") == "plau_cyl"),
