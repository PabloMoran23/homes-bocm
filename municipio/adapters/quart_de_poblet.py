...
BASE = "https://www.quartdepoblet.es"
...
CATALOGO_URBANISMO = f"{SEDE}/catalogoservicios.aspx?area=1537&ambito=1"
ICV_WFS = "https://terramapas.icv.gva.es/0702_Planeamiento"
ICV_LAYER = "ms:InventarioSuSuz"

HUB_SLUGS = {
    "gestion-de-residuos",
    "proyectos-2024-ivace",
    "movilidad-sostenible-ivace",
    "subvenciones-del-ivace-instituto-valenciano-para-la-competitividad-empresarial-para-poligonos",
    "botella-de-vidrio",
    "botella-de-plastico",
    "lata-de-aluminio",
    "caja-de-carton",
    "poligonos-ivace-2023",
}

RE_LICENCIA = re.compile(
    r"(?i)(licencia|licència|llic[eè]ncia|declaraci[oó]n responsable|comunicaci[oó]n previa|"
    r"primera ocupaci[oó]n|obra[s]? (?:major|menor)|autorizaci[oó]|informe urban)",
)
RE_PROYECTO = re.compile(
    r"(?i)(urban|planeam|plan (?:parcial|especial|general)|pgou|pge|pri|peri|"
    ...
    r"participaci[oó]|reforma interior|urbanitz|parcel|cat[aá]logo de protecc|"
    r"plan general|pgou|bop|aprobaci[oó]n definitiva)",
)
RE_NOISE = re.compile(
    r"(?i)(subvenci[oó]n|ivace|igualtat|cultural|festiu|ve[iï]nal|musical|empadron|"
    r"compte general|matr[ií]cula iae|borsa de treball|cessi[oó] d.us|residu|botella|"
    r"lata de aluminio|caja de cart|pol[ií]gonos ivace|ministerio de fomento|carreteras del estado)",
)
RE_SECTOR_TOKEN = re.compile(
    r"(?i)\b((?:UE|SD|PP|PRI|PERI|PE|PAI|RES|SAU|SUZ)[\s\-]?(?:IND\s*)?[\dA-Z]+(?:[\s,\-yY/]+[\dA-Z]+)*)\b",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_TRAMITE = re.compile(
    r'href="((?:https://quartdepoblet\.sedipualba\.es)?/carpetaciudadana/tramite\.aspx\?idtramite=\d+)"[^>]*>'
    r"\s*([^<]+)",
    re.I,
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if "expropi" in n:
        return "expropiación"
    if "plan parcial" in n or "pp " in n or "moli" in n or "molí" in n:
        return "plan parcial"
    if "modificaci" in n and ("plan general" in n or "pge" in n or "pgou" in n):
        return "modificación plan general"
    if "catálogo de protecc" in n or "catalogo de protecc" in n:
        return "catálogo de protecciones PGE"
    if "informaci" in n and "p" in n and "blica" in n:
        return "información pública"
    if "pgou" in n or "plan general" in n or "pge" in n:
        return "plan general"
    if "bop" in n and "aprob" in n:
        return "aprobación definitiva"
    if re.search(r"\b(sd|ue|sector|sau|suz)\b", n):
...
class QuartDePobletAyuntamientoAdapter(AyuntamientoAdapter):
    """DigitalValue/Zity (quartdepoblet.es) + sede Sedipualba + ICV WFS InventarioSuSuz."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.ine_mun = str(geom_cfg.get("ine_mun") or INE_MUN)
        self._wfs_cache: list[dict[str, Any]] | None = None
        self._wfs_by_key: dict[str, dict[str, Any]] | None = None

    def _fetch(self, url: str, *, timeout: int = 60, retries: int = 3, encoding: str | None = None) -> str:
    ...
    def _is_proyecto_article(self, item: dict[str, Any]) -> bool:
        ...
        if "urbanismo" not in cats and not any("urban" in c for c in cats):
            return False
        ...
        if item.get("filesGroup") and (
            ...
            or slug.startswith("modificacion-")
            or slug.startswith("pgou")
    ...
    def _collect_tramites(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(self.catalogo_urbanismo)
        ...
        for href, title in RE_TRAMITE.findall(html):
            ...
            if not href.startswith("http"):
                href = urllib.parse.urljoin(f"{self.sede_base}/", href)
            rows.append(
                {
                    "titulo": title,
                    "url": href,
    ...
    def _collect_tablon_rss(self) -> list[dict[str, Any]]:
        try:
            raw = self._fetch(self.tablon_rss, encoding="iso-8859-1")
        except urllib.error.URLError:
            return []
        rows: list[dict[str, Any]] = []
        try:
        ...
        while start < 8000:
            ...
            for member in members:
                ...
                rec: dict[str, Any] = {
                    "titulo": titulo[:500],
                    "fecha": f_aprob,
                    "url": f"{BASE}/es/desarrollo-urbano-sostenible",
