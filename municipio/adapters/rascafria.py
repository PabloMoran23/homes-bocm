...
WP_BASE = "https://www.rascafria.org"
SEDE_BASE = "https://rascafria.sedelectronica.es"
MUNICIPIO = "Rascafría"
ID_PREFIX = "rascafria"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "RASCAFRÍA"

WP_URBANISMO_CATEGORIES: tuple[int, ...] = ()  # sin categoría WP dedicada; búsqueda por términos

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/tu-ayuntamiento/urbanismo/",
    f"{WP_BASE}/ordenacion-del-territorio-y-obras-publicas/",
    f"{WP_BASE}/tu-ayuntamiento/ordenanzas-municipales/",
    f"{WP_BASE}/aprobacion-inicial-en-los-terminos-que-figuran-en-el-expediente-de-la-modificacion-de-las-normas-subsidiarias-urbanisticas-de-1985/",
]
...
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|AD|AN|AI|PAU|AR|S)-[\w\d-]+)\b",
)
...
RE_BOARD_PREVIEW = re.compile(
    r'href="(https://rascafria\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
def _licencia_tipo(title: str) -> str:
    ...
    if "actividad" in n or "apertura" in n:
        return "licencia de actividad"
    if "autotaxi" in n or "taxi" in n:
        return "licencia autotaxi"
...
def _sector_ilike_parts(text: str) -> list[str]:
    low = text.lower()
    for marker in (" bocm", " aprob", " anuncio", " del pgou", " nnss", " de aldea"):
        if marker in low:
...
class RascafriaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress TownPress + sede espublico gestiona + WFS SITCM (geometría partial)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-rascafria/1.0")},
        )
    ...
    def _collect_wp_search_posts(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        ...
        for term in WP_SEARCH_TERMS:
            try:
                posts = self._fetch_json(
                    ...
                    f"?search={urllib.parse.quote(term)}&per_page=50&_fields=id,link,title,date"
                )
    ...
    def _collect_wp(self) -> list[dict[str, Any]]:
        ...
        for rec in self._collect_wp_api_posts():
            ...
            by_key[key] = rec
        for rec in self._collect_wp_search_posts():
            key = rec["url"] + "|" + rec["titulo"]
            by_key[key] = rec
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        urbanismo_url = f"{self.wp_base}/tu-ayuntamiento/urbanismo/"
        tramites: list[dict[str, Any]] = [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Formularios licencia de obra mayor y menor",
                "url": urbanismo_url,
                "source": "ayuntamiento",
                "nota": "PDFs solicitud licencia obra mayor/menor e informes",
                "origen": "wp_tramite",
            },
            {
                "id": _stable_id("lic", f"{self.wp_base}/tu-ayuntamiento/ordenanzas-municipales/"),
                "fecha_concesion": None,
                "tipo": "ordenanza fiscal licencias urbanísticas",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Tasa por licencias urbanísticas (ordenanzas municipales)",
                "url": f"{self.wp_base}/tu-ayuntamiento/ordenanzas-municipales/",
                "source": "ayuntamiento",
                "nota": "Ordenanza fiscal tasa licencias urbanísticas",
                "origen": "wp_tramite",
            },
            {
                ...
                "url": self.board_url,
                "source": "ayuntamiento",
                "nota": "Tablón espublico (vacío en scraping; anuncios en web municipal)",
                "origen": "sede_tablon",
            },
        ]
        try:
        ...
        for m in re.finditer(
            ...
            if pdf_url in seen_urls:
                continue
            if not RE_LICENCIA.search(pdf_url):
                continue
    ...
    def _collect_sit_ambitos(self) -> list[dict[str, Any]]:
        ...
        for f in feats:
            ...
            rec: dict[str, Any] = {
                ...
                "url": f"{self.wp_base}/tu-ayuntamiento/urbanismo/",
                "source": "ayuntamiento",
                "origen": "sit_wfs",
                "ambito_sit": name,
            }
