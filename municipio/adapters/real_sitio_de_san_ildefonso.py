...
ID_PREFIX = "real-sitio-de-san-ildefonso"
PLAI_MUNICIPIO = 181
PLAI_PROVINCIA = 40
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (
        ...
        "Concejalía de Urbanismo — trámites y normativa",
    ),
    (
    ...
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=181",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=181",
        "Planeamiento en información pública (PLAI JCYL)",
    ),
    (f"{SEDE_BASE}/dossier/.0", "Catálogo de trámites — sede electrónica"),
]
...
RE_SECTOR = re.compile(
    ...
    r"valsain|navalhorno|paseo de bolonia|pradera de navalhorno|los alijares|"
    r"ampliaci[oó]n instalac)",
)
...
class RealSitioDeSanIldefonsoAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress lagranja-valsain.com + sede espublico + PLAI JCYL + geometría parcial IDECyL WFS."""
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-real-sitio-de-san-ildefonso/1.0")},
        )
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "tablon": sum(1 for r in rows if r.get("origen") in ("tablon", "info_tablon")),
            "semillas": sum(1 for r in rows if r.get("origen") == "web_semilla"),
        }
