...
WP_BASE = "https://www.reduena.com"
SEDE_BASE = "https://reduena.sedelectronica.es"
MUNICIPIO = "Redueña"
ID_PREFIX = "reduena"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "REDUEÑA"

WP_URBANISMO_CATEGORIES = (25,)  # Urbanismo

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/servicios-municipales/urbanismo/",
    f"{WP_BASE}/normas-subsidiarias/",
    f"{WP_BASE}/licencias-de-obras/",
...
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|AD|AN|AI|PAU|S|SAU)-[\w\d.-]+)\b",
)
...
RE_BOARD_PREVIEW = re.compile(
    r'href="(https://reduena\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
def _sector_ilike_parts(text: str) -> list[str]:
    low = text.lower()
    for marker in (" bocm", " aprob", " anuncio", " del pgou", " nnss", " de reduena", " de reduña"):
        if marker in low:
...
class ReduenaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Freepress + sede espublico gestiona + WFS SITCM (geometría partial)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-reduena/1.0")},
        )
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        tramites: list[dict[str, Any]] = [
            {
                "id": _stable_id("lic", f"{self.wp_base}/licencias-de-obras/"),
                "fecha_concesion": None,
                "tipo": "trámite licencia de obra",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Licencias de obras — información y formularios",
                "url": f"{self.wp_base}/licencias-de-obras/",
                "source": "ayuntamiento",
                "nota": "Formularios obra mayor, declaración responsable urbanística",
                "origen": "wp_tramite",
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Sede electrónica — trámites y tablón",
                "url": f"{self.sede_base}/",
                "source": "ayuntamiento",
                "nota": "espublico gestiona: tablón, transparencia, factura electrónica",
                "origen": "sede_tramite",
            },
            {
            ...
            {
                "id": _stable_id("lic", f"{self.wp_base}/wp-content/uploads/2023/02/Licencia-de-obra-mayor-Reduena.pdf"),
                "fecha_concesion": "2023-02-01",
                "tipo": "formulario licencia obra mayor",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Solicitud licencia de obra mayor",
                "url": f"{self.wp_base}/wp-content/uploads/2023/02/Licencia-de-obra-mayor-Reduena.pdf",
                "source": "ayuntamiento",
                "origen": "wp_formulario",
            },
            {
                "id": _stable_id("lic", f"{self.wp_base}/wp-content/uploads/2024/03/declaracion-responsable-urbanistica-def.pdf"),
                "fecha_concesion": "2024-03-01",
                "tipo": "declaración responsable urbanística",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Declaración responsable urbanística",
                "url": f"{self.wp_base}/wp-content/uploads/2024/03/declaracion-responsable-urbanistica-def.pdf",
                "source": "ayuntamiento",
                "origen": "wp_formulario",
            },
        ]
        try:
            html = self._fetch(f"{self.wp_base}/licencias-de-obras/")
        except urllib.error.URLError:
            return tramites
        seen_urls: set[str] = set()
        for m in re.finditer(
            r'href="(https://reduena\.sedelectronica\.es/[^"]+)"',
            html,
            re.I,
        ):
            url = m.group(1)
    ...
    def _collect_sit_ambitos(self) -> list[dict[str, Any]]:
        ...
        for f in feats:
            ...
            rec: dict[str, Any] = {
                ...
                "url": f"{self.wp_base}/servicios-municipales/urbanismo/",
                "source": "ayuntamiento",
                "origen": "sit_wfs",
                "ambito_sit": name,
            }
