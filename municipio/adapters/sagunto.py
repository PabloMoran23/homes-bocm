...
class SaguntoAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, timeout: int = 60, retries: int = 3) -> str:
        ...
        for attempt in range(retries):
            ...
            req = urllib.request.Request(
                url,
                headers={"User-Agent": self.config.get("user_agent", "poc-bocm-sagunto/1.0")},
            )
            try:
    ...
    def _page_title(self, html: str, fallback: str = "") -> str:
        for pat in (
            ...
            if m:
                t = _clean(m.group(1))
                t = re.sub(r"\s*[-|].*Sagunto.*$", "", t, flags=re.I).strip()
                if t and len(t) > 3:
                    return t[:500]
        return fallback
