...
WEB_BASE = "https://www.sanantoniodebenageber.es"
SEDE_BASE = "https://sanantoniodebenageber.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
TRANSPARENCY_URL = f"{SEDE_BASE}/transparency/57255e6b-c9fd-4ce4-8629-c2ef8b15a670/"
URBANISMO_DOCS_URL = f"{WEB_BASE}/es/pagina/documentos-para-descargar"
MUNICIPIO = "San Antonio de Benagéber"
ID_PREFIX = "san-antonio-de-benageber"
...
DEFAULT_TRANSPARENCY_FOLDERS: list[dict[str, str]] = [
    {
        ...
        "nota": "21 documentos en portal transparencia (carga AJAX Wicket)",
    },
    {
        "titulo": "7.3. Normativa Urbanística",
        "url": TRANSPARENCY_URL,
        "nota": "17 documentos en portal transparencia",
    },
    {
        "titulo": "7.4. Obras Públicas e Infraestructuras",
        "url": TRANSPARENCY_URL,
        "nota": "0 documentos en portal transparencia",
    },
]
...
RE_LICENCIA = re.compile(
    ...
    r"obra (?:mayor|menor)|llic[eè]ncia|urba-\d+)",
)
RE_PROYECTO = re.compile(
    ...
    r"montesano|unidad de ejecuci[oó]n|dogv|iate|impacto ambiental)",
)
RE_BOARD_NON_URBAN = re.compile(
    ...
    r"nombramiento.*polic[ií]a|agente de la polic[ií]a)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://sanantoniodebenageber\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
RE_SECTOR_TOKEN = re.compile(
    r"(?i)\b((?:R|T|UE|UEC|UEP)[\s\-]?\d+[A-Z0-9\-]*|MONTESANO(?:\s+[\d.]+)?|SECTOR\s+[A-Z0-9\-]+)\b",
)
...
class SanAntonioDeBenageberAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal Adaptive Theme + sede espublico gestiona + ICV InventarioSuSuz WFS."""
    ...
    def _fetch(self, url: str, *, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-san-antonio-de-benageber/1.0")},
        )
    ...
    def _parse_wfs_feature(self, feat_el: ET.Element) -> dict[str, Any] | None:
        ...
        if props.get("cod_ine_mun") != self.ine_mun:
            return None
        pp = _strip_html(str(props.get("pp") or ""))
        ue = _strip_html(str(props.get("ue") or ""))
        titulo = pp
        if ue and ue.lower() not in titulo.lower():
            titulo = f"{pp} ({ue})" if pp else ue
        if not titulo:
            titulo = f"Sector {props.get('id', '')}"
    ...
    def _enrich_geometry(self, rec: dict[str, Any]) -> None:
        if record_geometry(rec):
            return
        blob = " ".join(str(rec.get(k) or "") for k in ("titulo", "descripcion", "pp", "ue", "expte"))
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "id": _stable_id("lic", URBANISMO_DOCS_URL),
                "fecha_concesion": None,
                "tipo": "formularios licencias urbanismo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Documentos urbanismo — licencias y comunicaciones (URBA-01..12)",
                "url": URBANISMO_DOCS_URL,
                "source": "ayuntamiento",
                "nota": "Impresos URBA en web municipal; sin registro público de concesiones",
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "info": sum(1 for r in rows if r.get("origen") in ("sede_tablon", "sede_tramite", "web_formularios")),
        }
