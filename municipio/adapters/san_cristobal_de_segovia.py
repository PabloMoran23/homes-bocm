...
DIPSEGOVIA_BASE = "https://www.dipsegovia.es/web/ayuntamiento-de-san-cristobal-de-segovia"
SEDE_BASE = "https://sancristobaldesegovia.sedelectronica.es"
...
ID_PREFIX = "san-cristobal-de-segovia"
PLAI_MUNICIPIO = 170
PLAI_PROVINCIA = 40
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (f"{DIPSEGOVIA_BASE}/urbanismo", "Urbanismo — archivo PLAI y contacto"),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=170",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=170",
...
RE_PREVIEW = re.compile(
    r'href="(https://sancristobaldesegovia\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://sancristobaldesegovia\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class SanCristobalDeSegoviaAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-san-cristobal-de-segovia/1.0")},
        )
