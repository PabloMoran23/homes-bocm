...
WEB_BASE = "https://www.sanestebandegormaz.org"
SEDE_BASE = "https://sanestebandegormaz.sedelectronica.es"
...
ID_PREFIX = "san-esteban-de-gormaz"
PLAI_MUNICIPIO = 162
PLAI_PROVINCIA = 42
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    (WEB_BASE, "Web municipal — noticias y avisos"),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia={PLAI_PROVINCIA}&municipio={PLAI_MUNICIPIO:03d}",
        "Archivo planeamiento urbanístico aprobado (PLAU JCYL)",
    ),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia={PLAI_PROVINCIA}&municipio={PLAI_MUNICIPIO:03d}",
...
RE_PREVIEW = re.compile(
    r'href="(https://sanestebandegormaz\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://sanestebandegormaz\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class SanEstebanDeGormazAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress municipal + sede espublico gestiona + PLAU/PLAI JCYL + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plai_url = str(
            ...
            or (
                f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S"
                f"&provincia={self.plai_provincia}&municipio={self.plai_municipio:03d}"
            )
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-san-esteban-de-gormaz/1.0")},
        )
    ...
    def _collect_tramites_catalog(self) -> list[dict[str, Any]]:
        ...
        for url, titulo in self.seed_pages:
            if url in seen:
                continue
            if not RE_LICENCIA.search(titulo) and not RE_PROYECTO.search(titulo):
                if "dossier" not in url and "plau" not in url.lower() and "plai" not in url.lower():
                    continue
            seen.add(url)
            rows.append({"titulo": titulo[:500], "url": url, "origen": "catalogo_tramites"})
        return rows
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for url, titulo in self.seed_pages:
            if not RE_LICENCIA.search(titulo) and "dossier" not in url:
                continue
            rows.append(
                {
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_plai_seed():
            ...
            if rec and rec["id"] not in seen:
                ...
                rows.append(rec)
        for url, titulo in self.seed_pages:
            rec = self._tramite_to_proyecto({"titulo": titulo, "url": url, "origen": "web_semilla"})
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_board():
