...
WP_BASE = "https://www.aytosanlorenzo.es"
SEDE_BASE = "https://sede.aytosanlorenzo.es/GDCarpetaCiudadano"
TRANSP_BASE = "https://transparencia.aytosanlorenzo.es"
TRAMITES_BASE = "https://tramites.aytosanlorenzo.es"
MUNICIPIO = "San Lorenzo de El Escorial"
ID_PREFIX = "san-lorenzo-de-el-escorial"

TABLON_ALL = f"{SEDE_BASE}/Tablon.do?action=verAnuncios"
URBANISMO_TRAMITES = f"{TRAMITES_BASE}/urbanismo-obras-y-servicios/"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "SAN LORENZO DE EL ESCORIAL"

DEFAULT_SEED_PAGES: list[str] = [
    f"{TRANSP_BASE}/urbanismo-y-obras-publicas/planeamiento/",
    f"{WP_BASE}/normativa-municipal/modificaciones-puntuales/",
    f"{WP_BASE}/normativa-municipal/normas-subsidiarias-san-lorenzo-escorial/",
    f"{WP_BASE}/servicios/urbanismo/",
]
...
DEFAULT_TRAMITE_URLS: list[str] = [
    f"{TRAMITES_BASE}/solicitud-de-informe-de-necesidad-de-titulo-habilitante/",
    f"{TRAMITES_BASE}/solicitud-de-licencia-urbanistica-de-obra-menor/",
    f"{TRAMITES_BASE}/solicitud-de-licencia-urbanistica-de-obra-mayor/",
    f"{TRAMITES_BASE}/licencia-de-apertura-de-caracter-general/",
    f"{TRAMITES_BASE}/licencias-de-apertura-para-bar-rte-o-similares/",
    f"{TRAMITES_BASE}/solicitud-de-cambio-de-titularidad-para-un-bar-rte/",
    f"{TRAMITES_BASE}/viabilidad-urbanistica-para-puesta-en-marcha-de-actividades/",
    f"{TRAMITES_BASE}/cambio-de-titularidad/",
    f"{TRAMITES_BASE}/licencias-de-apertura-calificadas/",
]
...
RE_LICENCIA = re.compile(
    ...
    r"viabilidad urban|titulo habilitante|cambio de titularidad)",
)
RE_PROYECTO = re.compile(
    ...
    r"orden de ejecuci|modificaci[oó]n|estudio (?:ac[uú]stico|ambiental)|ua-\d|ue-\d|apd-\d|sau-\d|"
    r"proyecto de|catalogo|memoria|normas urban)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|APD|SAU|UA|AD|AN|AI)-\d+[A-Z0-9-]*)\b",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_PDF_HREF = re.compile(r'href="([^"]+\.pdf[^"]*)"', re.I)
RE_TRAMITE_LINK = re.compile(
    r'<a[^>]+href="(https://tramites\.aytosanlorenzo\.es/[^"]+)"[^>]*>([^<]{5,200})</a>',
    re.I,
)
...
def _proyecto_tipo(title: str, default: str = "urbanismo") -> str:
    n = title.lower()
    if re.search(r"\bue-\d+\b", n) or re.search(r"\bapd-\d+\b", n) or re.search(r"\bsau-\d+\b", n):
        return "planeamiento"
    if "informaci" in n:
...
def _sector_ilike_parts(text: str) -> list[str]:
    s = text.strip()
    low = s.lower()
    for marker in (" del pgou", " pgou", " bocm", " aprob", " anuncio", " nnss", " modificación"):
        if marker in low:
...
class SanLorenzoDeElEscorialAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress (normativa/planeamiento) + sede eAdmin tablón + trámites WP + SITCM WFS."""
    ...
    def _fetch(self, url: str, data: bytes | None = None) -> str:
        ...
        with urllib.request.urlopen(req, timeout=60) as resp:
            ...
            charset = resp.headers.get_content_charset() or (
                "iso-8859-1" if "aytosanlorenzo.es" in url and "transparencia" not in url else "utf-8"
            )
            if "Tablon.do" in url or "Registrar.do" in url:
                charset = "iso-8859-1"
    ...
    def _collect_sit_ambitos(self) -> list[dict[str, Any]]:
        ...
        for f in feats:
            ...
            rec: dict[str, Any] = {
                ...
                "url": f"{TRANSP_BASE}/urbanismo-y-obras-publicas/planeamiento/",
                "source": "ayuntamiento",
                "origen": "sit_wfs",
                "ambito_sit": name,
            }
    ...
    def _collect_tramites_licencias(self) -> list[dict[str, Any]]:
        ...
        for m in RE_TRAMITE_LINK.finditer(html):
            ...
            if "urbanismo-obras-y-servicios" in url:
                continue
