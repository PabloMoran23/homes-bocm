...
WP_BASE = "https://www.sanmartindevaldeiglesias.es"
SEDE_BASE = "https://sanmartindevaldeiglesias.sedelectronica.es"
MUNICIPIO = "San Martín de Valdeiglesias"
ID_PREFIX = "san-martin-de-valdeiglesias"

URBANISMO_URL = f"{WP_BASE}/areas/obras-urbanismo/"
SITCM_VISOR_URL = "https://www.madrid.org/cartografia/sitcm/html/visor.htm?municipio=133"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "SAN MARTÍN DE VALDEIGLESIAS"

DEFAULT_NORMATIVA_PDFS: list[dict[str, str]] = [
    {
        "url": f"{WP_BASE}/wp-content/uploads/ayuntamiento/normativa-municipal/UE25_BOCM_20201113.PDF",
        "titulo": "Aprobación definitiva UE-25 Dotacional Ermita Ecce Homo (BOCM 13/11/2020)",
        "fecha": "2020-11-13",
    },
    {
        "url": f"{WP_BASE}/wp-content/uploads/ayuntamiento/normativa-municipal/OR11_BOCM_20201113.PDF",
        "titulo": "Ordenanza 11 — Licencias de apertura (BOCM 13/11/2020)",
        "fecha": "2020-11-13",
    },
    {
        "url": f"{WP_BASE}/wp-content/uploads/ayuntamiento/normativa-municipal/Comercial_BOCM_20201113.PDF",
        "titulo": "Ordenanza comercial (BOCM 13/11/2020)",
        "fecha": "2020-11-13",
    },
    {
        "url": f"{WP_BASE}/wp-content/uploads/ayuntamiento/normativa-municipal/Vivienda_BOCM_20201113.PDF",
        "titulo": "Ordenanza de vivienda (BOCM 13/11/2020)",
        "fecha": "2020-11-13",
    },
    {
        "url": f"{WP_BASE}/wp-content/uploads/ayuntamiento/normativa-municipal/BOCM-20201102-2.pdf",
        "titulo": "Normativa urbanística municipal (BOCM 02/11/2020)",
        "fecha": "2020-11-02",
    },
    {
        "url": f"{WP_BASE}/wp-content/uploads/2023/06/BOCM-20230421-77.pdf",
        "titulo": "Publicación BOCM urbanismo (21/04/2023)",
        "fecha": "2023-04-21",
    },
]

RE_PREVIEW = re.compile(
    r'href="(https://sanmartindevaldeiglesias\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra (?:mayor|menor)|"
    r"primera ocupaci[oó]n|cedula urban|c[eé]dula urban|mesas y sillas)",
)
RE_PROYECTO = re.compile(
    ...
    r"reparcel|sector|parcela|bando|unidad(?:es)? de ejecuci[oó]n|ordenanza|"
    r"\b(?:UE|SAU|AD|AN|AI|PAU|S)-\d+[A-Z0-9-]*\b)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|empleo|jurado|selecci[oó]n de personal|"
    r"activaci[oó]n profesional|igualdad|bolsa|monitor deportivo|m[uú]sica)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|SAU|AD|AN|AI|PAU|S)-\d+[A-Z0-9-]*)\b",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_URBAN_PDF = re.compile(
    r'href="(https://www\.sanmartindevaldeiglesias\.es/wp-content/uploads/[^"]+\.(?:pdf|PDF))"',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    n = title.lower()
    if re.search(r"\bue-\d+", n) or "unidad de ejecuci" in n:
        return "unidad de ejecución"
    if re.search(r"\bsau-\d+", n):
        return "sector"
    if re.search(r"\bs-\d+\b", n) or "sector" in n:
        return "sector"
...
def _sector_ilike_parts(text: str) -> list[str]:
    low = text.lower()
    for marker in (" bocm", " aprob", " anuncio", " del ", " de san", " valdeiglesias"):
        if marker in low:
...
class SanMartinDeValdeiglesiasAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress + sede espublico eHome + ámbitos SITCM WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.normativa_pdfs = list(self.config.get("normativa_pdfs") or DEFAULT_NORMATIVA_PDFS)
        ...
        req = urllib.request.Request(
            ...
            headers={
                "User-Agent": self.config.get(
                    "user_agent", "poc-bocm-san-martin-de-valdeiglesias/1.0"
                )
            },
        )
    ...
    def _collect_urbanismo_form_pdfs(self) -> list[dict[str, Any]]:
        try:
        ...
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for m in RE_URBAN_PDF.finditer(html):
            pdf = m.group(1)
            name_low = Path(urllib.parse.unquote(pdf)).name.lower()
            if pdf in seen:
                continue
            if not any(
                k in name_low
                ...
                continue
            seen.add(pdf)
            titulo = re.sub(r"[-_]+", " ", Path(name_low).stem).strip()
            rows.append(
                {
        ...
        return [
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Obras y urbanismo — licencias, declaraciones responsables y cédulas",
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
                "nota": "Página informativa del área de urbanismo",
                "origen": "wp_urbanismo",
            },
            {
    ...
    def _collect_normativa_seeds(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        rec: dict[str, Any] = {
            "id": _stable_id("proy", "nnss-1999"),
            "municipio": MUNICIPIO,
            "titulo": "Normas Subsidiarias de Planeamiento Municipal (matriz 25/09/1999)",
            "fecha": "1999-09-25",
            "tipo": "normas subsidiarias",
            "url": SITCM_VISOR_URL,
            "source": "ayuntamiento",
            "origen": "wp_urbanismo",
            "nota": "Consultable en Visor SIT Comunidad de Madrid (municipio 133)",
        }
        self._enrich_geometry(rec)
        rows.append(rec)
        for item in self.normativa_pdfs:
            pdf = item["url"]
            titulo = item["titulo"]
            rec = {
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_licencia_info_pages():
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for rec in self._collect_urbanismo_form_pdfs():
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_sit_ambitos():
            add(rec)
        for rec in self._collect_normativa_seeds():
            add(rec)
        for item in self._collect_board():
