...
DIPSEGOVIA_BASE = "https://www.dipsegovia.es/web/ayuntamiento-de-san-pedro-de-gaillos"
SEDE_BASE = "https://sanpedrodegaillos.sedelectronica.es"
...
ID_PREFIX = "san-pedro-de-gaillos"
PLAI_MUNICIPIO = 184
PLAI_PROVINCIA = 40
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=184",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=184",
...
RE_PREVIEW = re.compile(
    r'href="(https://sanpedrodegaillos\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://sanpedrodegaillos\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class SanPedroDeGaillosAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-san-pedro-de-gaillos/1.0")},
        )
    ...
    def _collect_dipsegovia_urbanismo(self) -> list[dict[str, Any]]:
        ...
        trans_m = re.search(
            r"(https://sanpedrodegaillos\.sedelectronica\.es/transparency[^\"<\s]*)",
            html,
            re.I,
        )
        if trans_m:
