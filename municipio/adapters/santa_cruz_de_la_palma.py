...
WEB_BASE = "https://www.santacruzdelapalma.es/web/sclapalma"
SEDE_BASE = "https://sede.santacruzdelapalma.es"
MUNICIPIO = "Santa Cruz de La Palma"
ID_PREFIX = "sclp"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-santa-cruz-de-la-palma"
GEOBDP_MUNICIPIO = "https://geobdp.grafcan.es/core/municipios/38037/"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WEB_BASE}/areas/informacion/10",
    f"{WEB_BASE}/documentos/anuncios-convocatorias",
    f"{WEB_BASE}/documentos/documentos-utiles",
    f"{WEB_BASE}/noticia/noticias-sclapalma",
    f"{SEDE_BASE}/transparencia",
]
...
RE_PORTAL_LINK = re.compile(r'<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', re.I | re.S)
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?santacruzdelapalma\.es)?/[^"]+\.(?:pdf|PDF)[^"]*)"',
    re.I,
)
RE_DOC_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?santacruzdelapalma\.es)?/web/sclapalma/documentos/descargar/[^"]+)"',
    re.I,
)
RE_NOTICIA_HREF = re.compile(
    r'href="(https?://(?:www\.)?santacruzdelapalma\.es/web/sclapalma/noticias/[^"]+)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (
        ...
        if m:
            ...
            if t and len(t) > 3 and "not found" not in t.lower():
                return t[:500]
    return fallback
...
class SantaCruzDeLaPalmaAyuntamientoAdapter(AyuntamientoAdapter):
    """Portal corporativo Bootstrap + sede Maggioli + SITCAN CKAN + GEOBDP Grafcan (Canarias)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-santa-cruz-de-la-palma/1.0")},
        )
    ...
    def _load_geobdp_catalog(self) -> dict[str, str]:
        return {doc_id: title for title, doc_id in self._load_geobdp_index().items()}
    ...
    def _discover_noticias(self) -> list[str]:
        urls: list[str] = []
        seen: set[str] = set()
        keywords = (
            "urban",
        ...
        for page in range(1, 6):
            listing = (
                f"{self.web_base}/noticia/noticias-sclapalma"
                if page == 1
                else f"{self.web_base}/noticia/noticias-sclapalma?page={page}"
            )
            try:
            ...
            for m in RE_NOTICIA_HREF.finditer(html):
                ...
                seen.add(loc)
                try:
                    article = self._fetch(loc)
                except urllib.error.URLError:
                    continue
                low = f"{loc} {_strip_html(article)[:1500]}".lower()
    ...
    def _collect_portal_pages(self) -> list[dict[str, Any]]:
        ...
        page_urls = list(dict.fromkeys([*self.seed_pages, *self._discover_noticias()]))
        for page_url in page_urls:
            ...
            blob = f"{title} {page_url} {_strip_html(html)[:3000]}"
            ...
            for m in RE_DOC_HREF.finditer(html):
                doc_url = self._abs_url(m.group(1))
                name = unescape(urllib.parse.unquote(doc_url.rsplit("/", 1)[-1].replace("+", " ")))
                doc_blob = f"{title} {name} {doc_url}"
                if not RE_PROYECTO.search(doc_blob) and not RE_LICENCIA.search(doc_blob):
                    continue
                rows.append(
                    {
                        "titulo": name[:500] if len(name) > 5 else f"{title}: {name}"[:500],
                        "fecha": _fecha_from_blob(f"{name} {doc_url}"),
                        "url": page_url,
                        "pdf_url": doc_url,
                        "blob": doc_blob,
                        "origen": "portal_documento",
                        "urls": [doc_url, page_url],
                    }
                )
            for m in RE_PORTAL_LINK.finditer(html):
                href = m.group(1)
                anchor = _strip_html(m.group(2))
                if not anchor or len(anchor) < 5:
                    continue
                link_blob = f"{title} {anchor} {href}"
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        tramites = [
            ("Sede electrónica — trámites urbanismo y licencias", f"{self.sede_base}/"),
            ("Transparencia — sede electrónica", f"{self.sede_base}/transparencia"),
            (
                "Solicitud de licencia de obra mayor",
                f"{self.web_base}/documentos/descargar/60/SOLICITUD+DE+LICENCIA+DE+OBRA+MAYOR",
            ),
            (
                "Solicitud de licencia de obra menor",
                f"{self.web_base}/documentos/descargar/61/SOLICITUD+DE+LICENCIA+DE+OBRA+MENOR",
            ),
            (
                "Solicitud informe cédula urbanística",
                f"{self.web_base}/documentos/descargar/62/SOLICITUD+INFORME+CEDULA+URBANISTICA",
            ),
        ]
        rows: list[dict[str, Any]] = []
        for titulo, url in tramites:
            rows.append(
                {
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {"rows": len(rows), "status": "ok", "source": "portal_sede_sitcan"}
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for row in self._collect_sitcan():
            ...
            if proy:
                raw.append(proy)
        catalog = self._load_geobdp_index()
        seen_docs: set[str] = set()
        for title_key, doc_id in catalog.items():
            ...
            proy = self._to_proyecto(
                {
                    "titulo": title_key,
                    "fecha": _fecha_from_blob(title_key),
                    "url": f"https://geobdp.grafcan.es/core/documentos/{doc_id}.html",
                    "blob": title_key,
                    "origen": "geobdp_catalog",
                    "urls": [f"https://geobdp.grafcan.es/core/documentos/{doc_id}.html"],
                }
            )
            if proy:
                raw.append(proy)
