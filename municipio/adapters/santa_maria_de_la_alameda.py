...
WP_BASE = "https://santamariadelaalameda.com"
SEDE_BASE = "https://santamariadelaalameda.sedelectronica.es"
MUNICIPIO = "Santa María de la Alameda"
ID_PREFIX = "santa-maria-de-la-alameda"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "SANTA MARÍA DE LA ALAMEDA"

ORDENANZAS_URL = (
    f"{WP_BASE}/portal-de-transparencia/normativa-municipal/ordenanzas/"
)
AVANCE_PGOU_URL = f"{WP_BASE}/avance-plan-general/"
BOARD_URL = f"{SEDE_BASE}/board/"
DOSSIER_URL = f"{SEDE_BASE}/dossier"
TRANSPARENCY_URL = f"{SEDE_BASE}/transparency"
HISTORICO_BANDOS_URL = f"{WP_BASE}/tablon-de-anuncios/historico-bandos/"

URBANISMO_ORDENANZA_PDFS: list[dict[str, str]] = [
    {
        "url": (
            f"{WP_BASE}/wp-content/uploads/Ordenanzas/"
            "02.-%20ORDENANZA%20LICENCIAS%20URBAN%C3%8DSTICAS.pdf"
        ),
        "titulo": "Ordenanza reguladora de licencias urbanísticas",
    },
    {
        "url": (
            f"{WP_BASE}/wp-content/uploads/Ordenanzas/"
            "13.-%20ORDENANZA%20FISCAL%20IMPUESTO%20CONSTRUCCIONES%20Y%20OBRAS.pdf"
        ),
        "titulo": "Ordenanza fiscal impuesto construcciones y obras",
    },
    {
        "url": (
            f"{WP_BASE}/wp-content/uploads/Ordenanzas/"
            "10.-%20ORDENANZA%20FISCAL%20REGULADORA%20LICENCIAS%20AGRUPACION%20Y%20SEGREGACION.pdf"
        ),
        "titulo": "Ordenanza licencias agrupación y segregación",
    },
    {
        "url": (
            f"{WP_BASE}/wp-content/uploads/Ordenanzas/"
            "25.-%20ORDENANZA%20REGULADORA%20Y%20FISCAL%20DE%20LA%20TASA%20POR%20"
            "LICENCIAS%20DE%20ACTIVIDAD%20Y%20FUNCIONAMIENTO.pdf"
        ),
        "titulo": "Ordenanza tasa licencias de actividad y funcionamiento",
    },
    {
        "url": (
            f"{WP_BASE}/wp-content/uploads/Ordenanzas/"
            "23.-%20ORDENANZA%20GESTION%20DE%20RESIDUOS%20DE%20CONSTRUCCION%20Y%20DEMOLICION.pdf"
        ),
        "titulo": "Ordenanza gestión residuos de construcción y demolición",
    },
]

RE_PREVIEW = re.compile(
    r'href="(https://santamariadelaalameda\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    ...
    r"construcci[oó]n|demolici[oó]n|vado|terrazas)",
)
RE_PROYECTO = re.compile(
    ...
    r"reparcel|bando.*parcel|edicto|avance|cat[aá]logo|memoria|planos|"
    r"limpieza.*parcel|parcela|ordenanza.*urban|"
    r"\b(?:P-\d+|UE|UA|AD|AN|AI|PAU|SAU|S)-\d+[A-Z0-9-]*\b)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|funcionario|empleado|plusvalia|basura|"
    r"residuos domiciliarios|vehiculos|notificaci[oó]n expediente|igualdad|"
    r"convocatoria.*plaza|jurado|iae\b|empleo p[uú]blico|bolsa de empleo|"
    r"protecci[oó]n civil|covid|vacunaci[oó]n|perros peligrosos|toros|fiestas)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:P-\d+|UE|UA|AD|AN|AI|PAU|SAU|S)-\d+[A-Z0-9-]*)\b",
)
...
RE_CATALOG = re.compile(
    r'href="((?:https://santamariadelaalameda\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?santamariadelaalameda\.com)?/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    n = title.lower()
    if "avance" in n or "pgou" in n or "plan general" in n:
        return "planeamiento"
...
class SantaMariaDeLaAlamedaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress Elementor + sede espublico gestiona + SITCM WFS (geometría partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.historico_bandos_url = str(
            self.config.get("historico_bandos_url") or HISTORICO_BANDOS_URL
        )
        self.urbanismo_pdfs = list(
            self.config.get("urbanismo_ordenanza_pdfs") or URBANISMO_ORDENANZA_PDFS
        )
        ...
        req = urllib.request.Request(
            ...
            headers={
                "User-Agent": self.config.get(
                    "user_agent", "poc-bocm-santa-maria-de-la-alameda/1.0"
                )
            },
        )
    ...
    def _collect_ordenanza_pdfs(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for item in self.urbanismo_pdfs:
            url = str(item["url"])
            titulo = str(item.get("titulo") or Path(url).name)
            rows.append(
                {
                    "titulo": titulo[:500],
        ...
        try:
            ...
            return rows
        seen = {r["pdf_url"] for r in rows}
        for m in RE_PDF_HREF.finditer(html):
            pdf = self._abs_wp(m.group(1))
            if pdf in seen:
                continue
            label = unescape(Path(urllib.parse.unquote(pdf)).name)
            if not RE_LICENCIA.search(label) and not RE_PROYECTO.search(label):
                continue
            seen.add(pdf)
            rows.append(
                {
    ...
    def _collect_avance_pgou(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        try:
            html = self._fetch(self.avance_pgou_url)
        except urllib.error.URLError:
            return rows
        h1_m = RE_H1.search(html)
        titulo = _strip_html(h1_m.group(1)) if h1_m else "Avance Plan General"
        rec: dict[str, Any] = {
            "id": _stable_id("proy", self.avance_pgou_url),
            "municipio": MUNICIPIO,
            "titulo": titulo[:500],
            "fecha": "2022-12-14",
            "tipo": "planeamiento",
            "url": self.avance_pgou_url,
            "source": "ayuntamiento",
            "origen": "wp_avance_pgou",
        }
    ...
    def _collect_historico_bandos(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(self.historico_bandos_url)
        except urllib.error.URLError:
            return []
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for m in re.finditer(
            ...
            if href.startswith("http://wp-content") or href.startswith("/wp-content"):
                href = self._abs_wp(href)
            if not href.startswith(self.wp_base) and ".pdf" not in href.lower():
                continue
            blob = f"{label} {href}"
            ...
            rec: dict[str, Any] = {
                ...
                "url": self.historico_bandos_url,
                "source": "ayuntamiento",
                "origen": "historico_bando",
            }
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Ordenanzas municipales — licencias y tasas urbanísticas",
                "url": self.ordenanzas_url,
                "source": "ayuntamiento",
                "nota": "PDFs ordenanzas reguladoras de licencias",
            ...
            },
            {
                ...
                "url": DOSSIER_URL,
                "source": "ayuntamiento",
                "nota": "Procedimientos licencia obra, comunicación previa, etc.",
                "origen": "catalogo_tramites",
            },
            {
                "id": _stable_id("lic", TRANSPARENCY_URL),
                "fecha_concesion": None,
    ...
    def _ordenanza_to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        return {
            "id": _stable_id("lic", row["pdf_url"]),
            "fecha_concesion": row.get("fecha"),
            "tipo": "ordenanza licencia",
            "distrito": None,
            "lat": None,
            "lon": None,
            "titulo": row["titulo"],
            "url": row.get("url") or self.ordenanzas_url,
            "source": "ayuntamiento",
            "nota": "Normativa reguladora; no concesión publicada",
            "origen": row.get("origen"),
            "pdf_url": row["pdf_url"],
        }

    def _ordenanza_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    def _tramite_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if RE_LICENCIA.search(row["titulo"]) and not re.search(
            r"(?i)planeam|actuaci[oó]n urban|modificaci[oó]n del planeamiento|urban|normas",
            row["titulo"],
        ):
            return None
        rec: dict[str, Any] = {
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_licencia_info_pages():
            if rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_ordenanza_pdfs():
            rec = self._ordenanza_to_licencia(item)
        ...
        return {
            ...
            "ordenanzas": sum(1 for r in rows if r.get("origen") == "ordenanza_pdf"),
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_avance_pgou():
            add(rec)
        for item in self._collect_board():
            add(self._board_to_proyecto(item))
        for item in self._collect_ordenanza_pdfs():
            add(self._ordenanza_to_proyecto(item))
        for item in self._collect_historico_bandos():
            add(item)
        for item in self._collect_tramites():
        ...
        return {
            ...
            "avance_pgou": sum(1 for r in rows if r.get("origen") == "wp_avance_pgou"),
            "historico_bandos": sum(1 for r in rows if r.get("origen") == "historico_bando"),
