...
WEB_BASE = "https://www.santamariadelparamo.es"
SEDE_BASE = "https://santamariadelparamo.sedelectronica.es"
...
ID_PREFIX = "santa-maria-del-paramo"
...
DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    (
        f"{WEB_BASE}/es/documentos/solicitud-de-licencia-o-autorizacion-urbanistica",
        "Solicitud de Licencia o Autorización Urbanística",
    ),
    (
        f"{WEB_BASE}/es/formularios/tramites-generales",
        "Trámites generales (licencias y declaraciones urbanísticas)",
    ),
    (
        f"{WEB_BASE}/es/normas-urbanisticas-municipales",
        "Normas urbanísticas municipales",
    ),
    (
        f"{WEB_BASE}/areas-municipales/documentacion/urbanismo",
        "Documentación urbanismo (área municipal)",
    ),
]

DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=157",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=157",
...
RE_PREVIEW = re.compile(
    r'href="((?:https://santamariadelparamo\.sedelectronica\.es)?/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://santamariadelparamo\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_H2 = re.compile(r"<h1[^>]*>([^<]+)</h1>", re.I)
...
class SantaMariaDelParamoAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal web + sede espublico gestiona + PlanPublica PLAU + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=157"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=157"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-santa-maria-del-paramo/1.0")},
        )
    ...
    def _parse_board_links(self, html: str, origen: str) -> list[dict[str, Any]]:
        ...
        for m in RE_PREVIEW.finditer(html):
            ...
            text_m = re.search(rf'href="[^"]*preview-document/[^"]+"[^>]*>([^<]+)<', local, re.I)
            titulo = ""
            if title_m:
        ...
        for url, fallback_title in self.seed_urls:
            ...
            try:
                ...
                if title_m:
                    ...
                    titulo = re.sub(r"\s*-\s*Santa María del Páramo.*$", "", titulo, flags=re.I).strip()
            except (urllib.error.URLError, OSError, ConnectionError):
                pass
