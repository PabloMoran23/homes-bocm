...
SEDE_BASE = "https://soria.sedelectronica.es"
...
DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    (
        f"{SEDE_BASE}/catalog/t/c52dcecf-5159-41d1-8fd8-1d2cf7b69a2a",
        "Declaración responsable para reparaciones y actuaciones sencillas",
    ),
    (
    ...
    (
        f"{SEDE_BASE}/catalog/t/b3e341df-0750-405c-bbfd-911d503efd70",
    ...
    (
        f"{SEDE_BASE}/catalog/t/56a24794-bb82-4b2f-9704-c6b5a07a3061",
        "Solicitud de licencia de Primera ocupación o utilización",
    ),
    (
    ...
    (
        f"{SEDE_BASE}/catalog/t/639337f0-58f8-4ee0-9acd-498b5e97d4ed",
...
RE_PREVIEW = re.compile(
    r'href="(https://soria\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://soria\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class SoriaAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + PlanPublica PLAU/PLAI + IDECyL WFS (capital Soria)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia={PLAU_PROVINCIA}&municipio={PLAU_MUNICIPIO}"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia={PLAU_PROVINCIA}&municipio={PLAU_MUNICIPIO}"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-soria/1.0")},
        )
