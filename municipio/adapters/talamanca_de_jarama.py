...
WP_BASE = "https://www.talamancadejarama.org"
SEDE_BASE = "https://talamancadejrama.sedelectronica.es"
MUNICIPIO = "Talamanca de Jarama"
ID_PREFIX = "talamanca-de-jarama"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "TALAMANCA DE JARAMA"

NORMAS_URL = f"{WP_BASE}/administracion/normas-subsidiarias/"
BOARD_URL = f"{SEDE_BASE}/board"
IMPRESOS_SITEMAP = f"{WP_BASE}/wp-sitemap-posts-impresos-1.xml"

DEFAULT_NNSS_PDFS: list[dict[str, str]] = [
    {
        "url": f"{WP_BASE}/wp-content/uploads/2022/10/NORMAS-URBANISTICAS-Enero2006.pdf",
        "titulo": "NNSS Talamanca — Normas urbanísticas enero 2006",
    },
    {
        "url": f"{WP_BASE}/wp-content/uploads/2022/10/Ordenacion-y-Gestion.pdf",
        "titulo": "NNSS Talamanca — Ordenación y gestión (plano)",
    },
    {
        "url": f"{WP_BASE}/wp-content/uploads/2022/10/Ambitos-de-proteccion-y-afecciones.pdf",
        "titulo": "NNSS Talamanca — Ámbitos de protección y afecciones (plano)",
    },
    {
        "url": f"{WP_BASE}/wp-content/uploads/2024/07/RAT-2024.pdf",
        "titulo": "RAT 2024 — Revisión de las Normas Subsidiarias",
    },
]

RE_PREVIEW = re.compile(
    r'href="(https://talamancadejrama\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra menor|obra mayor|"
    r"primera ocupaci[oó]n|ocupaci[oó]n de (?:v[ií]a|zona)|consulta t[eé]cnica|terrazas|vado)",
)
RE_PROYECTO = re.compile(
    ...
    r"reparcel|bando|edicto|avance|cat[aá]logo|memoria|planos|rat\b|"
...
RE_EXCLUDE = re.compile(
    ...
    r"convocatoria.*plaza|jurado|iae\b|empleo p[uú]blico|limpieza|bolsa de empleo)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|UA|AD|AN|AI|PAU|SAU|S)-\d+[A-Z0-9-]*)\b",
)
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "pgou" in n or "avance" in n:
        return "planeamiento"
    if "nnss" in n or "normas subsidiarias" in n or "normas urban" in n or "rat" in n:
        return "planeamiento"
    if "convenio" in n:
        return "convenio"
    if "bando" in n or "edicto" in n:
        return "edicto"
    if re.search(r"\bue-\d+", n) or re.search(r"\bs-\d+", n):
...
def _sector_ilike_parts(text: str) -> list[str]:
    s = text.strip()
    low = s.lower()
    for marker in (" del pgou", " pgou", " bocm", " aprob", " anuncio", " nnss", " talamanca"):
        if marker in low:
...
class TalamancaDeJaramaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress (NNSS + impresos) + sede espublico (tablón) + SITCM WFS (geometría partial)."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.nnss_pdfs = list(self.config.get("nnss_pdfs") or DEFAULT_NNSS_PDFS)
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-talamanca-de-jarama/1.0")},
        )
    ...
    def _collect_impresos(self) -> list[dict[str, Any]]:
        ...
        seen: set[str] = set()
        try:
            sitemap = self._fetch(IMPRESOS_SITEMAP)
        except urllib.error.URLError:
            return rows
        for m in RE_SITEMAP_LOC.finditer(sitemap):
            ...
            if not RE_LICENCIA.search(slug.replace("-", " ")) and not RE_PROYECTO.search(
                slug.replace("-", " ")
            ):
                continue
            ...
            try:
                ...
                if h1_m:
                    titulo = _strip_html(h1_m.group(1))
            except urllib.error.URLError:
                pass
            rows.append(
                {
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "id": _stable_id("lic", self.normas_url),
                "fecha_concesion": None,
                "tipo": "normas subsidiarias / planeamiento",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Normas subsidiarias de planeamiento urbanístico",
                "url": self.normas_url,
                "source": "ayuntamiento",
                "nota": "Documentación NNSS y RAT 2024",
                "origen": "normas_web",
            },
            {
                "id": _stable_id("lic", f"{self.wp_base}/administracion/descargas-de-impresos-municipales/"),
                "fecha_concesion": None,
                "tipo": "impresos urbanismo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Descargas de impresos municipales (licencias y obras)",
                "url": f"{self.wp_base}/administracion/descargas-de-impresos-municipales/",
                "source": "ayuntamiento",
                "nota": "Formularios licencia obra, declaración responsable, etc.",
                "origen": "impresos_index",
            },
        ]
    ...
    def _nnss_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        rec: dict[str, Any] = {
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_board():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_impresos():
        ...
        return {
            ...
            "impresos": sum(1 for r in rows if r.get("origen") == "impreso_wp"),
            "info": sum(
                1 for r in rows if r.get("origen") in ("sede_tablon", "normas_web", "impresos_index")
            ),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_nnss_pdfs():
            add(self._nnss_to_proyecto(item))
        ...
        return {
            ...
            "status": "ok",
            "tablon": sum(1 for r in rows if r.get("origen") == "tablon"),
            "nnss": sum(1 for r in rows if r.get("origen") == "nnss_pdf"),
