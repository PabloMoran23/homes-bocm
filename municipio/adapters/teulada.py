...
WEB_BASE = "https://www.teuladamoraira.com.es"
SEDE_BASE = "https://teuladamoraira.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
TRANSPARENCY_URL = f"{SEDE_BASE}/transparency/"
GEOPORTAL_URL = "https://geoportal.teuladamoraira.org"
AGENDA_URBANA_URL = "https://agendaurbanateuladamoraira.es"
URBANISMO_WEB_URL = (
    "https://www.teuladamoraira.com.es/attm/Web_php/index.php"
    "?contenido=subapartados_woden&id_boto=43"
)
MUNICIPIO = "Teulada"
ID_PREFIX = "teulada"
...
DEFAULT_TRANSPARENCY_FOLDERS: list[dict[str, str]] = [
    {
        ...
        "nota": "276 documentos en portal transparencia sede (carga AJAX Wicket)",
    },
]
...
RE_PROYECTO = re.compile(
    ...
    r"agenda urbana|banco de tierras)",
)
RE_BOARD_NON_URBAN = re.compile(
    ...
    r"selecci[oó]n de personal|provisiones de puestos|notoriedad.*inmatriculaci)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://teuladamoraira\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
class TeuladaAyuntamientoAdapter(AyuntamientoAdapter):
    """Woden CMS + sede espublico gestiona + ICV InventarioSuSuz WFS + Geoportal Geonet."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-teulada/1.0")},
        )
    ...
    def _collect_info_proyectos(self) -> list[dict[str, Any]]:
        return [
            {
                "titulo": "Geoportal municipal — consulta PGOU y expedientes",
                "fecha": None,
                "url": GEOPORTAL_URL,
                "blob": "visor Geonet visorteulada.geonet.es capa expedientes municipales",
                "origen": "geoportal_info",
            },
            {
                "titulo": "Agenda Urbana Teulada-Moraira",
                "fecha": None,
                "url": AGENDA_URBANA_URL,
                "blob": "agenda urbana participación planeamiento",
                "origen": "agenda_urbana",
            },
            {
                "titulo": "Área municipal de Urbanismo",
                "fecha": None,
                "url": URBANISMO_WEB_URL,
                "blob": "urbanismo web municipal Woden",
                "origen": "web_urb",
            },
        ]
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "source": "ayuntamiento",
                "nota": "Requiere identificación; visor GIS capa expedientes en geoportal",
                "origen": "sede_tramite",
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Geoportal — consulta PGOU y parcelas",
                "url": GEOPORTAL_URL,
                "source": "ayuntamiento",
                "nota": "Visor Geonet (visorteulada.geonet.es); sin API REST pública",
                "origen": "geoportal_info",
            },
        ]
    ...
    def _row_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        rec = {
            "id": _stable_id("proy", f"{row.get('origen', 'info')}:{key}"),
            "municipio": MUNICIPIO,
            "titulo": title,
            "fecha": row.get("fecha"),
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_info_proyectos():
            add(self._row_to_proyecto(item))
