...
WEB_BASE = "https://tordesillas.ayuntamientosdevalladolid.es"
SEDE_BASE = "https://tordesillas.sedelectronica.es"
PLAI_BASE = "https://servicios.jcyl.es/PlanPublica"
WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/ows"
MUNICIPIO = "Tordesillas"
ID_PREFIX = "tordesillas"
PLAI_MUNICIPIO = 165
PLAI_PROVINCIA = 47
...
DEFAULT_INFO_PAGES: list[tuple[str, str]] = [
    ...
    (f"{SEDE_BASE}/transparency", "portal transparencia — urbanismo y obras públicas"),
    (f"{WEB_BASE}/el-municipio/urbanismo", "urbanismo web municipal (Liferay)"),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=47&municipio=165",
        "archivo planeamiento PLAI JCYL (Tordesillas)",
    ),
    ("https://urbanismoenred.jcyl.es/", "Urbanismo en Red JCYL"),
]

RE_PREVIEW = re.compile(
    r'href="(https://tordesillas\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://tordesillas\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"normas urban|num\b|instrumento|sued|sau|peri|pech|"
    r"ordenaci[oó]n detallada|actuaci[oó]n urban|valdegalindo|villamarci)",
)
RE_NOISE = re.compile(
    r"(?i)(acta del pleno|decreto bases|concurso|fiestas|subvenci[oó]n|"
    r"ordenanza fiscal|plusval[ií]a|tasa de piscinas|albergue|taurin|toros|"
    r"oposicion|empleo p[uú]blico|licitaci[oó]n bar|arrastre)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_YMD = re.compile(r"\b((?:19|20)\d{2})(\d{2})(\d{2})\b")
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"SUED[-\s]?\d+|SUNC[-\s]?\d+|SAU[-\s]?\d+|"
    r"SECTOR\s+\d+|SECTOR\s+SUED[-\s]?\d+"
    r")\b",
)
...
def _proyecto_tipo(title: str, instrumento: str = "") -> str:
    ...
    if "plan parcial" in blob:
        return "plan parcial"
    if "plan especial" in blob or "peri" in blob or "pech" in blob:
        return "plan especial"
    if "proyecto de urbaniz" in blob:
    ...
    if "sector" in blob or "sued" in blob or "sau" in blob:
...
def _sector_codes_from_text(text: str) -> list[str]:
    ...
    for m in RE_SECTOR_CODE.finditer(text or ""):
        ...
        code = re.sub(r"SUED-(\d+)", r"SUED-\1", code)
...
class TordesillasAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay ayuntamientosdevalladolid + sede espublico + PLAI JCyL + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-tordesillas/1.0")},
        )
        opener = self._opener if sede else urllib.request.build_opener()
        timeout = 90 if sede else 30
