...
DIPSEGOVIA_BASE = "https://www.dipsegovia.es/web/ayuntamiento-de-torre-val-de-san-pedro"
SEDE_BASE = "https://torrevaldesanpedro.sedelectronica.es"
...
ID_PREFIX = "torre-val-de-san-pedro"
PLAI_MUNICIPIO = 206
PLAI_PROVINCIA = 40
...
DEFAULT_SEED_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=40&municipio=206",
        "Archivo planeamiento urbanístico aprobado (PLAI JCYL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=40&municipio=206",
        "Planeamiento en información pública (PLAI JCYL)",
    ),
    (
        f"{SEDE_BASE}/transparency/3e5646c8-74e2-4464-a4b5-f182833529b5/",
        "Transparencia — urbanismo y obras públicas",
    ),
    (f"{SEDE_BASE}/dossier", "Catálogo de trámites — sede electrónica"),
]

RE_PREVIEW = re.compile(
    r'href="(https://torrevaldesanpedro\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://torrevaldesanpedro\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class TorreValDeSanPedroAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def _fetch(self, url: str, *, sede: bool = False, timeout: int | None = None) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-torre-val-de-san-pedro/1.0")},
        )
    ...
    def _collect_dipsegovia_urbanismo(self) -> list[dict[str, Any]]:
        """Proyectos y documentos listados en la página de urbanismo del portal DipSegovia."""
        ...
        for m in re.finditer(
            r"<i[^>]*fa-folder[^>]*>\s*</i>\s*([^<]{10,200})",
            html,
            re.I,
        ):
            titulo = re.sub(r"\s+", " ", unescape(m.group(1))).strip(" .")
            if len(titulo) < 12:
                continue
            key = titulo.lower()
            if key in seen:
        ...
        for m in re.finditer(
            r'href="(/documents/[^"]+)"[^>]*title="([^"]{8,200})"[^>]*class="document',
            html,
            re.I,
        ):
            doc_path, doc_title = m.group(1), unescape(m.group(2).strip())
            doc_url = doc_path if doc_path.startswith("http") else f"https://www.dipsegovia.es{doc_path}"
            key = doc_url
            if key in seen:
                continue
            seen.add(key)
        ...
        trans_m = re.search(
            r"(https://torrevaldesanpedro\.sedelectronica\.es/transparency/[^\"<\s]+)",
            html,
            re.I,
        )
        if trans_m:
