...
SEDE_BASE = "https://aytotorrejoncalzada.sedelectronica.es"
WP_BASE = "https://www.aytotorrejoncalzada.es"
MUNICIPIO = "Torrejón de la Calzada"
ID_PREFIX = "torrejon-de-la-calzada"

SITCM_VISOR_URL = "https://idem.comunidad.madrid/cartografia/sitcm/html/visor.htm?municipio=149"
UMAP_MAP_URL = (
    "https://umap.openstreetmap.fr/es/map/mapa-interactivo-de-intervenciones-urbanas_1197502"
)
UMAP_DATALAYER_URL = (
    "https://umap.openstreetmap.fr/es/datalayer/1197502/"
    "2eef579e-2d23-4282-ab26-704a83385e4e/"
)

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "TORREJÓN DE LA CALZADA"
...
RE_PROYECTO = re.compile(
    ...
    r"edicto|bocm|intervenci[oó]n|obra p[uú]blica|asfalt|aparcamiento|"
    r"ecoparque|mejora|renovaci[oó]n|\b(?:UE|API|S)-[A-Z0-9][A-Z0-9.-]*\b)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|iae\b|plusval[ií]a|calendario fiscal|jurado|presupuest|"
    r"selecci[oó]n de personal|funcionario|bii\b|residuos|sanci[oó]n.*tr[aá]fico|"
    r"casetas fiestas|convocatoria de jls|decreto de ausencia)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|API|S)-[A-Z0-9][A-Z0-9.-]*)\b",
)
...
def _proyecto_tipo(blob: str) -> str:
    ...
    if re.search(r"(?i)intervenci|asfalt|aparcamiento|ecoparque|renovaci", blob):
        return "obra pública"
...
class TorrejonDeLaCalzadaAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona + WordPress + SITCM WFS + uMap intervenciones urbanas."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.umap_datalayer_url = str(
            self.config.get("umap_datalayer_url") or UMAP_DATALAYER_URL
        )
        self.umap_map_url = str(self.config.get("umap_map_url") or UMAP_MAP_URL)
    ...
    def _collect_umap_intervenciones(self) -> list[dict[str, Any]]:
        try:
            data = self._fetch_json(self.umap_datalayer_url)
        except (urllib.error.URLError, json.JSONDecodeError):
            return []
        if not isinstance(data, dict):
            return []
        ...
        for feat in data.get("features") or []:
            ...
            fid = str(feat.get("id") or name)
            rec: dict[str, Any] = {
                "id": _stable_id("proy", f"umap:{fid}"),
                "municipio": MUNICIPIO,
                "titulo": name[:500],
                "fecha": None,
                "tipo": "intervención urbana",
                "url": self.umap_map_url,
                "source": "ayuntamiento",
                "origen": "umap_intervenciones",
            }
            if isinstance(geom, dict) and geom.get("type") and geom.get("coordinates"):
                rec["geom_geojson"] = geom
                rec["geometry_source"] = "portal_geojson"
                rec["geometry_source_url"] = self.umap_datalayer_url
                rec["coord_source"] = "portal_geometry_centroid"
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Trámites para licencias municipales (modelos normalizados)",
                "url": f"{self.wp_base}/tramites/",
                "source": "ayuntamiento",
                "nota": "Modelos 01-10: licencia, declaración responsable, comunicación previa, etc.",
                "origen": "wp_tramites",
            },
            {
                "id": _stable_id(
                    "lic",
                    f"{self.wp_base}/wp-content/uploads/2024/01/"
                    "Ordenanza-reguladora-del-regimen-de-licencia-y-declaracion-responsable.pdf",
                ),
    ...
    def _collect_transparency_info(self) -> dict[str, Any]:
        return {
            ...
            "source": "ayuntamiento",
            "nota": "48 documentos en sección 7 (Wicket AJAX; consulta manual)",
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "info": sum(
                1
                for r in rows
                if r.get("origen") in ("sede_tablon", "sede_tramite", "wp_tramites", "wp_ordenanza")
            ),
        }
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_sit_ambitos():
            add(rec)
        for rec in self._collect_umap_intervenciones():
            add(rec)
        for item in self._collect_board():
        ...
        return {
            ...
            "umap": sum(1 for r in rows if r.get("origen") == "umap_intervenciones"),
