...
WP_BASE = "https://torremochadejarama.es"
SEDE_BASE = "https://torremochadejarama.sedelectronica.es"
MUNICIPIO = "Torremocha de Jarama"
ID_PREFIX = "torremocha-de-jarama"

DESARROLLO_URL = f"{WP_BASE}/desarrollo/"
FOTOVOLTAICOS_URL = f"{WP_BASE}/proyectos-fotovoltaicos/"
...
WFS_MUNICIPIO = "TORREMOCHA DE JARAMA"

RE_PREVIEW = re.compile(
    r'href="(https://torremochadejarama\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra (?:mayor|menor)|"
    r"primera ocupaci[oó]n|cedula urban|c[eé]dula urban)",
)
RE_PROYECTO = re.compile(
    ...
    r"reparcel|sector|parcela|bando|unidad(?:es)? de ejecuci[oó]n|fotovolt|"
    r"\b(?:UE|S)-\d+[A-Z0-9-]*\b)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|empleo|jurado|selecci[oó]n de personal|"
    r"activaci[oó]n profesional|igualdad|bolsa|cobranza|iae|ibi|calendario fiscal)",
)
...
RE_WP_PDF = re.compile(
    r'href="(https://torremochadejarama\.es/wp-content/uploads/[^"]+\.pdf)"',
    re.I,
)
...
def _sector_ilike_parts(text: str) -> list[str]:
    low = text.lower()
    for marker in (" bocm", " aprob", " anuncio", " del ", " de torre"):
        if marker in low:
...
class TorremochaDeJaramaAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress torremochadejarama.es + sede espublico + ámbitos SITCM WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.desarrollo_url = str(self.config.get("desarrollo_url") or DESARROLLO_URL)
        self.fotovoltaicos_url = str(self.config.get("fotovoltaicos_url") or FOTOVOLTAICOS_URL)
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-torremocha-de-jarama/1.0")},
        )
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            {
                "id": _stable_id("lic", self.desarrollo_url),
                "fecha_concesion": None,
                "tipo": "trámites urbanísticos",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Desarrollo — normas subsidiarias y planeamiento urbanístico",
                "url": self.desarrollo_url,
                "source": "ayuntamiento",
                "nota": "Página informativa del área de desarrollo/urbanismo",
                "origen": "wp_desarrollo",
            },
            {
    ...
    def _collect_nnss_seed(self) -> list[dict[str, Any]]:
        rec: dict[str, Any] = {
            "id": _stable_id("proy", "nnss-vigente"),
            "municipio": MUNICIPIO,
            "titulo": "Normas Subsidiarias de Planeamiento Municipal de Torremocha de Jarama (vigente)",
            "fecha": None,
    ...
    def _collect_fotovoltaicos(self) -> list[dict[str, Any]]:
        try:
            html = self._fetch(self.fotovoltaicos_url)
        except urllib.error.URLError:
            return []
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        ...
        for name, titulo in (
            ("bisbita", "Proyecto fotovoltaico Bisbita — Torremocha de Jarama"),
            ("colimbo", "Proyecto fotovoltaico Colimbo — Torremocha de Jarama"),
            ("la-cereal", "Proyecto fotovoltaico La Cereal — conjunto Bisbita/Colimbo"),
        ):
            key = f"pv:{name}"
            if key in seen:
                continue
            ...
            rec: dict[str, Any] = {
                ...
                "titulo": titulo,
                "fecha": "2023-03-30",
                "tipo": "proyecto fotovoltaico",
                "url": self.fotovoltaicos_url,
                "source": "ayuntamiento",
                "origen": "wp_fotovoltaicos",
            }
            rows.append(rec)
        ...
        for m in RE_WP_PDF.finditer(html):
            ...
            rec = {
                ...
                "municipio": MUNICIPIO,
                "titulo": f"Proyecto fotovoltaico — {titulo}"[:500],
        ...
        drive_url = "https://drive.google.com/drive/folders/1t0zoE30iCT4L09AwMkNE2H4Y10OehuR7"
        if drive_url not in seen:
            seen.add(drive_url)
            rows.append(
                {
                    "id": _stable_id("proy", drive_url),
                    "municipio": MUNICIPIO,
                    "titulo": "Expedientes fotovoltaicos — alegaciones y reparos del Ayuntamiento",
                    "fecha": None,
                    "tipo": "proyecto fotovoltaico",
                    "url": drive_url,
                    "source": "ayuntamiento",
                    "origen": "drive_fotovoltaicos",
                }
            )
        return rows
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_sit_ambitos():
            add(rec)
        for rec in self._collect_nnss_seed():
            add(rec)
        for rec in self._collect_fotovoltaicos():
            add(rec)
        for item in self._collect_board():
        ...
        pv_n = sum(1 for r in rows if r.get("origen", "").startswith("wp_fotovoltaicos") or r.get("origen") == "drive_fotovoltaicos")
        return {
            "rows": len(rows),
            "status": "ok",
            "sit_ambitos": sit_n,
