...
DEFAULT_INFO_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{SEDE_BASE}/citizen-service/2c314061-c7de-4583-aea1-04ba11588afd",
        "servicio en línea URBANISMO — licencias y planeamiento",
    ),
    (f"{DRUPAL_BASE}/normativa", "normativa municipal"),
    (
        "http://www.jcyl.es/plau/lplanes.plau?municipio=09299",
        "archivo planeamiento JCyL (Trespaderne)",
    ),
    (
        "https://ovc.diputaciondeburgos.es/",
        "visor cartográfico OVC Diputación Burgos",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://trespaderne\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://trespaderne\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class TrespaderneAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal Toools (burgos.es) + sede espublico + PLAI JCyL + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-trespaderne/1.0")},
        )
