...
SEDE_BASE = "https://utiel.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WEB_BASE = "https://www.utiel.es"
PGOU_URL = f"{WEB_BASE}/es/pagina/plan-general-ordenacion-urbana"
...
RE_PROYECTO = re.compile(
    ...
    r"cambio de uso|normas subsidiarias|homologaci[oó]n|ordenanza|e[oó]lic|egm|av[ií]cola|"
    r"explotaci[oó]n|tollo|melero|medina|funerari|ordenaci[oó]n)",
)
RE_BOARD_NON_URBAN = re.compile(
    ...
    r"matrimonio|delegaci[oó]n de funciones|bolsa de t[eé]cnico|jurado|"
    r"venta directa|convenio.*abogados|asesoramiento jur[ií]dico)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://utiel\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_PGOU_PDF = re.compile(
    r'href="(/sites/www\.utiel\.es/files/[^"]+\.pdf)"[^>]*(?:title="([^"]*)")?[^>]*>([^<]{0,120})',
    re.I,
)
RE_MOD_NUM = re.compile(r"(?i)modificaci[oó]n\s*(?:n[ºo°.]?\s*)?(\d+)")
...
def _proyecto_tipo(titulo: str) -> str:
    ...
    if "estudio de detalle" in n or "est_det" in n:
...
class UtielAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona (tablón) + web utiel.es PGOU + ICV WFS zonificación (partial)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-utiel/1.0")},
        )
    ...
    def _match_icv_zone(self, titulo: str) -> dict[str, str] | None:
        ...
        for zone in ICV_ZONES:
            ...
            if den and den in norm:
                score = 100.0
            elif "PGOU" in norm or "PLAN GENERAL" in norm or "NORM" in norm:
                score = 50.0
            else:
    ...
    def _should_enrich_board_geometry(self, rec: dict[str, Any], *, licencia: bool) -> bool:
        ...
        if licencia:
            return bool(re.search(r"(?i)pol[ií]gono|parcela|diseminad|tollo|melero", titulo))
        if rec.get("tipo") in {"planeamiento", "información pública", "modificación PGOU"}:
            return True
        return bool(
            re.search(
                r"(?i)planeamiento|normas subsidiarias|pgou|modificaci[oó]n|plan general|egm|tollo",
                titulo,
            )
        )
    ...
    def _collect_pgou_seeds(self) -> list[dict[str, Any]]:
        ...
        def add(titulo: str, pdf_path: str, *, origen: str = "pgou_web") -> None:
            key = pdf_path
            if key in seen:
                return
            seen.add(key)
            url = f"{self.web_base}{pdf_path}" if pdf_path.startswith("/") else pdf_path
            rec: dict[str, Any] = {
        ...
        for m in RE_PGOU_PDF.finditer(html):
            ...
            titulo = link_text or title_attr or pdf_path.split("/")[-1]
            low = pdf_path.lower()
            if not any(
                k in low or k in titulo.lower()
                for k in ("mod", "pgou", "planeam", "memoria", "aprobacion", "ordenacion", "plano")
            ):
                continue
            mod_m = RE_MOD_NUM.search(titulo)
            if mod_m:
                titulo = f"Modificación PGOU nº {mod_m.group(1)} — {title_attr or pdf_path.split('/')[-1]}"
            elif "pgou_1986" in low:
                titulo = "PGOU 1986 (base)"
            elif "memoria_y_normas" in low:
                titulo = "PGOU — memoria y normas"
            add(titulo, pdf_path)

        add("PGOU 1986 — plan general de ordenación urbana", self.pgou_url, origen="pgou_index")
        return rows
        ...
        return [
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Tablón de anuncios — sede electrónica Utiel",
                "url": self.board_url,
                "source": "ayuntamiento",
            ...
            },
            {
                ...
                "source": "ayuntamiento",
                "nota": "Licencias y comunicaciones previas vía sede (autenticación)",
                "origen": "sede_tramite",
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo — área municipal",
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
                "nota": "Enlace PGOU, PIC y sede electrónica",
                "origen": "web_tramite",
            },
        ]
        ...
        return [
            {
                ...
                "municipio": MUNICIPIO,
                "titulo": "Plan General de Ordenación Urbana — documentación",
                "fecha": None,
                "tipo": "plan general",
                "url": self.pgou_url,
                "source": "ayuntamiento",
            ...
            {
                "id": _stable_id("proy", ICV_VISOR_URL),
                "municipio": MUNICIPIO,
    ...
    def _board_is_urban(self, row: dict[str, Any]) -> bool:
        ...
        if any(k in proc for k in ("planeamiento", "licencia", "urban", "actividad", "obra", "medio ambiente")):
            return True
        if "urban" in cat:
            return True
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if RE_LICENCIA.search(blob) and not RE_PROYECTO.search(blob):
            if "informaci" not in blob.lower():
                return None
        proc = (row.get("procedimiento") or "").lower()
        if not RE_PROYECTO.search(blob) and "planeamiento" not in proc and "medio ambiente" not in proc:
            return None

        tipo = "urbanismo"
        if re.search(r"(?i)informaci[oó]n p[uú]blica|estudio de integraci[oó]n|revisi[oó]n aai", blob):
            tipo = "información pública"
        elif re.search(r"(?i)planeamiento|normas subsidiarias|pgou", blob):
            tipo = "planeamiento"
        elif re.search(r"(?i)egm|tollo|pol[ií]gono", blob):
            tipo = "planeamiento"
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_proyecto_info_pages():
            add(rec)
        for seed in self._collect_pgou_seeds():
            add(seed)
        for seed in self._collect_icv_proyectos():
            add(seed)
        for item in self._collect_board():
        ...
        return {
            "rows": len(rows),
            "status": "ok",
            "pgou_web": sum(1 for r in rows if r.get("origen") in ("pgou_web", "pgou_index")),
