...
WP_BASE = "https://www.valdemanco.org"
SEDE_BASE = "https://valdemanco.sedelectronica.es"
MUNICIPIO = "Valdemanco"
ID_PREFIX = "valdemanco"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "VALDEMANCO"

IMPRESOS_URL = f"{WP_BASE}/impresos/"
SERVICIOS_URL = f"{WP_BASE}/servicios-municipales/"
ORDENANZAS_URL = f"{SEDE_BASE}/transparency/04c03607-6da4-46cd-8d65-b743eb02a665/"
...
RE_PREVIEW = re.compile(
    r'href="(https://valdemanco\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"edicto|bando.*parcel|actuaci[oó]n urban|ordenanza|"
    r"\b(?:UE|SAU|S)-[\w\d.\-]+\b)",
)
RE_EXCLUDE = re.compile(
    ...
    r"jurado|juez de paz|pe[oó]n|iae|cobranza|empleo|oposici[oó]n|monitor)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|SAU|S)-[\w\d.\-]+)\b",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_YMD = re.compile(r"\b((?:19|20)\d{2})(\d{2})(\d{2})\b")
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?valdemanco\.org)?/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
class ValdemancoAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress (impresos) + sede espublico (tablón/ordenanzas) + SIT WFS ámbitos."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-valdemanco/1.0")},
        )
    ...
    def _parse_preview_links(self, html: str, origen: str, page_url: str) -> list[dict[str, Any]]:
    ...
    def _parse_board_table(self, html: str, origen: str) -> list[dict[str, Any]]:
        ...
        if not tbody_m:
            return self._parse_preview_links(html, origen, self.board_url)
        rows: list[dict[str, Any]] = []
        for tr in re.findall(r"<tr>(.*?)</tr>", tbody_m.group(1), re.S):
            ...
            rows.append(
                {
                    "titulo": titulo[:500],
                    "fecha": _parse_fecha_dmy(fecha_cell) or _fecha_from_blob(titulo),
    ...
    def _collect_ordenanzas(self) -> list[dict[str, Any]]:
        try:
            ...
            return []
        return self._parse_preview_links(html, "ordenanzas_transparencia", self.ordenanzas_url)
        ...
        return [
            {
                ...
                "url": self.board_url,
                "source": "ayuntamiento",
                "nota": "Anuncios y exposiciones públicas; sin licencias urbanísticas publicadas actualmente",
                "origen": "sede_tablon",
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Impresos municipales — licencias de obra",
                "url": self.impresos_url,
                "source": "ayuntamiento",
            ...
            },
            {
                ...
                "fecha_concesion": None,
                "tipo": "sede electrónica instancias",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Sede electrónica — presentar instancia urbanística",
                "url": f"{self.sede_base}/info.0",
                "source": "ayuntamiento",
                "nota": "Presentación electrónica vía Cl@ve (espublico gestiona)",
                "origen": "sede_instancia",
            },
        ]
    ...
    def _fetch_geometry(self, titulo: str) -> dict[str, Any] | None:
        if RE_EXCLUDE.search(titulo or ""):
            return None
        geom, meta = resolve_ambito_geometry(self.wfs_municipio, titulo or "")
        if geom:
            return {
                "geom_geojson": geom,
            ...
            if feat:
                ...
                if merged:
                    ...
                    return {
                        ...
                        "geometry_source": "portal_wfs",
                        "geometry_source_url": (
                            f"{self.wfs_url}?service=WFS&request=GetFeature&CQL_FILTER="
    ...
    def _form_to_licencia(self, row: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "lon": None,
            "titulo": row["titulo"],
            "url": row.get("url") or self.impresos_url,
            "source": "ayuntamiento",
    ...
    def _row_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        rec: dict[str, Any] = {
            ...
            "url": row.get("url") or row.get("page_url") or self.impresos_url,
            "source": "ayuntamiento",
            "origen": row.get("origen"),
        }
        if row.get("pdf_url"):
    ...
    def _row_to_licencia(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not RE_LICENCIA.search(blob):
            return None
        key = row.get("expediente") or row.get("url") or blob
        return {
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            "rows": len(rows),
            "status": "ok",
            "formularios": sum(1 for r in rows if r.get("origen") == "impresos_web"),
    ...
    def update_licencias(self, out_jsonl: Path, state_path: Path) -> dict[str, Any]:
        ...
        for rec in self._collect_licencia_info_pages():
            existing[rec["id"]] = rec
        for item in self._collect_impresos():
            existing[self._form_to_licencia(item)["id"]] = self._form_to_licencia(item)
        for item in self._collect_board():
            rec = self._row_to_licencia(item)
            if rec:
                existing[rec["id"]] = rec
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "ordenanzas": sum(1 for r in rows if r.get("origen") == "ordenanzas_transparencia"),
