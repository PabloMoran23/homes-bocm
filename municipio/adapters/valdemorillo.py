...
DEFAULT_NNSS_PDFS: list[dict[str, str]] = [
    {"url": f"{WP_BASE}/wp-content/uploads/2022/02/acuerdo.pdf", "titulo": "NNSS Valdemorillo — Acuerdo"},
    {"url": f"{WP_BASE}/wp-content/uploads/2022/02/catalogo.pdf", "titulo": "NNSS Valdemorillo — Catálogo"},
    {"url": f"{WP_BASE}/wp-content/uploads/2022/02/indice_nurbanisticas.pdf", "titulo": "NNSS Valdemorillo — Índice"},
    {"url": f"{WP_BASE}/wp-content/uploads/2022/02/memoria.pdf", "titulo": "NNSS Valdemorillo — Memoria"},
    {
        "url": f"{WP_BASE}/wp-content/uploads/2022/02/nurbanisticas_valdemorillo.pdf",
        "titulo": "NNSS Valdemorillo — Normas urbanísticas",
    },
    {"url": f"{WP_BASE}/wp-content/uploads/2022/02/planos-1_5.pdf", "titulo": "NNSS Valdemorillo — Planos 1-5"},
    {"url": f"{WP_BASE}/wp-content/uploads/2022/02/planos-6_10.pdf", "titulo": "NNSS Valdemorillo — Planos 6-10"},
    {"url": f"{WP_BASE}/wp-content/uploads/2022/02/planos-11_15.pdf", "titulo": "NNSS Valdemorillo — Planos 11-15"},
    {"url": f"{WP_BASE}/wp-content/uploads/2022/02/planos-16_18.pdf", "titulo": "NNSS Valdemorillo — Planos 16-18"},
]

RE_PREVIEW = re.compile(
    r'href="(https://aytovaldemorillo\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
...
RE_LICENCIA = re.compile(
    ...
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra menor|vado)",
)
RE_PROYECTO = re.compile(
    ...
    r"reparcel|bando|edicto|avance|cat[aá]logo|memoria|planos|"
    r"\b(?:UE|UA|AD|AN|AI|PAU|SAU|S)-\d+\b)",
)
RE_EXCLUDE = re.compile(
    r"(?i)(padr[oó]n|presupuest|funcionario|empleado|plusvalia|basura|"
    r"residuos|vehiculos|notificaci[oó]n expediente|igualdad|cineg[eé]tica|"
    r"convocatoria.*plaza|jurado|iae\b|infofma|barbacoa|ordenanza de convivencia)",
)
...
def _sector_ilike_parts(text: str) -> list[str]:
    s = text.strip()
    low = s.lower()
    for marker in (" del pgou", " pgou", " bocm", " aprob", " anuncio", " nnss", " valdemorillo"):
        if marker in low:
...
class ValdemorilloAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress urbanismo + sede espublico (tablón) + SITCM WFS (geometría partial)."""
    ...
    def _parse_board_table(self, html: str, origen: str) -> list[dict[str, Any]]:
        tbody_m = re.search(r'<tbody[^>]*id="idd"[^>]*>(.*?)</tbody>', html, re.S | re.I)
        if not tbody_m:
    ...
    def _collect_urbanismo_tramites(self) -> list[dict[str, Any]]:
        try:
        ...
        for m in RE_CATALOG.finditer(html):
            ...
            if not RE_LICENCIA.search(titulo) and not RE_PROYECTO.search(titulo):
                continue
            rows.append({"titulo": titulo[:500], "url": url, "origen": "urbanismo_tramite"})
        return rows
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Concejalía de Urbanismo — formularios y trámites",
                "url": self.urbanismo_url,
                "source": "ayuntamiento",
                "nota": "Catálogo de trámites licencias y declaraciones responsables",
            ...
            },
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Portal de transparencia — Urbanismo y obras públicas",
                "url": self.transparency_url,
                "source": "ayuntamiento",
                "nota": "Documentación PGOU/NNSS (navegación Wicket)",
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_board():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_urbanismo_tramites():
        ...
        return {
            ...
            "tramites": sum(1 for r in rows if r.get("origen") == "urbanismo_tramite"),
            "info": sum(1 for r in rows if r.get("origen") in ("sede_tablon", "urbanismo_web", "transparencia")),
        }
