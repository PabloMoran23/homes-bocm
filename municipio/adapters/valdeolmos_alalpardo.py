...
WP_BASE = "https://www.valdeolmos-alalpardo.org"
SEDE_BASE = "https://valdeolmos-alalpardo.sedelectronica.es"
MUNICIPIO = "Valdeolmos-Alalpardo"
ID_PREFIX = "valdeolmos-alalpardo"

WFS_BASE = "https://idem.comunidad.madrid/geoserver3/ows"
WFS_TYPE = "sitcm:VPLA_V_AMBITO"
WFS_MUNICIPIO = "VALDEOLMOS-ALALPARDO"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/normativa-urbanistica/",
    f"{WP_BASE}/normativa-urbanistica/normas-subsidiarias/",
    f"{WP_BASE}/normativa-urbanistica/planes-especiales/",
    f"{WP_BASE}/portal-de-transparencia/planeamiento-urbanistico-ayuntamiento/",
]

DEFAULT_LICENCIA_PAGES: list[str] = [
    f"{WP_BASE}/normativa-urbanistica/",
    f"{WP_BASE}/wp-login-phploggedouttruewp_langes_es/tramites-de-urbanismo/",
    f"{WP_BASE}/wp-login-phploggedouttruewp_langes_es/tramites-de-urbanismo/solicitud-de-licencia-de-obra-mayor/",
    f"{WP_BASE}/wp-login-phploggedouttruewp_langes_es/tramites-de-urbanismo/formulario-declaracion-responsable-urbanistica-de-obras/",
    f"{WP_BASE}/wp-login-phploggedouttruewp_langes_es/tramites-de-urbanismo/formulario-declaracion-responsable-inicio-de-obras/",
    f"{WP_BASE}/wp-login-phploggedouttruewp_langes_es/tramites-de-urbanismo/formulario-declaracion-responsable-de-primera-ocupacion-2020/",
    f"{WP_BASE}/wp-login-phploggedouttruewp_langes_es/tramites-de-urbanismo/solicitud-de-licencia-de-segregacion-o-division/",
]

RE_PREVIEW = re.compile(
    r'href="(https://valdeolmos-alalpardo\.sedelectronica\.es/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra menor|obra mayor|"
    r"primera ocupaci[oó]n|segregaci[oó]n|divisi[oó]n)",
)
RE_PROYECTO = re.compile(
    ...
    r"edicto|actuaci[oó]n urban|suelo|convenio|zona especial|z\.e\.p\.a|"
    r"planos de ordenaci[oó]n|memoria de planteamiento|catalogo|"
    r"\b(?:UE|AD|AN|AI|PAU|S|SAU|APD)-\d+[A-Z0-9-]*\b)",
)
RE_EXCLUDE = re.compile(
    ...
    r"tasa.*funerari|ordenanza fiscal)",
)
RE_AMBIT_CODE = re.compile(
    r"(?i)\b((?:UE|AD|AN|AI|PAU|S|SAU|APD)-\d+[A-Z0-9-]*)\b",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_FECHA_YM = re.compile(r"/(\d{4})/(\d{2})/")
RE_BOCM_DATE = re.compile(r"BOCM[- ](\d{4})(\d{2})(\d{2})")
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?(?:valdeolmos-alalpardo\.org|valdeolmos-alalpardo\.eu))?/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
def _proyecto_tipo(title: str) -> str:
    ...
    if "zona especial" in n or "z.e.p.a" in n or "zpc" in n:
...
class ValdeolmosAlalpardoAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress normativa urbanística + sede espublico gestiona + ámbitos SIT WFS."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-valdeolmos-alalpardo/1.0")},
        )
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "id": _stable_id("lic", f"{WP_BASE}/tramites-urbanismo"),
                "fecha_concesion": None,
                "tipo": "trámites licencia y obra",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Trámites de urbanismo — solicitudes y declaraciones responsables",
                "url": f"{WP_BASE}/wp-login-phploggedouttruewp_langes_es/tramites-de-urbanismo/",
                "source": "ayuntamiento",
                "nota": "Formularios web; concesiones individuales en tablón cuando se publiquen",
                "origen": "tramites_web",
            },
        ]
    ...
    def _collect_licencia_forms(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.licencia_pages:
            try:
                ...
                continue
            page_title = _strip_html(re.search(r"<title>([^<]+)</title>", html, re.I).group(1) if re.search(r"<title>", html) else page_url)
            if RE_LICENCIA.search(page_title) or RE_LICENCIA.search(page_url):
                key = page_url
                if key not in seen:
                    seen.add(key)
                    rows.append(
    ...
    def backfill_licencias(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        return {
            "rows": len(rows),
            "status": "ok",
            "tramites": sum(1 for r in rows if r.get("origen") in ("tramites_web", "formulario_web")),
