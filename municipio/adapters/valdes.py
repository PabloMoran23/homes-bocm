...
BASE = "https://www.valdes.es"
SEDE_BASE = "https://ayuntamientodevaldes.sede.e-ayuntamiento.es"
ICAST_URBANISMO = "http://urbanismo.i-cast.es/Valdes/"
...
WFS_FILTER = "Instrumento LIKE '%VALD%'"
...
RE_PROYECTO = re.compile(
    ...
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|gesti[oó]n urban|reforma interior|"
    r"luarca|cadavedo|otur|barzana|trevias)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://www\.valdes\.es)?/documents/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Vald[eé]s.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
def _tipo_proyecto(denominacion: str, clasificacion: str = "") -> str:
    ...
    if "plan general" in blob or "pgou" in blob or "pgo " in blob or blob.startswith("pgo"):
        return "PGOU"
    if "plan especial" in blob or "pe " in blob or "reforma interior" in blob:
...
class ValdesAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay valdes.es + RPGUR Asturias; geometría parcial WFS visorrpgur."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "34")
    ...
    def _fetch(self, url: str, *, data: bytes | None = None, encoding: str = "utf-8") -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-valdes/1.0"),
        }
        if data is not None:
    ...
    def _crawl_liferay_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in re.finditer(r'href="(https://www\.valdes\.es/[^"#?]+)"', html, re.I):
        ...
        rows.append(
            {
                ...
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo y vivienda — trámites y normativa",
                "url": info_url,
                "source": "ayuntamiento",
                "nota": "Sin tablón público scrapeable; sede e-ayuntamiento inaccesible",
            }
        )

        sede_id = _stable_id("lic", SEDE_BASE)
            ...
            rows.append(
                {
                    ...
                    "lat": None,
                    "lon": None,
                    "titulo": "Sede electrónica — licencias urbanísticas",
                    "url": SEDE_BASE,
                    "source": "ayuntamiento",
                    "nota": "Sede ayuntamientodevaldes.sede.e-ayuntamiento.es inaccesible (timeout)",
                }
            )

        icast_id = _stable_id("lic", ICAST_URBANISMO)
        if icast_id not in seen:
            seen.add(icast_id)
            rows.append(
                {
                    "id": icast_id,
                    ...
                    "lat": None,
                    "lon": None,
                    "titulo": "Visor urbanístico i-cast (Valdés)",
                    "url": ICAST_URBANISMO,
                    "source": "ayuntamiento",
                    "nota": "Portal urbanismo.i-cast.es no responde desde el agente",
                }
            )
