...
WP_BASE = "https://www.valenciadedonjuan.es"
SEDE_BASE = "https://valenciadedonjuan.sedelectronica.es"
...
ID_PREFIX = "valencia-de-don-juan"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/secciones/areas/urbanismo-y-obras/",
    (
        f"{WP_BASE}/solicitudes-y-descargas/"
        "archivo-de-planeamiento-urbanistico-y-ordenacion-del-territorio-vigente-plau/"
    ),
]
...
DEFAULT_TRAMITE_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{SEDE_BASE}/catalog/t/0f211517-fa3a-4ddd-ada1-75ce64a718bf",
...
RE_PREVIEW = re.compile(
    r'href="(https://valenciadedonjuan\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://valenciadedonjuan\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
RE_PROYECTO = re.compile(
    ...
    r"pau\b|pa integrada)",
)
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"NC\d+|ULD[- ]?I[- ]?\d+|PA[- ]?[A-Z]?\d+|UA\.?\d+|"
    r"PE[- ]?\d+|AR[- ]?P\d+|UNC\d+|"
...
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?valenciadedonjuan\.es)?/wp-content/uploads/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
def _proyecto_tipo(blob: str, instrumento: str = "") -> str:
    ...
    if "estudio de detalle" in n or re.search(r"\bed\b", n):
    ...
    if "plan parcial" in n or "sector" in n:
        return "plan parcial"
    if "pau" in n or "pa integrada" in n:
        return "plan de actuación"
    if "licencia" in n:
        return "licencia"
...
class ValenciaDeDonJuanAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress + sede espublico gestiona + PlanPublica PLAU + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=188"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=188"
        )
        raw_seeds = self.config.get("seed_pages") or DEFAULT_SEED_PAGES
        self.seed_pages = [str(u) for u in raw_seeds]
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-valencia-de-don-juan/1.0")},
        )
    ...
    def _collect_wp_seeds(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        seen: set[str] = set()
        for url in self.seed_pages:
            ...
            for m in RE_PDF_HREF.finditer(html):
                ...
                if not pdf_url.startswith("http"):
                    pdf_url = f"{self.wp_base}{pdf_url}"
                if pdf_url in seen:
                    continue
                seen.add(pdf_url)
                titulo = unescape(pdf_url.rsplit("/", 1)[-1].replace("-", " ").replace(".pdf", ""))
                rows.append(
                    {
                        "titulo": titulo[:500],
                        "url": pdf_url,
                        "fecha": _fecha_from_blob(pdf_url),
                        "origen": "web_municipal",
                    }
                )
            for m in re.finditer(
                ...
                link, titulo = m.group(1), unescape(m.group(2).strip())
                if not link.startswith("http"):
                    link = f"{self.wp_base}{link}" if link.startswith("/") else f"{self.wp_base}/{link}"
                if link in seen or "valenciadedonjuan" not in link:
                    continue
    ...
    def backfill_proyectos(self, out_jsonl: Path) -> dict[str, Any]:
        ...
        for item in self._collect_board():
            ...
            if rec and rec["id"] not in seen:
                seen.add(rec["id"])
                rows.append(rec)
        for item in self._collect_wp_seeds():
