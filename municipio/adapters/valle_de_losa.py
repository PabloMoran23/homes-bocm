...
WP_BASE = "https://www.valledelosa.es"
SEDE_BASE = "https://valledelosa.sedelectronica.es"
...
RE_PREVIEW = re.compile(
    r'href="(?:https://valledelosa\.sedelectronica\.es/)?preview-document/([a-f0-9-]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://valledelosa\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class ValleDeLosaAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=9&municipio=908"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=9&municipio=908"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-valle-de-losa/1.0")},
        )
    ...
    def _parse_board_table(self, html: str, origen: str) -> list[dict[str, Any]]:
        ...
        for tr in re.findall(r"<tr>(.*?)</tr>", tbody_m.group(1), re.S):
            ...
            preview = self._preview_url(link_m.group(1)) if link_m else self.board_url
            rows.append(
                {
                    "titulo": titulo[:500],
                    "doc_label": doc[:500],
                    ...
                    "url": preview,
                    "pdf_url": preview if link_m else None,
                    "origen": origen,
                }
            )
        return rows
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if RE_LICENCIA.search(blob) and not RE_PROYECTO.search(blob):
            return None
        categoria = str(row.get("categoria") or "").lower()
        if categoria in {"urbanismo", "anuncios"} and RE_PROYECTO.search(blob):
            pass
        elif categoria == "urbanismo":
            pass
