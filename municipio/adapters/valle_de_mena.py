...
SEDE_BASE = "https://valle-de-mena.sedelectronica.es"
...
RE_PREVIEW = re.compile(
    r'href="(https://valle-de-mena\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://valle-de-mena\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class ValleDeMenaAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal Toools (burgos.es) + sede espublico + PlanPublica PLAU + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09provincia=37&municipio=342municipio=410"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=09provincia=37&municipio=342municipio=410"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-valle-de-mena/1.0")},
        )
