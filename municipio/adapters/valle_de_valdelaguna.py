...
WEB_BASE = "https://www.valledevaldelaguna.es"
SEDE_BASE = "https://valledevaldelaguna.sedelectronica.es"
...
ID_PREFIX = "valle-de-valdelaguna"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WEB_BASE}/informacion-general",
    f"{WEB_BASE}/noticias",
    f"{WEB_BASE}/noticias/modificacion-puntual-normas-subsidiarias-municipales",
]
...
DEFAULT_LICENCIA_INFO_PAGES: list[tuple[str, str]] = [
    (f"{SEDE_BASE}/board", "Tablón de anuncios — licencias y urbanismo"),
    (f"{SEDE_BASE}/catalog", "Catálogo de trámites — sede electrónica"),
    (f"{WEB_BASE}/informacion-general", "Información general — planeamiento urbanístico"),
    (f"{SEDE_BASE}/", "Sede electrónica — trámites urbanísticos"),
]

RE_PREVIEW = re.compile(
    r'href="(https://valledevaldelaguna\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://valledevaldelaguna\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_DRUPAL_NEWS = re.compile(
    r'href="((?:https://www\.valledevaldelaguna\.es)?/noticias/[^"#?]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_DRUPAL_TITLE = re.compile(r"<title>([^<|]+)", re.I)
RE_DRUPAL_DATE = re.compile(r'datetime="(\d{4}-\d{2}-\d{2})"', re.I)
...
class ValleDeValdelagunaAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal Toools + sede espublico gestiona + PlanPublica PLAU + IDECyL WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=414"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=09&municipio=414"
        )
        raw_info = self.config.get("licencia_info_pages") or DEFAULT_LICENCIA_INFO_PAGES
        self.licencia_info_pages: list[tuple[str, str]] = []
        for item in raw_info:
            if isinstance(item, dict):
                self.licencia_info_pages.append((str(item["url"]), str(item.get("titulo") or item["url"])))
            elif isinstance(item, (list, tuple)) and len(item) >= 2:
                self.licencia_info_pages.append((str(item[0]), str(item[1])))
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-valle-de-valdelaguna/1.0")},
        )
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for url, titulo in self.licencia_info_pages:
            rows.append(
                {
    ...
    def _collect_drupal_news(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return rows
        for m in RE_DRUPAL_NEWS.finditer(html):
            ...
            try:
                ...
                date_m = RE_DRUPAL_DATE.search(detail)
                if date_m:
                    fecha = date_m.group(1)
                title_m = RE_DRUPAL_TITLE.search(detail)
                if title_m:
