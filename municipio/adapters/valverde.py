...
WP_BASE = "https://aytovalverde.org"
SEDE_BASE = "https://aytovalverde.sedelectronica.es"
MUNICIPIO = "Valverde"
ID_PREFIX = "valverde"
SITCAN_PACKAGE = "planeamiento-urbanistico-de-valverde"

DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/servicios/urbanismo-y-vivienda/",
    f"{WP_BASE}/servicios/sede-electronica/",
    f"{WP_BASE}/valverde-inicia-la-adaptacion-plan-general-ordenacion-al-modelo-turistico-insular/",
    f"{WP_BASE}/ayuntamiento-garantiza-la-construccion-la-cancha-polideportiva-san-andres-se-ajusta-al-planeamiento-municipal/",
    f"{WP_BASE}/convocatoria-vecinal-telematica-sobre-el-plan-de-movilidad-urbano-sostenible/",
    f"{WP_BASE}/bienvenido-a-la-encuesta-del-plan-de-movilidad-de-valverde/",
    f"{SEDE_BASE}/board",
]

RE_PREVIEW = re.compile(
    r'href="(https://aytovalverde\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    ...
    r"tasa por (?:licencia|prestaci[oó]n))",
)
RE_PROYECTO = re.compile(
    ...
    r"texto refundido|suspensi[oó]n de determinaciones|callejero|plusval[ií]a)",
)
RE_WP_LINK = re.compile(r'<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', re.I | re.S)
RE_PDF_HREF = re.compile(
    r'href="((?:https?://(?:www\.)?aytovalverde\.org)?/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
class ValverdeAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress PageLines + sede espublico gestiona + SITCAN CKAN + GEOBDP Grafcan (Canarias)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-valverde/1.0")},
        )
    ...
    def _parse_board_table(self, html: str) -> list[dict[str, Any]]:
        ...
        for tr in re.findall(r"<tr>(.*?)</tr>", tbody_m.group(1), re.S):
            ...
            blob = f"{titulo} {procedimiento} {categoria} {descripcion}"
            rows.append(
                {
                    "titulo": titulo[:500],
                    "fecha": fecha,
                    ...
                    "blob": blob,
                    "origen": "sede_tablon",
                    "urls": [link_m.group(1)] if link_m else [self.board_url],
    ...
    def _collect_board(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return []
        for rec in self._parse_board_table(html):
            by_url[rec["url"]] = rec
    ...
    def _discover_wp_posts(self) -> list[str]:
        ...
        for i in range(1, 5):
            ...
            for m in RE_SITEMAP_LOC.finditer(xml):
                ...
                ) and "plan-de-empleo" not in low and "plan-extraordinario" not in low:
                    if loc not in seen:
                        seen.add(loc)
                        urls.append(loc)
        return urls
    ...
    def _collect_licencia_tramites(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "id": _stable_id("lic", f"{self.wp_base}/servicios/urbanismo-y-vivienda/"),
                "fecha_concesion": None,
                "tipo": "trámite informativo",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Urbanismo y vivienda — formularios y trámites",
                "url": f"{self.wp_base}/servicios/urbanismo-y-vivienda/",
                "source": "ayuntamiento",
                "origen": "tramite_informativo",
            },
        ]
