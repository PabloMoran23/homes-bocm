...
SEDE_BASE = "https://vejerdelafrontera.sedelectronica.es"
BOARD_URL = f"{SEDE_BASE}/board"
WEB_BASE = "https://www.vejer.es"
URBANISMO_URL = f"{WEB_BASE}/es/ayuntamiento/urbanismo"
MUNICIPIO = "Vejer de la Frontera"
ID_PREFIX = "vejer-de-la-frontera"

DEFAULT_URBANISMO_PAGES: list[str] = [
    URBANISMO_URL,
    f"{WEB_BASE}/es/ayuntamiento/urbanismo?view=categories",
    f"{WEB_BASE}/es/ayuntamiento/urbanismo/14-planeamiento",
    f"{WEB_BASE}/es/ayuntamiento/urbanismo/220-proyectos-de-actuacion",
    f"{WEB_BASE}/es/ayuntamiento/urbanismo/288-ed-aip-3-plaza-de-la-paz-ii",
    f"{WEB_BASE}/es/ayuntamiento/urbanismo/155-modificado-proyecto-urbanizable-suo-1-buenavista",
    f"{WEB_BASE}/es/ayuntamiento/urbanismo/169-aip-1-calle-santiago",
    f"{WEB_BASE}/es/ayuntamiento/urbanismo/172-aip-2-plaza-de-la-plaza-de-la-paz",
    f"{WEB_BASE}/es/pgou-2016",
    f"{WEB_BASE}/es/convocatorias/anuncios",
]
...
RE_PROYECTO = re.compile(
    ...
    r"\b(?:AR|AO|SLN|SLV|SUO|SUS|AIP|ED)-\d+[A-Z0-9-]*\b|consulta p[uú]blica|"
    r"documento t[eé]cnico|adaptaci[oó]n.*loua|actuaci[oó]n|urbanizable|nnss|peprich)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"cobranza iae|padrones|polic[ií]a local|ausencia alcald|"
    r"contrataci[oó]n laboral|orden d[ií]a pleno|censo electoral)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://vejerdelafrontera\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
...
RE_EXCLUDE_FORM = re.compile(
    ...
    r"modelo de solicitud|solicitudes de urbanismo$|nuevos modelos afo$)",
)
RE_URBANISMO_PATH = re.compile(
    r"(?i)(/es/ayuntamiento/urbanismo|/es/pgou-|phocadownload|\?download=|\.pdf(?:\?|$))",
)
...
def _proyecto_tipo(blob: str) -> str:
    b = blob.lower()
    if "aip" in b or "estudio de detalle" in b:
        return "estudio de detalle / AIP"
    if "suo-" in b or "urbanizable" in b:
        return "sector urbanizable"
    if "plan especial" in b or "peprich" in b:
...
class VejerDeLaFronteraAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico gestiona (tablón) + web Joomla Phoca Download (urbanismo PDFs)."""
        ...
        req = urllib.request.Request(
            ...
            headers={
                "User-Agent": self.config.get(
                    "user_agent",
                    "Mozilla/5.0 (compatible; poc-bocm-vejer-de-la-frontera/1.0)",
                )
            },
        )
    ...
    def _discover_urbanismo_links(self, html: str, base_url: str) -> list[str]:
        links: list[str] = []
        for m in RE_LINK.finditer(html):
            href = m.group(1)
            if not RE_URBANISMO_PATH.search(href):
                continue
            url = self._abs_url(href, base_url).split("#")[0].rstrip("/")
            if url.startswith(self.web_base) and url not in links:
                links.append(url)
        return links
    ...
    def _collect_urbanismo_web(self) -> list[dict[str, Any]]:
        ...
        while queue:
            ...
            for m in RE_LINK.finditer(html):
                ...
                is_doc = (
                    "?download=" in doc_url
                    or "phocadownload" in doc_url
                    or re.search(r"(?i)\.(pdf|zip)(?:\?|$)", doc_url)
                )
                if not is_doc:
                    continue
                blob = f"{anchor} {doc_url} {page_url}"
            ...
            for link in self._discover_urbanismo_links(html, page_url):
                if link not in visited and link not in queue and len(visited) < 40:
                    queue.append(link)
