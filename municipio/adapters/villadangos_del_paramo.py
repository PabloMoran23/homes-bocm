...
WEB_BASE = "https://www.aytovilladangosdelparamo.es"
SEDE_BASE = "https://villadangosdelparamo.sedelectronica.es"
MUNICIPIO = "Villadangos del Páramo"
ID_PREFIX = "villadangos-del-paramo"
...
DEFAULT_SEED_URLS: list[tuple[str, str]] = [
    ...
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=205",
        "Planeamiento urbanístico en información pública (Junta CYL)",
    ),
    (
        "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=205",
        "Archivo planeamiento urbanístico aprobado (Junta CYL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://villadangosdelparamo\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="(https://villadangosdelparamo\.sedelectronica\.es/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class VilladangosDelParamoAyuntamientoAdapter(AyuntamientoAdapter):
    """OpenCms web (Diputación León) + sede espublico tablón + SIUCyL WFS sectores."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-villadangos-del-paramo/1.0")},
        )
