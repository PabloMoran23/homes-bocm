...
SEDE_BASE = "https://villamurieldecerrato.sedelectronica.es"
...
ID_PREFIX = "villamuriel-de-cerrato"
PLAI_MUNICIPIO = 217
PLAI_PROVINCIA = 34
...
DEFAULT_INFO_PAGES: list[tuple[str, str]] = [
    ...
    (
        f"{SEDE_BASE}/citizen-service/256bf975-701d-4ee8-81e3-f2815195d8a9",
        "servicio en línea — información pública",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=34&municipio=217",
        "Archivo planeamiento urbanístico aprobado (JCyL)",
    ),
    (
        f"{PLAI_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=34&municipio=217",
        "Planeamiento en información pública (JCyL)",
    ),
]

RE_PREVIEW = re.compile(
    r'href="(https://villamurieldecerrato\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://villamurieldecerrato\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
...
class VillamurielDeCerratoAyuntamientoAdapter(AyuntamientoAdapter):
    """Sede espublico + PLAI JCyL + IDECyL WFS (geometría partial). Sin web corporativa."""
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-villamuriel-de-cerrato/1.0")},
        )
