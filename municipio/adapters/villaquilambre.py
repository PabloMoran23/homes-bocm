...
WP_BASE = "https://www.villaquilambre.es"
WP_API = f"{WP_BASE}/wp-json/wp/v2"
SEDE_BASE = "https://villaquilambre.sedelectronica.es"
MUNICIPIO = "Villaquilambre"
ID_PREFIX = "villaquilambre"

WFS_BASE = "https://idecyl.jcyl.es/geoserver/urbanismo/wfs"
WFS_MUNICIPIO = "Villaquilambre"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{WP_BASE}/atencion-al-ciudadano/urbanismo/",
    f"{WP_BASE}/atencion-al-ciudadano/urbanismo/edictos/",
    f"{WP_BASE}/atencion-al-ciudadano/urbanismo/proyectos/",
    f"{WP_BASE}/atencion-al-ciudadano/urbanismo/plan-general-de-ordenacion-urbana/",
    f"{WP_BASE}/atencion-al-ciudadano/urbanismo/estudio-detalle-sur-01/",
    f"{WP_BASE}/atencion-al-ciudadano/urbanismo/licencias/",
    f"{WP_BASE}/atencion-al-ciudadano/urbanismo/informacion-tecnica/",
    (
        f"{WP_BASE}/atencion-al-ciudadano/urbanismo/"
        "convenio-urbanistico-de-obtencion-de-terrenos-de-las-calles-remeson-y-juan-de-juni/"
    ),
    "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=24&municipio=241",
    "https://servicios.jcyl.es/PlanPublica/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=24&municipio=241",
]

RE_PREVIEW = re.compile(
    r'href="(https://villaquilambre\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra mayor|obra menor|"
    r"primera ocupaci[oó]n|lpo\b|solicitud de obra|ocupaci[oó]n de v[ií]a p[uú]blica)",
)
RE_PROYECTO = re.compile(
    ...
    r"edicto|bando.*parcel|actuaci[oó]n urban|urbanizaci[oó]n|convenio urban|"
    r"memoria|planos|bocyl|sur-\d+|ua-\d+|su-nc)",
)
RE_WP_EXCLUDE = re.compile(
    r"(?i)(fiestas|cine|empleo|concurso|deporte|bols[ií]n taurino|violencia de g[eé]nero|"
    r"subvenci[oó]n deportiv|empadron|tribut|matrimonio|carnaval|turismo activo|"
    r"senderos|ornitol[oó]gic|museo|gastronom|ciami|igualdad)",
)
RE_EXCLUDE = re.compile(
    ...
    r"incendio forestal|bolsa de trabajo|bop n)",
)
...
RE_PDF_HREF = re.compile(
    r'href=["\']((?:https?://(?:www\.)?villaquilambre\.es)?(?:/wordpress)?/wp-content/uploads/[^"\']+\.pdf[^"\']*)["\']',
    re.I,
)
...
def _normalize_sector_code(raw: str) -> str:
    ...
    code = re.sub(r"SU-NC-(\d+)", r"SU-NC-\1", code)
    return code
...
def _proyecto_tipo(title: str) -> str:
    n = title.lower()
    if "estudio de detalle" in n or re.search(r"\bsur-\d+\b", n):
    ...
    if "sector" in n or re.search(r"\bsu-nc\b", n):
        return "sector"
    return "urbanismo"


class VillaquilambreAyuntamientoAdapter(AyuntamientoAdapter):
    """WordPress urbanismo + sede espublico tablón + IDECyL WFS (geometría partial)."""
    ...
    def _fetch(self, url: str) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-villaquilambre/1.0")},
        )
        ...
        for page_url in self.seed_pages:
            ...
            for m in RE_PDF_HREF.finditer(html):
                ...
                if not RE_PROYECTO.search(blob) and not RE_LICENCIA.search(blob):
                    if "urban" not in page_url.lower() and "plai" not in page_url.lower():
                        continue
                rows.append(
                    {
                        "titulo": titulo[:500],
    ...
    def _pdf_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        rec: dict[str, Any] = {
            ...
            "fecha": row.get("fecha"),
            "tipo": _proyecto_tipo(blob),
            "url": row.get("url") or row.get("pdf_url") or f"{WP_BASE}/atencion-al-ciudadano/urbanismo/",
            "source": "ayuntamiento",
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        pages = [
            ...
            (
                f"{WP_BASE}/atencion-al-ciudadano/urbanismo/licencias/",
                "formularios y trámites de licencias urbanísticas",
            ),
            (
                f"{WP_BASE}/atencion-al-ciudadano/urbanismo/",
                "urbanismo — documentación y trámites",
            ),
