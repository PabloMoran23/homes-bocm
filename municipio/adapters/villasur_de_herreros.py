...
WEB_BASE = "https://www.villasurdeherreros.es"
SEDE_BASE = "https://villasurdeherreros.sedelectronica.es"
...
ID_PREFIX = "villasur-de-herreros"
...
RE_PREVIEW = re.compile(
    r'href="(https://villasurdeherreros\.sedelectronica\.es/preview-document/[^"]+)"',
    re.I,
)
RE_CATALOG = re.compile(
    r'href="((?:https://villasurdeherreros\.sedelectronica\.es)?/catalog/t/[^"]+)"[^>]*>([^<]+)</a>',
    re.I,
)
RE_LICENCIA = re.compile(
    r"(?i)(licencia|licencias|solicitud de licencia|comunicaci[oó]n previa|"
    r"declaraci[oó]n responsable|autorizaci[oó]n (?:previa|urban)|obra (?:mayor|menor)|"
    r"primera ocupaci[oó]n|recepci[oó]n de obras)",
)
RE_PROYECTO = re.compile(
    ...
    r"unidad(?:es)? de ejecuci[oó]n|suelo urbano|urrez)",
)
RE_SECTOR_CODE = re.compile(
    r"(?i)\b("
    r"(?:SUR|SU[- ]?NC)\s*SE\s*\d+|"
    r"SECTOR\s+(?:SUR|SU[- ]?NC)\s*SE\s*\d+|"
    r"sector\s*\d+"
    r")\b",
)
...
def _normalize_sector_code(raw: str) -> str:
    ...
    code = re.sub(r"SU\s*NC", "SU-NC", code)
    return code.strip()
...
def _proyecto_tipo(blob: str, instrumento: str = "") -> str:
    ...
    if "plan parcial" in n or "sector" in n:
        return "plan parcial"
    if "modificaci" in n and "num" in n:
        return "modificación NUM"
    if "planeam" in n or "pgou" in n:
        return "planeamiento"
    return "urbanismo"


class VillasurDeHerrerosAyuntamientoAdapter(AyuntamientoAdapter):
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.plau_url = str(
            ...
            or f"{PLAU_BASE}/searchVPubDocMuniPlau.do?bInfoPublica=N&provincia=09&municipio=463"
        )
        self.plai_url = str(
            self.config.get("plai_url")
            or f"{PLAU_BASE}/searchVPubDocMuniPlai.do?bInfoPublica=S&provincia=09&municipio=463"
        )
    ...
    def _fetch(self, url: str, *, sede: bool = False) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-villasur-de-herreros/1.0")},
        )
