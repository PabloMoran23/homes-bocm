...
BASE = "https://www.villaviciosa.es"
SEDE_BASE = "https://villaviciosa.sedelectronica.e-ayuntamiento.es"
RPGUR_BASE = "https://www54.asturias.es/rpgur/action/publico"
MUNICIPIO = "Villaviciosa"
ID_PREFIX = "villaviciosa"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{BASE}/pgou",
    f"{BASE}/normativa-urbanistica1",
    f"{BASE}/instrumentos-de-planeamiento-vigentes",
    f"{BASE}/instrumentos-de-gestion-urbanistica",
    f"{BASE}/convenios-urbanisticos",
    f"{BASE}/es/urbanismo",
]
...
RE_PROYECTO = re.compile(
    ...
    r"cat[aá]logo|actuaci[oó]n urban|ordenanza|pepu|pep\b|gesti[oó]n urban)",
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_DOC_LINK = re.compile(
    r'href="((?:https://www\.villaviciosa\.es)?/documents/[^"]+\.pdf[^"]*)"',
    re.I,
)
...
RE_TABLON_ROW = re.compile(
    r'href="([^"]*anuncio\.aspx\?id=\d+)"[^>]*>.*?'
    r'<td><span[^>]*>(\d{1,2}/\d{1,2}/\d{4})[^<]*</span></td>\s*<td>\s*([^<]+?)\s*</td>',
    re.S | re.I,
)
...
def _page_title(html: str, fallback: str = "") -> str:
    for pat in (r"<h1[^>]*>([^<]+)", r"<title>([^<]+)"):
        ...
        if m:
            ...
            t = re.sub(r"\s*[-|].*Villaviciosa.*$", "", t, flags=re.I).strip()
            if t and len(t) > 3:
                return t[:500]
    return fallback
...
def _tipo_proyecto(denominacion: str, clasificacion: str = "") -> str:
    ...
    if "plan general" in blob or "pgou" in blob or "pgo " in blob:
        return "PGOU"
    if "plan especial" in blob or "pepu" in blob or "pep " in blob:
...
class VillaviciosaAyuntamientoAdapter(AyuntamientoAdapter):
    """Liferay villaviciosa.es + sede e-ayuntamiento + RPGUR Asturias; geometría parcial WFS."""
    ...
    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        ...
        self.rpgur_concejo_id = str(self.config.get("rpgur_concejo_id") or "76")
    ...
    def _fetch(self, url: str, *, data: bytes | None = None, encoding: str = "utf-8") -> str:
        ...
        headers = {
            "User-Agent": self.config.get("user_agent", "poc-bocm-villaviciosa/1.0"),
        }
        if data is not None:
    ...
    def _load_geom_index(self) -> dict[str, dict[str, Any]]:
        ...
        params = {
            ...
            "srsName": "EPSG:4326",
            "CQL_FILTER": "Instrumento LIKE '%VILLAVICIOSA%'",
        }
    ...
    def _collect_tablon_rows(self) -> list[dict[str, str]]:
        url = f"{SEDE_BASE}/tablondeanuncios/default.aspx"
        try:
        ...
        for m in RE_TABLON_ROW.finditer(html):
            link = self._abs_url(m.group(1), SEDE_BASE)
            fecha = m.group(2)
            title = _strip_html(m.group(3))
            rows.append({"url": link, "fecha": fecha, "titulo": title})
        return rows

    def _collect_tablon_proyectos(self) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        for item in self._collect_tablon_rows():
            title = item["titulo"]
            ...
            rows.append(
                {
                    ...
                    "municipio": MUNICIPIO,
                    "titulo": title[:500],
                    "fecha": _parse_fecha_dmy(item["fecha"]),
    ...
    def _crawl_liferay_documents(self) -> list[dict[str, Any]]:
        ...
        while queue and len(visited) < self.max_crawl_pages:
            ...
            for m in re.finditer(r'href="(https://www\.villaviciosa\.es/[^"#?]+)"', html, re.I):
    ...
    def _collect_licencias(self) -> list[dict[str, Any]]:
        ...
        for item in self._collect_tablon_rows():
            ...
            rows.append(
                {
                    "id": rec_id,
                    "fecha_concesion": _parse_fecha_dmy(item["fecha"]),
                    "tipo": "anuncio tablón",
                    "distrito": None,
                    "lat": None,
                    "lon": None,
        ...
        info_pages = [
            (f"{BASE}/es/urbanismo", "Urbanismo — trámites y licencias"),
            (f"{BASE}/normativa-urbanistica1", "Normativa urbanística"),
            (f"{SEDE_BASE}/tablondeanuncios/default.aspx", "Tablón de anuncios electrónico"),
        ]
