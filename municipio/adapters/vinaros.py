...
URBANISME_BASE = "https://urbanisme.vinaros.es"
CORP_BASE = "https://www.vinaros.es"
MUNICIPIO = "Vinaròs"
ID_PREFIX = "vinaros"
INE_MUNICIPIO = "12138"
...
DEFAULT_SEED_PAGES: list[str] = [
    f"{URBANISME_BASE}/es/contenido/planeamiento-urbanistico-municipal",
    f"{URBANISME_BASE}/es/contenido/modificaciones-del-pgou-2001",
    f"{URBANISME_BASE}/es/contenido/planes-parciales-i-planes-de-reforma-interior",
    f"{URBANISME_BASE}/es/node/302",
    f"{URBANISME_BASE}/es/node/275",
    f"{URBANISME_BASE}/es/contenido/estudios-de-detalle",
    f"{URBANISME_BASE}/es/contenido/planes-especiales",
    f"{URBANISME_BASE}/es/contenido/programas-e-instrumentos-de-planeamiento-en-tramite-de-aprobacion-o-desarrollo",
]
...
RE_PROYECTO = re.compile(
    ...
    r"cambio de uso|ordenanza|rectificaci[oó]n|unidad de ejecuci[oó]n|actuaci[oó]n|"
    r"asfalt|alumbrado|pai|paa|pmus|\bue[\.\-\s]*\d+|\bp[\.\-\s]*\d+|\bsui[\s-]?\d+|\bsur[\s-]?\d+)",
)
RE_BOARD_NON_URBAN = re.compile(
    r"(?i)(selecci[oó]n de personal|nombramiento|convocatoria.*empleo|"
    r"modificaci[oó]n de cr[eé]ditos|subvenci[oó]n|precios p[uú]blicos|ordenanza fiscal|"
    r"declaraci[oó]n de bienes|notoriedad|convocatoria.*pleno|juez de paz|contractaci[oó]n)",
)
RE_BOARD_ROW = re.compile(r"<tr[^>]*>\s*<td class=\"class_name\".*?</tr>", re.I | re.S)
RE_BOARD_CELL = re.compile(r'class="(class_[^"]+)"[^>]*>(.*?)</td>', re.I | re.S)
RE_PREVIEW_LINK = re.compile(
    r'href="((?:https://vinaros\.sedelectronica\.es)?/preview-document/[a-f0-9-]+)"',
    re.I,
)
RE_FECHA_DMY = re.compile(r"(\d{1,2})/(\d{1,2})/(\d{4})")
RE_YEAR = re.compile(r"\b((?:19|20)\d{2})\b")
RE_SECTOR_CODE = re.compile(
    r"(?i)\b(?:sector|ue|sui|sur|sut|p\.?\s*|unidad de ejecuci[oó]n)\s*([A-Z]?\s*\d+)\b",
)
...
def _proyecto_tipo(blob: str) -> str:
    b = blob.lower()
    if re.search(r"\bpai\b", b) or "programa de actuaci" in b:
        return "PAI"
    ...
    if re.search(r"\bsui[\s-]?\d+", b) or re.search(r"\bsur[\s-]?\d+", b) or "sector" in b:
        return "sector urbanizable"
    if "asfalt" in b or "alumbrado" in b:
        return "obra pública"
    return "planeamiento"
...
def _parse_gml_feature(member: ET.Element, ine_municipio: str) -> dict[str, Any] | None:
    feat = member[0]
...
class VinarosAyuntamientoAdapter(AyuntamientoAdapter):
    """Drupal 8 (urbanisme) + sede espublico gestiona + ICV WFS InventarioSuSuz (geometría partial)."""

    def __init__(self, slug: str, config: dict[str, Any] | None = None, base_url: str = ""):
        super().__init__(slug, config, base_url or URBANISME_BASE)
    ...
    def _fetch(self, url: str, *, timeout: int = 60) -> str:
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": self.config.get("user_agent", "poc-bocm-vinaros/1.0")},
        )
    ...
    def _load_wfs_ambitos(self) -> list[dict[str, Any]]:
        ...
        while start < 20_000:
            ...
            for member in members:
                parsed = _parse_gml_feature(member, self.ine_municipio)
                if parsed:
                    rows.append(parsed)
            start += len(members)
            if len(members) < 200:
    ...
    def _collect_seed_docs(self) -> list[dict[str, Any]]:
        ...
        for page_url in self.seed_pages:
            ...
            for m in re.finditer(r'href="([^"]+)"[^>]*>([^<]{3,200})</a>', html, re.I):
                href = unescape(m.group(1))
                link_title = _strip_html(m.group(2))
                href_l = href.lower()
                if not any(
                    x in href_l
                    for x in (
                        "sharepoint.com",
                        "/sites/urbanisme/files",
                    )
                ):
                    continue
                if not any(
                    x in href_l
                    for x in (".pdf", ".zip", ".rar", "sharepoint.com")
                ):
                    continue
                doc_url = href if href.startswith("http") else self._abs_url(href, URBANISME_BASE)
                if doc_url in seen:
                    continue
                seen.add(doc_url)
                name = link_title or unescape(urllib.parse.unquote(Path(href).name))
                titulo = f"{page_title} — {name}"
                rows.append(
                    {
                        "titulo": titulo[:500],
                        "fecha": _fecha_from_blob(f"{name} {href}"),
                        "url": doc_url,
                        "page_url": page_url,
    ...
    def _collect_licencia_info_pages(self) -> list[dict[str, Any]]:
        return [
            ...
            {
                "id": _stable_id("lic", f"{URBANISME_BASE}/es/contenido/inicio"),
                "fecha_concesion": None,
                "tipo": "portal urbanismo municipal",
                "distrito": None,
                "lat": None,
                "lon": None,
                "titulo": "Portal de Urbanismo — trámites y documentación",
                "url": f"{URBANISME_BASE}/es/contenido/inicio",
                "source": "ayuntamiento",
                "nota": "PGOU, modificaciones y trámites de licencias (Drupal urbanisme.vinaros.es)",
                "origen": "web_tramite",
            },
            {
    ...
    def _board_is_urban(self, row: dict[str, Any]) -> bool:
        ...
        if RE_BOARD_NON_URBAN.search(blob):
            if not re.search(
                r"(?i)(pgou|planeam|urban|licencia|obra|sector|suelo|informaci[oó]n p[uú]blica|actuaci)",
                blob,
            ):
                return False
    ...
    def _board_to_proyecto(self, row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not RE_PROYECTO.search(blob) and not any(
            k in proc for k in ("planeamiento", "genérico", "actuacions", "urban")
        ):
            return None
        key = row.get("expediente") or row["url"]
        rec = {
