-- Ficha ubicación: tramitación del visor + fecha_aprob en expedientes SIGMA.
...
AS $$
  ...
  tram AS (
    ...
    FROM homes.sigma_vis_tramite vt
    WHERE vt.expediente_grupo IN (SELECT expediente_grupo FROM sigma)
    GROUP BY vt.expediente_grupo
  )
  SELECT CASE
    ...
    ELSE jsonb_build_object(
      ...
      COALESCE(
        (
          SELECT jsonb_object_agg(t.expediente_grupo, t.rows)
          FROM tram t
        ),
        '{}'::jsonb
      ),
