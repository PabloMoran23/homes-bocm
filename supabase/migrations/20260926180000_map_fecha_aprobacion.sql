-- Fecha del mapa: la más reciente entre el anuncio del boletín y la aprobación SIGMA.
-- Así un expediente aprobado en 2024 no queda oculto por un anuncio de 2022.

CREATE OR REPLACE FUNCTION homes._proyecto_fecha_mapa(p_bocm date, p_aprob text)
RETURNS date
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT GREATEST(
    p_bocm,
    CASE
      WHEN p_aprob ~ '^\d{4}-\d{2}-\d{2}' THEN substring(p_aprob, 1, 10)::date
      ELSE NULL
    END
  );
$$;
...
BEGIN
  ...
  WITH base AS MATERIALIZED (
    ...
    WHERE homes._proyecto_de_municipio(p_slug, p.municipio_slug, p.municipio, p.bocm_municipio)
      AND homes._proyecto_fecha_mapa(p.bocm_pub_date, p.fecha_aprob) IS NOT NULL
      AND homes._proyecto_fecha_mapa(p.bocm_pub_date, p.fecha_aprob) >= p_from
      AND homes._proyecto_fecha_mapa(p.bocm_pub_date, p.fecha_aprob) <= p_to
  ),
