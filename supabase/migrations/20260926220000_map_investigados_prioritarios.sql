-- Proyectos con ficha de investigación: nombre popular y siempre en el mapa, dibujados encima.

CREATE OR REPLACE FUNCTION homes._proyecto_nombre_popular(p_id text)
RETURNS text
LANGUAGE sql
STABLE
AS $$
  ...
  JOIN homes.proyecto_investigacion i ON i.id = d.investigacion_id
  WHERE d.proyecto_id = p_id
    AND d.clave = 'nombre_publico'
    AND d.publicable
  ORDER BY i.investigado_at DESC, d.orden, d.id
  LIMIT 1;
$$;

INSERT INTO homes.proyecto_dato_extra (
  investigacion_id, proyecto_id, bloque, clave, etiqueta, valor,
  confianza, fuente_tipo, fuente_nombre, destacado, publicable, orden
)
SELECT i.id, i.proyecto_id, 'situacion', 'nombre_publico', 'Nombre',
  CASE i.proyecto_id
    WHEN '711/2013/20329' THEN 'Ribera del Calderón'
    WHEN '135/2018/00489' THEN 'Madrid Nuevo Norte'
  END,
  'alta', 'oficial',
  CASE i.proyecto_id
    WHEN '711/2013/20329' THEN 'Ayuntamiento de Madrid'
    WHEN '135/2018/00489' THEN 'Crea Madrid Nuevo Norte'
  END,
  true, true, -1
FROM homes.proyecto_investigacion i
WHERE i.proyecto_id IN ('711/2013/20329', '135/2018/00489')
  AND NOT EXISTS (
    SELECT 1 FROM homes.proyecto_dato_extra d
    WHERE d.investigacion_id = i.id AND d.clave = 'nombre_publico'
  );
...
AS $$
  SELECT jsonb_build_object(
    ...
    'nombrePopular', homes._proyecto_nombre_popular(p.id),
    'investigado', EXISTS (
...
BEGIN
  ...
  IF bbox_active THEN
    ...
    poly_take AS (
      SELECT * FROM (SELECT * FROM poly_ranked LIMIT lim_polys) ranked
      UNION
      SELECT * FROM destacados
    ),
    poly_ok AS (
      SELECT jsonb_build_object(
        'type', 'Feature',
