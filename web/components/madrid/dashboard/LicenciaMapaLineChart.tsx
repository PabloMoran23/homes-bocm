...
import {
  LICENCIA_MAPA_CONFIG,
  LICENCIA_MAPA_LEYENDA,
} from "@/lib/licencia-mapa-config";
...
function collectSeriesIds(
  ...
  for (const id of LICENCIA_MAPA_LEYENDA) {
...
export function LicenciaMapaLineChart({
  title,
  seriesByYear,
  seriesByMonth,
  granularity = "year",
  ...
  const chartData = useMemo(() => {
    const datasets = seriesIds.map((id) => {
      const cat = id as LicenciaMapaCategoria;
      const cfg = LICENCIA_MAPA_CONFIG[cat];
      const color = cfg?.bg ?? "#64748b";
      return {
        label: cfg?.label ?? id,
  ...
  return (
    <ChartCard
      title={title}
      subtitle={`Iconos del mapa · ${granularityLabel(granularity)} · pulsa la leyenda para ocultar una serie`}
