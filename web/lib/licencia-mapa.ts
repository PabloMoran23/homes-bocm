...
export const LICENCIA_MAPA_CONFIG: Record<LicenciaMapaCategoria, CategoriaConfig> = {
  dr_residencial: {
...
export function createLicenciaDivIcon(
  ...
  return L.divIcon({
    className: "",
    html: `<div style="width:${sizePx}px;height:${sizePx}px;margin:${-sizePx / 2}px 0 0 ${-sizePx / 2}px;background:${cfg.bg};border:${border};border-radius:50%;box-shadow:0 2px 6px rgba(15,23,42,0.25);display:flex;align-items:center;justify-content:center;box-sizing:border-box">${svg}</div>`,
...
export function createCentroBusquedaDivIcon(): L.DivIcon {
  ...
  return L.divIcon({
    className: "",
    html: `<div style="width:${sizePx}px;height:${sizePx}px;margin:${-sizePx / 2}px 0 0 ${-sizePx / 2}px;background:#0f766e;border:3px solid #fff;border-radius:50%;box-shadow:0 2px 8px rgba(15,118,110,0.45);display:flex;align-items:center;justify-content:center">${CENTRO_SVG}</div>`,
    iconSize: [sizePx, sizePx],
    iconAnchor: [sizePx / 2, sizePx / 2],
  });
}
...
export function createSigmaDivIcon(size: "sm" | "md" = "md"): L.DivIcon {
  ...
  return L.divIcon({
    className: "",
    html: `<div style="width:${sizePx}px;height:${sizePx}px;margin:${-sizePx / 2}px 0 0 ${-sizePx / 2}px;background:#0ea5e9;border:2px solid #0369a1;border-radius:50%;box-shadow:0 2px 5px rgba(3,105,161,0.35);display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#fff">Σ</div>`,
    iconSize: [sizePx, sizePx],
    iconAnchor: [sizePx / 2, sizePx / 2],
  });
}
